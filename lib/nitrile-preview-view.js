...
///console.log(`image ${fsrc} changed`);
  ...
  updateBlocks() {
    ...
    this.toAll().then( ([all,config,subs]) => {
        ...
        this.config = config;
        this.subs = subs;
        this.title = this.config.latex.title ? this.config.latex.title : 'Untitled'
        this.author = this.config.latex.author ? this.config.latex.author : ''
...
`;
        ...
        data = await epub.generateAsync(this.parser,this.parser.escape(this.title),this.parser.escape(this.author),htmls,chaps,this.imagemap,this.config.latex.isArticle,this.dirname());
