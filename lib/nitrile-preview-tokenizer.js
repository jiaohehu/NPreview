...
const re_token = /\\\;|\\\,|\\\:|\\\\|\\\{|\\\}|\\sqrt\[.*?\]|\\begin\{\w+\}|\\end\{\w+\}|(\\mathscr|\\mathcal|\\mathbb|\\mathrm|\\mathit)\{[A-Za-z0-9]*\}|\\[a-zA-Z]+|./g;
...
const re_fontvariants = /^\\(mathit|mathrm|mathscr|mathbb|mathcal)\{(.*)\}$/;
...
class NitrilePreviewTokenizer {
  ...
  toInnerSvg (v, compact=false) {
    ...
    /// one group, such as '\\brace',     [Array], '[', ']', ...
    /// one group, such as '\\leftright', [Array], '[', ']', ...
    /// one group, such as '\\beginend',  [Array], '[', ']', ...
...
console.log('ncol=',ncol);
          ...
          cols_w.fill(12);
          rows_h.fill(12);
          /// find out the max row height for each row
          for (let j=0; j < ncol; ++j) {
            for (let i=0; i < nrow; ++i) {
              if (p[i][j]) {
                var [w_,h_,mid_,s_,g_,q_] = p[i][j];
                rows_h[i] = Math.max(rows_h[i],h_);
              }
            }
          }
          /// find out the max col 
          for (let j=0; j < ncol; ++j) {
            for (let i=0; i < nrow; ++i) {
              if (p[i][j]) {
                var [w_,h_,mid_,s_,g_,q_] = p[i][j];
...
console.log('s_=',s_);
          ...
          //var [w_,h_,mid_,s_,g_,q_] = this.toInnerSvg(['\\brace',p[0][0],'','',''],compact);
        ...
        case '\\dot':
          ...
          o.push(s_);
          o.push(`</svg>`);
          o.push(`<circle cx='${w/2}pt' cy='1.5pt' r='0.5pt' class='MyLine'/>`);
          return [w,h,mid,o.join('\n'),0,0];
          break;
        ...
        case '\\vec':
          ...
          var y1 = 1.5;
          o.push(`<line x1='${x0}pt'  y1='${y0}pt'  x2='${x1}pt'     y2='${y1}pt' class='MyLine'/>`);
          o.push(`<line x1='${x1}pt'  y1='${y1}pt'  x2='${x1-1.5}pt' y2='0pt' class='MyLine'/>`);
          o.push(`<line x1='${x1}pt'  y1='${y1}pt'  x2='${x1-1.5}pt' y2='3pt' class='MyLine'/>`);
          return [w,h,mid,o.join('\n'),0,0];
          break;
        ...
        case '\\breve':
          ...
          x1 *= 1.33;
          y1 *= 1.33;
          o.push(`<path d='M ${x0},${y0} a1,1 0 0,0 ${x1} ${y1}' class='MyLine'/>`);
          return [w,h,mid,o.join('\n'),0,0];
          break;
        ...
        case '\\tilde':
          ...
          x1 *= 1.33;
          y1 *= 1.33;
          o.push(`<path d='M ${x0},${y0} Q ${(x0+xm)/2},0 ${xm},${ym} Q ${(xm+x1)/2},${3*1.33} ${x1},${y1}' class='MyLine'/>`);
          return [w,h,mid,o.join('\n'),0,0];
          break;
