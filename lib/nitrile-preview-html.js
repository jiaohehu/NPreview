...
`
  ...
  translateHtml (all,config,ispreview) {
    ...
    for (var block of all) {
      ...
      if (ispreview && config.previewOutlineViewEnabled) {
        if (sig === 'PART') {
        } else if (sig === 'HDGS') {
        } else if (sig === 'ERRO') {
        } else if (config.previewOutlineViewSection && this.isInsideSection(dept,config.previewOutlineViewSection)) {
        } else {
      ...
      switch (sig) {
        case 'PART': {
          var text = data;
          p.push(['PART',id,label_text,dept,this.escape(text),o.length,'']);
        ...
        case 'ERRO': {
          ...
          if (this.isarticle) {
            switch (hdgn) {
              ...
              case 1:
                heading = 'SECTION';
                p.push([heading,id,label_text,dept,this.escape(text),o.length,'']);
        ...
        case 'PARA': {
          var text = data;
          text = this.unmask(text);
          o.push(`<div id='${id}' class='TEXT' dept='${dept}' fName='${fname}' subrow='${subrow}' rows='${row1} ${row2}' style='${this.divcssstyle}; ; margin-top:2em; margin-bottom:0;'>`);
          o.push(`${showidtext}`);
          o.push(`<p style='${this.paracssstyle}' >`);
          o.push(text);
          o.push('</p>');
        ...
        case 'PRIM': {
          ...
          text = this.unmask(text);
          o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' subrow='${subrow}' rows='${row1} ${row2}' style='${this.divcssstyle}; margin-bottom:0; '>`);
          o.push(`${showidtext}`);
          o.push(`<p style='${this.paracssstyle}' >`);
        ...
        case 'TEXT': {
          var text = data;
          text = this.unmask(text);
          o.push(`<div id='${id}' class='TEXT' dept='${dept}' fName='${fname}' subrow='${subrow}' rows='${row1} ${row2}' style='${this.divcssstyle}; ; margin-top:0; margin-bottom:0;'>`);
          o.push(`${showidtext}`);
          o.push(`<p style='${this.paracssstyle}' >`);
          if (blockcount == 1) {
            o.push(text);
...
//console.log(data);
        ...
        case 'math': {
          if (!fencecmd.label) {
            ...
            ///NOTE: equation
            var num = autonum.idenEquation(isarticle);
            p.push(['math',id,label_text,num,'',o.length,'']);
            o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' subrow='${subrow}' rows='${row1} ${row2}' style='${this.divcssstyle};' >`);
            o.push(`${showidtext}`);
            o.push(`<p style='${this.paracssstyle}' >`);
            var text = data;
            if (text.length == 1) {
              for (var s of text) {
                s = this.tokenizer.parse(s,true);
                o.push(`<span style='display:block;text-align:center;position:relative'>${s} `);
                o.push(` <span style='position:absolute;right:0px;'>(${num}) ${showlabeltext} </span>`);
                o.push(`</span> `);
              }
            } else if (text.length > 1) {
              var j = 0;
              for (var s of text) {
                ...
                o.push(`<span style='display:block;text-align:center;position:relative'>${s} `);
                o.push(` <span style='position:absolute;right:0px;'>(${num}${subnum}) ${showlabeltext} </span>`);
                o.push(`</span> `);
                j += 1;
              }
            }
        ...
        case 'code': {
          ...
          if (fencecmd.label) {
            var num = autonum.idenListing(isarticle);
