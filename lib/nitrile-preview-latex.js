...
`;
  ...
  to_xecjk_document() {
    ...
    var p_packages=this.to_core_packages();
    var p_extra_packages = this.to_extra_packages();
    var p_fonts = this.to_xecjk_fonts();
    return     `\
%!TeX program=XeLatex
\\documentclass[${p_documentclassopt||''}]{${p_documentclass}}
\\usepackage{ucs}
...
`
  ...
  to_lualatex_fonts () {
    if(this.conf('xelatex.fonts')){
      var ss = this.conf('lualatex.fonts').split('\t');
      ss = ss.map(s => {
        let [ccode,fnt] = s.split(',').map(x => x.trim());
        return `\\newfontfamily\\${ccode}{${fnt}}`;
      });
  ...
  to_xelatex_fonts () {
    if(this.conf('xelatex.fonts')){
      var ss = this.conf('xelatex.fonts').split('\t');
      ss = ss.map(s => {
        let [ccode,fnt] = s.split(',').map(x => x.trim());
