...
class NitrilePreviewParser {
  ...
  readFromLines(lines) {
    ...
    */
    ...
    while (lines.length > 0) {
      ...
      } else if (this.is_tabr(para)){

        var {data,caption,label} = this.parse_tabr(para);
        sig = 'TABR';
        o.push({mode,rmap,row1, row2, sig, html, data, caption, label,  para, fencecmd,  subrow});
        block_type = 'tabr';
        continue;

      } else if (this.is_pict(para)){

        var {caption,label,data} = this.parse_pict(para);
        sig = 'PICT';
        o.push({mode,rmap,row1, row2, sig, html, data, caption, label,  para, fencecmd,  subrow});
        block_type = 'pict';
        continue;
      ...
      } else if ((v = this.re_hlst.exec(para[0])) !== null) {

        var data = this.parse_hlst(para);
        sig = 'HLST';
        o.push({mode,rmap,row1,row2,sig,html,data,para,fencecmd,subrow});
  ...
  read_para (lines) {
    ...
    /// now parse the data based on the 'type'
    
    if(type == 'spcl' && bull=='@'){
      data = parse_pict(text);
    } 
    else if (type == 'spcl' && bull=='&'){
      data = parse_tabr(text);
    } 
    else if (type == 'spcl' && bull=='='){
      data = parse_tabr(text);
    }
      ...
      data = parse_samp(text);
    }
    else {
      data = parse_text(text);
    }

    return {para,text,caption,label,title,hdgn,sig,data,isfenced,fenceid};
  }
  ...
  */
  ...
  translateBlocks() {
    ...
    for (var block of this.blocks) {
      var {rmap,id,row1,row2,sig,dept,level,data,caption,label,idnum,para,fencecmd,subrow,subfname} = block;
      this.block = block;
