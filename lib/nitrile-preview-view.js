...
console.log('onDidChangeCurosPosition');
...
///console.log(`load image from file: ${imgid}, ${src}`);
  ...
  refreshView() {
    ...
    if (atom.config.get('nitrile-preview.viewMode') === 'normal') {
      this.innerHTMLs = [];
      ...
      this.innerHTMLs = [];
      this.innerHTMLs.push(this.toHeadings(this.chaps,this.isarticle));
      this.innerHTMLs.push(`<main class='PAGE' style='${geometry_text}; padding-left:1cm; padding-right:7.9cm; ' >`);
      this.innerHTMLs.push(`<p class='TITLE' style='${this.parser.titlecssstyle}' >`);
      this.innerHTMLs.push(`${this.parser.escape(this.title)}`);
      this.innerHTMLs.push(`</p>`);
      this.innerHTMLs.push(`<p class='AUTHOR' style='${this.parser.authorcssstyle}' >`);
