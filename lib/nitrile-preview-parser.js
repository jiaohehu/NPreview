...
class NitrilePreviewParser {
  ...
  constructor() {
    ...
    this.re_fence = /^(\s*\`\`\`|\s*\~\~\~|\s*\<\<\<|\s*\<\<\=|\s*\/\/\/|\s*\-\-\-|\s*\=\=\=|\s*\+\+\+|\s*\"\"\"|\s*\$\$\$)\s*(|\w+|\[.*\])$/;
    this.re_bullet_text = /^(\S+)\s+(\:)(|\s+.*)$/;
    this.re_unmask = /\s\_{2}(.*?)\_{2}|\s\_{1}(.*?)\_{1}|\s\`\`\s*(.*?)\s*\`\`|\`\s*(.*?)\s*\`|\[(.*?)\]\((.*?)\)|\s\*(\w+)\*/g;
    this.re_local = /^\:([\w\-]+)$/;
    this.re_full = /^([\w\-]+)\:([\w\-]+)$/;
    this.re_directive  = /^\%\s+\!(\w+)\b(.*)$/;
    this.re_texcomment = /^\%\s+\!TEX/;
    this.re_ntrflag    = /^\%\s+\!NTR\b(.*)$/;
    this.re_ntrflagset = /^\%\s+\!NTR\b\s*(\w+)\s*\=\s*(.*?)\s*$/;
  ...
  toFlow(lines,fname,o=null) {
    ...
    while (lines.length > 0) {
      ...
      var [nread, para, _flags, lines, the_fence, fencecmd ] = this.readPara(lines);
      ...
      if (sampspaces > 0) {
        if ((v = this.re_leadspaces.exec(para[0])) !== null) {
      ...
      // this is the TABU block
      if (the_fence === '+++') {
        var [data, n] = this.trimPara(para);
        n = (stepspaces > 0 && n >= stepspaces) ? Math.floor(n/stepspaces) : 0;
        var [data,fence] = this.trimFences(data);
        var data = this.parseTABB(data);
        o.push([id,row,row2,'TABU',n,data,para,fencecmd,caption,base,label,fname]);
        caption = ''; label = ''; fencecmd = {};
        continue;
      }
      ...
      if (the_fence === '<<=') {
        ...
        var [data,fence] = this.trimFences(data);
        var data = this.parseTERM(data);
        o.push([id,row,row2,'SPEC',n,data,para,fencecmd,caption,base,label,fname]);
        caption = ''; label = ''; fencecmd = {};
        continue;
      }
      ...
      if (nblocks == 1 && para.length == 1 && !flags.title) {
        var text = para[0];
        flags.title = text;
        var data = [0,text];
  ...
  readPara (lines) {
    ...
    for (var i=0; i < lines.length; ++i) {
      ...
      if (num_solid == 0 && ( v = this.re_directive.exec(line)) !== null) {
        if (v[2]) {
          /// such as
          ///
          /// .adjust .33 .33
      ...
      if (num_solid == 0) {
        if (this.re_texcomment.test(line)) {
          continue
        } else if (this.re_ntrflag.test(line)) {
          var v = this.re_ntrflagset.exec(line);
          if (v) {
            var key = v[1];
            var val = v[2];
            flags[key] = val;
...
///console.log(config);
  ...
  wrapSample (para, config_sampwrap) {
    var o = [];
    var l = [];
    const maxn = config_sampwrap;
