...
class NitrilePreviewParser {
  ...
  constructor() {
    ...
    this.re_nitrileconf = /^\s(\w+)\.(\w+)\s*=\s*(.*)$/u;
    this.re_blank = /^(\s+)(.*)$/;
    this.re_plain = /^(\S+\s*)(.*)$/;
  ...
  newConfig() {
    ...
    config.CONTEX.toc='';///when set to 1 '\placecontent' will be inserted
    config.CONTEX.pages='';
    config.CONTEX.twocolumns='';
    ...
    config.LATEX.twocolumns='';///set to '1' to enable
    ...
    config.HTML.toc='';///when set to 1 a customized TOC will be inserted
    config.HTML.trace = 0;
    ...
    config.SBDC = {};
    config.PLST = {};
    config.PLST.padding = '1.5';///1.5em padding-left for <UL> and <OL>
    config.ILST = {};
    config.ILST.padding = '1.5';///1.5em padding-left for <UL> and <OL>
    config.DLST = {};
    config.DLST.padding = '0';///The additional padding-left for <DD>. If set to 1 it is 1em
    config.TOPI = {};
    config.LONG = {};
    ...
    while (lines.length > 0) {
      ...
      if (block_type === 'SAMP') {
        ...
        if (block_type0 === 'SAMP') {
          ...
          var data = para;
          sig = 'SAMP';
          o.push({fname,mode,rmap,row1,row2,sig,html,data,para,fencecmd,subrow});
        }
        continue
        ...
        if(v){
          ...
          /// # Part:: GRAMMER (#mygrammer)
          var re_part = /^part::\s+(.*)$/i
          if(hdgn == 1) {
            if((v=re_part.exec(data))!==null){
              part = 1;
              data = v[1];
            }
          }

          o.push({fname,mode,rmap,row1,row2,sig,html,part,hdgn,level,data,para,fencecmd,subrow});
        }
      ...
      } else if ((v = this.re_ilst.exec(para[0])) !== null) {

        var data = this.parse_ilst(para);
        sig = 'ILST';
        o.push({fname,mode,rmap,row1,row2,sig,html,data,para,fencecmd,subrow});
        block_type = 'ilistpara';
        continue
        ...
        var data = text;
        sig = 'TEXT';
        o.push({fname,mode,rmap,row1,row2,sig,html,lead,leadn,data,para,fencecmd,subrow});
        block_type = 'TEXT';
        ...
        if((v=re1.exec(s))!==null){
          ...
          sig = 'SAMP';
          var mode = 1;
          o.push({fname,mode,rmap,row1,row2,mode,sig,html,data,para,fencecmd,subrow});
          block_type = 'SAMP';
          ...
          var standalone = 1;
          o.push({fname,mode,rmap,row1,row2,sig,html,standalone,data,para,fencecmd,subrow});
          block_type = 'TEXT';
        }
    ...
    while ((v = this.re_unmask.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        if(cnt.startsWith('#')){
          ...
          var mytext = this.rubify(mytext,this.xrmap);
          newtext += mytext;
        }
  ...
  */
  ...
  async readModeAsync(dirname) {
    ...
    for( var d of this.mode ) {
      let { subfname } = d;
      if(subfname){
        let fsubfname = path.join(dirname,subfname);
        let out = await this.readFileAsync(fsubfname);
        let lines = out.split('\n');
        d.subparser = new NitrilePreviewParser();
        d.subparser.readFromLines(lines);
        d.subparser.fname = subfname;
      }
    }
  ...
  mergeRoot (root) {
    ...
    if(root.length==1) {
      var item = root[0];
      this.config = item.subparser.config;
      //this.rmap = [...item.subparser.rmap, ...this.rmap];
      //this.rmap = this.rmap.concat(item.subparser.rmap);
      for(var d of item.subparser.rmap){
        this.rmap.push(d);
      }
    }
  ...
  mergeSubs (subs) {
    ...
    for (var sub of subs) {
      ...
      db[subrow] = {fname:subfname,flow:subparser.blocks,rmap:subparser.rmap};
    }
    var o = [];
        ...
        case 'SBDC': {
          ...
          if (flow) {
            for (var f of flow) {
              ...
              // add all master's rmap entry to the end of the 'sub-rmap' entry
              for(var d of this.rmap){
                f.rmap.push(d);
              }
              o.push(f)
            }
  ...
  translateBlocks() {
    ...
    this.needblank = 0;
    for (var block of this.blocks) {
      var {rmap,row1,row2,sig,level,data,para,fencecmd,subrow} = block;
      this.block = block;
