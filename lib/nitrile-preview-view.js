...
console.log('NitrilePreviewView::destroy() called');
  ...
  update() {
    ...
    if (book.length) {
      ...
      var parser = new NitrilePreviewHtml();
      parser.toBookBlocks(book,this.dirname())
        .then(([blocks,flags]) => {
            ...
            this.htmls.push(`</p>`);
            this.htmls = parser.translateBlocksOutlineOnly(this.toConfig(),this.blocks,this.htmls);
            this.htmls.push(`</div>`);
      ...
      blocks = parser.idenBlocks(blocks,true);
      this.book = [];
      this.flags = flags;
      this.blocks = blocks;
      this.htmls = [];
