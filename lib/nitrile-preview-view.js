...
console.log('autoScroll is on');
  ...
  async getBlocks() {
    ...
    if (book.length) {
      ...
      this.flags = this.parser.parseFLAGS(this.book);
      this.config = this.parser.toConfig(this.flags);
      var proms = this.subs.map ( sub => this.updateSub(sub) );
      for (var prom of proms) {
        var flow = await prom;
        var [subblocks] = flow;
        for (var subblock of subblocks) {
...
console.log('getting flow from myflow');
  ...
  async fetchFlow (sub,fsubfname) {
    try {
      ...
      this.myflows.set(fsubfname,flow);
      return flow;
    } catch (e) {
  ...
  updateBlocks() {
    ...
    this.getBlocks().then( () => this.refreshView() );
    return;
