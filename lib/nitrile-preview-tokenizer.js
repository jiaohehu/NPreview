...
class NitrilePreviewTokenizer {
  ...
  toDiagramSvg (str) {
    ...
    for (var mathSymbol of pjson.mathSymbols) {
      ...
      if (this.used.has(id)) {
        if(id=='myINT'){
          defs.push(`<text id='${id}' dy='9pt' textLength='${width}pt' lengthAdjust='' style='stroke:none;fill:inherit;font-size:${this.fs}pt'>${unicode}</text>`);
          continue;
        }
        defs.push(`<text id='${id}' dy='9pt' textLength='${width}pt' lengthAdjust='spacingAndGlyphs' style='stroke:none;fill:inherit;font-size:${this.fs}pt'>${unicode}</text>`);
      }
    }
    var defs= `\
<defs>
  ...
  toSvg (v) {
    ...
    for (var mathSymbol of pjson.mathSymbols) {
      ...
      if (this.used.has(id)) {
        ...
        defs.push(`<text dy='${dy}' dx='${dx}' id='${id}' textLength='${width}pt' lengthAdjust='spacingAndGlyphs' style='stroke:none; fill:inherit; font-size:${this.fs}pt;'>${unicode}</text>`);
      }
    }
    var s = `\
<svg 
