...
///console.log(`image ${fsrc} changed`);
  ...
  async toAll() {
    ...
    var proms = subs.map(sub => this.getSubFlow(fname, sub, this.parser));
    for (var j in proms) {
  ...
  updateBlocks() {
    ...
    this.toAll().then( ([all,flags,subs]) => {
...
`;
        ...
        var isarticle = true;
        data = await epub.generateAsync(this.parser,this.parser.escape(this.title),this.parser.escape(this.author),htmls,chaps,this.imagemap,isarticle,this.dirname());
