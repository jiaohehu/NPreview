...
class NitrilePreviewDiagram {
  ...
  toDiagram(para,notes) {
    ...
    for (var row = 0; row < para.length; row++) {
      ...
      this.row = row;
      this.line = line;
      var exit_flag = this.parseLine(line,notes);
      if (exit_flag) {
        break;
      }
    }
  ...
  parseLine(line,notes) {
    ...
    if ((v = this.re_pic_command.exec(line)) !== null) {
      ...
      if (pic) {
        ...
        pic = this.translator.trim_left(pic);
        for (var s of pic) {
  ...
  extract_next_expr(s,g=null) {
    ...
    if (s.charAt(0)==='(') {
      ...
      var arg1 = 0;
      for(var n=1; n > 0; ++n){
        var [a,s] = this.extract_next_expr(s,g);
        if (typeof a === 'number') {
          arg1 = this.exec_scalar_operator(op,arg1,a);
    ...
    if ((v=this.re_scalar_func.exec(s))!==null) {
      ...
      var v = this.exec_scalar_func(func_name,args);
      return [v,s];
    } 
    /// if this is a float
  ...
  */
  ...
  exec_scalar_func(func_name,args) {
    switch(func_name) {
      case 'log':
  ...
  exec_scalar_operator(op,arg1,arg2) {
    switch(op) {
      case '*':
  ...
  do_cartesian(opt,txt,g,floats) {
    ...
    switch( cmd ) {
      ...
      case 'yplot'://CARTESIAN
        ...
        var m = this.assertFloat(floats[2],4,this.MIN,this.MAX);
        var n = (max-min)*m/A.grid;
