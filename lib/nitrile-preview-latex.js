...
\\usepackage{csquotes}`
  ...
  async convert (filename) {
    ...
    //console.log(pjson);
    console.log(pjson.configSchema);

    console.log(this.getConfig('step',{},pjson.configSchema));
    console.log(this.getConfig('indentMargin',{},pjson.configSchema));
    console.log(this.getConfig('flushLeftParagraph',{},pjson.configSchema));
    
    var out = await this.readFileAsync('my.md');
    let lines = out.split('\n');
    let [blocks,flags] = this.toBLOCKS(lines,'my',0);
    let config = { step:2, indentMargin:0.85, flushLeftParagraph:true };
    let olines = this.translate(atom.config,blocks,true);
    let packages = this.toRequiredPackages();
    let data = `% !TEX program = lualatex
