...
console.log(is_sample);
      ...
      if (is_sample) {
        var [data, m] = this.trimParaAt(para,N_sampspaces); /// n is to be ignored
      ...
      if (true) {
        if (this.re_enum.test(data[0])) {
          data = this.parseENUM(data);
          o.push([id,row,row2,'ENUM',n,data,para,fencecmd,caption,base,label,fname]);
          caption = ''; label = ''; fencecmd = {};
          continue;
  ...
  readPara (lines) {
    ...
    for (var i=0; i < lines.length; ++i) {
      ...
      if (num_solid == 0 && this.re_ntrcmd.test(line)) {
        var v = this.re_ntrcmd.exec(line);
      ...
      if (num_solid == 0 && this.re_texcomment.test(line)) {
        var v = this.re_ntrflag.exec(line);
        var key = v[1];
        var val = v[2];
        flags[key] = val;
        para.push('');
