...
///console.log(`image ${fsrc} changed`);
  ...
  async toAll() {
    ...
    var all = this.parser.idenBlocks(all,config);
  ...
  refreshView() {
    ...
    if (this.isfullpage) {
      var fullpage_style = this.parser.toPageCssStyleText(this.config);
    } else {
      var fullpage_style = '';
    }

    var ispreview = true;/// ispreview flag is true
    this.htmls = this.parser.translateHtml(this.all,this.config,ispreview,this.row,this.column);
    this.innerHTMLs = [];
    this.innerHTMLs.push(`<style>${this.parser.stylesheet}</style>`);
  ...
  updateBlocks() {
    ...
    this.toAll().then( ([all,config,subs]) => {
        ...
        this.config = config;
        this.subs = subs;
        this.title = this.config.ALL.title ? this.config.ALL.title : 'Untitled'
        this.author = this.config.ALL.author ? this.config.ALL.author : ''
        this.isReport = this.config.ALL.isReport;
  ...
  async saveAs (savefilepath) {
    ...
    try {
      ...
      if (this.saveastype === 'latex-complete') {
        ...
        var htmlines = parser.translateHtml(this.all,this.config);
        const cssrules = []
...
`;
        ...
        data = await epub.generateAsync(this.parser,this.parser.escape(this.title),this.parser.escape(this.author),htmlines,chaps,this.imagemap,this.config,this.dirname());
