...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  rubify (src) {
    ...
    while (j < src.length) {
      i0 = src.length;
      found = 0;
      for (var rubyitem of this.xblock.rmap) {
  ...
  to_ref (str) {
    for (var j=0; j < this.xparser.blocks.length; ++j) {
      var block = this.xparser.blocks[j];
      var {id,sig,label,floatname,idnum,more,refid,id} = block;
      label = label||'';
      if(sig=='MATH'){
        if( str.localeCompare(label)===0) {
          //return `${floatname}.\\ref{${str}}`;
          return this.do_ref(sig,str,floatname,idnum,refid,id);
          break;
        }
        if(more&&more.length){
