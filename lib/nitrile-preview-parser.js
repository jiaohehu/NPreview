...
class NitrilePreviewParser {
  ...
  constructor() {
    ...
    this.re_unmask = /(?<!\S)_{2}(.*?)_{2}|(?<!\S)_{1}(.*?)_{1}|``(.*?)``|`(.*?)`|\[(.*?)\]\((.*?)\)|\s\*([\w\-]+)\*|\[\[&(\w+);\]\]|\[\[\^(\S+)\^\]\]|\[\[([\w\-]*#[\w\-]+)\]\]|(?<!\S)\'(\S+)\'/g;
    ...
    while (lines.length > 0) {
      ...
      if (block_type === 'SAMP' && (block_type0 === 'plistpara' || 
        ...
        if (v2) {
          ...
          var data = text;
          sig = 'HDGS';
          o.push({id,row1,row2,sig,html,hdgn,data,para,fencecmd,base,subrow,fname});
          continue;
        }
  ...
  idenBlocks(all,config) {
    ...
    return autonum.idenBlocks(all,config,this.isepub);
  }
