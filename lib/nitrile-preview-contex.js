...
class NitrilePreviewContex extends NitrilePreviewParser {
  ...
  do_hdgs(block){
    var o = [];
    var {id,row1,row2,sig,part,hdgn,dept,title,subfname} = block;
    subfname=subfname||'';
    ...
    if(part){
      o.push(`\\startpart[title={${dept}~~${text}},reference={${this.xlabel}},bookmark={${dept} ${raw}}]`);
      ...
      if(this.config.CONTEX.titlepage){
      }else{
        o.push(`\\blank\\noindent{\\tfd ${text}}`);
        o.push(`\\blank`);
      }
  ...
  do_samp(block){
    ...
    if(this.xfencecmd.style==1){
      ...
      body = body.join('\n');
      o.push(`\\setupnarrower[left=${this.xfencecmd.left}mm]`);
  ...
  do_quot(block){
    ...
    o.push(`\\setupnarrower[left=${this.xleft}mm,right=${this.xright}mm]`);
  ...
  do_long(block){
    ...
    rows = rows.map ( (row,i) => {
      ...
      if(i==0){
        row = row.map(x => x.map(y => `{\\bf ${y}}`));
        row = row.map(x => x.map(y => this.polish(y)));
      }else{
        row = row.map(x => x.map(y => this.polish(y))); 
      }
    ...
    var vlines = this.xfencecmd.convlines;
    var hlines = this.xfencecmd.conhlines;
    var vpadding = this.xfencecmd.convpadding;
    var hpadding = this.xfencecmd.conhpadding;
  ...
  do_tabr(block){
    var o = [];
    var {id,row1,row2,sig,cols,caption,label,subfname} = block;
    ...
    var vlines = this.xfencecmd.convlines;
    var hlines = this.xfencecmd.conhlines;
    var vpadding = parseInt(this.xfencecmd.convpadding);
    var hpadding = parseInt(this.xfencecmd.conhpadding);
    var hlines = this.toArray(hlines);
    var vlines = this.toArray(vlines);
    ...
    var text = s.join('\n');
    if(label){
      caption = `Table ${this.xidnum} : ${this.caption_text}`;
    }
    o.push(`\\placefloat`);
    o.push(`[here]`);
    o.push(`[${label}]`);
  ...
  do_pict(block){
    var o = [];
    var {id,row1,row2,sig,rows,caption,label,subfname} = block;
    subfname=subfname||''; o.push(`%${sig} ${subfname} ${1+row1} ${1+row2}`);
    caption=caption||'';
    label=label||'';
    var s = [];
    ...
    var text = s.join('\n');
    if(label){
      caption = `Figure ${this.xidnum} : ${this.caption_text}`;
    }
    o.push(`\\placefloat`);   
    o.push(`[here]`);   
    o.push(`[${label}]`);
  ...
  do_diag(block){
    var o = [];
    var {id,row1,row2,sig,label,body,subfname} = block; 
    subfname=subfname||''; o.push(`%${sig} ${subfname} ${1+row1} ${1+row2}`);
    if(0){
      (new NitrilePreviewDiagramMP(this)).def_pic(body);
    } 
  ...
  do_math(block){
    ...
    subfname=subfname||'';
    o.push(`%MATH ${subfname} ${1+row1} ${1+row2}`);
    if(label){
      o.push(`\\placeformula`);
    }
  ...
  do_frmd(block){
    var o = [];
    var {id,row1,row2,sig,body,para,fencecmd,base,subrow,subfname} = block;
    subfname=subfname||'';
    o.push(`%FRMD ${subfname} ${1+row1} ${1+row2}`);
    var n = body.length;
    var solid = '\\ '.repeat(80);
    o.push(`\\blank`);
    o.push(`\\startMPcode`);
    ...
    o.push(`label.rt(btex {\\tt\\switchtobodyfont[12pt]${solid}} etex, (0,0));`);
    body.forEach((x,i) => {
      x = this.escapeFramed(x);
      o.push(`label.rt(btex {\\tt\\switchtobodyfont[12pt]${x}} etex, (0,-${i}*o));`);
    });
  ...
  */
  ...
  toContexDocument() {
    ...
    if(this.config.CONTEX.titlepage){
      ...
      ///TOC
      if(1){
        titlelines.push(`\\setupcombinedlist[content][list={part,chapter,section}]`);
        titlelines.push(`\\completecontent[criterium=all]`);
        titlelines.push('');
      } else {
        titlelines.push(`\\setupcombinedlist[content][list={part,section,subsection}]`);
        titlelines.push(`\\placecontent`);
        titlelines.push('');
      }
    }
...
\\stoptext
  ...
  polish(s){
    if(s){
      if(this.xmonospace && this.xconfontsize){
        s= `{\\tt\\tf${this.xconfontsize} ${s}}`;
      }else if(this.xconfontsize){
        s= `{\\tf${this.xconfontsize} ${s}}`;
      }
    }
    return s;
  }
