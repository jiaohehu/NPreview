...
class NitrilePreviewLatex extends NitrilePreviewParser {
  ...
  translateLatex (all,config) {
    ...
    for (var block of all) {
      ...
      this.row2 = row2;
      this.fname = fname;
      const label_text = (label) ? `\\label{${base}:${label}}` : '';
      ...
      switch (sig) {
        ...
        case 'SAMP': {
          var text = data;
          const xleft = config.latexSampleMargin;
          const xsize = config.latexSampleSize;
          o.push(`\\begin{flushleft}`);
          o.push(`\\begin\{adjustwidth\}{${xleft}cm}{}`)
          if (config.latexSampleWrap) {
            var text = this.wrapSample(text,config.latexSampleWrap);
          }
        ...
        case 'imgs': {
          ...
          if (fencecmd.label) {
            o.push(`\\begin{figure}`);
            o.push(`\\caption{${caption_text}}${label_text}`)
            o.push(`\\centering`);
            o.push(text);
            o.push('\\end{figure}');
            o.push('');
        ...
        case 'tabular': {
          ...
          var vlines = this.toArray(fencecmd.vlines);
          var hlines = this.toArray(fencecmd.hlines);
          var vlines = vlines.map(v => parseInt(v));
          pcol = this.insertTabularVlines(vlines,pcol.split(''));
          var header = rows.shift();
          var header = header.map(x => `\\textbf{${x}}`);
          o.push(`\\begin{center}`);
          o.push(`\\begin{tabular}{${pcol}}`);
          if (hlines.indexOf('t')>=0){
            if (fencecmd.booktabs){
              o.push('\\toprule');
            } else {
              o.push('\\hline');
            }
          ...
          o.push(this.toSingleLineRows(hlines,fencecmd.booktabs,rows));
        ...
        case 'tabulary': {
          ...
          o.push(this.toParagraphRows(hlines,fencecmd.booktabs,rows));
          if(hlines.indexOf('b')>=0){
            if(fencecmd.booktabs){
              o.push('\\bottomrule');
            }else{
        ...
        case 'tabularx': {
          ...
          ///***NOTE: tabularx is percular of naming its columns
        ...
        case 'math': {
          ...
          if (fencecmd.label) {
            ...
            if (count == 1) {
              ...
              ss = ss.map(x => x.map(y => `\\displaystyle ${y}`));
              ss = ss.map(x => x.join(' & '));
              ss = ss.map(x => `${x} \\\\`);
              if (fencecmd.alignequalsign) {
                o.push(`\\[`);
                o.push(`\\begin{array}{rl}`);
                o.push(ss.join('\n\\noalign{\\medskip}\n'));
                o.push(`\\end{array}`);
                o.push(`\\]`);
                o.push('');
        ...
        case 'listing': {
          ...
          if (caption_text) {
            myopts.push(`title={${caption_text}}`);
          }
          if (label) {
...
///console.log(o);
  ...
  toParagraphRows(hlines,booktabs,rows) {
    var o = [];
    if (rows.length == 0) {
      o.push('(empty) ');
    } else {
      for (var k in rows){
        var row = rows[k];
        if(k > 0){
          if(hlines.indexOf('*')>=0 && booktabs){
            o.push('\\midrule');
          }else if(hlines.indexOf('*')>=0){
            o.push('\\hline');
          }else if(hlines.indexOf('*1')>=0){
            o.push('\\noalign{\\smallskip}');
          }else if(hlines.indexOf('*2')>=0){
            o.push('\\noalign{\\medskip}');
          }else if(hlines.indexOf('*3')>=0){
            o.push('\\noalign{\\bigskip}');
          }else{
            ///DO NOTHING
          }
  ...
  toSingleLineRows(hlines,booktabs,rows) {
    var o = [];
    if (rows.length == 0) {
      o.push('(empty) ');
    } else {
  ...
  toLatexDocument(config,texlines) {
    ...
    var title = config.latexTitle ? config.latexTitle : 'Untitled'
    var author = config.latexAuthor ? config.latexAuthor : ''
    var documentclass = config.latexDocumentClass ? config.latexDocumentClass : ''
    if (!documentclass) {
      documentclass = config.latexIsArticle ? "article" : "report";
    }
    var documentclassopt = config.latexTwoColumnEnabled?"twocolumn":"";
    var data = `% !TEX program = ${config.latexEngine}
${mylines.join('\n')}
