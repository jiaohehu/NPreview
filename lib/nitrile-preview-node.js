...
class NitrilePreviewNode {
  ...
  async toLatex (filename) {
    ...
    parser.translateLatex(all,config);
  ...
  async toHtml (filename) {
    ...
    parser.isepub = 0;
    let [all,config] = await this.toAll(filename, parser);
    parser.translateHtml(all,config);
...
${htmls.join('\n')}
  ...
  async toEpub (filename) {
    ...
    let data = await epub.generateAsync(parser,parser.escape(title),parser.escape(author),all,imagemap,config,dirname);
