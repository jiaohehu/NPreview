...
`;
  ...
  do_dlst(block) {
    ...
    for (var item of items) {
      ...
      if(type=='text'){
        ...
        key = this.escape(key);
        text = this.unmask(text);
        o.push(`<dt style='${dtstyle}'>&#x2022; &#160; <span style='font-family:monospace;font-size:${this.config.HTML.fscode}em'>${key}</span> &#160; ${text}</dt>`);
      }
    }
    o.push('</dl>');
    o.push('');
  ...
  do_verb(block) {
    ...
    text = body.join('\n');
    text = `<table border='0' style='font-size:${this.config.HTML.fslisting}em;border-collapse:collapse;'>${text}</table>`;
...
${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
  ...
  to_outer_svg(s,w,h,mid,defs){
    ...
    opt += ` style='vertical-align:${VA}em; outline:${this.config.HTML.mathoutline}px solid orange'`;
  ...
  make_line_svg(math,label,idnum,gather){
    ...
    return {s,w,h,shiftdist,defs,label,idnum};
  }
