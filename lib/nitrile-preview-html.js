...
class NitrilePreviewHtml extends NitrilePreviewParser {
  ...
  do_verb(block) {
    ...
    if(this.floatname){
      o.push(`<figcaption> <span class='nicaption'>${this.caption_text}</span> </figcaption>`);
    }
    o.push(text);
    o.push('</figure>');
    o.push('');
  ...
  do_samp(block) {
    ...
    var text = body.join('\n');
    o.push(`<pre ${this.toATTR(block)} style='margin-left:${this.conf('step')}em;'><span class='nicode'>${text}</span></pre>`);
    o.push('');
    block.html = o.join('\n');
  }
  do_pass(block) {
  ...
  do_pict(block) {
    ...
    for(var blk of rows){
      ...
      var qq = imgs.map( img => {
        ...
        return(`<td style='${tdstyle}'> <span class='nicaption'>${sub_text}</span> </td>`);
      });
      if(mode && mode.width){
  ...
  make_math_svg(math,label,idnum,more,gather){
    ...
    if(1){///always a gather
      var data = data.map( x => {
          ...
          return (`<td style='${math_style}'>${s}</td><td style='${idnum_style}'><span class='nicaption'>${idnum_text}</span></td>`);
      });
...
`;
  ...
  toATTR(block){
    ...
    htmlattrs.push(`type='${sig.toLowerCase()}'`);
