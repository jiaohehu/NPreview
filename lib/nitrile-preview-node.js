const { NitrilePreviewLualatex, NitrilePreviewPdflatex } = require('./nitrile-preview-latex');
const { NitrilePreviewContex } = require('./nitrile-preview-contex');
const { NitrilePreviewHtml } = require('./nitrile-preview-html');
const { NitrilePreviewEpub } = require('./nitrile-preview-epub');
const { NitrilePreviewMemor } = require('./nitrile-preview-memor');
...
class NitrilePreviewNode {
  ...
  async toLualatex (filename) {

    const parser = new NitrilePreviewLualatex();
  ...
  async toPdflatex (filename) {

    const parser = new NitrilePreviewPdflatex();
  ...
  async toMemor (filename) {

    const parser = new NitrilePreviewMemor();
  ...
  async toEpub (filename) {
    ...
    const parser = new NitrilePreviewEpub();
    await parser.readFromFileAsync(filename)
    await parser.readModeAsync();
    parser.idenBlocks();
    parser.translateBlocks();

    /// create a EPUB-archieve by calling generateAsync()
    var dirname = path.dirname(filename);
    let data = await parser.generateAsync(parser,dirname);
