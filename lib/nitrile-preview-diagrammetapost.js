...
class NitrilePreviewDiagramMETAPOST extends NitrilePreviewDiagram {
  ...
  do_label(opt,label,coords){
    ...
    for (var i = 0; this.valid(coords, i); ++i) {
      ...
      var x = z0[0];
      var y = z0[1];
      var label0 = this.toString(all_labels[i], label0);
  ...
  fills() {
    var s = [];
    if (this.fillcolor) {
      s.push(`withcolor \\mpcolor{${this.fillcolor}}`);
    }
    return s.join(' ');
  }
  ...
  draws() {
    ...
    if (this.linecolor) {
      s.push(`withcolor \\mpcolor{${this.linecolor}}`);
    }
    return s.join(' ');
  }
  ...
  dots() {
    var s = [];
    if (this.dotcolor) {
      s.push(`withcolor \\mpcolor{${this.dotcolor}}`);
    }
    return s.join(' ');
  }
  ...
  my_shape(align,coords,mypath,w,h,sx,sy) {     
    ...
    for (var i = 0; this.valid(coords, i); i++) {
      ...
      var y = z0[1];
      if (this.fillcolor) {
        s.push(`fill (${mypath}) ${myshifted} xscaled(${sx}) yscaled(${sy}) shifted(${x},${y}) scaled(u) ${this.fills()};`);
      }
      if (this.linewidth !== '0') {
