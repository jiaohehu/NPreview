...
console.log("observed acitve editor:");
console.log(item.id);
console.log(this.id);
          if (!this.id) {

console.log("going to switch editor!");

            this.switchEditor(item);
            this.updateBlocks();

          } else if (item.id && item.id.toString() === this.id.toString()) {
...
console.log('autoScroll is on');
  ...
  async getBlocks() {
    ...
    this.book = book;
    if (book.length) {
      this.subs = this.parser.parseFILES(this.book);
      var promises = [];
      for (var sub of this.subs) {
        promises.push(this.getSubFlow(sub));
      }
    }
    return [blocks,flags,book];
  ...
  getSubFlow (sub) {
    ...
    if (sublevel < 0) {                
      sub.flow = this.parser.toBlocks([subpart],subfname,subrow,sublevel);

    } else if (this.myflows.has(subfname)) {
      sub.flow = this.myflows.get(subfname);

    } else if (!this.myfiles.has(subfname)) {
      var fsubfname = path.join(this.dirname(),subfname);
      var file = new File(fsubfname);
      this.myfiles.set(subfname,file);
      this.disposables.add(
        file.onDidChange( () => {
            this.fetchSubFlow(sub, fsubfname);
        })
      );
      this.fetchSubFlow(sub, fsubfname);
    }
  ...
  fetchSubFlow (sub, fsubfname) {
    this.readFileAsync(fsubfname).then( out => {
            sub.flow = this.parser.toBlocks(out.split('\n'),sub.subfname,sub.subrow,sub.sublevel);
            this.myflows.set(sub.subfname,sub.flow);
            console.log(`subfname ${sub.subfname} reloaded: length: ${sub.flow[0].length}`);
        }).catch( e => {
            var o = [];
            o.push(this.parser.newHDGS(e.toString(),sub.subfname,sub.subrow,sub.sublevel));
            sub.flow = [o,{},[]];
            this.myflows.set(sub.subfname,sub.flow);
            console.log(`subfname ${sub.subfname} load error: ${e.toString()}`);
        });
  }
  ...
  updateBlocks() {
    ...
    this.getAllBlocks().then( ([blocks,flags,book]) => {
      ...
      /// also register for watch for file content changes for sub-document
      this.subs = this.parser.parseFILES(this.book);
      for (var sub of this.subs) {
        this.getSubFlow(sub);
      }
