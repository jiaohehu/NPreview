...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  do_identify(block, A) {
    ...
    var { sig, hdgn, subn, name, style, floatname, parser } = block;
    subn = subn||0;
    name = name||'';
    /// generate css ID
    A.id++;
    ...
    if (sig == 'PART') {
      ...
      if(block.style.n && block.style.n > 1){
        for(let i=1; i < block.style.n; ++i) {
          idnum = A.floats.get(floatname);
          idnum += 1;
          A.floats.set(floatname,idnum);
          if (A.chapters) {
  ...
  do_samp(block) {
    ...
    o.push(`<blockquote><pre >${text}</pre></blockquote>`);
    o.push('');
    block.html = o.join('\n');
  }
  do_hrle(block) {
    ...
    o.push(this.to_info(block));
    text = this.unmask(text);
    o.push(`<hr style='text-align:center' />`);
    o.push('');
    block.html = o.join('\n');
  }
  do_prim(block) {
  ...
  do_text(block) {
    ...
    let text = this.untext(body);
    if(nspace){
      o.push(`<blockquote>${text}</blockquote>`);
    }else{
      o.push(`<p>${text}</p>`);
    }
    o.push('');
  ...
  to_math(itms,style) {
    var text = this.math_to_htmltable(itms,style.idnum,style.label,style.floatname,style.align);
    return text;
  }
  to_img(g) {
    var src = g.src;
  ...
  to_rows(itms,style){
    ...
    let text = this.itms_to_rows(itms,style.n);
    return text;
  }
  to_tabulate(rows,style) {
    ...
    let text = this.itms_to_tabulate(rows,style.border);
    return text;
  }
  to_longtable(itms, style) {
    ...
    return this.itms_to_longtable(itms, style.fr, style.hline, style.glue, style.n, caption, label, style.floatname);
  }
  to_multi(itms,style){
    itms = itms.map(s => {
      s = this.unmask(s);
...
`;
  ...
  pp_to_multi_itemized(itms,fr,n,name){
    var d = [];
...
${j+n >= itms.length && hlines.indexOf('b') >= 0 ? 'border-bottom:1px solid;' : ''} '>${x}</td>`);
  ...
  math_to_htmltable(itms,idnum,label,floatname,align) {
    idnum = idnum||'';
    label = label||'';
    var idnums = idnum.split(',');
    var labels = label.split(',');
    let islabeled = (floatname == 'Equation');
    var data = itms.map( (p,i) => {
      let idnum = idnums[i]||'';
      let label = labels[i]||'';
      var line = this.make_line_svg(p.math, label, islabeled, idnum, 0);
      return line;
    });
    ...
    if (1) {///always a gather and flushleft
      var data = data.map(x => {
        var { s, w, label, idnum } = x;
        var dist = (max_w - w) / 2;
        if (idnum) {
  ...
  to_info(block){
    return `<span ${this.to_attr(block)}/>`;
  }
}
