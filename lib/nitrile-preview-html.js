...
${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
  ...
  do_diag(block){
    ...
    var o = []; 
    lines=lines||[];
    var {s,vw,vh} = this.diagram.to_diagram(lines,this.parser.block.parser.notes);
  ...
  do_frmd(block) {
    ...
    var o = []; 
    lines=lines||[];
    var {s, vw, vh} = this.framed.to_framed(lines);
    var text = `<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' fill='currentColor' stroke='currentColor' viewBox='0 0 ${vw} ${vh}' style='width:100%; ' >${s}</svg>`;
    if (islabeled) {
      o.push(`<figure ${this.to_attr(block)} >`);
      o.push(this.to_caption_text(floatname, idnum, caption));
  ...
  to_diagram(ss,style){
    var { s, vw, vh } = this.diagram.to_diagram(ss,this.parser.block.parser.notes);
    if (style && style.width) {
      let str = style.width;
      str = this.str_to_css_width(str);
  ...
  to_framed(ss, style) {
    var { s, vw, vh } = this.framed.to_framed(ss);
    let bs = `border:1px solid;padding:1px;box-sizing;border-box;`
    if (style && style.width) {
      let str = style.width;
      str = this.str_to_css_width(str);
      var text = `<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' fill='currentColor' stroke='currentColor' viewBox='0 0 ${vw} ${vh}' style='${bs};width:${str}; ' >${s}</svg>`;
    } else {
      var text = `<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' fill='currentColor' stroke='currentColor' viewBox='0 0 ${vw} ${vh}' style='${bs};width:100%; ' >${s}</svg>`;
    }
    return text;
  }
  to_verbatim(ss){
