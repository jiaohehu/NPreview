...
\\setuplayout
 [header=5em,
  ...
  cutspace=4cm,
  width=middle,
  height=\\dimexpr\\paperheight-\\topspace+\\footerheight+\\footerdistance-5cm\\relax]
\\setupindenting[yes,medium]
\\setscript[hanzi] % hyphenation
\\setuphead[chapter][style=bold,number=yes]
\\setuphead[section][style=bold,number=yes]
\\setuphead[subsection][style=bold,number=yes]
\\setuphead[subsubsection][style=bold,number=yes]
...
\\stoptext
  ...
  toDisplayMath(o,id,sig,fname,row1,row2,data,fencecmd,isalignequalsign) {
    if (0 && isalignequalsign) {
      ...
      data = data.map(x => `\\startformula ${x} \\stopformula`);
      var text = data.join('\n');
      o.push(text);
      o.push('');
    }
  ...
  toEquation(o,id,sig,fname,row1,row2,data,fencecmd,isalignequalsign) {
    ...
    if (0 && isalignequalsign) {
      ...
      ss = ss.map(x => `\\startformula ${x} \\stopformula`);
      o.push(`\\placeformula`);
      o.push(ss.join('\n'));
      o.push('');
    }
