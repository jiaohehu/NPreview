...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  do_prog(block) {
    ...
    var text = lines.join('\n');
    var text = `<table class='PROG' border='0' style='border-collapse:collapse;'>${text}</table>`;
...
${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
  ...
  do_diag(block){
    ...
    var o = []; 
    lines=lines||[];
    var {s,vw,vh} = this.diagram.toDiagram(lines,notes);
  ...
  /// escape the string for dialog
  smooth_for_diag(unsafe) {
    unsafe = '' + unsafe; /// force it to be a string when it can be a interger
    return unsafe
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
  ...
  style (type, text) {
    ...
    switch (type) {
      ...
      case 'var': {
        text = text.trim();
        return `<var>${this.smooth_for_diag(text)}</var>`
        break;
      }
      default: {
