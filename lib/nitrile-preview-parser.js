  ...
  toBlocks(lines,fname,subrow,plevel,o=null) {
    ...
    while (lines.length > 0) {
      ...
      if ((v = this.re_sbjt.exec(para[0])) !== null) {
        var data = v[1];
        n = 0;
        o.push([id,row,row2,'SBJT',0,data,para,fencecmd,dept,fig,caption,base,label,fname,subrow,plevel]);
        caption = ''; label = ''; fencecmd = {};
        continue;
      }
  ...
  idenSubs (config,subs) {
    ...
    for (var sub of subs) {
      ...
      for (subblock of subblocks) {
        subblock[P_fname]  = sub.subfname;
        subblock[P_subrow] = sub.subrow;
        subblock[P_plevel] = sub.sublevel;
        o.push(subblock);
      }
    }
