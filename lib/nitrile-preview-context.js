...
class NitrilePreviewContext extends NitrilePreviewTranslator {
  ...
  constructor(parser) {
    ...
    this.config.title = '';
    this.config.author = '';
    this.config.papersize = '';///A4,A5, etc.
    this.config.layout='';//this should be done by +=
    this.config.bodyfont='';//should be +=, for \setupbodyfont[] 
    this.config.diagfontsizept = 12;
    this.config.distance = 2;///inter-image dist in percentage of page width for PICT
    this.config.twocolumn = 0;///set to 1 to enable
    this.config.maxn = 44;//maximum line number for each "float" Program
    this.config.chapter = '\\bfd';
    this.config.section = '\\bfa';
    this.config.subsection = '\\bf';
    this.config.subsubsection = '\\bf';
    this.config.subsubsubsection = '\\bf';
    this.config.toc = 0;///when set to 1 '\placecontent' will be inserted
    this.config.titlepage = 0;//1=title page will be generated
    this.config.trace = 0;
    this.config.step = 5;//5mm left-padding for some
    this.config.nicaption = 'sm';
    this.config.nipass = 'sm';
    this.config.nisamp = 'sm';
    this.config.nitabr = 'sm';
    this.config.nilong = 'sm';
    this.config.nitabb = 'sm';
    this.config.nitabu = 'sm';
    this.config.niprog = 'xsm';
    this.config.longpadding = '1 3';
    this.config.longvlines = '*';
    this.config.longhlines = 't m b r';
    ...
    while(n < lines.length){
      if(1){
        ...
        for(let i=0; i < maxn && n < lines.length; ++i,++n){
          ...
          var line = this.fontify_latex(line);
  ...
  do_tabr(block){
    ...
    //var text = this.tabr_cols_to_btable(cols);
    if(islabeled){
      o.push(`\\placetable`);
      o.push(`[here]`);
      o.push(`[${label}]`);
  ...
  polish(unsafe){
    ...
    unsafe = this.fontify_context(unsafe);
    return unsafe;
  }
  ...
  smooth (unsafe) {
    ...
    unsafe = unsafe.replace(/\$/g,    "\0char36\0")
    unsafe = unsafe.replace(/\\\\/g,  "\0char92\0")
    unsafe = unsafe.replace(/_/g,     "\0char95\0") // these need to happen after because _ and ^ could be a subscript or superscript
    unsafe = unsafe.replace(/\^/g,    "\0char94\0")
    unsafe = unsafe.replace(/\{/g,    "\0char123\0")
    unsafe = unsafe.replace(/\}/g,    "\0char125\0")
    unsafe = unsafe.replace(/\\/g,    "\0char92\0")
  ...
  to_context_document() {
    ...
    if(this.conf('papersize')){
      var s=`\\setuppapersize[${this.conf('papersize')}]`;
      p_papersize=s;
    }
    /*
