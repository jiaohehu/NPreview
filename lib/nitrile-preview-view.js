        ...
        editor.getBuffer().onDidStopChanging(() => {
          ...
          if (this.editor) {
            if (atom.config.get('nitrile-preview.liveUpdate')) {
              console.log('live update is enabled, going to update');
              this.lines = this.editor.getBuffer().getLines();
              this.updateBlocks();
            } else {
              console.log('live update is disabled, no update');
            }
          }
        })
      );
...
console.log('NitrilePreviewView::destroy() called');
  ...
  async getAllBlocks() {
    ...
    var [blocks,flags,book] = this.parser.toBlocks(this.lines,fname,0);
    if (book.length) {
      [blocks,flags] = await this.parser.toBookBlocks(book,this.dirname());
    }
  ...
  refreshView() {
    ...
    this.htmls.push(`</p>`);
    this.htmls = this.parser.translateHtml(this.toConfig(),this.blocks,this.isarticle,this.hiddendepts,this.htmls);
    this.htmls.push(`</div>`);
  ...
  updateBlocks() {
    ...
    this.getAllBlocks().then( ([blocks,flags,book]) => {
      ...
      var { title = '', author = '', outline = '' } = flags;
      title = title ? title : 'Untitled';

      var {documentclass=''} = flags;
      if (isarticle) {
      ...
      var isoutline = this.getBool(outline);
      blocks = this.parser.idenBlocks(blocks,isarticle,isoutline);
  ...
  openLinkedFile (event) {
    ...
    if (node) {
      var nodeName = ''+node.nodeName;
      var className = ''+node.className;
      var isOutline = node.getAttribute('isOutline');
      var fName = node.getAttribute('fName');
      if (nodeName === 'MARK' && fName) {
...
console.log('toggleSectionalContents');
    ...
    if (node) {
      ...
      var nodeName = ''+node.nodeName;
      var className = ''+node.className;
      var dept = node.getAttribute('dept');
      ...
      if (nodeName === 'MARK' && dept ) {
        console.log('ok')
        if (this.hiddendepts.hasOwnProperty(dept)) {
          delete this.hiddendepts[dept];
console.log(`deleted ${dept}`);
console.log(this.hiddendepts);
        } else {
          this.hiddendepts[dept] = true;
console.log(`added ${dept}`);
console.log(this.hiddendepts);
        }
        this.blocks = this.parser.hideBlocks(this.blocks,this.hiddendepts);
        this.refreshView();
      }
    }
  }
  ...
  async saveAs (savefilepath) {
    ...
    try {
      ...
      if (this.saveastype === 'latex-chapter') {
        ...
        var olines = latex.translateLatex(this.toConfig(),this.blocks,false);
        data = olines.join('\n');

      } else if (this.saveastype === 'latex-complete') {

        var latex = new NitrilePreviewLatex();
        var olines = latex.translateLatex(this.toConfig(),this.blocks,this.article);
        var texFamily = atom.config.get('nitrile-preview.texFamily');
...
<div class='PAGE'>
<p class='TITLE'> ${this.parser.escape(this.title)} </p>
<p class='AUTHOR'> ${this.parser.escape(this.author)} </p>
<p class='DATE'> ${this.parser.escape(new Date().toLocaleDateString())} </p>
${this.htmls.join('\n')}
</div>
</body>
</html>`;
