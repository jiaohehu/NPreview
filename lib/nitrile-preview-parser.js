...
class NitrilePreviewParser {
  ...
  toBLOCKS(lines,fname,plevel,o) {
    ...
    */
    ...
    /// a %!BOOK block
    if (lines.length && lines[0].match(/^\%\!BOOK/)) {
    ...
    while (lines.length > 0) {
      ...
      if (v) {
        var ins = v[1];
        if (para[1]) {
          var v2 = para[1].match(this.re_local);
          if (v2) {
      ...
      if (this.flags['verbatim'] && para[0].match(/^\s\s\s\s/)) {
      ...
      /// this is the DESC block
      var re = /^(\@)\s+(\S.*)$/;
      if (re.test(para[0])) {
        var data = this.parseDESC(para,re);
      ...
      if ((v = re.exec(para[0])) !== null) {
        ...
        var text = v[2];
        if (para[1]) {
          var v1 = para[1].match(this.re_local);
          if (v1) {
            var local = v1[1];
  ...
  trimFences (para) {
    ...
    */
    ...
    return [para,the_fence,this.splitFenceCmd(the_fencecmd)];
  }
  ...
  parseTABB (para) {
    ...
    if (para.length) {
      ...
      if (pp.length === 1) {
        /// either 'lines' or 'colons'
        if (this.re_tabb_colons.test(para[0])) {
          form = 'colons';
        } else {
          form = 'lines';
        }
    ...
    if (form === 'colons') {
      ...
      var para2 = [];
      for (var line of para) {
        var v = this.re_tabb_colons.exec(line); 
        if (!line) {
          para2.push('');
        } else if (v) {
          para2.push(v[1]);
          ...
          if (para2.length) {
            para2[para2.length-1] = this.joinLine(para2[para2.length-1],line);
          }
        }
      }
      para = para2;
