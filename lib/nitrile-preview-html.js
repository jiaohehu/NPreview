...
//console.log(data);
        ...
        case '': {
          ...
          if (is_sample) {
            ...
            o.push(`${showidtext}`);
            //var text = this.wrapSample(text,'html',config);
            ...
            o.push(`${showidtext}`);
            o.push(`<p style='${this.paracssstyle}' >`);
            /// Add five #160 to simulate a text indent.
            ///
            ///***IMPORTANT: Need to use nbsp and not text-indent: because text-indent is broken!.
            ///  Observed in Safari as well as in Firefox all math SVG are weired. Safari display them with leading spaces
            ///  Firefox squish them and they look like a thin vertical line. ***cannot use text-indent***
            if (normalparacount > 0) {
              o.push(`${'&#160;'.repeat(5)}`);
            }
            if (lead1) {
              o.push(`<b>${lead1}</b>`);
            } else if (lead2) {
              o.push(`<b><i>${lead2}</i></b>`);
            }
            o.push(text);
            o.push(`</p>`);
            o.push('</div>');
