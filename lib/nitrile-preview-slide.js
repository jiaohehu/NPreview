...
`;
  ...
  to_html_solution(tops){
    let my = tops.shift();
    let d = [];
    //let title = `${this.unmask(my.text)} <button onclick="document.getElementById('${my.idnum}').hidden^=true">&#x2026;</button>`;
    let title = ` <button shown='false' onclick="document.getElementById('${my.idnum}').hidden^=true, this.shown^=true, this.innerHTML=this.shown?'&#x25BE;':'&#x25B8;'">&#x25B8;</button> ${this.unmask(my.text)}`;
