...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  untext(ss) {
    ...
    let style = {};
    let v;
    if(ss[0].startsWith('~~~')&&ss[n-1].startsWith('~~~')){ 
      let s0 = ss[0].slice(3);
      ss = ss.slice(1,n-1);
      if(ss.length){
      ...
      if(this.para_is_rows(ss)){
        let data = this.para_to_rows(ss);
        return this.to_rows(data,style);
      }
      ...
      if (v[1] !== undefined) {
        ...
        if(ss.length){
          ...
          if(fenceid==='diagram'){
            ...
            let pp = this.para_to_math(ss);
            newtext += this.to_math(pp,style);
          }else{
