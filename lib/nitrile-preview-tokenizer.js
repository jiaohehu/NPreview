...
console.error(g);
  ...
  getNextGroup (tokens,isskipblank) {
    ...
    if (tokens[1] === '_' && tokens[3] === '^') {
      [tokens,token0] = this.getThisToken(tokens);
      [tokens,token1] = this.getThisToken(tokens);
      [tokens,group2] = this.getNextGroup(tokens,true);
      [tokens,token3] = this.getThisToken(tokens);
      [tokens,group4] = this.getNextGroup(tokens,true);
      return [tokens,['\\subsup',token0,group2,group4]];
    }
    if (tokens[1] === '^' ) {
      [tokens,token0] = this.getThisToken(tokens);
      [tokens,token1] = this.getThisToken(tokens);
      [tokens,group2] = this.getNextGroup(tokens,true);
      return [tokens,['\\sup',token0,group2]];
    }
    if (tokens[1] === '_' ) {
      [tokens,token0] = this.getThisToken(tokens);
      [tokens,token1] = this.getThisToken(tokens);
      [tokens,group2] = this.getNextGroup(tokens,true);
      return [tokens,['\\sub',token0,group2]];
    }
    group = tokens[0];
    tokens = tokens.slice(1);
    return [tokens,group];
    ...
    if (Array.isArray(v)) {
      switch (v[0]) {
        ...
        case '\\sup':  
          ...
          var o = [];
          o.push(s1_);
          o.push(`<svg x='${w1_}pt' y='0' width='${w2_/2.0}' height='${h2_/2.0}' viewbox='0 0 ${w2_*1.333} ${h2_*1.333}'>`);
          o.push(s2_);
          o.push(`</svg>`);
          return [w1_+(w2_/2.0),h1_,mid1_,o.join('\n')];
          break;
