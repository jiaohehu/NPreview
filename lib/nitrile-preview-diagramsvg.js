...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_finalize(s) {
    ...
    o.push('<defs>');
    o.push(`<marker id='markerArrow' markerWidth='13' markerHeight='13' refX='10' refY='6' orient='auto'> <path d='M2,4 L2,8 L10,6 z' stroke='context-stroke' fill='context-stroke'/>`);
    o.push(`<marker id='startArrow' markerWidth='13' markerHeight='13' refX='3' refY='6' orient='auto'> <path d='M11,4 L11,8 L3,6 z' stroke='context-stroke' fill='context-stroke'/>`);
    o.push('</defs>');
    o.push(s);    
    return [o.join('\n'),vw,vh];
  }
  ...
  do_apple(opt,txt,coords) {
    ...
    var p = this.readCoordsLine(null,mypath);
    var u = this.u;
    for (var i = 0; this.valid(coords, i); i++) {
      var z0 = this.point(coords, i);
      var x = (z0[0]);
      var y = (z0[1]);
      if (opt === '.top') {
        var p = this.shiftcoords(p,-0.5,0);
        var p = this.shiftcoords(p,x,y);
        var d = this.coordsToD(p);
      } else if (opt === '.bot') {
        var p = this.shiftcoords(p,-0.5,-1);
        var p = this.shiftcoords(p,x,y);
        var d = this.coordsToD(p);
      } else if (opt === '.rt') {
        var p = this.shiftcoords(p,0,-0.5);
        var p = this.shiftcoords(p,x,y);
        var d = this.coordsToD(p);
      } else if (opt === '.lft') {
        var p = this.shiftcoords(p,-1,-0.5);
        var p = this.shiftcoords(p,x,y);
        var d = this.coordsToD(p);
      } else {
        var p = this.shiftcoords(p,-0.5,-0.5);
        var p = this.shiftcoords(p,x,y);
        var d = this.coordsToD(p);
      }
