...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  constructor(parser) {
    ...
    this.config.bodyfontsize = 12; // font size for the entire document
    this.config.mathfontsize = 12; // font size for SVG generated in 'pt'
...
${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
  ...
  smooth (unsafe) {
    ...
    unsafe = unsafe.replace(this.re_all_diacritics, (match,p1,p2) => {
          // a~dot, a~ddot, a~bar ...
          switch(p2){
            case 'dot':      return `${p1}&#x0307;`;
            case 'ddot':     return `${p1}&#x0308;`;
            case 'bar':      return `${p1}&#x0305;`;
            case 'mathring': return `${p1}&#x030A;`;
            case 'hat':      return `${p1}&#x0302;`;
            case 'check':    return `${p1}&#x030C;`;
            case 'grave':    return `${p1}&#x0300;`;
            case 'acute':    return `${p1}&#x0301;`;
            case 'breve':    return `${p1}&#x0306;`;
            case 'tilde':    return `${p1}&#x0303;`;
            default: return match;
          }
      })
    ...
    unsafe = unsafe.replace(this.re_all_symbols, (match,p1) => {
          try{
            var v = this.tokenizer.get_symbol(p1,'html');
            return v;
          }catch(e){
            return match;
          }
      })
    unsafe = unsafe.replace(/\\\\/g,  "\\")//should be the last
    return unsafe;
  }
...
`;
  ...
  to_bodyfontsize(){
    return this.conf('bodyfontsize');
  }
  to_mathfontsize(){
    return this.conf('mathfontsize');
  }
}
