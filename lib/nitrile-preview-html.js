...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  do_identify(block, A) {
    ...
    if (sig == 'PART') {
      ...
      block.idnum = idnum;
    } else if ((sig == 'MATH' || sig=='QUAT') && islabeled) {
      if (block.more && block.more.length) {
        block.more.forEach(x => {
          A.equations++;
          if(name=='h'){
            x.idnum = `${A.chapters}-${A.equations}`;
          }else{
            x.idnum = `${A.equations}`;
          }
        })
  ...
  do_plst(block) {
    ...
    for (var item of items) {
      ...
      if(more && more.length){
        more.forEach((plitem) => {
          let {lines,type} = plitem;
          lines = lines.map(x => this.polish(x));
          if(type==='pre'){
            text += `<pre>${lines.join('\n')}</pre>`;
          }else{
            text += `<p>${this.unmask(this.join_para(lines))}</p>`;
          }
        });
      }
      switch (bull) {
        case 'OL': {
          if(count==0){
            count++;
            o.push(`<ol ${this.to_attr(block)} ><li value='${value}'>${text}`);
          }else{
            o.push(`<ol ><li value='${value}'>${text}`);
          }
          break;
        }
        case 'UL': {
          if(count==0){
            count++;
            o.push(`<ul ${this.to_attr(block)} ><li value='${value}'>${text}`);
          }else{
            o.push(`<ul ><li value='${value}'>${text}`);
          }
          break;
        }
        case 'LI': {
          o.push(`</li>`);
          o.push(`<li value='${value}'>${text}`);
          break;
        }
        case '/OL': {
          o.push(`</li>`);
          if(bullet){
            o.push(`</ol></li><li value='${value}'>${text}`);
          }else{
            o.push(`</ol>`);
          }
          break;
        ...
        case '/UL': {
          o.push(`</li>`);
          if(bullet){
            o.push(`</ul></li><li value='${value}'>${text}`);
          }else{
            o.push(`</ul>`);
          }
          break;
  ...
  do_text(block) {
    var {id,row1,row2,sig,leadn,lead,text,isverse,isstory} = block;
    var o = []; 
...
${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
  ...
  to_verbatim(ss){
    ss = ss.map(x => this.polish(x));
    return `<code style='white-space:pre'>${ss.join('\n')}</code>`;
  }
  to_style(text,type) {
