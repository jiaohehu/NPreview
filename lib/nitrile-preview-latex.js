...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  do_hdgs(block){
    var {hdgn,text,label,parser} = block;
    var o = [];
    o.push('');
    o.push(this.to_info(block));
    var raw = text;
    ...
    var subn=parser&&parser.sublevel||0;
    hdgn+=subn;
    var star='';
    if(hdgn==0){
      if(parser&&parser.subname=='h'){              
  ...
  do_plst(block){
    ...
    for (var item of items) {
      ...
      switch (bull) {
        case 'OL': {
          o.push(`\\begin{enumerate}[${nosep},style=unboxed,leftmargin=*]`);
          break;
        }
        case 'UL': {
          o.push(`\\begin{itemize}[${nosep},style=unboxed,leftmargin=*]`);
          break;
        }
        case 'DL': {
          o.push(`\\begin{description}[${nosep},style=unboxed,font=\\normalfont]`)
        ...
        case 'LI': {
          if (dl){
            ...
            if (desc) {
              o.push(`\\item[{\\bfseries{}${key}}] \\hfill\\\\${desc}${extra_text}`);
            } else {
              o.push(`\\item[{\\bfseries{}${key}}]`);
            }
          } else if (ds) {
            let { key, desc, cat, rb, rt, q } = ds;
            var nbsp = '\\ ';
            if (cat == 'rmap') {
              key = `${rb}${String.fromCharCode('0xb7')}${rt}`;
              key = this.polish(key);
              desc = this.unmask(desc);
              o.push(`\\item \\textbf{${key}}  ${nbsp}${desc}${extra_text}`);
            }
            else if (cat == 'quoted') {
              key = `${q}${key}${q}`;
              key = this.polish(key);
              desc = this.unmask(desc);
              o.push(`\\item \\texttt{\\textbf{${key}}}  ${nbsp}${desc}${extra_text}`);
            }
            else {
              key = this.polish(key);
              desc = this.unmask(desc);
  ...
  do_samp(block){
    ...
    o.push(`\\begin{flushleft}`);
    o.push(`\\begin{enumerate}[nosep,style=unboxed,leftmargin=${this.to_conf_step()}mm]`);
    o.push(lines.join('\\\\\n'));
    o.push('\\end\{enumerate\}');
    o.push('\\end\{flushleft\}')
    block.latex = o.join('\n');
  ...
  to_tabular(rows) {
    ...
    let ww = rows.map(row => row.length);
    let ncols = ww.reduce((acc,cur) => Math.max(acc,cur),0);
    ww = '1'.repeat(ncols).split('');
    let text = this.rows_to_tabular(rows,ww,{},0);
    return text;
  }
  to_style (text,type) {
    type = type || '';
  ...
  to_info(block) {
    var { name, hdgn, sig, row1, row2, parser } = block;
    var subf = parser&&parser.fname || '';
    var name = parser&&parser.subname || '';
    var subn = parser&&parser.sublevel || '';
    var sig = sig || '';
    var row1 = row1 || 0;
    var row2 = row2 || 0;
    var hdgn = hdgn || 0;
    return (`%${sig}:${hdgn}:${name} {subf:${subf}:${subn}} {row:${row1}:${row2}}`);
  }
