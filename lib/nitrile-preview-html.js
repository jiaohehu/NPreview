...
`
  ...
  translateHtml (all,config,ispreview=false,editorrow=-1,editorcolumn=-1) {
    ...
    this.block = [];
    this.config = config;
    this.isarticle = !config.latex.isReport;
    this.autonum.start();
    for (var block of all) {
      ...
      this.fontsize = fencecmd.fontsize;
      console.log('james: setting from fencecmd: this.fontsize =',this.fontsize);
...
//console.log(data);
  ...
  */
  ...
  style (type, text) {
    ...
    switch (type) {
      case 'inlinemath': {
        var s = this.tokenizer.parse(text,this.fontsize);
