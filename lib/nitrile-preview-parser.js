...
class NitrilePreviewParser {
  ...
  newConfig() {
    ...
    config.LATEX.documentclass='';///set to 'book','scrbook', etc., default to 'report' if 'config.LATEX.haschapter'  or 'article' if not
    config.LATEX.coverpage='';
    config.LATEX.haschapter = 0;
    ...
    for (var line of para) {
      ...
      if (action === 'push') {
        if (bullet.endsWith('.') || bullet.endsWith(')')) {
          items.push(['OL',bullet,value,text]);
          levels.push([lead,'OL']);
          bull = 'OL';
        } else if (bullet == '--') {
          items.push(['OL',bullet,value,text]);
          ...
          items.push(['UL',bullet,value,text]);
          levels.push([lead,'UL']);
          bull = 'UL';
        }
      } else if (action === 'pop') {
        ...
        if (bull === 'OL') {
          items.push(['LI',bullet,value,text]);
        } else {
          items.push(['LI',bullet,value,text]);
        }
      } else {
