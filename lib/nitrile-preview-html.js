...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  do_dlst(block) {
    ...
    var o = []; 
    const dtstyle=`text-indent:-${this.to_conf_step()}em;margin-left:${this.to_conf_step()}em;`;
    ...
    for (var item of items) {
      ...
      if(type=='rmap'){
        ...
        key = `${quote}${key}${quote}`;
        text = this.unmask(text);
        o.push(`<div style='${dtstyle}'>${bull} <tt><strong>${key}</strong></tt>  ${text}</div>`);
      }
      else{
        key = this.polish(key);
        text = this.unmask(text);
  ...
  do_samp(block) {
    ...
    if(parser.sample==1){
      ...
      if(this.is_line_indented(lines[0])){
        ...
        o.push(`<p ${this.to_attr(block)} style='margin-left:${this.to_conf_step()}em;'>${text}</p>`);
        o.push('');
        ...
        o.push(`<dl ${this.to_attr(block)} style='margin-left:${this.to_conf_step()}em;'>${text}</dl>`);
        o.push('');
      }else{
      ...
      var text = lines.join('\n');
      o.push(`<pre ${this.to_attr(block)} style='margin-left:${this.to_conf_step()}em;'>${text}</pre>`);
      o.push('');
      block.html = o.join('\n');
    }
  }
  ...
  do_quot(block) {
    ...
    text = `${lq}${text}${rq}`;
    var o = [];
    o.push(`<blockquote ${this.to_attr(block)} style='margin-left:${this.to_conf_step()}em;right-margin:0;${this.xstyle}'>`);
    o.push(text);
    o.push(`</blockquote>`);
    o.push('');
    block.html = o.join('\n');
  ...
  do_data(block){
    ...
    var o = []; 
    var text = this.rows_to_table(rows);
    o.push(`<dl ${this.to_attr(block)} style='margin-left:${this.to_conf_step()}mm' >${text}</dl>`);
    o.push('');
    block.html = o.join('\n');
  }
  do_tabr(block){
...
${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
  ...
  do_ref (sig,label,floatname,idnum,refid,id,chapters){
    if(sig=='HDGS'){
      return `<a href='#${id}'>Section&#160;${idnum}</a>`;
    }
    else if(sig=='MATH'){
      return `<a href='#${id}'>Expression&#160;(${idnum})</a>`;
    }
    if(floatname){
      return `<a href='#${id}'>${floatname}&#160;${idnum}</a>`;
    }
