...
class NitrilePreviewLatex extends NitrilePreviewParser {
  ...
  translateLatex (all,config) {
    ...
    /// the 'blocks' argument is an array of blocks; the 'isreport'
    ...
    for (var block of all) {
      ...
      this.fs = fencecmd.fs;
      this.xsize = this.toLatexFontsize(this.fs);
      ...
      switch (sig) {
        ...
        case 'DLST': {
          ...
          o.push(`\\begin{description}\[nosep,style=unboxed,leftmargin=${this.xleft},font=\\normalfont]`);
        ...
        case 'PLST': {
          ...
          for (var item of data) {
            ...
            if (item.length === 3) {
              switch (lead) {
                case 'OL': {
                  o.push(`\\begin{enumerate}[nosep,leftmargin=${this.xleft}]`);
                  o.push(`\\item\[${value}.\] ${text}`);
                  break;
                }
                case 'UL': {
                  o.push(`\\begin{itemize}[nosep,leftmargin=${this.xleft}]`);
                  o.push(`\\item ${text}`);
                  break;
                }
                case 'LI': {
        ...
        case 'VERB': {
          ...
          if (name === 'listing') {
            this.toListing(o,id,sig,dept,fname,row1,row2,data,fencecmd);
          } else if (this.xnumbers) {
            o.push(`\\begin{Verbatim}[numbers=${this.xnumbers},numbersep=${this.xnumbersep}]`);
            o.push(text.join('\n'));
            o.push(`\\end{Verbatim}`);
            o.push('');
          } else if (1) {
        ...
        case 'ITEM': 
          var text = data;
          o.push(`\\begin{flushleft}`);
          o.push(`\\begin\{adjustwidth\}{${this.xleft}}{}`)
          text = text.join('\n');
          text = this.unmask(text);
          text = `{${this.xsize}{}${text}}`;
          o.push(text);
        ...
        case 'LIST':
          ...
          text.unshift(`\\begin{description}[nosep,style=unboxed,leftmargin=0,font=\\normalfont${this.xsize}]`);
          text.push(`\\end{description}`);
          text = text.join('\n');
          o.push(`\\begin{flushleft}`);
          o.push(`\\begin\{adjustwidth\}{${this.xleft}}{}`);
          o.push(text);
        ...
        case 'TBLR': {
          ...
          var header = header.map(x => `{${this.xsize}{}${x}}`);
          if (1) {
            ///buid table into 's'
            var s = [];
          ...
          if (this.caption_text) {
            ...
            o.push(`\\caption{${this.caption_text}}${this.label_cmd}`);
            o.push(s.join('\n'));
            o.push(`\\end{table}`);
          } else {
            o.push(`\\begin{center}`);
...
\\usepackage[export]{adjustbox}`
  ...
  toLatexDocument(config,texlines) {
    ...
    if (!documentclass) {
      documentclass = (this.isreport) ? "report" : "article";
    }
...
\\end{document}\n`;
  ...
  toEquation(o,id,sig,dept,fname,row1,row2,data,fencecmd,isalignequalsign) {
    ...
    o.push(`\\begin{equation${star}}${this.label_cmd}`);
    o.push(`\\begin{split}`);
    o.push(ss.join('\\\\\n'));
    o.push(`\\end{split}`);
  ...
  toEquations(o,id,sig,dept,fname,row1,row2,data,fencecmd,isalignequalsign) {
    ...
    var star = fencecmd.star||'';
    if (isalignequalsign) {
      o.push(`\\begin{align${star}}${this.label_cmd}`);
    } else {
      o.push(`\\begin{gather${star}}${this.label_cmd}`);
    }
    var ss = [];
    for(var bl of bls) {
      var s0 = bl.join(' ');
  ...
  toSubequations(o,id,sig,dept,fname,row1,row2,data,fencecmd) {
    var bls = data;
    var j = 0;
    var star = fencecmd.star||'';
    o.push(`\\begin{subequations}${this.label_cmd}`);
    for(var bl of bls) {
  ...
  toMultline(o,id,sig,dept,fname,row1,row2,data,fencecmd) {
    var bls = data;
    var ss = [];
    for(var bl of bls) {
      var s0 = bl.join(' ');
    ...
    var star = fencecmd.star||'';
    o.push(`\\begin{multline${star}}${this.label_cmd}`);
    o.push(ss.join('\\\\\n'));
    o.push(`\\end{multline${star}}`);
    o.push('');
  }

  toListing(o,id,sig,dept,fname,row1,row2,data,fencecmd) {
    var myopts=[];
    var onecolumn = fencecmd.onecolumn;
    myopts.push(`basicstyle=${this.xsize}`);
    myopts.push('numberstyle=\\scriptsize');
    myopts.push('breaklines=true');
    myopts.push('postbreak=\\textrightarrow\\space');
    myopts.push(`numbers=${this.xnumbers}`);
    myopts.push(`numbersep=${this.xnumbersep}`);
    if (this.caption_text) {
      myopts.push(`caption={${this.caption_text}}`);
    }
    if (this.label_text) {
      myopts.push(`label={${this.label_text}}`);
    }
    var text = data;
