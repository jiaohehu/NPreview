...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  constructor() {
    ...
    this.xconfig.layout = 0;///set to '1' to enable layout
    this.xconfig.geometry = ''///should use += to add individual options
    ...
    this.xconfig.extra = '';///should use += to add individual options
  ...
  do_hdgs(block){
    var o = [];
    var {subn,hdgn,idnum,text,label} = block;
    o.push('');
    o.push(this.to_info(block));
    var raw = text;
    var text = this.smooth(text);//note that we can't use escape here
                                 //because it might add \jp
    ...
    var star='';
    if(level==0){
      if(this.xparser.ismaster){
        o.push(`\\chapter${star}{${idnum}${text}}${this.to_latexlabelcmd(label)}`);
        if(star) o.push(`\\addcontentsline{toc}{chapter}{${idnum} ${raw}}`);
      }else{
      ...
      o.push(`\\section${star}{${idnum}${text}}${this.to_latexlabelcmd(label)}`);
      if(star) o.push(`\\addcontentsline{toc}{section}{${idnum} ${raw}}`);
    }
    else if(level==2){
      o.push(`\\subsection${star}{${idnum}${text}}${this.to_latexlabelcmd(label)}`);
      if(star) o.push(`\\addcontentsline{toc}{subsection}{${idnum} ${raw}}`);
    }
    else{
      o.push(`\\subsubsection${star}{${idnum}${text}}${this.to_latexlabelcmd(label)}`);
    }
    block.latex = o.join('\n');
  }
  do_dlst(block){
  ...
  do_plst(block){
    ...
    for (var item of items) {
      ...
      switch (bull) {
        case 'OL': {
          o.push(`\\begin{enumerate}[${nosep},leftmargin=${this.xconfig.step}mm]`);
          o.push(`\\item\[${value}\] ${text}`);
          break;
        }
        case 'UL': {
          o.push(`\\begin{itemize}[${nosep},leftmargin=${this.xconfig.step}mm]`);
          o.push(`\\item ${text}`);
          break;
        }
        case 'LI': {
  ...
  do_nlst(block){
    ...
    o.push('');
    o.push(this.to_info(block));
    o.push(`\\begin{enumerate}[leftmargin=${this.xconfig.step}mm]`);
    for (var item of items) {
      ...
      if(type=='nlst'){
        ...
        for (var i=0; i < body.length; ++i) {
          ...
          line = this.polish(line,this.xconfig.nicode);
          if (line.length == 0) {
            line = '~';
          } else {
            line = `{\\ttfamily{}${line}}`;
  ...
  do_llst(block){
    var o = [];
    var {body,caption,label,islabeled} = block;
    o.push('');
    o.push(this.to_info(block));
    var opts=[];
    if(islabeled){
  ...
  do_tabu(block){
    ...
    if(0){
      ...
      o.push(`\\begin{xtabular}{@{}p{${this.xconfig.step}mm}@{}${pcol}@{}}`);
      o.push(d.join('\n'));
      o.push(`\\end{xtabular}`);
      block.latex = o.join('\n');
    }else{
      o.push(`\\begin{flushleft}`);
      o.push(`\\begin{adjustwidth}{${this.xconfig.step}mm}{0mm}`);
      o.push(`\\begin{tabbing}`);
  ...
  do_diag(block){
    var {body,wide,parser,notes,caption,label,islabeled} = block;
    var mp = new NitrilePreviewDiagramMP(this,parser.notes);
    var {s,xm,ym,unit} = mp.toDiagram(body,notes);
    var d = [];
  ...
  do_ref (sig,label,floatname,idnum) {
    ...
    if(floatname){
      var secsign = String.fromCharCode(0xA7);
      return `${secsign}${floatname}~\\ref{${label}}`;
    }
  ...
  */
  ...
  polish_tabr_header(s){
    return this.polish(s,this.xconfig.nitabr,'\\bfseries');
  }
  polish_long(s){
    return this.polish(s,this.xconfig.nilong);
  }
  polish_long_header(s){
    return this.polish(s,this.xconfig.nilong,'\\bfseries');
  }
  polish_pass(s){
