...
console.log('NitrilePreviewView::destroy() called');
  ...
  update() {
    ...
    if (blocks.length == 1 && blocks[0][3] === '!BOOK') {
      ...
      this.toContentBlocks(this.bookBlock,this.dirname())
        .then((out) => {
  ...
  async toContentBlocks(bookBlock,dirname) {
      ...
      if (v) {
        ...
        if (dirname && subfname) { 
          ...
          try {
            ...
            myBlocks.push(subparser.toErrorBlock(`No file: '${subfname}'`,subfname,sublevel));
            console.log(e.stack);
            continue;
          }
          var subparser = new NitrilePreviewParser(); 
...
console.log(href);
        atom.workspace.open(href,{pending:true,searchAllPanes:true});
      }
    }
