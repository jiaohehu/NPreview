...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  constructor() {
    ...
    this.xconfig.documentclassopt = '';///for += document class options  
  ...
  do_diag(block){
    var {lines,wide,parser,notes,caption,label,islabeled} = block;
    lines=lines||[];
    var mp = new NitrilePreviewDiagramMP(this,parser.notes);
    var {s,xm,ym,unit} = mp.toDiagram(lines,notes);
    var d = [];
  ...
  */
  style (type, text) {
    ...
    switch (type) {
      ...
      case 'var': {
        text = text.trim();
        return `\\textsl{${this.escape_diag(text)}}`
        break;
      }
      default: {
