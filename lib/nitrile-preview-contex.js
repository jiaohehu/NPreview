...
const { NitrilePreviewDiagramMP, NitrilePreviewDiagramMF } = require('./nitrile-preview-diagrammp');
...
class NitrilePreviewContex extends NitrilePreviewParser {
  ...
  do_prog(block){
    var o = [];
    var {id,row1,row2,sig,wide,body,label} = block;
    o.push(this.to_info(block));
    var n = 0;
    ...
    while(n < body.length){
      if(1){
        ...
        for(let i=0; i < maxn && n < body.length; ++i,++n){
          ...
          d.push(`\\NC ${lineno} \\NC ${line}\\NC\\NR`);
        };
      } 
      var text = d.join('\n');
      var caption = this.xcaption + cont;
      o.push(`\\placefigure`);
      o.push(`[here]`);
      o.push(`[${label}]`);
      ...
      o.push(`}`);
      o.push('%');
      label = '';//so that it is not used again for the second float
      cont = ' (Cont.)';
    }
    block.latex = o.join('\n');
  }
  ...
  do_tabr(block){
    ...
    o.push(`[here]`);
    o.push(`[${this.xlabel}]`);
    o.push(`{${this.polish_caption(this.unmask(this.xcaption))}}`);
    o.push('{%');
    o.push(text);
    o.push('}');
    o.push('');
  ...
  do_diag(block){
    ...
    var {id,row1,row2,sig,body,parser} = block; 
    o.push(this.to_info(block));
    var mp = new NitrilePreviewDiagramMF(this,parser.notes);///Use MetaFun implementation
