...
class NitrilePreviewAutonum {
  ...
  idenBlocks () {
    ...
    for (var block of this.parser.blocks) {
      var {id,row1,row2,sig,hdgn,name,sublevel,text,float,caption,label,islabeled} = block;
      caption=caption||'';
      label=label||'';
      ...
      if (sig === 'HDGS') {
        ...
        if(name=='part'){
          ...
          let level = parseInt(sublevel) + parseInt(hdgn);
          var idnum = this.idenNewHdgs(1+level);//1=chapter, 2=section
          dept = idnum;
          title = text;
        ...
        //program number
        block.idnum = '';
        if (sig  === 'PICT' && float && islabeled) {
          block.idnum = this.idenFigure();
          block.floatname = 'Figure';
        } else if (sig  === 'DIAG' && float && islabeled) {
          block.idnum = this.idenFigure();
          block.floatname = 'Figure';
        } else if (sig  === 'FRMD' && float && islabeled) {
          block.idnum = this.idenFigure();
          block.floatname = 'Figure';
        } else if (sig === 'PROG' && float && islabeled) {
          block.idnum = this.idenListing();
          block.floatname = 'Program';
        } else if (sig === 'TABR' && float && islabeled) {
          block.idnum = this.idenTable();
          block.floatname = 'Table';
