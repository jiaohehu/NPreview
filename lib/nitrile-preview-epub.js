...
class NitrilePreviewEpub {
  ...
  async generateAsync (parser,dirname) {
    ...
    let title = config.ALL.title||'Untitled';
    let author = config.ALL.author||'';
    ...
    var htmls = [];
    htmls.push(`<p style='font-size:175%; text-align:center; margin:1em 0; ' >${title}</p>`);
    htmls.push(`<p style='font-size:125%; text-align:center; margin:1em 0; ' >${author}</p>`);
    ...
    for (var block of blocks) {
      let {saveas,dept,name,level,title,refid,html} = block;
console.log('dept=',dept,'name=',name,'level=',level,'title=',title,'saveas=',saveas);
    ...
    this.toc = this.build_toc(tocs); ///tocs.map(x => `<li><a href='${x.saveas}'>${x.title}</a></li>`).join('\n');
...
`);
  ...
  build_toc(tocs){
    tocs = tocs.map(x => {
      if(Array.isArray(x)){
        return this.build_toc(x);
      }
      return `<li><a href='${x.saveas}'>${x.title}</a></li>`;
    })
    tocs = tocs.join('\n');
    tocs = `<ol style='list-style-type:none;' epub:type='list'> ${tocs} </ol>`;
    return tocs;
  }
