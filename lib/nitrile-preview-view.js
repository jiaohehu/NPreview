  ...
  */
  ...
  update() {
    ...
    if (editor) {
      ...
      /// try to determine the curr dir
      var dirname = path.dirname(editor.getPath());
...
${this.html}
  ...
  tolualatexarticle () {
    ...
    /// window
    this.saveastype = 'lualatexarticle';
    atom.workspace.paneForItem(this).saveItemAs(this);
  }
  ...
  tolualatexchapter () {
    ...
    /// and then show it in a new editor
    /// window
    this.saveastype = 'lualatexchapter';
    atom.workspace.paneForItem(this).saveItemAs(this);
    return;
  ...
  getPath () {
    ...
    if (this.editor) {
      switch (this.saveastype) {
        case 'html': {
          var basename = path.basename(this.editor.getPath() + ".html");
          break;
        }
        case 'lualatexarticle':
        case 'lualatexchapter':
        case 'lualatexbook': {
          var basename = path.basename(this.editor.getPath() + ".tex");
          break;
        }
        default: {
          var basename = path.basename(this.editor.getPath());
          break;
        }
      }
  ...
  async saveAs (savefilepath) {
    ...
    try {
      var data = '';
      if (this.saveastype === 'lualatexarticle') {
        data = await latexparser.toARTICLE(this.blocks);
      } else if (this.saveastype === 'lualatexchapter') {
        data = await latexparser.toCHAPTER(this.blocks);
      } else if (this.saveastype === 'lualatexbook') {
        data = await latexparser.toBOOK(this.blocks,this.dirname);
      } else {
        data = this.prepareHTMLData();
      }
      fs.writeFileSync(savefilepath, data+'\n');
      atom.notifications.addSuccess('write to ' + savefilepath + ' completed, '
