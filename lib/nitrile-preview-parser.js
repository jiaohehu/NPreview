...
class NitrilePreviewParser {
  ...
  constructor() {
    ...
    this.re_unmask = /(?<!\S)\_{2}(.*?)\_{2}|(?<!\S)\_{1}(.*?)\_{1}|\`\`(\s*.*?\s*)\`\`|\`(\s*.*?\s*)\`|\[(.*?)\]\((.*?)\)|\s\*([\w\-]+)\*|\[\[\&(\w+)\;\]\]|\[\[\^(\S+)\^\]\]|\[\[\#([\w\:\-]+)\#\]\]|(?<!\S)\'(\S+)\'/g;
  ...
  toFlow(lines,plevel,fname,o=null) {
    ...
    [3] sig: the signature of the block: 'TEXT', 'SAMP', 'LIST'. For a fenced
        block such as 'diagram', it is assigned as 'diagram' For a fenced
        block such as 'diagram*', it is also assigned 'diagram', and then the
        next parameter is assigned the string '*'.
    [4] star: the '*' character if fenced block is named such as 'diagram*'
    ...
    */
    ...
    while (lines.length > 0) {
      ...
      /// prepare the HTML id
      var id = `nitrile-preview-${base}-${nblocks}`;

      /// merge the attributes from the_caption_block if any
      fencecmd = {...fencecmd, ...the_caption_block};
      the_caption_block = {};

      /// change the ID to be include the label if set by user
      if (fencecmd.label) {
        var id = `nitrile-preview-${base}-${fencecmd.label}`;
      }
      ...
      if (block_type === 'SAMP' && block_type0 === 'PLST') {
        ...
        if (fence) {
          o.push([id,row,row2,'SAMP','',data,para,fencecmd,'',base,'',fname]);
        } else {
          o.push([id,row,row2,'ITEM','',data,para,fencecmd,'',base,'',fname]);
        }
        continue;
        ...
        if (v2) {
          ...
          /// #### Part 1: Background ####
          var text = v2[2];
          o.push([id,row,row2,'PART','',text,para,fencecmd,'',base,'',fname]);
          continue;
        } else if (v) {
          /// # Chapter 1 (#mychap1)
          /// ## Section 1 (#mysec1)
          ...
          var text = v[2];
          var v;
          if ((v=this.re_label.exec(text))!==null) {
            text = v[1];
            fencecmd.label = v[2];
          }
          var data = [hdgn,text];
          o.push([id,row,row2,'HDGS','',data,para,fencecmd,'',base,'',fname]);
          continue;
        }
        ...
        if (sig === 'imgs') {
          ...
          o.push([id,row,row2,'IMGS',star,data,para,fencecmd,'',base,'',fname]);

        } else if (sig === 'diagram') {

          data = this.parseDIAGRAM(data);
          o.push([id,row,row2,'DIAG',star,data,para,fencecmd,'',base,'',fname]);

        } else if (sig === 'tabbing') {

          data = this.parseTABL(data);
          o.push([id,row,row2,'TABB',star,data,para,fencecmd,'',base,'',fname]);

        } else if (sig==='math'||sig==='equation'||sig==='subequations'||sig==='equations'||sig==='multline') {

          data = this.parseEQTN(data);
          o.push([id,row,row2,'MATH',star,data,para,fencecmd,'',base,'',fname]);

        } else if (sig === 'tabular') {

          data = this.parseTABL(data);
          o.push([id,row,row2,'TBLR',star,data,para,fencecmd,'',base,'',fname]);

        } else if (sig === 'dt') {

          data = this.parseDT(data);
          o.push([id,row,row2,'DLST',star,data,para,fencecmd,'',base,'',fname]);

        } else {

          o.push([id,row,row2,'VERB',star,data,para,fencecmd,'',base,'',fname]);
        ...
        ///o.push([id,row,row2,data.name,'',data,para,fencecmd,'',base,'',fname]);

      } else if (this.isShortTable(para)) {

        var data = this.parseShortTable(para);
        o.push([id,row,row2,'TBLR','',data,para,fencecmd,'',base,'',fname]);
        block_type = 'tabular';
        continue
        ...
        if (block_type0 === 'longtablepara') {
          ...
          o.push([id,row,row2,'LONG','',data,para,fencecmd,'',base,'',fname]);
        }
        ...
        if (block_type0 === 'dtlistpara') {
          ...
          o.push([id,row,row2,'DLST','',data,para,fencecmd,'',base,'',fname]);
        }
        ...
        o.push([id,row,row2,'PLST','',data,para,fencecmd,'',base,'',fname]);
        block_type = 'PLST';
        continue
        ...
        o.push([id,row,row2,'CITE','',data,para,fencecmd,'',base,'',fname]);
        block_type = 'CITE';
        continue;
        ...
        o.push([id,row,row2,'PARA','',text,para,fencecmd,'',base,'',fname]);
        block_type = 'PARA';
        ...
        var data = [lead,text];
        o.push([id,row,row2,'PRIM','',data,para,fencecmd,'',base,'',fname]);
        block_type = 'PRIM';
        ...
        var data = [lead,text];
        o.push([id,row,row2,'SECO','',data,para,fencecmd,'',base,'',fname]);
        block_type = 'SECO';
        continue;
      } else {
        ...
        if ((v=this.re_displaymath.exec(text))!==null) {
          text = v[1];
          o.push([id,row,row2,'MATH','',[[text]],para,fencecmd,'',base,'',fname]);
          block_tyep = 'MATH';
        } if ((v=this.re_hruler.exec(text))!==null) {
          text = '* * *';
          o.push([id, row, row2, 'HRLE', '', text, para, fencecmd, '', base, '', fname]);
        } else {
          o.push([id,row,row2,'TEXT','',text,para,fencecmd,'',base,'',fname]);
          block_type = 'TEXT';
        }
        continue;
      }
  ...
  unmask (str) {
    ...
    while ((v = this.re_unmask.exec(str)) !== null) {
      ...
      } else if (v[10] !== undefined) { /// REF: [[#myfigure]] or [[myfile#myfigure]]
        var cnt = v[10];
  ...
  newPARTblock (count,subpart) {
    ...
    var fname = '';
    var plevel = 0;
    return [id,-1,-1,'PART',0,subpart,[],fencecmd,'',base,'',fname];
  ...
  new_hdgs_block (level,text,fname) {
    ...
    var para = [];
    var n = 0;
    return ['',-1,-1,'HDGS',n,[level,text],para,fencecmd,'',base,'',fname];
  ...
  toSubs (main) {
    ...
    for (var block of main) {
      const [id,row1,row2,brand,n,data,para,fencecmd,star,base,_label,fname] = block;
      switch (brand) {
        case 'CITE': {
          for (var sub of data) {
            o.push(sub);
  ...
  mergeBlocks (main,subs) {
    ...
    var o = [];
    for (var m of main) {
      const [id,row1,row2,brand,star,data,para,fencecmd,_,base,subrow,fname] = m;
      switch (brand) {
        case 'CITE': {
          for (var sub of data) {
            var {subfname} = sub;
            ...
            if (flow) {
              for (var f of flow) {
                ...
                f[10] = parseInt(subr);///NOTE: it has to be a number
                o.push(f)
              }
            }
          }
  ...
  toConfig (flags) {
    ...
    config.DIAG = {};
    config.Listing = {};
    config.Listing.numbers = 'left';
    config.Listing.numbersep = '12pt';
  ...
  idenBlocks(all,config) {
    ...
    return autonum.idenBlocks(all,config);
  }
