...
class SphinxPreviewHtml extends SphinxPreviewParser {
  ...
  /*
    ...
    */
  toLINES (blocks,plevel,ispreview,o) {
    const step = 0.25;
      ...
      switch (type) {
        case 'HDGS': {
          ...
          switch (cat) {
            ...
            case 3:
              ...
              break;
            case 4:
              o.push(`<h5>${this.escape(text)}</h5>`);
              break;
            default:
              o.push(`<h6>${this.escape(text)}</h6>`);
              break;
          }
          o.push(`</div>`);
          o.push('');
        ...
        case 'PICT': {
          if (ispreview) {
            ...
            if (inst === 'figure') {
              this.fignum += 1;
              o.push(`<b><i>Figure</i></b> ${this.fignum}. ${this.unmask(inst_t)}<br/>`);
            }
            for (var pp of data) {
              const [what,opts,srcs,sub] = pp;
              const src = srcs[0];
              const {width,height,frame} = opts;
              o.push(`<img src='' alt='${src}' style='width:40;height:40;outline:1px solid'/>`);
            }
            o.push('</p>');
            o.push('</div>');
            o.push('');
...
module.exports = {
  ...
  toPREVIEW (lines) {
    var o = [];
    var parser = new SphinxPreviewHtml();
