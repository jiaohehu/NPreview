  ...
  openLinkedFile (event) {
    ...
    if (node) {
      ...
      if (nodeName === 'A' && href) {
        ...
        if (v) {
          var id = v[1];
          var div_node = this.findParentElementById( node.parentNode, id);
          if (div_node) {
            var next_div_node = div_node.nextSibling;
            var parent_div_node = div_node.parentNode;
            if (parent_div_node && next_div_node) {
              if (next_div_node.className === 'DIV') {
                /// this is to collapse
...
console.log(the_blocks);
                ...
                /// of this <mark> node and then insert new HTML before
                /// its next sibling
                var new_element = document.createElement('div');
                new_element.innerHTML = html.join('\n');
                parent_div_node.insertBefore(new_element,next_div_node);
              }
            }
          }
        }
