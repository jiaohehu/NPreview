...
class NitrilePreviewTokenizer {
  ...
  toDiagramSvg (str) {
    ...
    for (var mathSymbol of pjson.mathSymbols) {
      ...
      if (this.used.has(id)) {
        if(id=='myINT'){
          defs.push(`<text class='MyText' id='${id}' dy='9pt' textLength='${width}pt' lengthAdjust=''>${unicode}</text>`);
          continue;
        }
        defs.push(`<text class='MyText' id='${id}' dy='9pt' textLength='${width}pt' lengthAdjust='spacingAndGlyphs'>${unicode}</text>`);
      }
    }
    var defs= `\
<defs>
...
${defs.join('\n')}
  ...
  to_text_element(text,keepwhitespace){
    ...
    /// display it as text.
    var s = `<text class='MyText' x='0' y='0' dy='0.75em' textLength='${totalw}pt' lengthAdjust='spacingAndGlyphs'>${newtext}</text>`
    var w = totalw;
    var h = 12.0;
    var mid = 6.0;
