  ...
  toFramedPict (text,config) {
    ...
    for (var pp of text) {
      pp = pp.map( x => {
          ...
          var { height } = opts;
          if (height) {
            return `{\\includegraphics[keepaspectratio=true,height=${height},width=${width}\\linewidth]{${src}}}`;
          } else {
            return `{\\includegraphics[keepaspectratio=true,width=${width}\\linewidth]{${src}}}`;
          }
      });
