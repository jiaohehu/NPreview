  ...
  unmask (line) {
    ...
    while ((v = re_unmask.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        var cnt = v[2]; 
        newtext += this.to_inlinemath(cnt);
        ...
        if(key=='em'){
          ...
          newtext += this.do_sqrt(cnt);
        }else if(key=='cbrt'){
          newtext += this.do_cbrt(cnt);
        }else if(key=='qdrt'){
          newtext += this.do_qdrt(cnt);
        }else if(key=='overline'){
          newtext += this.do_overline(cnt);
        }else{
          var cnt = v[0]; 
          cnt = this.escape(cnt);
          newtext += cnt;
        ...
        /// \[ ... \]
        var cnt = v[7]; 
        newtext += this.to_inlinemath(cnt,1);

      } else if (v[8] !== undefined) {

        /// \Alpha
        var cnt = v[8];
        if(this.is_symbol(cnt)){
          newtext += this.to_symbol(cnt);
        }else{
          var cnt = v[0]; 
          cnt = this.escape(cnt);
          newtext += cnt;
