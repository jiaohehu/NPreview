...
class NitrilePreviewTokenizer {
  ...
  toCleanup (g) {
    ...
    /// but spaces characters. It also does it for \math, \brace, \leftright, and \beginend
    /// so that its 5th element is the raw data; for example, for \text{hello world}, 
    ...
    if (v === undefined || v === null) {
      ...
      switch (v[0]) {
        ...
        case '\\frac':
          ...
          var hh_ = 2; /// add 1pt gap between numerator and denominator
          h = hh_ + h1_ + h2_;
          mid = h1_ + hh_/2;
          var o = [];
          if (w1_ < w2_) {
  ...
  to_text_element(text,keepwhitespace){
    ...
    for (var x of xx) {
      ...
      var width = this.measureText(x,this.fs);
      totalw += width;
      newtext += this.parser.escape(x);

    }

    /// not yet defined for this primitive, thus, we just
    /// display it as text.
    var s = `<text dy='${this.dy}em' textLength='${totalw}pt' lengthAdjust='spacingAndGlyphs' style='stroke:none;fill:inherit;font-size:${this.fs}pt'>${newtext}</text>`
    var w = totalw;
    var h = this.fs;
    var mid = this.fs/2;
