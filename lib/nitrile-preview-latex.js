...
class NitrilePreviewLatex extends NitrilePreviewParser {
  ...
  do_hdgs(block){
    var o = [];
    var {id,row1,row2,sig,part,hdgn,dept,title,para,base,subrow,subfname} = block;
    ...
    var use_chap=0;
    if(this.config.LATEX.documentstyle==0){
      if(this.ismaster){
        use_chap=1;
      } 
    } else if (this.config.LATEX.documentstyle==1){
      use_chap=0;
    } else if (this.config.LATEX.documentstyle==2){
      use_chap=1;
    }
  ...
  do_samp(block){
    ...
    if(this.xfencecmd.style==1){
      ...
      o.push(`\\begin{flushleft}`);
      o.push(`\\begin\{adjustwidth\}{${this.xleft}mm}{}`)
...
\\usepackage[export]{adjustbox}`
  ...
  toLatexDocument() {
    ...
    var titlelines = [];
    if(this.config.LATEX.titlepage){
