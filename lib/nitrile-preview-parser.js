...
class NitrilePreviewParser {
        ...
        if (!isNaN(m) && m > 0) {
          ...
          if (v) {
            if (v[1].length >= m) {
              ...
              var data = this.parseSample(data,flags); ///long line is to be broken up
              var fencecmd = {};
  ...
  parseSample (para, flags) {
    var o = [];
    const maxn = flags.twocolumn ? 42 : 80;
    for (var s of para) {
      var leadn = 0;
      var v = /^(\s*)/.exec(s);
      if (v) {
      ...
      while (s.length > maxn) {
        ...
        var s2 = '';
        if (s.length > maxn) {
          var k = this.findWordBeginAt(s, maxn);
          var s1 = s.slice(0,k);
          var s2 = s.slice(k);
        }
        o.push(s1);
