...
class NitrilePreviewView {
  ...
  constructor(data) {
    ...
    this.disposables.add(
      ...
      atom.commands.add(this.element, {
        ...
        'nitrile-preview:to-latex-no-prompt': () => {
          this.toLatexNoPrompt();
        },
  ...
  doReload () {
  console.log('doReload is called');
  console.log('doReload this.last_active_pane_item=',this.last_active_pane_item);
  ...
  registerEventHandlers (editor) {
    ...
    this.subscriptions.add(
      editor.onDidChangeCursorPosition((event) => {
        ...
        /// child document as preview if the cursor is inside the CITE 
        ...
        /// to be shown in its entirety. Once the cursor leaves the CITE 
...
///console.log(`image ${fsrc} changed`);
  ...
  doHilite () {
    ...
    /// set to point to the same line number as the CITE block row
    ...
    for (var block of this.all) {
      ...
      if (row >= row1 && row < row2) {
        ...
        if (node) {
          ...
          ///for NitrilePreview.
          node.style.outline = '7px solid orange';
  ...
  async toAll() {
    ...
    var [main, flags] = this.parser.toFlow(this.lines, 0, fname);
    var config = this.parser.toConfig(flags);
    var subs = this.parser.toSubs(main);
    var proms = subs.map(sub => this.getSubFlow(this.path, sub, this.parser));
    for (var j in proms) {
      subs[j].flow = await proms[j];
    }
    var all = this.parser.mergeBlocks(main, subs);
  ...
  async fetchFlow(sub, sublevel, fsubfname, parser) {
    try {
      ...
      var o = [];
      o.push(parser.newERROblock(e.toString(), sublevel, fsubfname));
      var flow = o;
      return flow;
    }
  }
  ...
  refreshView() {
    ...
    [this.htmls,this.chaps] = this.parser.translateHtml(this.all,this.config,ispreview,this.row,this.column);
  ...
  updateBlocks() {
    ...
    this.toAll().then( ([all,config,subs]) => {

        this.all = all;
        this.config = config;
        this.subs = subs;
        this.title = this.config.latexTitle ? this.config.latexTitle : 'Untitled'
        this.author = this.config.latexAuthor ? this.config.latexAuthor : ''
  ...
  getContentsBlocksForHDGS (id) {
    ...
    for (var j=0; j < this.all.length; ++j) {
      var block1 = this.all[j];
      var id1 = block1[0];
      var type1 = block1[3];
      if (type1 === 'HDGS' && id1 === id) {
        j += 1;
        for (var k=j; k < this.all.length; ++k) {
          var block2 = this.all[k];
          var type2 = block2[3];
          if (type2 === 'HDGS') {
            break;
          }
  ...
  toLatexNoPrompt () {
    ...
    var outfilepath = path.basename(this.path + ".tex");
    if (path.isAbsolute(this.dirname())) {
      outfilepath = path.resolve(this.dirname(),outfilepath);
    }

    // call this.saveAs directly without ATOM prompt for user
    this.saveastype = 'latex-complete';
    return this.saveAs(outfilepath);
  ...
  jumpToEditor (event) {
    ...
    /// This is guarentted by the 'translateHtml()' function.
    ///
  ...
  async saveAs (savefilepath) {
    ...
    try {
      ...
      if (this.saveastype === 'latex-complete') {

        var latex = new NitrilePreviewLatex();
        var olines = latex.translateLatex(this.all,this.config);
        var latexEngine = this.config.latexEngine;
        var documentclass = this.config.latexDocumentClass ? this.config.latexDocumentClass : ''
        if (!documentclass) {
          documentclass = this.config.latexIsArticle ? "article" : "book";
        }
        var documentclassopt = [];
        if (this.config.latexTwoColumnEnabled) { documentclassopt.push('twocolumn'); }
        var mykeys = Object.keys(this.config);
        var mylines = mykeys.map(x => `% !TEX nitrile ${x} = ${this.config[x]}`);

        data = `\% \!TEX program = ${latexEngine}
${mylines.join('\n')}
\\documentclass[${documentclassopt.join(',')}]{${documentclass}}
${latex.toRequiredPackages(this.config)}
...
`;
        ...
        var [htmls,chaps] = this.parser.translateHtml(this.all,this.config,ispreview);
...
`;
        ...
        data = await epub.generateAsync(this.parser,this.parser.escape(this.title),this.parser.escape(this.author),htmls,chaps,this.imagemap,this.config.latexIsArticle,this.dirname());
