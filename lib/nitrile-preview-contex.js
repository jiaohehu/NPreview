...
class NitrilePreviewContex extends NitrilePreviewParser {
  ...
  do_tabr(block){
    ...
    o.push(this.to_info(block));
    var text = this.cols_to_tabulate(cols);
    if(wide){
  ...
  do_ref (sig,label,floatname,idnum){
    if(sig=='MATH'){
      return `${floatname}.\\in[${label}]`;
    }
    if(sig=='HDGS'){
      return `${secsign}{\\goto{${idnum}}[${label}]}`;
    }
    if(sig){
      return `\\goto{${floatname}.${idnum}}[${label}]`;
    }
  ...
  */
  ...
  toContexDocument() {
    ...
    /// two column setting
    var istwocol = this.conf('twocolumn');
