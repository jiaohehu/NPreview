...
class NitrilePreviewDiagram {
  ...
  constructor(parser) {
    ...
    this.re_def_command = /^def\s+([a-zA-Z])\((.*?)\)\s*\=\s*(.*)$/;
    ...
    this.def_unit = 4;  // the length each grid unit, always in mm
  ...
  parseLine(line) {
    ...
    if ((v = this.re_pic_command.exec(line)) !== null) {
      ...
      if (pic) {
        ...
        this.commands.push(this.do_comment(`***ERROR: pic does not exist: ${name}`));
      }
      return false;
    }
    ///NOTE: *label* command
  ...
  execScalarFunc(func_name,args) {
    ...
    if (this.my_funcs.hasOwnProperty(func_name)) {
      ...
      var g = {};
      for(var i=0; i < g.args.length; ++i){
        var arg = g.args[i];
        var v = args[i];
        g[arg] = v;
      }
      var myexpr = `(${f.expr})`;
      return this.extractNextScalar(myexpr,g);
    }
    return NaN;
  }
