...
`;
  ...
  do_samp(block) {
    ...
    if (!mode) {
      ...
      text = text.join('<br/>');
      text = `<li>${text}</li>`; 
      o.push(`<ul id='${id}' class='P ${sig}' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' style='margin-left:${fencecmd.left}mm;font-size:${fencecmd.cssfontsize};padding-left:0;list-style-position:outside;' >${text}</ul>`);
      o.push('');
    }
    block.html = o.join('\n');
  }
