...
class NitrilePreviewContext extends NitrilePreviewLatex {
  ...
  to_math(pp,style) {
    ...
    let ss = pp.map(p => {
      var line = this.make_line(p.math,p.label,islabeled);
      return line;
    })
  ...
  to_longtable(itms, style, caption, label) {
    ...
    return this.itms_to_TABLE(itms, style.fr, style.hline, style.glue, style.n, caption, label);
  }
  to_multi(itms,style){
    itms = itms.map(s => {
      s = this.unmask(s);
...
\\stoptext
  ...
  itms_to_TABLE(itms, fr, hline, glue, n, caption, label) {
    var caption = caption || '';
    var label = label || '';
    var fr = fr || '';
    var glue = glue || '';
