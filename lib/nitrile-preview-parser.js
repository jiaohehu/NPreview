...
class NitrilePreviewParser {
  ...
  constructor() {
    ...
    this.re_unmask = /\s\_{2}(.*?)\_{2}|\s\_{1}(.*?)\_{1}|\s\`\`\s+(.*?)\s+\`\`|\s\`\s*(.*?)\s*\`|\[(.*?)\]\((.*?)\)/g;
    this.re_local = /^\:([\w\-]+)$/;
    this.re_directive = /^\.(\w+)$/;
  }
  ...
  addErrorBlock (str,fname,plevel) {
    /**
    Add a block that express an error encountered during
    translation.
    */
    this.blocks.push(['',-1,-1,'ERRO',0,e.toString(),[],'','','',fname,plevel]);
