...
class NitrilePreviewLatex extends NitrilePreviewParser {
  ...
  do_hdgs(block){
    ...
    if (this.config.LATEX.documentclass==''||
        this.config.LATEX.documentclass=='article') {
      switch (level) {
        case 0:
...
\\usepackage[export]{adjustbox}`
  ...
  toLatexDocument(config,texlines) {
    ...
    var documentclass = (config.LATEX.documentclass)?config.LATEX.documentclass:'article';
