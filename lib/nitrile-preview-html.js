    ...
    for (var block of blocks) {
      ...
      switch (sig) {
        ...
        case 'SAMP': {
          ...
          if (config.sampstyle === 'line') {
            text = text.map( x => `<dt style='${this.dtcssstyle}; white-space:pre-wrap; font-weight:normal;' >${x}</dt>` );
          } else {
            text = text.map( x => `<dt style='${this.dtcssstyle}; white-space:pre-wrap; ' ><code style='${this.codecssstyle}' >${x}</code></dt>` );
          }
          o.push(text.join('\n'));
          o.push(`</dl>`);
          o.push(`</div>`);
