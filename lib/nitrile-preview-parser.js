...
class NitrilePreviewParser {
  ...
  toFlow(lines,plevel,fname,o=null) {
    ...
    while (lines.length > 0) {
      ...
      if (is_sample && !listitemflag) {
        var [data, m] = this.trimParaAt(para,N_sampspaces); /// m is to be ignored
        o.push([id,row,row2,'SAMP','',data,para,fencecmd,caption,base,label,fname]);
        continue;
      }
      ...
      if (is_sample && listitemflag) {
        ...
        data = [fence,data];
        o.push([id,row,row2,'ITEM','',data,para,fencecmd,caption,base,label,fname]);
        continue;
      }
      ...
      if (the_fence === '```' ) {
        ...
        if (brand === 'imgs') {
          ...
          data = this.joinPara(data);
        } else if (brand === 'diagram') {
          data = this.parseDIAGRAM(data);
        } else if (brand === 'tabbing') {
          data = this.parseTABL(data);
        } else if (brand === 'tabular') {
          data = this.parseTABL(data);
        } else if (brand === 'longtable') {
          data = this.parseTABL(data);
        } else if (brand === 'math'||brand==='equation'||brand==='subequations'||brand==='equations'||brand==='multiline') {
          data = this.parseEQTN(data);
        } else if (brand === 'verb') {
          data = data; ///data taken as is
        } else {
          brand = 'listing';
          data = data; ///data taken as is
        }
        o.push([id,row,row2,brand,'',data,para,fencecmd,caption,base,label,fname]);
        continue;
      }
      ...
      if (v) {
        ...
        o.push([id,row,row2,'PLST','',data,para,fencecmd,caption,base,label,fname]);
        continue
      }

      /// this is the at-sign (@) description block
      if (this.re_listblock.test(para[0])) {
        listitemflag = 1;
        var data = this.parseLIST(para,this.re_listblock);
        o.push([id,row,row2,'LIST','',data,para,fencecmd,caption,base,label,fname]);
        continue;
      }

      /// this is the at-sign (@) description block
      if (this.re_descblock.test(para[0])) {
        var data = this.parseDESC('mono',para,this.re_descblock);
        o.push([id,row,row2,'DESC','',data,para,fencecmd,caption,base,label,fname]);
        continue;
      }
      ...
      if ((v = this.re_cite.exec(para[0])) !== null) {
        ...
        o.push([id,row,row2,'CITE','',data,para,fencecmd,caption,base,label,fname]);
        continue;
      }
      ...
      if ((v = this.re_hdgs.exec(para[0])) !== null) {
        ...
        if (v2) {
          ...
          /// #### Part 1: Background ####
          var text = v2[2];
          o.push([id,row,row2,'PART','',text,para,fencecmd,caption,base,label,fname]);
          continue;
        }
        var hdgn = v[1].length + plevel;
        var text = v[2];
        var data = [hdgn,text];
        o.push([id,row,row2,'HDGS','',data,para,fencecmd,caption,base,label,fname]);
        continue;
      }

      if ((v = this.re_para.exec(para[0])) !== null) {
        ...
        o.push([id,row,row2,'PARA','',text,para,fencecmd,caption,base,label,fname]);
        ...
        var data = [lead,text];
        o.push([id,row,row2,'PRIM','',data,para,fencecmd,caption,base,label,fname]);
        ...
        var data = [lead,text];
        o.push([id,row,row2,'SECO','',data,para,fencecmd,caption,base,label,fname]);
        continue;
      } else {

        // This is a regular paragraph block
        var text = this.joinPara(para);
        o.push([id,row,row2,'TEXT','',text,para,fencecmd,caption,base,label,fname]);
        continue;
      }
    }
    ...
    if (!flags.title) {
      if (o.length == 0) {
        ...
        var b = this.newHDGSblock(plevel, 'Untitled', fname);
        o.unshift(b);
      } else {
  ...
  newHDGSblock (level,text,fname) {
    ...
    var para = [];
    var n = 0;
    return ['',-1,-1,'HDGS',n,[level,text],para,fencecmd,caption,base,label,fname];

  }

  newERROblock (text,sublevel,fname) {
    ...
    var base = '';
    var label = '';
    var data = [sublevel,text];
    var para = [];
    return ['',-1,-1,'ERRO',0,data,para,fencecmd,caption,base,label,fname];
  ...
  toSubs (main) {
    ...
    for (var block of main) {
      const [id,row1,row2,brand,n,data,para,fencecmd,caption,base,label,fname] = block;
      switch (brand) {
        case 'CITE': {
          for (var sub of data) {
            o.push(sub);
  ...
  mergeBlocks (main,subs) {
    ...
    var o = [];
    for (var m of main) {
      const [id,row1,row2,brand,n,data,para,fencecmd,caption,base,label,fname] = m;
      switch (brand) {
        case 'CITE': {
          for (var sub of data) {
            var {subfname} = sub;
  ...
  _toConfig (flags) {
    ...
    config.latex.stepMargin=0.85;
    config.latex.isArticle=1;
    config.latex.sampleMargin=0.85;
  ...
  toConfig (flags) {
    ...
    config.latex.stepMargin=0.85;
    config.latex.isArticle=1;
    config.SAMP.margin=0.85;
    config.SAMP.fontsize='small';
    config.SAMP.wrap=0;

    config.listing.fontsize='small';
    config.tabular.fontsize='small';
    config.longtable.fontsize='small';
    config.imgs.fontsize='normalsize';
    config.verse.fontsize='normalsize';
    config.tabbing.fontsize='normalsize';
    config.dt.fontsize='normalsize';
    config.quot.fontsize='small';
    config.center.fontsize='normalsize';
    config.flushright.fontsize='normalsize';
    config.flushleft.fontsize='normalsize';
    config.math.fontsize='normalsize';
    config.verb.fontsize='normalsize';
  ...
  updateFenceOpt(fencecmd,sig,config){
    /// now grab all fenceopt
    for (var key in config) {
      if (key === sig) {
