...
`;
  ...
  do_dlst(block) {
    ...
    var o = []; 
    o.push(`<dl id='${id}' class='P ${sig}' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' >`);
  ...
  do_plst(block) {
    var {id,row1,row2,sig,data,para,dept,fencecmd,base,subrow} = block;
    var o = []; 
    o.push(`<p id='${id}' class='P ${sig}' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' >`);
    for (var item of data) {
      ...
      if (item.length === 3) {
        switch (lead) {
          case 'OL': {
            o.push(`<ol style='padding-left:${fencecmd.csspaddingleft}mm;list-style-position:outside;'><li value='${value}'>${text}`);
            break;
          }
          case 'UL': {
            o.push(`<ul style='padding-left:${fencecmd.csspaddingleft}mm;list-style-position:outside;'><li value='${value}'>${text}`);
            break;
          }
          case 'LI': {
  ...
  do_verb(block) {
    ...
    if (this.xname === 'listing') {
      o.push(`<figure id='${id}' class='P ${sig}' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' style='margin-left:0;margin-right:0;' >`);
  ...
  do_list(block) {
    ...
    text = text.join('\n');
    o.push(`<ul id='${id}' class='P ${sig}' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' style='list-style-position:inside;margin-left:${this.xleft};'>${text}</ul>`);
    o.push('');
    block.html = o.join('\n');
  } 
  do_samp(block) {
    var {id,rmap,row1,row2,mode,sig,data,para,dept,fencecmd,base,subrow} = block;
    var o = []; 
    var text = data;
    if (!mode) {
      ...
      text = text.join('\n');
      o.push(`<pre id='${id}' class='P ${sig}' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' style='margin-left:${fencecmd.left}mm;font-size:${fencecmd.cssfontsize};'>${text}</pre>`);
      o.push('');
    } else if (mode == 1) {
      text = this.joinPara(text);
      text = this.escape(text);
      if (rmap.length>0) { text = this.rubify(text,rmap); }
      o.push(`<p id='${id}' class='P ${sig}' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' style='margin-left:${fencecmd.left}mm;font-size:${fencecmd.cssfontsize};'>${text}</p>`);
      o.push('');
    } else if (mode == 3) {
      ...
      text = text.join('<br/>');
      text = `<li>${text}</li>`; 
      o.push(`<ul id='${id}' class='P ${sig}' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' style='margin-left:${fencecmd.left}mm;font-size:${fencecmd.cssfontsize};padding-left:${fencecmd.csspaddingleft}mm;list-style-position:outside;' >${text}</ul>`);
      o.push('');
    }
    block.html = o.join('\n');
  }
  do_hrle(block) {
    ...
    var o = []; 
    var text = data;
    o.push(`<p id='${id}' class='P ${sig}' style='height:3em;display:flex;flex-direction:row;' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' >`);
    text = this.unmask(text);
  ...
  do_text(block) {
    ...
    if (standalone) {
      text = this.unmask(text);
      o.push(`<p id='${id}' class='P ${sig}' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' style='margin-left:${fencecmd.left}mm;'>${text}</p>`);
    }
      ...
      o.push(`<p id='${id}' class='P TEXT' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' style='margin-bottom:0;text-indent:0; ' >`);
      o.push(text);
      o.push('</p>');
    }
    else {
      ...
      if (this.textblockcount == 1) {
        ...
        var csstextindent=`${fencecmd.csstextindent}mm`;
      }
      o.push(`<p id='${id}' class='P TEXT' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' style='margin-top:0;margin-bottom:0;text-indent:${csstextindent};' >${text}</p>`);
    }
    o.push('');
    block.html = o.join('\n');
  }
  do_quot(block) {
    ...
    o.push(`<p id='${id}' class='P ${sig}' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' style='font-size:${fencecmd.cssfontsize};margin-left:${fencecmd.left}mm;margin-right:${fencecmd.right}mm;'>`);
    o.push(`${lq}${text}${rq}`);
    o.push(`</p>`);
    o.push('');
    block.html = o.join('\n');
  ...
  do_tabb(block){
    ...
    var text = s.join('\n');
    var text = `<table border='0' style='font-size:${this.xcssfontsize};width:100%;border-collapse:collapse;'>${text}</table>`;
    o.push(`<figure id='${id}' class='P ${sig}' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' style='margin-left:0;margin-right:0;'>`);
    o.push(text);
    o.push('</figure>');
    o.push('');
  ...
  do_tblr_long(block){
    ...
    ${hlines.indexOf('m')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
    ...
    var text = s.join('\n');
    if (sig==='TBLR') {
      text = `<table border='0' style='font-size:${this.xcssfontsize};margin-left:auto;margin-right:auto;border-collapse:collapse;'>${text}</table>`;
    } else if (sig==='LONG') {
  ...
  do_math(block) {
    ...
    if (fencecmd.name=='equation') {
      if (isalignequalsign) {
        ...
        var mydata = this.toMathTabularRight(data);
        var mydata = mydata.map(x => x.join(''));
      }
      ...
      mydata = mydata.map((x,i) => {
          if(i==0) {
            ...
            return `<td rowspan='${rowspan}'>${numtext}</td>${x}`;
          } else {
            return x;
          }
      });
      ...
      if (isalignequalsign) {
        ...
        var mydata = this.toMathTabularCenter(data);
        var mydata = mydata.map(x => x.join(''));
      }
    ...
    text = `<table style='margin-left:${fencecmd.left}mm;margin-right:0;border-collapse:collapse;'>${text}</table>`;
    o.push(`<figure id='${id}' class='P ${sig}' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' style='margin-left:0;margin-right:0;margin-top:${fencecmd.vmargin}mm;margin-bottom:${fencecmd.vmargin}mm;'>`);
    o.push(text);
    o.push('</figure>');
    o.push('');
    block.html = o.join('\n');
  ...
  inlinemath (str) {
    ...
    if(this.tokenizer.isdisplaymath&&this.xsig==='TEXT'){
      return `<span style='display:inline-block;width:100%;text-align:left;margin-left:${this.xfencecmd.left}mm;'>${s}</span>`;
    }else{
      return s;
    }
  }
  ...
  displaymath (str) {
    ...
    if(this.tokenizer.isdisplaymath&&this.xsig==='TEXT'){
      return `<span style='display:inline-block;width:100%;text-align:left;margin-left:${this.xfencecmd.left}mm;margin-top:${this.config.HTML.vmargin}mm;margin-bottom:${this.config.HTML.vmargin}mm;'>${s}</span>`;
    }else{
      return s;
    }
  }
