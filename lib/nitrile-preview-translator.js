...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  to_ref (str) {
    for (var j=0; j < this.parser.blocks.length; ++j) {
      ...
      label = label||'';
      if( label && idnum && str.localeCompare(label)===0) {
        return this.do_ref(sig,label,floatname,idnum,refid,id,chapters);
        break;
      }
      if(more&&more.length){
        for(let k=0; k < more.length; k++){
          let x = more[k];
          if(x.label && x.idnum && str.localeCompare(x.label)===0){
