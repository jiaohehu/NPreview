...
console.log(title);     
      ...
      this.htmls.push(`</p>`);
      this.htmls = parser.translate(this.flags,this.blocks,true,this.htmls);
      this.htmls.push(`</div>`);
      ...
      this.generateContentsBlocks(bookBlock,this.dirname())
        .then((out) => {
            ...
            this.htmls.push(`${parser.escape(new Date().toLocaleDateString())}`);
            this.htmls.push(`</p>`);
            this.htmls = parser.translateOutlineOnly(this.contentsFlags,this.contentsBlocks,this.htmls);
            this.htmls.push(`</div>`);
            const { scrollTop } = this.element;
            this.element.textContent = ''
            this.element.innerHTML = this.htmls.join('\n');
  ...
  async saveAs (savefilepath) {
    ...
    try {
      ...
      if (this.saveastype === 'lualatex-article') {
        ...
        var olines = translator.translate(this.flags,this.blocks,true);
        var {title='',author='',documentclass=''} = this.flags;
        title = title ? title : 'Untitled';
...
\\end{document}`;
        ...
        var translator = new NitrilePreviewLatex();
        var olines = translator.translate(this.contentsFlags,this.contentsBlocks,false);
        data = olines.join('\n');
