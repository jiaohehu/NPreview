...
class NitrilePreviewLmath extends NitrilePreviewTokenizer {
  ...
  // even though it is called toSVG it does not yield an SVG
  toSvg(v) {
  ...
  toInnerSvg (v, compact=false) {
    ...
    if (v === undefined || v === null) {
        ...
        case '\\operatorname':
          ...
          if (Array.isArray(v1)) {
            if (v1[0] === '\\brace') {
              ...
              str = '';//this should not have happened
            }
          } else {
      ...
      var math_cmd_info = this.findMathCommandInfo(v[0]);
      ...
      if(id){
        ...
        var s = ` ${v}`;//put in a space before it so it is distinguished from its neighbors
        return {s};
