...
class NitrilePreviewParser {
  ...
  toBlocks(lines,fname,subrow,plevel) {
...
console.log(para);
      ...
      /// if the paragraph read is empty, we know we
      ...
      /// prepare the label
      ///var label = `${base}:${nblocks}`; /// default label, such as 'd3js:15'
      var label = '';
      ...
      /// if flags.sample is set then checks to see if the first 
      /// line has enough leading spaces specified by this flag
      if (flags.sample) {
        var m = parseInt(flags.sample);
        if (!isNaN(m) && m > 0) {
          var v = this.re_sample.exec(para[0]);
          if (v) {
            if (v[1].length >= m) {
      ...
      if (the_fence === '```') {
        ...
        if (fencecmd.listing) {
          if (fencecmd.label) {
            label = `${base}:${fencecmd.label}`;
          }
          o.push([id,row,row2,'LSTG',n,data,para,fencecmd,dept,fig,caption,base,label,fname,subrow,plevel]);
        } else {
      ...
      if (the_fence === '~~~') {
        var [data, n] = this.trimPara(para);
        var [data,fence,fencecmd] = this.trimFences(data);
        o.push([id,row,row2,'VERB',n,data,para,fencecmd,dept,fig,caption,base,label,fname,subrow,plevel]);
        continue;
      }
      ...
      if (the_fence === '///') {
        ...
        if (fencecmd.figure) {
          ...
          o.push([id,row,row2,'FIGE',n,data,para,fencecmd,dept,fig,caption,base,label,fname,subrow,plevel]);
        } else {
  ...
  async fetchBookBlocks(lines,dirname) {
    ...
    for (var myOpener of myOpeners) {
      ...
      var subparser = new NitrilePreviewParser();
      myBlocks.push(subparser.newFILE(subfname,subrow,sublevel));
      [myBlocks] = subparser.toBlocks(subdata.split('\n'),subfname,subrow,sublevel,myBlocks);
      myBlocks.push(subparser.newFEND(subfname,subrow,sublevel));
    }
  ...
  trimFences (para) {
    ...
    if (v) {
      var the_fence = v[1];
      var the_fencecmd = this.toObject(v[2]);
