...
class NitrilePreviewParser {
  ...
  newConfig() {
    ...
    config.CONTEX.fscode='sm';
    config.CONTEX.fstabular='sm';
    config.CONTEX.fslisting='xsm';
    config.CONTEX.longpadding='1 3';
    config.CONTEX.longvlines='*';
    config.CONTEX.longhlines='t m b r';
    config.CONTEX.fssubcaption='xsm';
    ...
    config.LATEX.fscode='small';
    config.LATEX.fstabular='small';
    config.LATEX.fslisting='footnotesize';
    config.LATEX.fssubcaption='footnotesize';
    ...
    config.HTML.mathoutline=0;

    /// All the layout dimensins are in 'mm'
    config.CSS = {};
    config.CSS.niprog='font-size: 0.75em';
    config.CSS.nicode='font-size: 0.7em';
    config.CSS.nitab='font-size: 0.7em';
    config.CSS.nicap='font-size: 0.8em';
    config.CSS.nisvg='';
...
//console.log(block);
      ...
      if(sig0=='NLST'&&sig=='NLST'){
        ...
        if(re_fence.test(body[0])&&re_fence.test(body[body.length-1])){
          ...
          blk.row2 = block.row2;
          blk.para = blk.para.concat(block.para);
          blk.items.push({type:'samp',body:body});
          o.push(blk);
          continue;
          ...
          blk.row2 = block.row2;
          blk.para = blk.para.concat(block.para);
          blk.items.push({type:'pass',body:body});
          o.push(blk);
          continue;
          ...
          blk.row2 = block.row2;
          blk.para = blk.para.concat(block.para);
          blk.items.push({type:'text',text:this.joinPara(body),body:block.body});
          o.push(blk);
          continue;
      ...
      if(sig == 'LONG'){
        ...
        if(sig0=='TABB'){
          ...
          blk.row2 = block.row2;
          blk.para = blk.para.concat(block.para);
          blk.cols = blk.cols.concat(block.cols);
          o.push(blk);
  ...
  read_para (lines,n) {
    ...
    if(type == 'spcl'){
      if(fenceid == 'framed'){
        ...
        return {para,sig,wide,float,caption,label,cols };
      } 
      else if (fenceid == 'image'){
        sig = 'PICT';
        var rows = this.parse_pict(body);
        return {para,sig,wide,float,caption,label,rows};
      }
      ...
      var cols = this.parse_tabb(body);
      return {para,sig,caption,label,cols};
    }
    else if(type == 'math'){
      var math = this.parse_math(body);
      return {para,sig,label,math,gather,body};
    }
    else if(type == 'hdgs'){
      return {para,sig,hdgn,label,text};
    } 
    else if(type == 'quot'){
  ...
  */
  ...
  async readModeAsync(dirname) {
    ...
    for(let d of this.mode){
      ...
      if(name==='root' && subparser){
        ...
        ///and then append all the 'rmap' entry to the end of ours
  ...
  parse_tabb_spaces(para){

    var para = para.map(row => this.splitCells(row));
  ...
  parse_tabb(para) {
    ...
    if(para[0].trim().toUpperCase()==='CSV'){
      return this.parse_tabb_csv(para.slice(1));
    }

    return this.parse_tabb_spaces(para);
  }
