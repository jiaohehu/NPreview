...
`;
  ...
  itms_to_tabulate(rows,style){
    ...
    if(border){
      ...
      for (var j = 0; j < nrows; ++j) {
        ...
        if (this.pp_is_hline(pp)) {
          ...
          d.push(`<tr><td style='background-color:currentColor' colspan='${ncols + ncols - 1}'></td></tr>`);
          continue;
        }
    ...
    if(border){
      var text = `<table style='${css_style}' border='1'>${text}</table>`;
    }else if(frame){
      var text = `<table style='${css_style};border:1px solid' border='0'>${text}</table>`;
    }else{
