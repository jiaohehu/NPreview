...
class NitrilePreviewParser {
  ...
  constructor() {
    ...
    this.re_unmask = /(?<!\S)\_{2}(.*?)\_{2}|(?<!\S)\_{1}(.*?)\_{1}|\`\`(\s*.*?\s*)\`\`|\`(\s*.*?\s*)\`|\[(.*?)\]\((.*?)\)|\s\*([\w\-]+)\*|\[\[\&(\w+)\;\]\]|\[\[\^(\S+)\^\]\]|\[\[([\w\-]*\#[\w\-]+)\]\]|(?<!\S)\'(\S+)\'/g;
    ...
    while (lines.length > 0) {
      ...
      if (block_type === 'SAMP' && block_type0 === 'PLST') {
        ...
        if (sig === 'includes') {
          ...
          /// > ch5.md
          /// ```
          var data = this.parseINCL(para,row);
          o.push([id,row,row2,'INCL','',data,para,fencecmd,'',base,'',fname]);
          block_type = 'INCL';

        } else if (sig === 'imgs') {

          data = this.parseDIAGRAM(data);
          o.push([id,row,row2,'IMGS','',data,para,fencecmd,'',base,'',fname]);

        } else if (sig === 'diagram') {

          data = this.parseDIAGRAM(data);
          o.push([id,row,row2,'DIAG','',data,para,fencecmd,'',base,'',fname]);

        } else if (sig === 'tabbing') {

          data = this.parseTABL(data);
          o.push([id,row,row2,'TABB','',data,para,fencecmd,'',base,'',fname]);
          ...
          o.push([id,row,row2,'MATH','',data,para,fencecmd,'',base,'',fname]);
          ...
          o.push([id,row,row2,'VERB','',data,para,fencecmd,'',base,'',fname]);
        ...
        if(block_type0 === 'tabbpara'){
          ...
          o.push([id, row, row2, 'TABB', '', [items], para, fencecmd, '', base, '', fname]);
        }
        block_type = 'tabbpara';
        continue;
        ...
        o.push([id,row,row2,'QUOT','',data,para,fencecmd,'',base,'',fname]);
        block_type = 'QUOT';
        continue;
  ...
  unmask (str) {
    ...
    while ((v = this.re_unmask.exec(str)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        var pp = cnt.split('#');
        if (pp.length == 2) {
          if (!pp[0]) pp[0] = this.base;  
          if (!pp[1]) pp[1] = '1';
          let mystr = pp.join('-');
          newtext += this.ref(mystr);
        }
