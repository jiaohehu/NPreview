...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_apple(opt,txt,coords) {
    ...
    for (var i = 0; this.valid(coords, i); i++) {
      ...
      var y = (z0[1]);
      if (opt === '.top') {
        var p = this.shiftcoords(p0,-0.5,0);
        var p = this.shiftcoords(p,x,y);
        var d = this.coordsToD(p);
      } else if (opt === '.bot') {
        var p = this.shiftcoords(p0,-0.5,-1);
        var p = this.shiftcoords(p,x,y);
        var d = this.coordsToD(p);
      } else if (opt === '.rt') {
        var p = this.shiftcoords(p0,0,-0.5);
        var p = this.shiftcoords(p,x,y);
        var d = this.coordsToD(p);
      } else if (opt === '.lft') {
        var p = this.shiftcoords(p0,-1,-0.5);
        var p = this.shiftcoords(p,x,y);
        var d = this.coordsToD(p);
      } else {
        var p = this.shiftcoords(p0,-0.5,-0.5);
        var p = this.shiftcoords(p,x,y);
        var d = this.coordsToD(p);
      }
