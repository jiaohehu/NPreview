...
const tokenizer = require('./nitrile-preview-tokenizer');
...
class NitrilePreviewHtml extends NitrilePreviewParser {
  ...
  */
  style (type, text) {
    ...
    switch (type) {
      case 'inlinemath': {
        console.error('inlinemath:');
        console.error(text);
        var l = tokenizer.toTokens(text);
console.log(l);
        var g = tokenizer.toGroups(l);
console.log(g);
        var g = tokenizer.toSubsup(g);
console.log(g);
        var s = tokenizer.toSvg(g);
console.log(s);
        return s;
        break;
      }
