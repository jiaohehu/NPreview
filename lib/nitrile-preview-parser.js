...
class NitrilePreviewParser {
  ...
  constructor() {
    ...
    this.re_unmask = /\s\_{2}(.*?)\_{2}|\s\_{1}(.*?)\_{1}|\s\`\$\$(\s*.*?\s*)\$\$\`|\s\`\$(\s*.*?\s*)\$\`|\s\`(\s*.*?\s*)\`|\[(.*?)\]\((.*?)\)|\s\*(\w+)\*/g;
    this.re_local = /^\:([\w\-]+)$/;
    this.re_full = /^([\w\-]+)\:([\w\-]+)$/;
  ...
  unmask (text) {
    ...
    while ((v = this.re_unmask.exec(str)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        var cnt = raw.trim();
        newtext += ' ';
        newtext += this.style('displaymath',cnt);
      ...
      } else if (v[6] !== undefined && v[7] !== undefined) {
        /// uri: [](www.yahoo.com)
