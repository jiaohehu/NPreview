...
console.error(`load image from imagemap: ${imgid}, ${src}`);
      setTimeout( () => {
...
console.error(`load image from file: ${imgid}, ${src}`);
...
console.error(this.autonum.xrefs);
    this.autonum.start();

    if (this.isarticle) {
      [this.htmls,this.chaps] = this.parser.translateHtml(this.autonum,this.config,this.blocks,this.isarticle,0,0,this.htmls,this.chaps);
    } else {
      for (var i=0; i < this.subs.length; ++i) {
        [this.htmls,this.chaps] = this.parser.translateHtml(this.autonum,this.config,this.subs[i].flow[0],this.isarticle,this.subs[i].subrow,this.subs[i].sublevel,this.htmls,this.chaps);
      }
    }
...
console.error(this.autonum.xrefs);
    ...
    if (this.isEqual(xrefs_old,xrefs_new)) {
      console.error('references are the same.');
    } else {
      console.error('references have changed.');
      if (!this.updateFlag) {
    ...
    this.parser.xrefs = this.autonum.xrefs;
...
`;
        ...
        if (this.config.twocolumn) { documentclassopt.push('twocolumn'); }
...
`;
        ...
        data = await epub.generateAsync(this.parser.escape(this.title),this.parser.escape(this.author),this.htmls,this.chaps,this.imageidmap,this.imagemap,this.isarticle);
