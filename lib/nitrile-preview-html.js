...
class NitrilePreviewHtml extends NitrilePreviewParser {
  ...
  translateHtml (config,xrefs,blocks,isarticle,o) {
    ...
    o = o || [];
    const stepspaces = config['stepspaces'];
    const stepmargin = config['stepmargin'];
    const parskip = config['parskip'];
    const outlineonly = config['outlineonly'];
    const showsections  = this.toList(config['showsections']);
    var heading = '';
    this.block = [];
    this.xrefs = xrefs;
    for (var block of blocks) {
      ...
      const left = `${Math.floor(n/stepspaces)*stepmargin}cm`;
      switch (sig) {
        case 'FILE': {
        ...
        case 'HDGS': {
          ...
          switch (cat) {
            case 0:
              heading = 'CHAPTER';
              o.push(`<h1 id='${id}' class='${heading}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='position:relative; '>`);
              o.push(`Chapter ${dept} &#160; ${this.escape(text)}`);
              o.push(`<small style='position:absolute; '><mark>[${id}]</mark></small>`);
              o.push(`</h1>`);
              break;
            case 1:
              heading = 'SECTION';
              o.push(`<h2 id='${id}' class='${heading}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='position:relative; '>`);
            ...
            case 2:
              heading = 'SUBSECTION';
              o.push(`<h3 id='${id}' class='${heading}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='position:relative; '>`);
              o.push(`${dept} &#160; ${this.escape(text)}`);
              o.push(`<small style='position:absolute; '><mark>[${id}]</mark></small>`);
              o.push(`</h3>`);
              break;
            case 3:
              ...
              o.push(`<h4 id='${id}' class='${heading}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='position:relative; '>`);
              o.push(`${dept} &#160; ${this.escape(text)}`);
              o.push(`<small style='position:absolute; '><mark>[${id}]</mark></small>`);
              o.push(`</h4>`);
              break;
            case 4:
              ...
              o.push(`<h5 id='${id}' class='${heading}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='position:relative; '>`);
              o.push(`${dept} &#160; ${this.escape(text)}`);
              o.push(`<small style='position:absolute; '><mark>[${id}]</mark></small>`);
              o.push(`</h5>`);
              break;
            default:
              ...
              o.push(`<h6 id='${id}' class='${heading}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='position:relative; '>`);
              o.push(`${dept} &#160; ${this.escape(text)}`);
              o.push(`<small style='position:absolute; '><mark>[${id}]</mark></small>`);
              o.push(`</h6>`);
              break;
        ...
        case 'VRSE': {
          if (outlineonly && showsections.indexOf(dept) < 0) { break; }
        ...
        case 'CODE': {
          ...
          if (fencecmd.listing) {
            ...
            o.push(`<figcaption style='position:relative; '> Listing ${fig}: ${this.unmask(caption)} <small style='position:absolute; '><mark>[${id}]</mark></small> </figcaption>`);
        ...
        case 'PICT': {
          ...
          if (fencecmd.figure) {
            o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='margin-left:${left}'>`);
            o.push(`<figure style='margin:1em 0'>`);
            o.push(`<figcaption style='position:relative; '> Figure ${fig}: ${this.unmask(caption)} <small style='position:absolute; '><mark>[${id}]</mark></small> </figcaption>`);
            for (var j in text) {
              var pp = text[j];
        ...
        case 'EQTN': {
          ...
          if (text.length == 1) {
            for (var s of text) {
              ...
              o.push(`<span style='display:block;text-align:center;position:relative'>${s}<span style='position:absolute;right:0px;'>(${fig})<small style='position:absolute; '><mark>[${id}]</mark></small></span></span>`);
            }
          } else if (text.length > 1) {
            var j = 0;
            for (var s of text) {
              s = this.tokenizer.parse(s,true);
              o.push(`<span style='display:block;text-align:center;position:relative'>${s}<span style='position:absolute;right:0px;'>(${fig}${this.toSubfigNum(j)})<small style='position:absolute; '><mark>[${id}]</mark></small></span></span>`);
              j += 1;
            }
          }
          o.push(`</p>`);
        ...
        case 'DESC': {
          ...
          const xleft = `${Math.floor(xn/stepspaces)*stepmargin}cm`;
