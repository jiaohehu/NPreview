...
console.log(`EVENT: text editor: '${item.getPath()}'`);
  ...
  doHilite () {
    ...
    if (this.bookBlock) {
      ...
      var fname = '';
      if (row >= 0) {
        var line = this.bookBlock[6][row];
        if (line) {
          console.log(line);
          fname = this.toBookFile(line);
        }
      }
...
console.log('NitrilePreviewView::destroy() called');
  ...
  update() {
    ...
    var [blocks,flags] = parser.toBLOCKS(this.lines,fname,0);
    if (blocks.length == 1 && blocks[0][3] === '!BOOK') {

      this.bookBlock = blocks[0];
      this.flags = {};
      this.blocks = [];

      this.toBookPreviewBlocks(this.bookBlock,this.dirname())
        .then((out) => {
            var [blocks,flags] = out;
            var { title = '', author = '' } = flags;
            title = title ? title : 'Untitled';
            blocks = parser.idenBLOCKS(blocks,false);
            this.flags = flags;
            this.blocks = blocks;
            this.htmls = [];
            ...
            this.htmls.push(`</p>`);
            this.htmls = parser.translateBook(this.toConfig(),this.blocks,this.htmls);
            this.htmls.push(`</div>`);
      ...
      this.htmls.push(`${parser.escape(new Date().toLocaleDateString())}`);
      this.htmls.push(`</p>`);
      this.htmls = parser.translate(this.toConfig(),this.blocks,true,this.htmls);
      this.htmls.push(`</div>`);
      const { scrollTop } = this.element;
      this.element.textContent = '';
      this.element.innerHTML = this.htmls.join('\n');
  ...
  async toBookPreviewBlocks(bookBlock,dirname) {
    ...
    if (node) {
      ...
      if (nodeName === 'MARK' && className === 'BOOK') {
        ...
        if (next_div_node && !next_div_node.textContent) {
          ...
          var htmls = translator.translate(this.toConfig(),the_blocks,false);
  ...
  async saveAs (savefilepath) {
    ...
    try {
      ...
      if (this.saveastype === 'lualatex-article') {
        ...
        var olines = translator.translate(this.toConfig(),this.blocks,true);
        var {title='',author='',documentclass=''} = this.flags;
        title = title ? title : 'Untitled';
...
\\end{document}`;
        ...
        var translator = new NitrilePreviewLatex();
        var olines = translator.translate(this.toConfig(),this.blocks,false);
        data = olines.join('\n');
...
\\end{document}`;
        ...
        const olines = translator.translate(this.toConfig(),this.blocks,isarticle);
...
<div class='PAGE'>
<p class='TITLE'>
${translator.escape(title)}
</p>
<p class='AUTHOR'>
${translator.escape(author)}
</p>
<p class='DATE'>
${translator.escape(new Date().toLocaleDateString())}
</p>
${olines.join('\n')}
</div>
</body>
...
${olines.join('\n')}
  ...
  toConfig () {
    ...
    /// view
    var o = {}
    o.step = atom.config.get('nitrile-preview.step');
    o.indentMargin = atom.config.get('nitrile-preview.indentMargin');
    o.flushLeftParagraph = atom.config.get('nitrile-preview.flushLeftParagraph');
    return o;
  }
