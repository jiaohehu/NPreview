...
class NitrilePreviewHtml extends NitrilePreviewParser {
  ...
  /*
    ...
    */
  translateBlocks (config,blocks,isarticle,o) {
    ...
    for (var block of blocks) {
      ...
      switch (type) {
        case 'FILE': {
          o.push(`<div id='${id}' class='FILE' src='${fname}'>`);
          break;
        }
        case 'FILEND': {
          o.push(`</div>`);
          ...
          switch (cat) {
            case 0:
              if (isarticle) {
                ...
                o.push(`<small><mark isarticle='${isarticle}' class='ARTICLE' label='${id}'>[${id}]</mark></small>`);
                o.push(`</h1>`);
              }
              break;
            case 1:
  ...
  translateBlocksOutlineOnly (config,blocks,o) {
    o = o || [];
    for (var block of blocks) {
      const [id,row1,row2,type,n,data,para,fencecmd,refname,caption,fname,plevel] = block;
      switch (type) {
