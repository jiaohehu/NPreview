...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_circle(opt,txt,g,coords){
    var s = [];
    if (opt==='fill') {
      var drawopt = `stroke='none' ${this.to_fills(g,true)}`;
    } else {
      var drawopt = `${this.to_strokes(g)} fill='none'`;
    }
    var radius = g.r?parseFloat(g.r):1;
    var angle1 = parseFloat(g.start);       
    var angle2 = parseFloat(g.stop);       
    for (var i = 0; i < coords.length; i++){
      var z0 = this.point(coords, i);
      if (!this.isvalidpt(z0)) continue;
      var x = (z0[0]);
  ...
  p_path(coords,p,g){
    ...
    return(`<path d='${d}' ${this.to_strokes(g)} ${this.to_fills(g)} ${x.join(' ')}/>`);
  }
  ...
  p_hbar(x,y,g){
    ...
    var x = parseFloat(x);
    var dx = parseFloat(dx);
    var x2 = x + dx;
    var X2 = this.localx(x2);
    var Y2 = this.localy(y);
    o.push(`<line x1='${X}' y1='${Y}' x2='${X2}' y2='${Y2}' ${this.to_strokes(g)}/>`);
    return o.join('\n');
  }
  ...
  p_label(x,y,g){
    ...
    var label = g.label||'';
    var justi = g.anchor||'';
    console.log('justi=',justi);
    return this.drawtext(X,Y,label,justi);
  }
  ...
  p_ellipse(x,y,Rx,Ry,angle,g){
    ...
    o.push(`<path d='M${x1},${y1} A${Rx},${Ry},${-angle},1,0,${x2},${y2}' ${this.to_strokes(g)} ${this.to_fills(g)}/>`);//anti-clockwise
    o.push(`<path d='M${x1},${y1} A${Rx},${Ry},${-angle},1,1,${x2},${y2}' ${this.to_strokes(g)} ${this.to_fills(g)}/>`);//clockwise
    return o.join('\n');
  }
  ...
  p_arc(x,y,r,a1,a2,g){
    ...
    r = this.localdist(r);
    if (a2-a1 < 180) {
      return(`<path d='M${x1},${y1} A${r},${r},0,0,0,${x2},${y2}' ${this.to_strokes(g)} ${this.to_fills(g)}/>`);
    } else {
      return(`<path d='M${x1},${y1} A${r},${r},0,1,0,${x2},${y2}' ${this.to_strokes(g)} ${this.to_fills(g)}/>`);
    }
  }

  p_shape(x,y,w,h,mypath,g){
    var sx = this.assertFloat(g.sx,1);
    var sy = this.assertFloat(g.sy,1);
    var p = this.readCoordsLine(mypath);
    var p = this.scalecoords(p, sx, sy);//refsx,refsy
