...
\\page`);
  ...
  do_samp(block){
    ...
    if (this.features.mode1) {
      ...
      text = text.map( x => {
        ...
        ++n;
        x = `{${this.xconfontsize} ${x}}`;
        if(n==1){
          return `\\item ${x}`;
        }
        return `${x}`;
      });
      text = text.filter(x => x?true:false);
      text = text.join('\\\\\n');
      o.push(`\\setupnarrower[left=${fencecmd.left}mm]`);
      o.push(`\\startnarrower[left]`);
      o.push(`\\startitemize[packed]`);
  ...
  do_tabr(block){
    ...
    for(var j=0; j<nrows; ++j){
      ...
      if(j==0){
        pp = pp.map(x => `{${this.xconfontsize}\\bf ${x}}`);
        s.push(`\\HL`);
        s.push(`\\NC ${pp.join(' \\NC ')} \\NC\\NR`);
        s.push(`\\HL`);
      } else {
        pp = pp.map(x => `{${this.xconfontsize} ${x}}`);
    ...
    if (this.xname==='figure') {
      ...
      for(var blk of data){
        ...
        for(var img of blk){
          ...
          o.push(`{\\externalfigure[${srcs[0]}][width=${width},height=${height}]} {\\small ${sub_text}}`);
        }
  ...
  do_pict(block){
    ...
    var s = [];
    for(var blk of data){
      s.push(`{\\centeraligned{\\startcombination[${blk.length}*1]`);
      var n = blk.length;
      for(var img of blk){
        const {src,sub} = img;
        const imgsrc = this.toContextImageSrc(src);
        const sub_text = this.unmask(sub);
        const width = this.toContexLength(`${100/n}%`);
        s.push(`{\\externalfigure[${imgsrc}][width=${width}]} {\\small ${sub_text}}`);
      }
  ...
  do_tabb(block){
    ...
    ww.fill(1);
    ww = this.wwToOne(ww);
    ww = ww.map((x,i) => `\\setupTABLE[c][${i+1}][width=${x}\\textwidth]`);
    o.push(ww.join('\n'));
    o.push(`\\bTABLE[split=yes,option=stretch,frame=no,loffset=0pt,roffset=0pt,toffset=0pt,boffset=0pt,spaceinbetween=0pt]`);
    var s = [];
    for (var j = 0; j < nrows; ++j) {
      var pp = data.map(x => x[j] || '');
      pp = pp.map(x => this.unmask(x));
