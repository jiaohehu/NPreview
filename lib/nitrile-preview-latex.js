...
class NitrilePreviewLatex extends NitrilePreviewParser {
  ...
  do_topi(block){
    ...
    subfname=subfname||''; o.push(`%${sig} ${subfname} ${1+row1} ${1+row2}`);
    o.push(`\\begin{flushleft}`);
    o.push(`\\begin{description}\[nosep,style=unboxed,leftmargin=${0}mm,font=\\normalfont]`);
    for (var item of data) {
      var keys = item;
  ...
  do_tabr(block){
    ...
    if (fencecmd.name==='table') {
      ...
      o.push(`\\centering`);
      o.push(`\\phantomsection${this.latexlabelcmd}`);
      o.push(`Table ${this.xidnum} : ${this.caption_text} \\hfill\\\\`);
      o.push(text);
      o.push(`\\end{table}`);
    } else {
      o.push(`\\begin{center}`);
...
\\usepackage[export]{adjustbox}`
  ...
  toFramedLtpp (para,fencecmd) {
    ...
    for (var line of para) {
      ...
      o.push(`\\put(${x},${y}){\\ttfamily\\fontsize{12pt}{12pt}\\selectfont{}${line}}`);
      y += 12;
    }
