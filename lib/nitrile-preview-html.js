  ...
  */
  ...
  toFramedTabular (rows, maxj, ww, vlines, hlines) {
    ...
    for (var j=0; j < rows.length; ++j) {
      var row = rows[j];
      var row = row.map((x,i) => `<td style='text-align:left; padding:2; ${vlines.indexOf(i)>=0?'border-left:1px solid black;':''} ${vlines.indexOf(i+1)>=0?'border-right:1px solid black;':''} ${j>0&&hlines.indexOf('*')>=0?'border-top:1px solid black;':''} ${j==n-1&&hlines.indexOf('*')>=0?'border-bottom:1px solid black;':''} '>${x}</td>`);
