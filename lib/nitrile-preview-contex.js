...
class NitrilePreviewContex extends NitrilePreviewParser {
  ...
  do_verb(block){
    ...
    for (var k=0; k < text.length; ++k) {
      ...
      if(1){              
        var lineno = `${++linenum}`;
        var lineno = this.polish(lineno,this.config.CONTEX.fslisting);
  ...
  do_long(block){
    ...
    let [t,h] = this.convert_longpadding(this.config.CONTEX.longpadding);
    let vlines = this.toArray(this.config.CONTEX.longvlines);
    let hlines = this.toArray(this.config.CONTEX.longhlines);
  ...
  do_pict(block){
    ...
    for(var blk of rows){
      ...
      for(var img of images){
        ...
        sub_text = this.polish(sub_text,this.config.CONTEX.fssubcaption);
  ...
  escape_diag(text) {
    var fs = this.config.CONTEX.diagfontsizept;
    var fs = `${fs}pt`;
  ...
  */
  ...
  toContexDocument() {
    ...
    for(var i=0; i < this.blocks.length; ++i){
      ...
      if(this.config.CONTEX.twocolumns && sig=='HDGS' && hdgn==0){      
...
\\setupbodyfont[linuxlibertineo,${config.CONTEX.bodyfontsizept}pt]
...
\\stoptext
  ...
  make_line_array(math,label,gather,mainlabel,n){
    if (label == '#') {
      label = '[+]';
    } else if (label) {
      label = `[${label.slice(1)}]`;
    ...
    var math = this.formulamath(math);
    var lines = math.split('\n');
    lines = lines.filter(x => (x.length>0)?true:false);
    if(lines.length==1){
      var line = lines.join('');
      var pp = line.split('\r');
      if(pp.length == 1){
        pp.push('');//make it two columns
      }else if(pp.length >2){
        //need to merge columns that are longer than 2
        ///Otherwise three or more \NC will be inserted,
        ///this will case context to hang when compiling,
        ///thus we need to ensure only at most two pp's
        ///are there.
        while(pp.length > 2){
          var m = pp.pop();
          pp[pp.length-1] = this.joinLine(pp[pp.length-1],m);
        }
      }
      if(gather){
        var p = pp.join(' ');
  ...
  make_line(math,label,mainlabel,n){
    if (label == '#') {
      label = '[+]';
    } else if (label) {
      label = `[${label.slice(1)}]`;
    ...
    var line = '\\NC '
    line += '\\startmathalignment[distance=2pt]'
    line += `\n`;
    line += lines.join('\n');
    line += '\n';
    line += '\\stopmathalignment';
    line += ' \\NC\\NR'
    line += label;
    return line;
  }
  ...
  make_math(o,math,label,more,gather){
    var n = 0;//the number of labels in 'more'
    ...
    for(let k=0; k < more.length; k++){
      let x = more[k];
      var line_arr = this.make_line_array(x.math,x.label,gather,label,n);
      line_arr.forEach(x => o.push(x));
    }
