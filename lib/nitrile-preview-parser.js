...
class NitrilePreviewParser {
  ...
  newConfig() {
    ...
    config.TABR = {};
    config.TABR.hlines = 'm';
    config.TABR.vlines = '';
    config.TABR.vpadding = 1;
    config.TABR.hpadding = 6;
    config.TABR.cssfontsize = '0.8em';
    config.TABR.latfontsize = '\\footnotesize';
    config.TABR.confontsize = '\\tfx';
    config.SAMP = {};
  ...
  readFromLines(lines) {
    ...
    */
    ...
    while (lines.length > 0) {
      ...
      } else if (this.isTabbBlock(para)) {

        var data = this.parseTabbBlock(para);
        sig = 'TABB';
        o.push({rmap,row1,row2,sig,html,data,para,fencecmd,subrow});
        block_type = 'TABB';
        continue

      } else if (this.isTabrBlock(para)) {

        var data = this.parseTabrBlock(para);
        sig = 'TABR';
        o.push({rmap,row1,row2,sig,html,data,para,fencecmd,subrow});
        block_type = 'TABR';
        continue
  ...
  parseTabbBlock(para){
    ...
    var o = o.map( x => this.expandArray(x,maxj,'') );
    /// now change it so that data.length is the number of columns
    var data = [];
    var nrows = o.length;
    for(var j=0; j < maxj; ++j){
      var items = o.map(x => x[j]);
      data.push(items);
    }
    return data; 
  }
  ...
  parseTabbPara(para){
    ...
    var data = data.map(row => this.splitLineTwoSpaces(row));
