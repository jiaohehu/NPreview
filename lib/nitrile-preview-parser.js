...
class NitrilePreviewParser extends NitrilePreviewBase {
  ...
  read_md_lines(lines) {
    ...
    while (nlines < lines.length) {
      ...
      var block  = this.read_para(lines,nlines,mode,rmap,notes);
      var para = block.para;
      var sig = block.sig;
  ...
  read_para (lines,n,mode,rmap,notes) {
    let n0 = n;
      ...
      if((v=re_comm.exec(line))!==null){
        ...
        if ((v = re_nitrilemode.exec(nitrile_line)) !== null) {
          ...
          if(key==='config'){
            ...
            let text=val;
            let refid=n;
            mode.push({name,text,refid});
          }else if(key==='chapter'){
            let name='h';
            let subn=0;
            let subf=val;
            let refid=n;///let refid be the same as linenum
            mode.push({name,only,subn,subf,refid});
          }else if(key==='section'){
            let name='h';
            let subn=1;
            let subf=val;
    ...
    if(type == 'spcl'){
      ...
      var cols = this.parse_tabu(body,wide);
  ...
  translate_blocks(translator) {
    ...
    // var obj = this.config[translator.name];
