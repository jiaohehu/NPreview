...
class NitrilePreviewParser {
  ...
  constructor() {
    ...
    this.re_unmask = /(?<!\S)\{\{(.*?)\}\}|(?<!\S)``(.*?)``|(?<!\S)`(.*?)`|\$\{(.*?)\}|\$\(#([\w:]*)\)/g;
    this.re_jpchar = /[\u3000-\u303F]|[\u3040-\u309F]|[\u30A0-\u30FF]|[\uFF00-\uFFEF]|[\u4E00-\u9FAF]|[\u2605-\u2606]|[\u2190-\u2195]|\u203B/g;
    this.re_texmath = /\$\$(.*?)\$\$|\$(.*?)\$/g;
    this.re_texcomment  = /^%(.*)$/;
  ...
  read_para (lines) {
    ...
    for (let i=0; n < lines.length; ++i,++n) {
      ...
      if(i==0 && (v=re_spcl.exec(line))!==null){
        ...
        if((v=re_caption.exec(s))!==null){
          ...
          iscaption = 1;
          continue;
        } else if ((v=re_fence.exec(s))!==null){
          isfenced = 1;
          fenceid = v[2];
          continue;
        } else if ((v=re_sep.exec(s))!==null){
          continue;
        } else {
          body.push(s);
          continue;
    ...
    if(type == 'pict'){
      if(fenceid == 'framed'){
        ...
        return {para,lines,sig,caption,label,body};
        ...
        return {para,lines,sig,caption,label,rows,body};
      }
    } 
    else if (type == 'tabr'){
      var cols = this.parse_tabb(body);
      return {para,lines,sig,caption,label,cols,isfenced,fenceid,body};
    } 
    else if (type == 'tabb'){
      var cols = this.parse_tabb(body);
      return {para,lines,sig,caption,label,cols,isfenced,fenceid,body};
      ...
      return {para,lines,sig,label,maths,body};
    }
    else if(type == 'hdgs'){
      return {para,lines,sig,hdgn,label,text};
    } 
    else if(type == 'quot'){
      text = this.joinPara(body);
      return {para,lines,sig,text,body};
    }
    else if(type == 'plst'){
      var items = this.parse_plst(body);
      return {para,lines,sig,items,body};
    }
    else if(type == 'ilst'){
      ...
      return {para,lines,sig,data,body};
    }
    else if(type == 'samp'){
      body = this.trim_samp_body(body);
      return {para,lines,sig,body};
    } 
    else if(type == 'text'){
      var {leadn,lead,text} = this.parse_text(body);
      return {para,lines,sig,leadn,lead,text,body};
    } 
    else if(type){
      console.log(`unhandled type: (${type})`);
      console.log(para);
      return {para,lines,sig};
    }
    else {
  ...
  unmask (line) {
    ...
    while ((v = this.re_unmask.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        } else if(this.re_jpchar.test(cnt)){
          var mytext = cnt;
  ...
  async readModeAsync(dirname) {
    ...
    for(let d of this.mode){
      ...
      if(name==='root' && subparser){
        ...
        this.ismaster = 1;
        console.log('ismaster is set to 1');
