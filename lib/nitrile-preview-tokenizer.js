        ...
        if (pjson.fontVariants[key]) {
          for (var c of str) {
            ...
            if (val) {
              ...
              o.push( `<text class='MyText' x='${x}pt' dx='${dx}' dy='9pt'>${unicode}</text>` );
              x += width;
              w += width;
            }
          }
...
${news}
  ...
  toResizedSvg(str,dmath,x,y,rate,color) {
    ...
    for (var mathSymbol of pjson.mathSymbols) {
      let {dy,dx,id,unicode,width} = mathSymbol;
      if (this.used.has(id)) {
        pp.push(`<text class='MyText' id='${id}' dy='9pt' textLength='${width}pt' lengthAdjust='spacingAndGlyphs'>${unicode}</text>`);
      }
    }
    var defs= `\
<defs>
