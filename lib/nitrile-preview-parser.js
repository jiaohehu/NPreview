...
class NitrilePreviewParser {
  ...
  constructor() {
    this.block = null;  /// the active block during 'unmask'
  ...
  toBLOCKS(lines,fname,plevel,o) {
    ...
    */
    ...
    if (lines.length && lines[0] === '% !BOOK') {
      var data = lines;
      var fencecmd = {};
      o.push(['',0,lines.length-1,'!BOOK',0,data,lines,fencecmd,refname,caption,fname,plevel]);
    ...
    while (lines.length > 0) {
      ...
      if (flags.verbatim && this.re_four_spaces.test(para[0])) {
        var [data, n] = this.trimPara(para); /// n is to be ignored
        var fencecmd = {};
        o.push([id,row,row2,'VERB',0,data,para,fencecmd,refname,caption,fname,plevel]);
        continue;
      }
      ...
      if (the_fence === '```') {
        ...
        o.push([id,row,row2,'CODE',n,data,para,fencecmd,refname,caption,fname,plevel]);
        continue;
      }
      ...
      if (the_fence === '~~~') {
        ...
        var [data,fence,fencecmd] = this.trimFences(data);
        id = this.toSid(id,fencecmd.label);
        o.push([id,row,row2,'VERB',n,data,para,fencecmd,refname,caption,fname,plevel]);
        continue;
      }
      ...
      if (the_fence === '---') {
        ...
        var [data,fence,fencecmd] = this.trimFences(data);
        id = this.toSid(id,fencecmd.label);
        o.push([id,row,row2,'VRSE',n,data,para,fencecmd,refname,caption,fname,plevel]);
        continue
      }
      ...
      if (the_fence === '///') {
        ...
        id = this.toSid(id,fencecmd.label);
        o.push([id,row,row2,'PICT',n,data,para,fencecmd,refname,caption,fname,plevel]);
        continue;
      }
      ...
      if (the_fence === '===') {
        ...
        id = this.toSid(id,fencecmd.label);
        o.push([id,row,row2,'TABB',n,data,para,fencecmd,refname,caption,fname,plevel]);
        continue;
      }
      ...
      if (the_fence === '<<<') {
        ...
        id = this.toSid(id,fencecmd.label);
        o.push([id,row,row2,'TERM',n,data,para,fencecmd,refname,caption,fname,plevel]);
        continue;
      }
      ...
      if (the_fence === '\"\"\"') {
        ...
        id = this.toSid(id,fencecmd.label);
        o.push([id,row,row2,'QUOT',n,data,para,fencecmd,refname,caption,fname,plevel]);
        continue;
      }

      // this is the SPEC block
      if (the_fence === '@@@') {
        var [data, n] = this.trimPara(para);
        var [data,fence,fencecmd] = this.trimFences(data);
        var data = this.parseSPEC(data);
        id = this.toSid(id,fencecmd.label);
        o.push([id,row,row2,'SPEC',n,data,para,fencecmd,refname,caption,fname,plevel]);
        continue;
      }
      ...
      if (this.re_hyphen_hyphen.test(para[0])) {
        ...
        var fencecmd = {};
        o.push([id,row,row2,'DESC',n,data,para,fencecmd,refname,caption,fname,plevel]);
        continue;
      }
      ...
      if (v) {
        ...
        var data = [lead,text];
        var fencecmd = {};
        o.push([id,row,row2,'PRIM',0,data,para,fencecmd,refname,caption,fname,plevel]);
        continue;
      }
      ...
      if (v) {
        ...
        var data = [lead,text];
        var fencecmd = {};
        o.push([id,row,row2,'SECO',0,data,para,fencecmd,refname,caption,fname,plevel]);
        continue;
      }
      ...
      if (v) {
        ...
        var fencecmd = {};
        o.push([id,row,row2,'PLST',n,data,para,fencecmd,refname,caption,fname,plevel]);
        continue
      }
      ...
      if ((v = this.re_hdgs.exec(para[0])) !== null) {
        ...
        var fencecmd = {};
        o.push([sid,row,row2,'HDGS',0,data,para,fencecmd,refname,caption,fname,plevel]);
        continue;
      }
      ...
      if (nblocks == 1) {
        ...
        var fencecmd = {};
        o.push([id,row,row2,'HDGS',0,data,para,fencecmd,refname,caption,fname,plevel]);
        continue;
      }
      ...
      if (true) {
        ...
        var data = this.joinPara(data);
        var fencecmd = {};
        o.push([id,row,row2,'',n,data,para,fencecmd,refname,caption,fname,plevel]);
        continue;
      }
  ...
  async toBookBLOCKS(lines,dirname) {
    ...
    for (var myfile of myFiles) {
      let [sublevel,subfname] = myfile; 
      if (dirname && subfname) {
        var fsubfname = path.join(dirname,subfname);
        var subdata = '';
        try {
          await fs.statSync(fsubfname);
          var subdata = await this.readFileAsync(fsubfname);
        } catch(e) {
          var subparser = new NitrilePreviewParser();
          myBlocks.push(subparser.newFILE(subfname,sublevel));
          myBlocks.push(subparser.newHDGS(`no such file: '${subfname}'`,subfname,sublevel));
          myBlocks.push(subparser.newFILEND(subfname,sublevel));
          console.log(e.stack);
          continue;
        }
        var subparser = new NitrilePreviewParser();
    ...
    while ((v = this.re_unmask.exec(str)) !== null) {
      ...
      if (v[1]) {
        ...
        if (type === 'uri') {
          ...
          } else if (this.re_full.test(cnt)) { /// such as d3js:intro
            newtext += this.style('ref',cnt);
          } else {
  ...
  newFILE (fname,plevel) {
    ...
    return [id,-1,-1,'FILE',0,[],[],fencecmd,refname,caption,fname,plevel];

  }

  newFILEND (fname,plevel) {

    /// create a new 'FILEND' block

    var fencecmd = {};
    var refname = '';
    var caption = '';
    var id = `${this.toBase(fname)}::`;
    return [id,-1,-1,'FILEND',0,[],[],fencecmd,refname,caption,fname,plevel];
  ...
  newHDGS (str,fname,plevel) {
    ...
    var refname = '';
    var caption = '';
    return ['',-1,-1,'HDGS',0,[0,str],[],fencecmd,refname,caption,fname,plevel];
  ...
  idenBLOCKS (blocks,isarticle) {

    /// assign chapter/section/figure/listing numbers for each block
    /// will change the 'refname' field of the block. Returns
    /// a new block.
    ...
    for (var block of blocks) {
      ...
      /// overwrite the 'refname' field of block
      if (sec_text) {
        block[8] = sec_text;
      }
      o.push(block);
