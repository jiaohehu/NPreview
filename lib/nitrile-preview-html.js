      ...
      if(more && more.length){
        more.forEach((plitem) => {
          ...
          if(isbroad){
            ...
            extra_text += `<div>${this.untext(lines.join('\n'))}</div>`;
          }
        });
      }
      switch (bull) {
...
${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
  ...
  do_mult(block) {
    ...
    for (let j = 0; j < more.length; ++j) {
      ...
      var w = (1 - (0.02 * (n - 1))) / n;
      var gap = 0.02;
      o.push(`<table border='0' style='border-collapse:collapse;width:100%;'`);
      o.push(`<tr>`)
      for (let i = 0; i < n; ++i) {
        let text = pp[i] || '';
        o.push(`<td style='width:${w*100}%'>`);
        o.push(this.untext(text));
        o.push(`</td>`);
        if(i < n-1){
          o.push(`<td style='width:${gap*100}%'></td>`);
        }
      }
      o.push(`</tr>`);
      o.push(`</table>`)
  ...
  do_fige(block) {
    ...
    var o = []; 
    var text = this.untext(body.join('\n'));
    o.push(`<figure ${this.to_attr(block)} >`);
    o.push(this.to_caption_text(floatname, idnum, caption));
    o.push(text);
    o.push('</figure>');
