...
class NitrilePreviewNode {
  ...
  async to_slide_document_async (filename) {
    ...
    /// construct an htmlfil always the same as the input + .html
  ...
  async to_article_document_async (filename) {
    ...
    /// translator blocks, the translation is stored with each block
    /// at its .latex member.
    const translator = new NitrilePreviewArticle(parser);
    translator.translate_blocks();

    /// now construct a Context document
    const data = translator.to_article_document();
  ...
  async to_xelatex_document_async (filename) {
    ...
    /// translator blocks, the translation is stored with each block
    /// at its .latex member.
    const translator = new NitrilePreviewXelatex(parser);
    parser.translate_blocks(translator);

    /// now construct a Context document
    const data = translator.to_xelatex_document();
  ...
  async to_xecjk_document_async (filename) {
    ...
    /// translator blocks, the translation is stored with each block
    /// at its .latex member.
    const translator = new NitrilePreviewXecjk(parser);
    parser.translate_blocks(translator);

    /// now construct a Context document
    const data = translator.to_xecjk_document();
  ...
  async to_lualatexja_document_async (filename) {
    ...
    /// translator blocks, the translation is stored with each block
    /// at its .latex member.
    const translator = new NitrilePreviewLualatexja(parser);
    parser.translate_blocks(translator);

    /// now construct a Context document
    const data = translator.to_lualatexja_document();
  ...
  async to_lualatex_document_async (filename) {
    ...
    /// translator blocks, the translation is stored with each block
    /// at its .latex member.
    const translator = new NitrilePreviewLualatex(parser);
    parser.translate_blocks(translator);
  ...
  async to_pdflatex_memoir_document_async (filename) {
    ...
    /// translator blocks, the translation is stored with each block
    /// at its .latex member.
    const translator = new NitrilePreviewMemoir(parser);
    parser.translate_blocks(translator);
  ...
  async to_lualatex_memoir_document_async (filename) {
