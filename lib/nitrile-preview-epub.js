...
class NitrilePreviewEpub extends NitrilePreviewParser {
      ...
      switch (bull) {
        case 'OL': {
          if(count==0){
            ...
            o.push(`<ol class='PLST' style='${STYLE}'><li value='${value}'>${text}`);
          }
          break;
        }
        case 'UL': {
          if(count==0){
            ...
            o.push(`<ul class='PLST' style='${STYLE}'><li value='${value}'>${text}`);
          }
          break;
        }
        case 'LI': {
  ...
  do_samp(block) {
    ...
    var text = body.join('\n');
    o.push(`<pre class='SAMP' ${this.toATTR(block)} style='margin-left:${this.conf('step')}em;'>${text}</pre>`);
    o.push('');
    block.html = o.join('\n');
  }
  do_pass(block) {
    ...
    o.push(`<p class='PASS' ${this.toATTR(block)} style='margin-left:${this.conf('step')}em;'>${text}</p>`);
    o.push('');
    block.html = o.join('\n');
  }
  do_hrle(block) {
  ...
  do_pict(block) {
    ...
    for(var blk of rows){
      ...
      var qq = imgs.map( img => {
        ...
        return(`<td style='${tdstyle}'> ${sub_text} </td>`);
      });
      if(mode && mode.width){
        s.push(`<table class='PICT' border='0' style='width:${100*mode.width}%;'>`);
      }else{
        s.push(`<table class='PICT' border='0' style='width:100%;'>`);
      }
  ...
  do_quot(block) {
    ...
    text = `${lq}${text}${rq}`;
    var o = [];
    o.push(`<blockquote class='${sig}' ${this.toATTR(block)} style='margin-left:${this.conf('step')}em;right-margin:0;${this.xstyle}'>`);
    o.push(text);
    o.push(`</blockquote>`);
    o.push('');
    block.html = o.join('\n');
...
${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
  ...
  do_math(block) {
    var {id,row1,row2,sig,math,label,wide,idnum,more} = block;
    var o = []; 
    var data = this.make_math_svg(math,label,idnum,more,wide);
    var text = data.join('\n');
  ...
  do_ref (sig,label,floatname,idnum,saveas,id){
    if(sig=='MATH'){
      return `<a class='niref' href='${saveas}#${id}'>${floatname}.${idnum}</a>`;
    }
    if(sig=='HDGS'){
  ...
  to_outer_svg(s,w,h,mid,defs){
    ...
    var s = `<svg class='EXPR' ${opt}> <defs>${defs.join('\n')}</defs> ${s} </svg>`;
    return s;
  }
  ...
  make_line_svg(math,label,idnum,wide){
    var d = this.tokenizer.toMathSvgArray(math,true,wide);
    d = d.filter(x => (x.w > 0));
  ...
  make_math_svg(math,label,idnum,more,wide){
    var data = [];
    var line = this.make_line_svg(math,label,idnum,wide);
    data.push(line);
    if(more&&more.length){
      for(let k=0; k < more.length; k++){
        let x = more[k];
        var line = this.make_line_svg(x.math,x.label,x.idnum,wide);
        data.push(line);
      }
    }
...
`;
  ...
  async generateAsync (parser,dirname) {
    ...
    var mytitle = parser.conf('title');
    var myauthor = parser.conf('author');
    var imgsrcs = blocks.map(x => x.imgsrcs);

    ///get the stylesheet from 'css' property
    stylesheet = parser.conf('css');
