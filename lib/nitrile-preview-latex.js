    ...
    for (var block of all) {
      ...
      switch (sig) {
        ...
        case 'tabular': {
          ...
          if(fencecmd.label) {
            o.push(`\\begin{figure}[h!]${label_text}`);
          ...
          if (config.latexTableStyle=='full' || config.latexTableStyle=='standard') {
            o.push(`\\toprule`);
          }
          var row = text.shift();
          if (1) {///HEADER
            row = row.map(x => x.map(y => `\\textbf{${y}}`));
            var nn = row.map(x => x.length);
            var maxn = nn.reduce((acc,cur) => Math.max(acc,cur));
            for (var i=0; i < maxn; ++i) {
              var qq = row.map(x => x[i]);
          ...
          for (var row of text) {
            ...
            if (config.latexTableStyle==='full') {
              o.push(`\\midrule`);
            } else {
              o.push(`\\noalign{\\medskip}`);
            }
