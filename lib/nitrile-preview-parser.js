    ...
    while (lines.length > 0) {
      ...
      if (the_fence === '```' ) {
        ...
        if (brand === 'imgs') {
          ...
          data = this.parseTABL(data);
        } else if (brand === 'tabulary') {
          data = this.parseTABL(data);
        } else if (brand === 'tabularx') {
          data = this.parseTABL(data);
        } else if (brand === 'longtabu') {
          data = this.parseTABL(data);
        } else if (brand === 'math') {
...
///console.log(config);
  ...
  isSkipping (config, dept) {
    if (config.previewOutlineViewEnabled && !this.isSectionShowing(dept,config.previewExpandSections)) {
      return true;
    } else {
      return false;
    }
  ...
  toAdjustedColumns (x_count,adjust) {
    ...
    o = o.map(x => parseFloat(x));
    o = o.map(x => x.toFixed(3));
    return o;
  }
  ...
  parseTABULAR (para) {
    ...
    for (var row of para) {
      ...
      /// second column
      if (re_twospaces.test(row)) {
        row = `{}${row}`;
      }
      var pp = this.splitLineTwoSpaces(row);
      var pp = pp.map(s => s.trim());
      var pp = pp.map(s => s=='{}'?'':s);
      var pp = pp.map(s => s.replace(/\\\\$/,'\n'));
      for (var i in pp) {
        if (i < s0.length) {
          s0[i] = this.joinLine(s0[i],pp[i]);
        } else {
          s0.push(pp[i]);
        }
      }
