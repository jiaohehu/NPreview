...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  p_label(x, y, txt, ts, ta, g) {
    ...
    var x = this.localx(x);
    var y = this.localy(y);
    var labeldx = this.localdist(this.config.labeldx);
    var labeldy = this.localdist(this.config.labeldy);

    /// (x,y) is now in SVG-coordinates. 

    var gapx = my_svglabel_dx + labeldx; 
    var gapy = my_svglabel_dy + labeldy;
    var dx = 0;
    var dy = 0;
    if (ta==='lrt') {
