...
class NitrilePreviewParser {
    ...
    */
    ...
    while (lines.length > 0) {
      ...
      if (block_type === 'SAMP') {
        ...
        sig = 'SAMP';
        o.push({id,row1,row2,sig,html,data,para,fencecmd,subrow});
        continue;
        ...
        if (v2) {
          ...
          sig = 'HDGS';
          var level = hdgn;
          o.push({id,row1,row2,sig,html,hdgn,level,data,para,fencecmd,subrow});
          continue;
        }
        ...
        var data = text;
        sig = 'TEXT';
        o.push({id,row1,row2,sig,html,lead,leadn,data,para,fencecmd,subrow});
        block_type = 'TEXT';
  ...
  new_hdgs_block (text) {
    ...
    var level = 0;
    var hdgn = 0;
    return ({id,row1,row2,sig,html,level,hdgn,data,para,fencecmd,subrow});
  ...
  translateBlocks(options={}) {
    ...
    var editorrow = options.editorrow||0;
    var editorcolumn = options.editorcolumn||0;
    this.ispreview = ispreview;
    this.textblockcount = 0;
    this.block = [];
    this.haschapter = this.config.haschapter;
    for (var block of this.blocks) {
      var {id,row1,row2,sig,level,data,para,fencecmd,subrow} = block;
      this.block = block;
