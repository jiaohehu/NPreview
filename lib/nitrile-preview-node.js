...
const pjson = require('./nitrile-preview-config');
const fs = require('fs');
...
.nitrile-preview table tr th { padding: 2px 6px; }
.nitrile-preview table tr td { padding: 2px 6px; }
.nitrile-preview table tr th { text-align: center; }
.nitrile-preview .TABB { table tr th { border-top: 1px solid rgb(51, 51, 51); border-bottom: 1px solid rgb(51,51,51); }}
...
class NitrilePreviewNode {
  ...
  async toLatex (filename) {
    ...
    let lines = out.split('\n');
    let [blocks,flags,book] = parser.toBlocks(lines,path.basename(filename),0);
    if (book.length) {
      [blocks,flags] = await parser.toBookBlocks(book,path.dirname(filename));
      var isarticle = false;
    } else {
      var isarticle = true;
    }
    ...
    for (var key in schema) {
      if (schema.hasOwnProperty(key)) {
        config[key] = parser.getConfig(key,flags,schema);
    ...
    var title = flags.title ? flags.title : 'Untitled'
    var author = flags.author ? flags.author : ''
    var documentclass = flags.documentclass ? flags.documentclass : ''
    if (!documentclass) {
...
\\end{document}\n`;
  ...
  async toHtml (filename) {
    ...
    var out = await parser.readFileAsync(filename);
    let lines = out.split('\n');
    var [blocks,flags,book] = parser.toBlocks(lines,path.basename(filename),0);
    if (book.length) {
      [blocks,flags] = await parser.toBookBlocks(book,path.dirname(filename));
      var isarticle = false;
    } else {
