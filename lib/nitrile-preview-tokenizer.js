...
console.error(g);
  ...
  getNextCommand (items ) {
    ...
    /// each item could be a primitive or also a '\\brace' 
    /// or '\\leftright' group
    ...
    if (Array.isArray(item0)) {
      ...
      items = items.slice(1);
      [items,group] = this.getNextGroup(items);
      return [items,['\\sqrt',group]];

    } else if (item0 === '\\frac') {

      items = items.slice(1);
      [items,group1] = this.getNextGroup(items);
      [items,group2] = this.getNextGroup(items);
      return [items,['\\frac',group1,group2]];
      ...
      [items,group1] = this.getNextGroup(items);
      [items,group2] = this.getNextGroup(items);
      return [items,['\\binom',group1,group2]];

    } else {

      return this.getThisToken(items);
