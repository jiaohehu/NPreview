...
class NitrilePreviewLatex extends NitrilePreviewParser {
  ...
  translateLatex (all,config) {
    ...
    for (var block of all) {
      const [id,row1,row2,sig,n,data,para,fencecmd,caption,base,_label,fname] = block;
      this.block = block;
      this.base = base;
      this.row1 = row1;
      this.row2 = row2;
      this.fname = fname;
      const label_text = (_label)?`${base}:${_label}`:'';
      const label_cmd = (_label) ? `\\label{${label_text}}` : '';
      ...
      switch (sig) {
        ...
        case 'SAMP': {
          ...
          const xsize = config.latexSampleFontsize;
          o.push(`\\begin{flushleft}`);
          o.push(`\\begin\{adjustwidth\}{${xleft}cm}{}`)
        ...
        case 'tabular': {
          ...
          var fs = config.latexTabularFontsize;
        ...
        case 'longtable': {
          ...
          var fs = config.latexLongtableFontsize;
        ...
        case 'listing': {
          var myopts=[];
          var fs = config.latexListingFontsize;
          ...
          var text = data;
          if (onecolumn && config.latexTwoColumnEnabled) {
            o.push('\\onecolumn');
          }
