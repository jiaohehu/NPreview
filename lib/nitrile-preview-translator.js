  ...
  untext(line,isstory=null) {
    ...
    if(line.startsWith('~~~')&&line.endsWith('~~~')){ 
      ...
      if(this.para_is_multi(ss)){
        let cols = this.para_to_multi(ss);
        return this.to_multi(cols);
      }
      if(this.para_is_cols(ss)){
        let cols = this.para_to_cols(ss);
        return this.to_cols(cols);
      }
      if(this.para_is_list(ss)){
        let itms = this.para_to_list(ss);
        return this.to_list(itms);
      }
      if(this.para_is_rows(ss)){
        let data = this.para_to_rows(ss);
        return this.to_rows(data);
      }
      if(this.para_is_math(ss)){
        let data = this.para_to_math(ss);
        return this.to_math(data);
      }
      ss = this.join_backslashed_lines(ss);
      ss = this.join_indented_lines(ss);
      return this.to_verse(ss);
    } 
    if (line.startsWith('"""') && line.endsWith('"""')) {
      let ss = line.slice(3, n - 3).split('\n');
      ss = this.trim_samp_body(ss);
      return this.to_blockquote(ss);
    }
    if(isstory){
      let ss = line.split('\n');
      return this.to_story(ss);
    }
