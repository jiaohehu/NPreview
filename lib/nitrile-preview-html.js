...
`;
  ...
  do_hdgs(block) {
    var {id,row1,row2,sig,part,level,data,para,dept,fencecmd,base,subrow} = block;
    var o = [];
  ...
  do_dlst(block) {
    ...
    for (var item of data) {
      ...
      o.push(`<dd style='padding-left:${fencecmd.padding}em;' >${text}</dd>`);
    }
    */
    o.push('</dl>');
    o.push('');
  ...
  do_ilst(block){
    ...
    var o = [];
    o.push(`<ul ${this.xattr} style='${this.xstyle};padding-left:${fencecmd.padding}em;list-style-position:outside;'>`);
    var items = data;
    for(var item of items){
      ...
      if(dt){
        dt = this.escape(dt);
        //text = `\\type{${dt}} ${sep} ${text}`;
        text = `<li><code>${dt}</code> ${sep} ${text}</li>`;
      }
      o.push(text);
    }
    o.push(`</ul>`);
  ...
  do_samp(block) {
    ...
    if(fencecmd.style==1){
      ...
      //if(fencecmd.monospace){
        //text = text.map( x => `<code>${x}</code>` );
      //}
      text = text.map( x => `<dt>${x}</dt>` );
      text = text.join('\n');
      o.push(`<dl ${this.xhtmlattr}><div style='line-height:1'>${text}</div></dl>`);
      o.push('');
    }
    else{
      text = this.trimSampPara(text);
      ...
      text = text.map( x => `<dt>${x}</dt>` );
      text = text.join('\n');
      o.push(`<dl id='${id}' class='P ${sig}' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' style='line-height:1;margin-left:${fencecmd.left}mm;margin-top:${this.xtop};margin-bottom:${this.xbot};};'>${text}</dl>`);
      o.push('');
    } 
    block.html = o.join('\n');
  }
...
`;
  ...
  toXhtmlDocument() {
    ...
    geometry_opts.push(`font-family:roman`);
    geometry_opts.push(`font-size:${config.HTML.bodyfontsizept}pt`);
    var title = config.ALL.title||'Untitle';
    var author = config.ALL.author||'';
    title = this.escape(title);
    author = this.escape(author);
    var data = `\
