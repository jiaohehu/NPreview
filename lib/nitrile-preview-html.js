  ...
  itms_to_tabulate(rows,style){
    ...
    if(border){
      ...
      for (var j = 0; j < nrows; ++j) {
        ...
        if (this.pp_is_hline(pp)) {
          let ww = 'x'.repeat(ncols).split('').map(x => '');
          d.push(`<tr><td style='background-color:currentColor' colspan='${ncols}'></td></tr>`);
          continue;
        }
      ...
      for (var j = 0; j < nrows; ++j) {
        var pp = rows[j];
        if (this.pp_is_hline(pp)) {
          let ww = 'x'.repeat(ncols + ncols - 1).split('').map(x => '');
          d.push(`<tr><td style='border-top:1px solid;border-bottom:1px solid;' colspan='${ncols + ncols - 1}'></td></tr>`);
          continue;
        }
        pp = pp.map((x, i) => `<td style='padding:0'>${x}</td>`);
        let gap = `<td style='padding:0 1em'></td>`;
        var p = pp.join(gap);
        var p = `<tr>${p}</tr>`;
        d.push(p);
      }
