...
class NitrilePreviewHtml extends NitrilePreviewParser {
  ...
  do_samp(block) {
    ...
    var text = body.join('\n');
    o.push(`<pre ${this.toATTR(block)} style='margin-left:${this.conf('step')}em;'>${text}</pre>`);
    o.push('');
    block.html = o.join('\n');
  }
  do_pass(block) {
  ...
  do_csvd(block){
    ...
    for(var j=0; j<nrows; ++j){
      ...
      pp = pp.map((x,i) => `<td style='padding:0 3pt'>${x}</td>`);
      var p = pp.join('');
      var p = `<tr>${p}</tr>`;
      s.push(p);
    }
    var text = s.join('\n');
    var text = `<table ${this.toATTR(block)} border='1' style='${this.toMSTYLE(block)};border-collapse:collapse;'>${text}</table>`;
    o.push(text);
    o.push('');
    block.html = o.join('\n');
  }
...
${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
    ...
    s.push(`</tbody>`);
    var text = s.join('\n');
    text = `<table ${this.toATTR(block)} border='0' style='border-collapse:collapse;width:100%;'>${text}</table>`;
