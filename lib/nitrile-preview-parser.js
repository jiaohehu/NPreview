  ...
  toBlocks(lines,fname,subrow,plevel,o=null) {
    ...
    while (lines.length > 0) {
      ...
      if (the_fence === '~~~' || the_fence === '```') {
        ...
        } else if (fencecmd.program) {
          if (fencecmd.label) {
            label = `${base}:${fencecmd.label}`;
          }
          o.push([id,row,row2,'PRGM',n,data,para,fencecmd,dept,fig,caption,base,label,fname,subrow,plevel]);
        } else if (the_fence === '~~~') {
