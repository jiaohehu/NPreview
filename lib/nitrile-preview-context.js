...
class NitrilePreviewContext extends NitrilePreviewLatex {
  ...
  do_LLST(block){
    var {id,row1,row2,sig,wide,body,idnum,floatname,caption,label,islabeled} = block;
    var o = [];
    o.push('');
    o.push(this.to_info(block));
    var d = body.map((x,i) => {
  ...
  do_FIGE(block){
    var {rows,caption,label,islabeled,style,body} = block;
    var o = [];
    o.push(this.to_info(block));
  ...
  to_longtable(itms, style) {
    ...
    var caption = this.unmask(style.caption||'');
    return this.itms_to_longtable(itms, style.fr, style.hline, style.glue, style.n, caption, style.label, style.floatname);
  }
  to_multi(itms,style){
    itms = itms.map(s => {
      s = this.unmask(s);
    ...
    return this.itms_to_multi(itms,style.fr,style.hline,style.n);
  }
  ...
  to_imgrid(imgs,style){
    ...
    var text = this.imgs_to_combination(imgs,style.frame,style.n);
    return text;
  }
  to_br() {
    let text = '\\crlf';
...
\\stoptext
  ...
  itms_to_itemized(itms,fr,n){
    ...
    if(itms.length && itms[0].bull == 'OL'){
      ...
      if (n && n > 1) {
        return this.pp_to_multi_itemized(pp, fr, n, 'n')
      }else{
    ...
    if (itms.length) {
      ...
      if (n && n > 1) {
        return this.pp_to_multi_itemized(pp, fr, n, '')
      } else {
  ...
  imgs_to_combination(imgs,frame,n){
    var n = parseInt(n);
    var n = n || imgs.length;
    var m = Math.ceil(imgs.length/n);
    var w = (1-(0.02*(n-1)))/n;
    var d = [];
