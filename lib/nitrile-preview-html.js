...
`
  ...
  translateHtml (autonum,config,xrefs,blocks,isarticle,subrow,plevel,o) {
    ...
    for (var block of blocks) {
      ...
      this.block = block;
      const [id,row1,row2,sig,n,data,para,fencecmd,_dept,_fig,caption,base,label,fname] = block;
      ...
      const showidtext    = config.showids    ? `<mark style='font-size:small; position:absolute; left:0px; top:-1em; '>[${id}]</mark>` : '';
      ...
      switch (sig) {
        ...
        case 'SMPL': {
          if (this.isSkipping(config,dept)) break;
          var text = data;
          if (config.samplestyle === 'blockquote') {
            text = this.joinPara(text);
            text = this.escape(text);
            text = this.rubify(text);
            o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' subrow='${subrow}' rows='${row1} ${row2}' style='${this.divcssstyle}; margin-left:0; '>`);
            o.push(`${showidtext}`);
            o.push(`<blockquote style='${this.samplecssstyle}; margin:0 ${config.stepmargin}cm; ' >`);                
            o.push(text);
            o.push(`</blockquote>`);
            o.push(`</div>`);
            o.push('');
          } else if (config.samplestyle === 'text') {
            text = this.joinPara(text);
            text = this.escape(text);
            text = this.rubify(text);
            o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' subrow='${subrow}' rows='${row1} ${row2}' style='${this.divcssstyle}; margin-left:0; '>`);
            o.push(`${showidtext}`);
            o.push(`<p style='${this.samplecssstyle}' >`);                
            o.push(text);
            o.push(`</p>`);
            o.push(`</div>`);
            o.push('');
        ...
        case 'VERB':  
          ...
          o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' subrow='${subrow}' rows='${row1} ${row2}' style='${this.divcssstyle}; margin-left:${left}cm'>`);
          o.push(`${showidtext}`);
          o.push(`<span style='display:inline-block; width:100%; max-width:${vw}px; '>`);
          o.push( out );
          o.push(`</span>`);
          o.push(`</div>`);
          o.push('');
        ...
        case 'CODE':  
          ...
          if (fig) {
            o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' subrow='${subrow}' rows='${row1} ${row2}' style='${this.divcssstyle}; column-span:all; '>`);
            o.push(`${showidtext}`);
        ...
        case 'TABB': {
          ...
          if (fig) {
            o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' subrow='${subrow}' rows='${row1} ${row2}' style='${this.divcssstyle}; column-span:all; text-align:center; '>`);
            o.push(`${showidtext}`);
  ...
  toFramedSvg (para ) {
    ...
    o.push( `<text style='font-family:monospace;white-space:pre;font-size:10pt;' text-anchor='start' x='0pt' y='0pt' textLength='${vw}' lengthAdjust='spacing' >` );
    for (var i=0; i < npara; ++i) {
      ...
      var x = 0;
      o.push( `<tspan y='${(i+1)*10}pt' x='0pt'>${s}</tspan>` );
    }
    o.push( `</text>`);
    o.push( "</svg>" );
    return [o.join('\n'), vw, vh];
  ...
  toFramedTabb (text, maxj, ww, fencecmd) {
    ...
    for (var pp of text) {
      rowcount += 1;
      isfirstrow = (rowcount === 2);
      islastrow = (rowcount === text.length);
      pp = pp.map(x => x.split('\n'));
      if (rowcount == 1) {
