...
`;
  ...
  do_hdgs(block) {
    var {id,row1,row2,sig,hdgn,data,para,dept,fencecmd,base,subrow,fname} = block;
    var text = data;
    text = this.escape(text);
  ...
  do_verb(block) {
    ...
    if (this.xname === 'listing') {
      ...
      text = text.join('\n');
      text = `<pre>${text}</pre>`;
      o.push(`<figure id='${id}' class='P ${sig}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='font-size:${this.xcssfontsize}'>`);
  ...
  do_tblr_long(block){
    ...
    var s = [];
    if (sig==='TBLR') {
      s.push(`<table id='${id}' class='P ${sig}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' border='0' style='font-size:${this.xcssfontsize};margin-left:auto;margin-right:auto; ' >`);
    } else if (sig==='LONG') {
      s.push(`<table id='${id}' class='P ${sig}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' border='0' style='font-size:${this.xcssfontsize};width:100%; ' >`);
    }
  ...
  */
  ...
  ref (str) {
    ...
    /// if found, extracts the 'block.idnum'
    var idnum = str;
    for (var j=0; j < this.blocks.length; ++j) {
      ...
      var base = block.base;
      var saveas = block.saveas;
      var idnum = block.idnum;
