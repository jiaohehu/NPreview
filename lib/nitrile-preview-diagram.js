...
class NitrilePreviewDiagram {
  ...
  parseLine(line) {
    ...
    if ((v = this.re_area_command.exec(line)) !== null) {
      ...
      var [g,line] = this.readActionStyle(line);
      var coords = this.readCoordsLine(line);
      this.commands.push(this.p_path(coords,{fill:1},g));              
      this.commands.push(this.p_path(coords,{},g));              
      this.lastcoords = coords;
      return false;
    }
    ...
    if ((v = this.re_protractor_command.exec(line)) !== null) {
      ...
      var [g,line] = this.readActionStyle(line);
      var coords = this.readCoordsLine(line);
      this.commands.push(this.do_protractor(opt,txt,g,coords));
      this.lastcoords = coords;
      return false;
    }
      ...
      case 'ytick'://BARCHART
        for( var j=0; j < floats.length; j+=1 ) {
          ...
          if (Number.isFinite(num)){
            ...
            s.push(this.p_hbar(x,y,g));
            g.label = `${num}`;
            g.anchor = 'lft';
            if(Number.isFinite(x)&&Number.isFinite(y)){
              s.push(this.p_label(x,y,g));
            }
          }
        }
        break;
  ...
  do_shape(coords,w,h,mypath,g){
    ...
    for (var i = 0; i < coords.length; i++) {
      ...
      var x = z0[0];
      var y = z0[1];
      o.push(this.p_shape(x,y,w,h,mypath,g));
    }
    return o.join('\n');
  }
