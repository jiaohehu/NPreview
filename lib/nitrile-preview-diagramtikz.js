...
`;
  ...
  p_line(x1,y1,x2,y2,g){
    ...
    var o = [];
    if(g.dblarrow){
      o.push(`\\draw[<->,${this.to_draw(g)}] ${mypath};`)
    }
    else if(g.revarrow){
      o.push(`\\draw[<-,${this.to_draw(g)}] ${mypath};`)
    }
  ...
  p_label(x,y,txt,ts,ta,g){
    ...
    return `\\draw (${this.fix(x*unit)}mm,${this.fix(y*unit)}mm) node[${anchor}] {\\fontsize{${fs}pt}{${fs}pt}\\selectfont{}${textcolor}${txt}};`;
  }
  p_pie(x,y,radius,angle1,angle2,g){
    ...
    let mypath = `(${x*u}mm,${y*u}mm)--(${(x+x1)*u}mm,${(y+y1)*u}mm)arc[start angle=${angle1},end angle=${angle2},radius=${radius*u}mm]`
    var o = [];
    if (this.has_fills(g)) {
    ...
    for (var i in coords) {
      ...
      if (i == 0) {
        ...
        if (sweepflag) {//clockwise
          o.push(`[rotate around={${this.fix(Phi)}:(${this.fix(x0*unit)}mm,${this.fix(y0*unit)}mm)}] arc [start angle=${ang2},end angle=${ang1},x radius=${this.fix(Rx*unit)}mm,y radius=${this.fix(Ry*unit)}mm][reset cm]`);
        } else {//counter-clockwise
          o.push(`[rotate around={${this.fix(Phi)}:(${this.fix(x0*unit)}mm,${this.fix(y0*unit)}mm)}] arc [start angle=${ang1},end angle=${ang2},x radius=${this.fix(Rx*unit)}mm,y radius=${this.fix(Ry*unit)}mm][reset cm]`);
        }
        x0 = x;
        y0 = y;
      }
  ...
  to_draws(g) {
    var o = [];
    if (g.linedashed) {
      o.push(`dash pattern=2pt off 2pt on`);
    }
    ...
    if (g.linecolor) {
      o.push(`${this.to_colors(g.linecolor)}`);
    }
    return o.join(',');
  }
  to_fills(g) {
