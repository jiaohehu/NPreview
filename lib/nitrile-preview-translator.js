...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  do_phrase_ref (in_label) {
    for (var j=0; j < this.parser.blocks.length; ++j) {
      var block = this.parser.blocks[j];
      var {id,sig,label,style,floatname,idnum,more,refid,id} = block;
      ...
      for(let k=0; k < n; k++){
        ...
        if(this_label.localeCompare(in_label)===0){
          return this.to_ref(sig,this_label,floatname,this_idnum,refid,id);
        }        
      }
    }
    in_label = this.polish(in_label);
    return this.to_style(in_label,'overstrike');
  }
    ...
    while ((v = re_untouch.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        if(key=='ruby'){
          ...
          newtext += this.do_phrase_ref(cnt);
        }
        else if(key=='img'){
