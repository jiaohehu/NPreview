...
`;
  ...
  do_verb(block) {
    ...
    if (this.xname === 'listing') {
      ...
      o.push(`<figure id='${id}' class='P ${sig}' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' >`);
      o.push(text);
      o.push(`</figure>`);
      o.push('');
    }
  ...
  do_text(block) {
    var {id,row1,row2,sig,lead,leadn,data,para,dept,fencecmd,base,subrow} = block;
    var o = []; 
    var text = data;
    var v;
    if (leadn && leadn>0) {
  ...
  do_tabb(block){
    ...
    var text = s.join('\n');
    var text = `<table border='0' style='width:100%;border-collapse:collapse;'>${text}</table>`;
    o.push(`<figure id='${id}' class='P ${sig}' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' >`);
    o.push(text);
    o.push('</figure>');
    o.push('');
  ...
  do_tblr_long(block){
    ...
    var text = s.join('\n');
    if (sig==='TBLR') {
      text = `<table border='0' style='margin-left:auto;margin-right:auto;border-collapse:collapse;'>${text}</table>`;
    } else if (sig==='LONG') {
      text = `<table border='0' style='width:100%;border-collapse:collapse;'>${text}</table>`;
    }
    if (this.xname === 'table') {
      o.push(`<figure id='${id}' class='P ${sig}' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' style='font-size:${this.xcssfontsize};margin-left:0;margin-right:0;' >`);
  ...
  do_math(block) {
    ...
    text = `<table style='margin-left:auto;margin-right:auto;border-collapse:collapse;'>${text}</table>`;
    o.push(`<figure id='${id}' class='P ${sig}' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' >`);
    o.push(text);
    o.push('</figure>');
    o.push('');
