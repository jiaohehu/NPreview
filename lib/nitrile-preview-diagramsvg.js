...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  drawtext(x,y,label,justi) {
    ...
    if ((v=this.re_inlinemath.exec(label))!==null) {
      ...
      if(this.parser.config.HTML.bodyfontsizept  ) {
        var rate = this.parser.config.HTML.bodyfontsizept/12;
        w *= rate;
        h *= rate;
      } 
      w *= 1.3333;
      ...
      if (this.parser.config.HTML.bodyfontsizept) {
        var fontsize = `${this.parser.config.HTML.bodyfontsizept}pt`;
      } else {
        var fontsize = '';
      }
      s.push(`<text font-size='${fontsize}' text-anchor='${anchor}' ${this.texts()} x='${x}' y='${y}' dy='${dy}'>${label}</text>`);
    }
    return s.join('\n');
  }
