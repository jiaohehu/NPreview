...
class NitrilePreviewParser extends NitrilePreviewBase {
  ...
  read_para (lines,n) {
    ...
    for (let i=0; !done && n < lines.length; ++i,++n) {
      ...
      if(i==0 && (v=re_mult.exec(line))!==null){
        is_mult = 1;
        bull = v[1];
        line = v[2];
        type = 'mult';
      ...
      if(is_tabb){
        ...
        if(re_fullline.test(line)){
          if((v=re_tabb.exec(line))!==null){
            items.push(v[2]);
            continue;
          }
          if((v=re_tabb_null.exec(line))!==null){
            items.push('');
            continue;
          }
          break;
          ...
          let line1 = items.pop();
          line1=line1||'';
          line1 = this.join_line(line1,line);
          items.push(line1);
          continue;
        }
        continue;
    ...
    if(type == 'spcl'){
      ...
      var text = body.join('\n');
      return {para,sig,bull,text,body};
    } 
    else if(type == 'text'){
    ...
    if(1){
      ...
      for (var block of this.blocks) {
        ...
        switch(block.sig){
          ...
          case 'MULT': translator.do_mult(block); break;
