  ...
  translate (blocks,isarticle,o) {
    ...
    if (blocks.length == 1 && blocks[0][3] === '!BOOK') {
      ...
      for (var text of para) {
        ...
        var v = text.match(re);
        if (v) {
          o.push(`<code>${v[1]} <a href='file://${v[2]}'>${v[2]}</a></code><br/>`);
        } else {
          o.push(`<code>${text}</code><br/>`);
        }
      }
      o.push('</p>');
      o.push(`</div>`);
    ...
    const step = atom.config.get('nitrile-preview.step');
    ...
    for (var block of blocks) {
      ...
      switch (type) {
        case 'HDGS': {
          ...
          switch (cat) {
            ...
            case 2:
              ...
              break;
            case 3:
              o.push(`<h4 id='${id}' class='SUBSUBSECTION' rows='${row1} ${row2}'>`);
              o.push(`${refname} &#160; ${this.escape(text)}`);
              o.push(`<small><mark>[${id}]</mark></small>`);
              o.push(`</h4>`);
        ...
        case 'CODE': {
          ...
          if (fencecmd.listing) {
            var sid = this.toSid(id,fencecmd.label);
            o.push(`<div id='${sid}' class='${type}' rows='${row1} ${row2}' style='margin-left:${left}'>`);
            o.push(`<figure>`);
            o.push(`<figcaption>Listing ${refname}: <small><mark>[${sid}]</mark></small> ${this.unmask(caption)}</figcaption>`);
        ...
        case 'PICT': {
          ...
          if (fencecmd.figure) {
            ...
            o.push(`<div id='${sid}' class='${type}' rows='${row1} ${row2}' style='margin-left:${left}'>`);
            o.push(`<figure>`);
            o.push(`<figcaption>Figure ${refname}: <small><mark>[${sid}]</mark></small> ${this.unmask(caption)}</figcaption>`);
            for (var j in text) {  
              var pp = text[j];
...
console.log(pp);
        ...
        case 'DESC': {
          ...
          for (i=0; i < keys.length; ++i) {
            if (cat === 'mono') {
              o.push(`<dt><b><code>${this.escape(keys[i])}</code></b></dt>`);
            } else if (cat === 'strong') {
  ...
  style (type, text) {
    ...
    switch (type) {
      ...
      case 'uri': {
        const [cnt,href] = text;
        if (cnt) {
          return `<span>${this.escape(cnt)}</span><span style='word-break:break-all'>(<code>${this.escape(href)}</code>)</span>`
        } else {
          return `<span style='word-break:break-all'><code>${this.escape(href)}</code></span>`
        }
        break;
      }
      case 'ruby': {
