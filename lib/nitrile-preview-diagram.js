...
class NitrilePreviewDiagram {
  ...
  constructor (parser) {
    ...
    this.re_variable_index         = /^\*([A-Za-z]+)(\d+)\s*(.*)$/;
    this.re_variable_range         = /^\*([A-Za-z]+)\[(.*?)\]\s*(.*)$/;
    this.re_variable               = /^\*([A-Za-z]*)\s*(.*)$/;
    this.re_variable_s             = /^([A-Za-z]+)\s*(.*)$/;
    this.re_coord                  = /^\((.*?)\)\s*(.*)$/;
    this.re_offset = /^\<(.*?)\>\s*(.*)$/;
    this.re_relative = /^\[(.*?)\]\s*(.*)$/;
    ...
    this.mypaths = {}; // extra config stuff for the viewport
  ...
  toMetaPost (para) {
    ...
    for (line of para) {
      ...
      if ((v = this.re_apple_command.exec(line)) !== null) {
        ...
        if (!this.mypaths.apple) {
          o.push(`path apple; apple:=${mypath};`);
          this.mypaths.apple = 1;
        }
        for (var i=0; this.valid(coords,i); i++) {
          z0 = this.point(o, coords,i); 
          z0 = this.local(z0);
          ...
          var y = z0[1];
          if (my_opt==='.top') {
            o.push(`draw apple shifted(-0.5,0) shifted(${x},${y}) scaled(u) ${this.draws()};`);
          } else if (my_opt==='.bot') {
            o.push(`draw apple shifted(-0.5,-1) shifted(${x},${y}) scaled(u) ${this.draws()};`);
          } else if (my_opt==='.rt') {
            o.push(`draw apple shifted(0,-0.5) shifted(${x},${y}) scaled(u) ${this.draws()};`);
          } else if (my_opt==='.lft') {
            o.push(`draw apple shifted(-1,-0.5) shifted(${x},${y}) scaled(u) ${this.draws()};`);
          } else {
            if (this.fillcolor) {
              o.push(`fill apple shifted(-0.5,-0.5) shifted(${x},${y}) scaled(u) ${this.fills()};`);
            }
            if (this.linewidth !== '0') {
              o.push(`draw apple shifted(-0.5,-0.5) shifted(${x},${y}) scaled(u) ${this.draws()};`);
            }
          }
        }
        continue;
...
///console.log('y=',y);
  ...
  readCoordsLine (o, line, changeall=true) {
    ...
    while (line.length) {
      ...
      if ((v = this.re_variable_index.exec(line)) !== null) {
        var d    = v[1].trim();
        var i    = v[2].trim();
        line     = v[3];
      ...
      } else if ((v = this.re_variable_range.exec(line)) !== null) {
        var d    = v[1].trim();
        var range= v[2].trim();
        line     = v[3];
      ...
      } else if ((v = this.re_variable_s.exec(line)) !== null) {
        var d    = v[1].trim();
        line     = v[2];
