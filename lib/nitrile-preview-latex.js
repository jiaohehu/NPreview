...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  do_img (g) {
    ...
    this.imgs.push(src);
    if(width&&height){
      return `\\includegraphics[width=${width}mm,height=${height}mm]{${src}}`;
    }else if(width){
      return `\\includegraphics[width=${width}mm]{${src}}`;
    }else if(height){
      return `\\includegraphics[height=${height}mm]{${src}}`;
    }else{
  ...
  to_math(pp,style) {
    ...
    let ss = pp.map((p,i) => {
      var { line } = this.make_line(p.math,p.label,islabeled,1);
      return line;
    });
    o.push(ss.join('\\\\\n'))    
    o.push(`&&`);
  ...
  to_longtable(itms,style,caption,label){
    ...
    let text = this.itms_to_longtable(itms,style.fr,style.hline,style.glue,style.n,caption,label);
    var o = [];
    o.push('\\noindent');
    o.push(text);
    return o.join('\n');
  ...
  to_multi(itms,style){
    ...
    let text = this.itms_to_multi(itms,style.fr,style.hline,style.glue,style.n);
    var o = [];
    o.push('\\noindent');
    o.push(text);
    return o.join('\n');
...
`
  ...
  pp_to_multi_itemized(pp,fr,n){
    ...
    for(let j=0; j < n; ++j){
      ...
      t1 += tn;
      tn = k;
      d.push(`\\begin{list}{\\textbullet}{\\setlength\\topsep{0ex}\n\\setlength\\parsep{0ex}\n\\setlength\\itemsep{0ex}}\n${kk.join('\n')}\n\\end{list}`);
      if(j==n-1){
        d.push('\\\\')
      }else{
        d.push('&')
  ...
  rows_to_multi(rows,style) {
    ...
    for (let j = 0; j < nrows; ++j) {
      let pp = rows[j];//pp could be shorter than n
      pp = pp.slice(0,n);//pp is not longer than 'n'
      ...
      for (let i = 0; i < n; ++i) {
        let text = pp[i] || '';
        o.push(`\\begin{minipage}{\\linewidth}`);
        o.push(text);
        o.push(`\\end{minipage}`);
        if (i == n - 1) {
          o.push(`\\\\`);
  ...
  itms_to_multi(itms,fr,hline,glue,n) {
    ///all row data has been unmasked
    var fr = fr || '';
    var glue = glue || '';
    var hline = hline||0;
    ...
    for (let j = 0; j < itms.length; j+=n) {
      ...
      for (let i = 0; i < n; ++i) {
        let text = pp[i] || '';
        d.push(`\\begin{minipage}{\\linewidth}`);
        d.push(text);
        d.push(`\\end{minipage}`);
        if (i == n - 1) {
          d.push(`\\\\`);
