...
class SphinxPreviewParser {
  ...
  /*
  Return a string that is the HTML. The third
  argument is optional and can be omitted. It is
  set to the source file name if the source lines
  is to come from this file. This enables future
  xross reference search based on source filename.
  It is typically set to the basename of the file.
  The fourth parameter is 'plevel'; it is set to
  whatever dictated by the leading colons in the %BOOK
  block. For one colon it is zero, two colons it is
  one, three colons it is two, four colons it is three,
  etc. It is used to increase the sectioning indenting
  level. So when plevel is passed as 1 then HDGS/0 will
  become HDGS/1, essentially turning a chapter into
  a section, and section into subsection, etc.
  */
  toBLOCKS(lines,o,fname,plevel) {
    var inst = '';
    var inst_t = '';
    var inst_n = 0;
    var v = '';
    ...
    while (lines.length > 0) {
      ...
      if (nblocks == 1 && para[0] === '% !BOOK') {
        var data = para.slice(1);
        o.push([id,row,row2,'%BOOK',0,data,para,inst,inst_t,fname,plevel]);
        break;
      }

      /// first block is automatically treated as a HDGS/0 block
      if (nblocks == 1) {
        var text = this.joinPara(para);
        var data = [0,text];
        o.push([id,row,row2,'HDGS',0,data,para,inst,inst_t,fname,plevel]);
        continue;
      }
      ...
      if (the_fence === '---') {
        ...
        o.push([id,row,row2,'VRSE',n,data,para,inst,inst_t,fname,plevel]);
        continue
      }
      ...
      if (the_fence === '~~~') {
        var [data, n] = this.trimPara(para);
        var data = this.trimFences(data);
        o.push([id,row,row2,'VERB',n,data,para,inst,inst_t,fname,plevel]);
        continue;
      }
      ...
      if (the_fence === '///') {
        ...
        o.push([id,row,row2,'PICT',n,data,para,inst,inst_t,fname,plevel]);
        continue;
      }
      ...
      if (the_fence === '===') {
        ...
        o.push([id,row,row2,'TABB',n,data,para,inst,inst_t,fname,plevel]);
        continue;
      }
      ...
      if (the_fence === '<<<') {
        ...
        o.push([id,row,row2,'TERM',n,data,para,inst,inst_t,fname,plevel]);
        continue;
      }
      ...
      if (the_fence === '\"\"\"') {
        ...
        o.push([id,row,row2,'QUOT',n,data,para,inst,inst_t,fname,plevel]);
        continue;
      }
      ...
      if (re.test(para[0])) {
        ...
        o.push([id,row,row2,'DEF1',n,data,para,inst,inst_t,fname,plevel]);
        continue;
      }
      ...
      if (re.test(para[0])) {
        var data = this.parseDEF(para,re);
        o.push([id,row,row2,'DEF2',n,data,para,inst,inst_t,fname,plevel]);
        continue;
      }
      ...
      if (re.test(para[0])) {
        var data = this.parseDEF(para,re);
        o.push([id,row,row2,'DEF3',n,data,para,inst,inst_t,fname,plevel]);
        continue;
      }
      ...
      if (v) {
        ...
        var data = [lead,text];
        o.push([id,row,row2,'PRIM',0,data,para,inst,inst_t,fname,plevel]);
        continue;
      }
      ...
      if (v) {
        ...
        var data = [lead,text];
        o.push([id,row,row2,'SECO',0,data,para,inst,inst_t,fname,plevel]);
        continue;
      }
      ...
      if (v) {
        ...
        o.push([id,row,row2,'PLST',n,data,para,inst,inst_t,fname,plevel]);
        continue
      }
      ...
      if (true) {
        var [data, n] = this.trimPara(para);
        var data = this.joinPara(data);
        o.push([id,row,row2,'',n,data,para,inst,inst_t,fname,plevel]);
        continue;
      }
  ...
  unmask (text) {
    ...
    while ((v = re.exec(str)) !== null) {
      ...
      if (v[1]) {
        ...
        /// reff: [^/d3js.md]
        var fn = v[9].trim(); /// 'd3js.md'
        /// search in the 'this.blocks' member for
        var id = this.findIdForFname(fn,this.blocks);
        if (id >= 0) {
          console.log(fn);
          console.log(id);
          newtext += this.style('ref',id);
        } else {
          newtext += this.escape(`??${fn}??`);
        }
      }
