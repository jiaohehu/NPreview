...
class NitrilePreviewParser {
  ...
  constructor(name) {
    ...
    */
    ...
    this.samp = 0;///to be set by %^samp=1
  }
  ...
  newConfig() {
    ...
    config.LATEX.diagfontsizept=12;
    ...
    config.LATEX.documentclass='';///set to 'book','scrbook', etc.
    config.LATEX.docstyle=0;///0=auto;1=article;2=report
    config.LATEX.frontpage=0;//1=title page will be generated
    config.LATEX.maxn=44;//maximum line number for each "float" Program
    config.LATEX.step=5;//5mm left-padding for some
    config.LATEX.nipass='small';
    config.LATEX.nisamp='small';
    config.LATEX.nitabr='small';
    config.LATEX.nilong='small';
    config.LATEX.nitabb='small';
    config.LATEX.nitabu='small';
    config.LATEX.nicaption='small';
    config.LATEX.niprog='footnotesize';
    config.LATEX.extra='';
    ...
    config.MEMOR.bodyfontsizept=11.5;
    config.MEMOR.diagfontsizept=12;
    config.MEMOR.papersize='a4paper';
    config.MEMOR.twoside=0;///set to '1' to enable
    config.MEMOR.twocolumns=0;///set to '1' to enable
    config.MEMOR.latexengine='';///set to 'pdflatex' or 'lualatex', default to 'lualatex'
    ...
    while (nlines < lines.length) {
      ...
      if(sig=='NLST'){
        ...
        if(sig0=='NLST'){
          ...
          ///add a new item to blk
          blk.items.push({type:'nlst',bull:block.bull,text:this.joinPara(block.body),body:block.body});
          o.push(blk);
          ...
          ///first time
          block.items = [];
          block.items.push({type:'nlst',bull:block.bull,text:this.joinPara(block.body),body:block.body});
          o.push(block);
        ...
        if(sig0=='NLST'){
          ...
          let body = block.body;
          if(re_fence.test(body[0])&&re_fence.test(body[body.length-1])){

            body = body.slice(1,body.length-1);
            blk.items.push({type:'samp',body});
            o.push(blk);
            continue;
        
          } else if(re_tilda.test(body[0])&&re_tilda.test(body[body.length-1])){

            body = body.slice(1,body.length-1);
            blk.items.push({type:'pass',body});
            o.push(blk);
            continue;
            ...
            blk.items.push({type:'text',text,body});
            o.push(blk);
            continue;
  ...
  read_para (lines,n) {
    ...
    for (let i=0; n < lines.length; ++i,++n) {
      ...
      if(i==0 && (v=re_spcl.exec(line))!==null){
        ...
        if((v=re_fence.exec(s))!==null){
          isfenced = 1;
          fenceid = v[2];
          float=0;
          continue;
        }
        else if((v=re_labeled.exec(s))!==null){
          label = v[1];
          caption = v[2];
          iscaption = 1;
          float=1;
      ...
      if(i==0 && (v=re_nlst.exec(line))!==null){
        type = 'nlst';
        bull = v[1];
        let s = v[2];
        body.push(s);
      ...
      if(isspcl && (v=re_fence.exec(line))!==null){
        if(isfenced){
          isfenced = 0;
          iscaption = 0;
        }else{
    ...
    if(type == 'spcl'){
      if(fenceid == 'framed'){
        ...
        return {para,sig,wide,float,caption,label,body};
      } 
        ...
        return {para,sig,wide,float,caption,label,cols,body};
      } 
      else if (fenceid == 'image'){
        sig = 'PICT';
        var rows = this.parse_pict(body);
        return {para,sig,wide,float,caption,label,rows,body};
      }
      ...
      return {para,sig,caption,label,cols,body};
    }
    else if(type == 'math'){
      var math = this.parse_math(body);
      return {para,sig,label,math,wide,body};
    }
    else if(type == 'hdgs'){
      return {para,sig,hdgn,label,text,body};
    } 
    else if(type == 'quot'){
      ...
      var items = this.parse_ilst(body);
      return {para,sig,items,body};
    }
    else if(type == 'hlst'){
  ...
  unmask (line,cssfontrate) {
    ...
    while ((v = re_unmask.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        var cnt = v[2]; 
        newtext += this.inlinemath(cnt,cssfontrate);
  ...
  */
  ...
  async readModeAsync(dirname) {
    ...
    for(let d of this.mode){
      let {name,sublevel,text,subfname,subparser} = d;
      if(name==='root' && subparser){
        ///'subparser' is the root 
        var root_parser = subparser;
        ///we copy the config from the root
        ///and then merge the root's 'rmap' with our own.
        this.config = root_parser.config;
        var new_rmap = root_parser.rmap.map(x => x);
        for(var m of this.rmap){
          new_rmap.push(m);
        }
        this.rmap = new_rmap;
        this.ismaster = 1;
        ///found under what 'h*' the current doc is under
        for(var rm of root_parser.mode){
          if(rm.name==='h' && rm.subfname.localeCompare(this.fname)==0){
            for(let m of this.blocks){
              m.name=rm.name;
              m.subfname=rm.subfname;
              m.sublevel=rm.sublevel;
            }
          }
        }
      }else if(name==='part'){
  ...
  parse_dlst(para) {
    ...
    const re_dlst_astr1  = /^(\*)\s+(\S+)\s*(.*)$/u;
    const re_dlst_astr2  = /^(\*)\s+\{\s(.*?)\s\}\s*(.*)$/u;
    const re_dlst_astr3  = /^(\*)\s+``(.*?)``\s*(.*)$/u;
    const re_dlst_astr4  = /^(\*)\s+\$\{([^\{\}]+)\}\s*(.*)$/u;
    const re_dlst_astr5  = /^(\*)\s+"(.*?)"\s*(.*)$/u;
    const re_dlst_astr6  = /^(\*)\s+\{\{(.*?)\}\}\s*(.*)$/u;
    const re_dlst_astr7  = /^(\*)\s+`(.*?)`\s*(.*)$/u;

    var o = [];
    var re1 = re_dlst_astr1;
    var re2 = re_dlst_astr2;
    var re3 = re_dlst_astr3;
    var re4 = re_dlst_astr4;
    var re5 = re_dlst_astr5;
    var re6 = re_dlst_astr6;
    var re7 = re_dlst_astr7;
    for (var j = 0; j < para.length; ++j) {
      var s = para[j];
      var v;
      if((v=re6.exec(s)) !== null) {
        var bullet = v[1];
        var key    = v[2];
        var text   = v[3];
        var type   = 'emph';
  ...
  translateBlocks() {
    ...
    for (var block of this.blocks) {
      ...
      this.latexlabelcmd = this.xlabel?`\\label{${this.xlabel.slice(1)}}`:'';
      if(block.floatname){
        this.floatname = block.floatname;
        this.caption_text = `${block.floatname}.${this.xidnum} ${this.xcaption}`;
        this.caption_text = this.unmask(this.caption_text);
      } else if (this.xcaption){
        this.floatname = '';
        this.caption_text = `${this.xcaption}`;
        this.caption_text = this.unmask(this.caption_text);
      } else {
        this.floatname = '';
        this.caption_text = '';
  ...
  async readFromFileAsync(fname,dirname) {
    ...
    fsubfname = `${fsubfname.slice(0,fsubfname.length-path.extname(fsubfname).length)}.md`;
    console.log(fsubfname);
  ...
  to_ref (str) {
    for (var j=0; j < this.blocks.length; ++j) {
      ...
      if(sig=='MATH'){
        if( str.localeCompare(label)===0) {
          //return `${floatname}.\\ref{${str.slice(1)}}`;
          return this.do_ref(sig,str.slice(1),floatname,idnum,saveas,id);
          break;
        }
        if(more&&more.length){
          for(let k=0; k < more.length; k++){
            ...
            if(str.localeCompare(x.label)===0){
              //return `${floatname}.\\ref{${str.slice(1)}}`;
              return this.do_ref(sig,str.slice(1),floatname,x.idnum,saveas,id);
              break;
            }
          }
        }
    ...
    return this.do_ref('',str.slice(1),'','','','');
  }
}
