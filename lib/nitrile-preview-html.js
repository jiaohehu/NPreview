...
${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
  ...
  svg_array_to_dl(d){
    ...
    d = d.join('\n');
    var s = `<dl style='margin:0'>${d}</dl>`;
    var shiftdist = maxshiftdist;
    return {s,shiftdist};
  }

  svg_array_to_table(d){
    ...
    d = d.map( x => {
        ...
        var dist = maxshiftdist - shiftdist;
        let math_style = `left:${dist}pt;position:relative;text-align:left;`;
        return (`<td style='${math_style}'>${s}</td>`);
    });
    d = d.map(x => `<tr>${x}</tr>`);
    d = d.join('\n');
    var s = `<table border='0'>${d}</table>`;
    var shiftdist = maxshiftdist;
    return {s,shiftdist};
  }
  ...
  make_line(math,label,idnum){
    var d  = this.to_math_svg_array(math,true);
    if(d.length==1){
      var g = d[0];
      var {s,w,h,shiftdist} = g;
    } else {
      var {s,shiftdist} = this.svg_array_to_dl(d);
      var w = 0;
      var h = 0;
    }
  ...
  make_math(math,label,idnum,more,gather){
    var data = [];
    var line = this.make_line(math,label,idnum);
    data.push(line);
    if(more&&more.length){
      for(let k=0; k < more.length; k++){
        let x = more[k];
        var line = this.make_line(x.math,x.label,x.idnum);
        data.push(line);
      }
    }
    ///figure out maxshiftdist
