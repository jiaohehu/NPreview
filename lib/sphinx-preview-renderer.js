  ...
  */
  toHTML(lines) {
    ...
    while (lines.length > 0) {
      ...
      var [ para, lines ] = this.readPara(lines);
      ...
      if (/^\@{1,}\s+\S/.test(para[0])) {
        ...
        switch (cat.length) {
          ...
          case 3:
            for (i=0; i < keys.length; ++i) {
              o.push(`<dt><i>${this.escapeHTML(keys[i])}</i></dt>`);
            }
            o.push(`<dd style='margin-left:0.5cm'>${this.unmask(desc)}</dd>`);
            break;
          case 4:
            for (i=0; i < keys.length; ++i) {
              o.push(`<dt><tt>${this.escapeHTML(keys[i])}</tt></dt>`);
            }
            o.push(`<dd style='margin-left:0.5cm'>${this.unmask(desc)}</dd>`);
            break;
          default:
  ...
  unmask (text) {
    ...
    while ((v = re.exec(str)) !== null) {
      ...
      if (v[1]) {
        ...
        var cnt = v[8];
        cnt = this.escapeHTML(cnt);
        newtext = newtext.concat(` <a href='${cnt}'>${cnt}</a>`);
        //console.log('newtext='+newtext);
