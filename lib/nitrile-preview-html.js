...
`
        ...
        case 'CODE':  
          ...
          if (fig) {
            if (fencecmd.n) {
              ...
              text = text.map( x => this.escape(x) );
              var linenum = 0;
              text = text.map( x => `<span style='position:relative;'><span style='font-size:75%; position:absolute; left:-${N_lstlisting_numbers_xleftmargin}cm;' >${this.expandString(++linenum,4,' ')}</span>${x}</span>` );
              text = text.join('\n');
              ...
              o.push(`<pre style='${this.precssstyle}; padding-left:${N_lstlisting_numbers_xleftmargin}cm;' >${text}</pre>`);
              o.push(`</div>`);
              o.push('');
            } else {
              var text = data;
            ...
            if (fencecmd.n) {
              var text = data;
              text = this.toTranslatedCodeWithNumbers(text);
              o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' subrow='${subrow}' rows='${row1} ${row2}' style='margin:1em 0; margin-left:${left}cm'>`);
              o.push(text);
              o.push(`</div>`);
              o.push('');
  ...
  toTranslatedCodeWithNumbers (text) {
  
    ///var num = this.expandString(++linenum,4,' ');
    text = text.map( x => this.escape(x) );
    var linenum = 0;
