...
class NitrilePreviewDiagramMP extends NitrilePreviewDiagram {
  ...
  do_p_path(coords,p,g){
    ...
    for( var mypath of mypath_ss ) {
      if (mypath) {
        if(p.fill){
          o.push(`fill (${mypath}) ${withscaled} ${withshifted} scaled(u) ${this.do_fills(g)};`);
        }else{
          o.push(`${d} (${mypath}) ${withscaled} ${withshifted} scaled(u) ${this.to_draws(g)};`);
        }
      }
    }
    return o.join('\n');
  ...
  do_p_label(x,y,g){
    ...
    var t = g.label||'';
    var a = g.anchor||'';
    console.log('james,t=',t,'a=',a);
    var a = this.to_anchor_str(a);
    console.log('james,t=',t,'a=',a);
    if(a.length){
      a = `.${a}`;
    }
  ...
  do_p_shape(x,y,w,h,mypath,g){
    ...
    var mypathes = this.coordsToMetaPost(p,true);
    var mypath_ss = mypathes.split('\n');
    x = this.localx(x);//refsx,refsy,refx,refy
    y = this.localy(y);
    ...
    if (this.fillcolor) {
      for( var mypath of mypath_ss ) {
        if (mypath) {
          s.push(`fill (${mypath}--cycle) xscaled(${sx}) yscaled(${sy}) shifted(${x},${y}) scaled(u) ${this.to_fills(g)};`);
        }
      }
    }
    if (!this.nolineflag()) {
      for( var mypath of mypath_ss ) {
        if (mypath) {
          s.push(`draw (${mypath}) xscaled(${sx}) yscaled(${sy}) shifted(${x},${y}) scaled(u) ${this.to_draws(g)};`);
        }
      }
    }
    return s.join('\n');
