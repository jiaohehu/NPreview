...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  fence_to_tabulate(ss, style) {
    let rows = super.fence_to_tabulate_rows(ss);
  ...
  to_cols(itms,style){
    ...
    let text = this.itms_to_cols(itms,style);
    return text;
  }
  to_rows(itms,style){
    itms = itms.map(s => {
    ...
    let text = this.itms_to_rows(itms,style);
    return text;
  }
  ...
  to_longtable(itms, style) {
    ...
    return this.itms_to_longtable(itms, style, caption, label, style.floatname);
  }
...
`;
  ...
  itms_to_tabulate(rows,style){
    ...
    for (var j = 0; j < nrows; ++j) {
      ...
      if (this.pp_is_hline(pp)) {
        ...
        d.push(`<tr><td style='border-top:1px solid' colspan='${ncols}'></td></tr>`);
        continue;
      }
      pp = pp.map((x, i) => `<td style='padding:0 0.5em'>${x}</td>`);
      var p = pp.join(gap);
      var p = `<tr>${p}</tr>`;
      d.push(p);
    }
    ...
    var text = `<table style='${css_style}' border='${border}'>${text}</table>`;
    return text;
  }
  ...
  itms_to_multi(itms,style){
    ...
    for(let j=0; j < itms.length; j+=n){
      ...
      pp.forEach((text, i) => {
        let fr = frs[i];
        d.push(`<td style='padding:0 0.5em;width:${fr*100}%'>`);
        d.push(text);
        d.push(`</td>`);
      });
      d.push('</tr>');
