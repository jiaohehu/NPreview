...
class NitrilePreviewEpub {
  ...
  async generateAsync (parser,title,author,htmls,chaps,_imageidmap,imagemap,isarticle,dirname) {
...
///console.log('imgid='+imgid);
    ...
    if (isarticle) {
      ...
      this.toc = sections.map(x => `<li><a href='#${x[0]}'>${x[1]}</a></li>`).join('\n');
      ...
      for (var i=0; i < chaps.length; ++i) {
        ...
        var saveas = `content${n}.xhtml`;
        var refid  = `content${n}`;
        alls.push([heading,id,label,dept,text,ln,saveas,refid]);
      }

      /// replace all htmls

      htmls = parser.replaceRef(htmls,alls);

      /// extract only 'PART' or 'CHAPTER' entries
      ...
      this.toc = tocs.map(x => `<li><a href='${x[0]}'>${x[1]}</a></li>`).join('\n');
      this.toc = `<ol style='list-style-type:none;' epub:type='list'> ${this.toc} </ol>`;
    }
