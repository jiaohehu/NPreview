...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  unmask (line) {
    ...
    while ((v = re_unmask.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        var cnt = v[2]; 
        cnt = this.escape(cnt);
        newtext += this.to_style('code',cnt);
  ...
  to_config_lines() {
    ...
    if(this.parser && this.parser.config){
      const obj = this.parser.config[this.name];
  ...
  conf(key){
    if(this.parser && 
       this.parser.config && 
       this.parser.config[this.name] && 
       this.parser.config[this.name][key]){
      return this.parser.config[this.name][key];
    }
    if(this.config[key]){
  ...
  untouch (line) {
    ...
    var start_i = 0;
    var newtext = '';
    ///note that re_untouch cannot be a global variable because 'untouch' is a recursive call
    //const re_untouch = /(?<!\\)(\\\\)|(?<!\\)(\\[A-Za-z]+)|(?<!\\)(\\\()|(?<!\\)(\\\[)/g;
    const re_untouch = /(?<!\\)(\\\\)|(?<!\\)(\\[A-Za-z][A-Za-z0-9]*)|(?<!\\)(\\\()|(?<!\\)(\\\[)|(?<!\\)([A-Za-z]_[a-z0-9]+\^[a-z0-9]+)(?!=\w)|(?<!\\)([A-Za-z]_[a-z0-9]+)|(?<!\\)([A-Za-z]\^[a-z0-9]+)/g;
      while ((v = re_untouch.exec(line)) !== null) {
      var i = v.index;
      var txt = line.slice(start_i,i);
      var txt = this.escape(txt);
      newtext += txt;
      ...
      if (v[1] !== undefined) {
        ...
        var head_text = v[2];
        i += head_text.length;
        var key = head_text.slice(1);
        if(1){
          start_i = this.extract(line,i);
          var scan_text = line.slice(i,start_i);
          if(scan_text.startsWith('{')&&scan_text.endsWith('}')){
            var cnt = scan_text.slice(1,scan_text.length-1);
          }else if(scan_text.startsWith('{')){
            var cnt = scan_text.slice(1);
          }else if(scan_text.endsWith('}')){
        ...
        if(1){
          i = start_i;
          start_i = this.extract(line,i);
          var scan_text2 = line.slice(i,start_i);
          if(scan_text2.startsWith('{')&&scan_text2.endsWith('}')){
            var cnt2 = scan_text2.slice(1,scan_text2.length-1);
          }else if(scan_text2.startsWith('{')){
            var cnt2 = scan_text2.slice(1);
          }else if(scan_text2.endsWith('}')){
            var cnt2 = scan_text2.slice(0,scan_text2.length-1);
          }else{
            var cnt2 = scan_text2;
          }
        }
        try {
          var cnt_text = this.exec_cmd(key,cnt,cnt2);
          newtext += cnt_text;
        }catch (e){
          var cnt_text = `${head_text}${scan_text}${scan_text2}`;
          var cnt_text = this.escape(cnt_text);
          newtext += cnt_text;
        }
        ...
        try {
          var scan_text = this.exec_cmd('\\subsup',cnt,cnt2,cnt3);
          newtext += scan_text;
  ...
  exec_cmd(key,cnt,cnt2,cnt3){
    //note that 'cnt' and 'cnt2' has not yet been escaped 
    var newtext = '';
    if(key=='\\subsup'){
      this.issymbolmode++;
      newtext += this.do_subsup(cnt,cnt2,cnt3);
      this.issymbolmode--;
    }
    else if(key=='em'){
      newtext += this.to_style('em',this.untouch(cnt));
    }
    else if(key=='ruby'){
      var v;
      ...
      this.issymbolmode++;
      newtext += this.do_sqrt(this.untouch(cnt));
      this.issymbolmode--;
    }else if(key=='cbrt'){
      this.issymbolmode++;
      newtext += this.do_cbrt(this.untouch(cnt));
      this.issymbolmode--;
    }else if(key=='qdrt'){
      this.issymbolmode++;
      newtext += this.do_qdrt(this.untouch(cnt));
      this.issymbolmode--;
    }else if(key=='sfrac'){
      this.issymbolmode++;
      newtext += this.do_sfrac(this.untouch(cnt),this.untouch(cnt2));
      this.issymbolmode--;
    }else if(key=='overline'){
      this.issymbolmode++;
      newtext += this.do_overline(this.untouch(cnt));
      this.issymbolmode--;
    }else if(key=='dot'){
      this.issymbolmode++;
      newtext += this.do_dot(cnt);
      this.issymbolmode--;
    }
    else if(key=='ddot'){
      this.issymbolmode++;
      newtext += this.do_ddot(cnt);
      this.issymbolmode--;
    }
    else if(key=='hat'){
      this.issymbolmode++;
      newtext += this.do_hat(cnt);
      this.issymbolmode--;
    }
    else if(key=='check'){
      this.issymbolmode++;
      newtext += this.do_check(cnt);
      this.issymbolmode--;
    }
    else if(key=='bar'){
      this.issymbolmode++;
      newtext += this.do_bar(cnt);
      this.issymbolmode--;
    }
    else if(key=='grave'){
      this.issymbolmode++;
      newtext += this.do_grave(cnt);
      this.issymbolmode--;
    }
    else if(key=='acute'){
      this.issymbolmode++;
      newtext += this.do_acute(cnt);
      this.issymbolmode--;
    }
    else if(key=='breve'){
      this.issymbolmode++;
      newtext += this.do_breve(cnt);
      this.issymbolmode--;
    }
    else if(key=='tilde'){
      this.issymbolmode++;
      newtext += this.do_tilde(cnt);
      this.issymbolmode--;
    }
    else if(key=='mathring'){
      this.issymbolmode++;
      newtext += this.do_mathring(cnt);
      this.issymbolmode--;
    }
    else if(this.is_symbol(key)){
      this.issymbolmode++;
      newtext += this.to_symbol(key);
      newtext += this.untouch(cnt);
      this.issymbolmode--;
    }
    else{
      throw "error";
