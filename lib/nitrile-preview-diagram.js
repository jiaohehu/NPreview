...
class NitrilePreviewDiagram {
  ...
  constructor(parser,notes) {
    ...
    this.re_line_command = /^line(\.\w+|)\s+(.*)$/;
    this.re_area_command = /^area(\.\w+|)\s+(.*)$/;
    ...
    this.re_anglearc_command = /^anglearc(\.\w+|)\s+(.*)$/;
    ...
    this.def_barlength = 0.25; // the length equal number of grids
  ...
  initSettings() {
    ...
    this.def_linedashed = this.linedashed = this.toLinedashed('');
    this.def_linecap = this.linecap = this.toLinecap('');
    this.def_linejoin = this.linejoin = this.toLinejoin('');
    this.def_label = this.label = '';
    this.def_justi = this.justi = '';
  }
  ...
  parseLine(line) {
    ...
    if ((v = this.re_set_command.exec(line)) !== null) {
      ...
      else if (key === 'refsx') { this.refsx = this.assertFloat(val, this.def_refsx, 0.1, 10); } ///such as 1.5
      else if (key === 'refsy') { this.refsy = this.assertFloat(val, this.def_refsy, 0.1, 10); } ///such as 1.5
      else if (key === 'linesize') { this.linesize = this.assertLength(val, this.def_linesize); } ///such as '0', '0.1', '1pt', '1mm'
      else if (key === 'fillcolor') { this.fillcolor = this.assertColor(val, this.def_fillcolor); } ///such as 'red', '0.5red', or '0.8[red,green]'
      else if (key === 'linecolor') { this.linecolor = this.assertColor(val, this.def_linecolor); } ///such as 'red', '0.5red', or '0.8[red,green]'
      else if (key === 'dotcolor') { this.dotcolor = this.assertColor(val, this.def_dotcolor); } ///such as 'red', '0.5red', or '0.8[red,green]'
      else if (key === 'dotsize') { this.dotsize = this.assertLength(val, this.def_dotsize); } ///such as '4pt' or '4mm'
      else if (key === 'tickcolor') { this.tickcolor = this.assertColor(val, this.def_tickcolor); } ///such as 'red'
      else if (key === 'ticksize') { this.ticksize = this.assertLength(val, this.def_ticksize); } ///such as '1pt' or '1mm'
      else if (key === 'tickprotrude') { this.tickprotrude = this.assertFloat(val, this.def_tickprotrude, 0.1, 1.0); } ///such as '0.33'
      else if (key === 'fontcolor') { this.fontcolor = this.assertColor(val, this.def_fontcolor); }/// such as 'green', 'rgb(200,100,255)'
      else if (key === 'fontsize') { this.fontsize = this.assertLength(val, this.def_fontsize); }/// such as '14pt'
      else if (key === 'labelgapx') { this.labelgapx = this.assertInt(val, this.def_labelgapx, 0, 10); }///such as '0'
      else if (key === 'labelgapy') { this.labelgapy = this.assertInt(val, this.def_labelgapy, 0, 10); }///such as '0'
      else if (key === 'linedashed') { this.linedashed = this.toLinedashed(val); } ///such as 'evenly' or 'withdots'
      else if (key === 'linecap') { this.linecap = this.toLinecap(val); }///such as 'butt', 'rounded', 'squared'
      else if (key === 'linejoin') { this.linejoin = this.toLinejoin(val); }///such as 'mitered', 'beveled', 'rounded'
      else if (key === 'label') { this.label = this.toString(val,this.def_label); }///such as 'a', 'a\\b', 
      else if (key === 'justi') { this.justi = this.toString(val,this.def_justi); }///such as 'ctr', 'ctr\\rt\\lft', 
      this.commands.push(this.do_set(key));
      return false;
    }
    ...
    if ((v = this.re_line_command.exec(line)) !== null) {
      var stroke_opt = v[1].trim();
      var line = v[2];
      var cmd = 'line';
      var txt = '';
      var coords = this.readCoordsLine(line);
      this.commands.push(this.do_line(stroke_opt,txt,coords));
      this.lastcoords = coords;
      return false;
    }
    if ((v = this.re_area_command.exec(line)) !== null) {
      var stroke_opt = v[1].trim();
      var line = v[2];
      var cmd = 'area';
      var txt = '';
      var coords = this.readCoordsLine(line);
      this.commands.push(this.do_area(stroke_opt,txt,coords));
      this.lastcoords = coords;
      return false;
    }
    if ((v = this.re_fill_command.exec(line)) !== null) {
      ...
      var txt = '';
      var coords = this.readCoordsLine(line);
      this.commands.push(this.do_fill(stroke_opt,txt,coords));
      this.lastcoords = coords;
      return false;
    }
    if ((v = this.re_arrow_command.exec(line)) !== null) {
      ...
      var line = v[2];
      var coords = this.readCoordsLine(line);
      this.commands.push(this.do_arrow(fill_opt,'',coords));
      this.lastcoords = coords;
      return false;
    }
    if ((v = this.re_revarrow_command.exec(line)) !== null) {
      ...
      var line = v[2];
      var coords = this.readCoordsLine(line);
      this.commands.push(this.do_revarrow(fill_opt,'',coords));
      this.lastcoords = coords;
      return false;
    }
    if ((v = this.re_dblarrow_command.exec(line)) !== null) {
      ...
      var line = v[2];
      var coords = this.readCoordsLine(line);
      this.commands.push(this.do_dblarrow(fill_opt,'',coords));
      this.lastcoords = coords;
      return false;
    }
    if ((v = this.re_anglearc_command.exec(line)) !== null) {
      var anglearc_opt = v[1].trim();
      var [txt,line] = this.readCoordsText(v[2]);
      var coords = this.readCoordsLine(line);
      this.commands.push(this.do_anglearc(anglearc_opt,txt,coords));
      this.lastcoords = coords;
      return false;
    }
    ...
    if ((v = this.re_chord_command.exec(line)) !== null) {
      var chord_opt = v[1].trim();
      var [txt,line] = this.readCoordsText(v[2]);
      var coords = this.readCoordsLine(line);
      this.commands.push(this.do_chord(chord_opt,txt,coords));
      this.lastcoords = coords;
      return false;
    }
    if ((v = this.re_arc_command.exec(line)) !== null) {
      var arc_opt = v[1].trim();
      var [txt,line] = this.readCoordsText(v[2]);
      var coords = this.readCoordsLine(line);
      this.commands.push(this.do_arc(arc_opt,txt,coords));
      this.lastcoords = coords;
      return false;
    }
    if ((v = this.re_cseg_command.exec(line)) !== null) {
      var cseg_opt = v[1].trim();
      var [txt,line] = this.readCoordsText(v[2]);
      var coords = this.readCoordsLine(line);
      this.commands.push(this.do_cseg(cseg_opt,txt,coords));
      this.lastcoords = coords;
      return false;
    }
    ...
    if ((v = this.re_box_command.exec(line)) !== null) {
      ...
      var [txt,line] = this.readCoordsText(v[2]);
      var coords = this.readCoordsLine(line);
      this.commands.push(this.do_box(opt,txt,coords));
      this.lastcoords = coords;
      return false;
    }
    ...
    if ((v = this.re_updnprotractor_command.exec(line)) !== null) {
      ...
      var [txt,line] = this.readCoordsText(v[2]);
      var coords = this.readCoordsLine(line);
      this.commands.push(this.do_updnprotractor(opt,txt,coords));
      this.lastcoords = coords;
      return false;
    }
    if ((v = this.re_cartesian_command.exec(line)) !== null) {
      ...
      var [act,line] = this.readActionText(line);
      var floats = this.readFloatsLine(line);
      this.commands.push(this.do_cartesian(opt,g,act,floats));
      return false;
    }
    if ((v = this.re_barchart_command.exec(line)) !== null) {
      ...
      var [act,line] = this.readActionText(line);
      var floats = this.readFloatsLine(line);
      this.commands.push(this.do_barchart(opt,g,act,floats));
      return false;
    }
    if(line.length>0){
      this.commands.push(this.do_comment(`${line}`));
  ...
  extractNextExpr(s,g=null) {
    ...
    if (s.charAt(0)==='(') {
      ...
      var arg1 = 0;
      for(var n=1; n > 0; ++n){
console.log('before extract','s=',s);
        var [a,s] = this.extractNextExpr(s,g);
console.log('after extract','a=',a,'s=',s,'op=',op,'arg1=',arg1);
        if (typeof a === 'number') {
console.log('exec','op=',op,'arg1=',arg1,'a=',a,'s=',s);
          arg1 = this.execScalarOp(op,arg1,a);
console.log('after exec','arg1=',arg1,'s=',s);
...
console.log('james','op=',op,'arg1=',arg1,'arg2=',arg2);
    switch(op) {
      case '*':
  ...
  do_barchart(opt,g,act,floats) {
    ...
    switch( act ) {
      ...
      case 'bbox':
        ...
        s.push(this.do_p_path([x1,y1,x2,y1,x2,y2,x1,y2],{cycle:1},g));
        break;
        ...
        for( var j=0; j < floats.length; j+=1 ) {
          ...
          if (Number.isFinite(num)){
            ...
            if(num > 0){
              ...
              s.push(this.do_p_rect(x,y,w,h,g));
            }
          }
        }
        break;
      ...
      case 'ytick':
        for( var j=0; j < floats.length; j+=1 ) {
          ...
          if (Number.isFinite(num)){
            ...
            s.push(this.do_p_hbar(x,y,g));
            g.label = `${num}`;
            g.anchor = 'lft';
            s.push(this.do_p_label(x,y,g));
          }
        }
        break;
  ...
  do_cartesian(opt,g,act,floats) {
    ...
    switch( act ) {
      case 'setup':
        A.xorigin = this.assertFloat(floats[0],A.xorigin);    
        A.yorigin = this.assertFloat(floats[1],A.yorigin);    
        A.xgrid = this.assertFloat(floats[2],A.xgrid);  
        A.ygrid = this.assertFloat(floats[3],A.ygrid);  
        break;
      ...
      case 'xaxis'://DIAGRAM
        ...
        var y1 = A.yorigin;
        var y2 = A.yorigin;
        o.push(this.do_p_path([x1,y1,x2,y2],{arrowstart:1,arrowend:1},g));
        break;
      ...
      case 'xtick':
        for (var j=0; j < floats.length; ++j) {
          ...
          var y = A.yorigin;
          o.push(this.do_p_vbar(x,y,g));
          g.label = `${num}`;
          g.anchor = 'bot';
          o.push(this.do_p_label(x,y,g));
        }
        break;
      ...
      case 'ellipse'://DIAGRAM
        ...
        x += A.xorigin;
        y += A.yorigin;
        o.push(this.do_p_ellipse(x,y,Rx,Ry,angle,g));
        break;
      ...
      case 'yplot'://CARTESIAN
        ...
        var n = this.assertFloat(floats[2],10);
        for( var j=0; f && j <= n; j+=1 ) {
          var x = min;
          x += j/n * (max-min);
          var y = this.execScalarFunc(f,[x]);
          console.log('-------yplot','x=',x,'y=',y);
          x /= A.xgrid;
          y /= A.ygrid;
          x += A.xorigin;
          y += A.yorigin;
          if(Number.isFinite(x)&&Number.isFinite(y)){
            o.push(this.do_p_dot(x,y,g));
          }
        }
        break;
      ...
      case 'arrow'://DIAGRAM
        ...
        o.push(this.do_p_path(d,{arrowend:1},g));
        break;
      ...
      case 'arc' :
        ...
        if(Number.isFinite(x)&&Number.isFinite(y)&&Number.isFinite(r)&&Number.isFinite(a1)&&Number.isFinite(a2)){
          ...
          r = this.localdist(r);
          if (a2-a1 < 180) {
            o.push(`<path d='M${x1},${y1} A${r},${r},0,0,0,${x2},${y2}' ${this.strokes()} fill='none'/>`);
          } else {
            o.push(`<path d='M${x1},${y1} A${r},${r},0,1,0,${x2},${y2}' ${this.strokes()} fill='none'/>`);
          }
        }
        break;
