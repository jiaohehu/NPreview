  ...
  do_pict(block) {
    ...
    for(var blk of data){
      ...
      blk = blk.map( img => {
        ...
        img = {imgsrc,imgid,sub};
        imgsrcs.push(src);
        return img;
      });
      var pp = blk.map( img => {
        const {imgsrc,imgid} = img;
        return(`<td style='text-align:center;vertical-align:bottom;width:100%;'> <img id='nitrileimg-${imgid}' style='width:100%;' src='${imgsrc}'/> </td>`);
      });
      var qq = blk.map( img => {
        const {sub} = img;
        var sub_text = this.unmask(sub);
