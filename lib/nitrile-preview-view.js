...
console.log('NitrilePreviewView::destroy() called');
  ...
  update() {
    ...
    this.getBlocks(parser).then( ([blocks,flags,book]) => {
      ...
      this.htmls.push(`</p>`);
      this.htmls = parser.translateHtml(this.toConfig(),blocks,isarticle,isoutline,this.htmls);
      this.htmls.push(`</div>`);
...
console.log('toggleSectionalContents');
    ...
    if (node) {
      ...
      if (nodeName === 'MARK' && isOutline === 'true') {
        console.log('ok')
        var h_node = this.findParentHnodeById(node.parentNode,section);
        if (h_node) {
        ...
        if (next_div_node && !next_div_node.textContent) {
          /// expand!
          var parser = new NitrilePreviewHtml();
          var the_blocks = this.getContentsBlocksForHDGS(section);
          ...
          if (the_blocks.length == 0) {
            ...
            var htmls = parser.translateHtml(this.toConfig(),the_blocks,false,false);
            next_div_node.innerHTML = htmls.join('\n');
          }
      ...
      } else if (nodeName === 'MARK' && isOutline === 'false') {
        var h_node = this.findParentHnodeById(node.parentNode,section);
        if (h_node) {
  ...
  async saveAs (savefilepath) {
    ...
    try {
      ...
      if (this.saveastype === 'lualatex-article') {
        ...
        var olines = parser.translateLatex(this.toConfig(),this.blocks,true);
        var {title='',author='',documentclass=''} = this.flags;
        title = title ? title : 'Untitled';
        documentclass = documentclass ? documentclass : 'article';

        if (atom.config.get('nitrile-preview.texFamily').toLowerCase() === 'lualatex') {
...
\\end{document}`;
        ...
        const olines = parser.translateHtml(this.toConfig(),this.blocks,isarticle,false);
