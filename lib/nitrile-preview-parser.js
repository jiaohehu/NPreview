...
class NitrilePreviewParser {
  ...
  toFlow(lines,fname,o=null) {
    ...
    while (lines.length > 0) {
      ...
      if (the_fence === '~~~' ) {
        ...
        n = (stepspaces > 0 && n >= stepspaces) ? 1 : 0;
      ...
      if (nblocks == 1) {
        ...
        stepspaces = parseInt(flags.stepspaces) ? parseInt(flags.stepspaces) : stepspaces;
        sampspaces = parseInt(flags.sampspaces) ? parseInt(flags.sampspaces) : sampspaces;
        var data = [0,text];
  ...
  toConfig (flags) {
    ...
    for (var key in schema) {
      if (schema.hasOwnProperty(key)) {
        ...
        config[key] = this.getConfig(key,flags,entry);
  ...
  wrapSample (para, config) {
    var o = [];
    var l = [];
    const maxn = config.sampwrap;
