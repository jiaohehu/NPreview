...
class NitrilePreviewNode {
  ...
  async toHtml (filename) {
    ...
    /// get TOC
    var toc = this.getXHtmlToc(config.latex.isArticle,chaps);
    /// construct the final LATEX file
    let title = config.latex.title ? config.latex.title : 'Untitled'
    let author = config.latex.author ? config.latex.author : ''
...
${htmls.join('\n')}
  ...
  async toEpub (filename) {
    ...
    let title = config.latex.title ? config.latex.title : 'Untitled'
    let author = config.latex.author ? config.latex.author : ''
    var isarticle = config.latex.isArticle;
