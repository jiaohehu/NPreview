...
class NitrilePreviewParser {
  ...
  constructor() {
    ...
    this.re_rubyitem    = /^(%|\u{ff05})(\S+?)(\u{30fb})(\S+)/u;
    this.re_rubyitem_desc = /^(%|\u{ff05})(\S+?)(\u{30fb})(\S+)\s*(.*)$/u;
    this.re_flag        = /^(%)\s*#(.*)$/u;
    this.re_uri         = /^\w+:\/\//u;
  ...
  readFromLines(lines) {
    ...
    */
    ...
    while (lines.length > 0) {
      ...
      } else if((v=this.re_rubyitem.exec(para[0]))!==null){

        for( var s of para ) {
          if((v=this.re_rubyitem.exec(s))!==null){
            var rb = v[2];
            var rt = v[4];
            rmap.push([rb,rt]);
          }
