        ...
        case 'EQTN': {
          ...
          if (text.length == 1) {
            for (var s of text) {
              ...
              o.push(`<span style='display:block;text-align:center;position:relative'>${s}<span style='position:absolute;right:0px;'>(${fig})</span></span>`);
            }
          } else if (text.length > 1) {
            var j = 0;
            for (var s of text) {
              s = this.tokenizer.parse(s,true);
              o.push(`<span style='display:block;text-align:center;position:relative'>${s}<span style='position:absolute;right:0px;'>(${fig}${this.toSubfigNum(j)})</span></span>`);
              j += 1;
            }
          }
          o.push(`</p>`);
