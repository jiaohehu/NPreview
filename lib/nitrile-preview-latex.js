...
class NitrilePreviewLatex extends NitrilePreviewParser {
  ...
  translateLatex (config,xrefs,blocks,isarticle,o) {
    ...
    const showsections = config['showsections'];
    this.block = [];
    this.xrefs = xrefs;
        ...
        case 'PLST': {
          ...
          for (var item of data) {
            ...
            if (item.length === 3) {
              switch (lead) {
                case 'OL': {
                  o.push(`\\begin{enumerate}[nosep,leftmargin=${stepmargin}cm]`);
                  o.push(`\\item\[${bullet}.\] ${text}`);
                  break;
                }
                case 'UL': {
                  o.push(`\\begin{itemize}[nosep,leftmargin=${stepmargin}cm]`);
                  o.push(`\\item ${text}`);
                  break;
                }
                case 'LI': {
