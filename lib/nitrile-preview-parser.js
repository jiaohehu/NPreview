...
class NitrilePreviewParser extends NitrilePreviewBase {
  ...
  read_para (lines,n) {
    ...
    if(type == 'spcl'){
      ...
      if(body.length==1 && n0==0 && n==1){
        ...
        var {leadn,lead,text,nspace} = this.parse_text(body);
        return {para,sig,leadn,lead,text,nspace,body};
      }
    } 
    else if(type == 'note'){
      let name = body[0];
  ...
  parse_text(para) {
    ...
    if ((v1 = re_prim.exec(para[0])) !== null ||
      ...
      nspace = this.get_line_nspace(para[0]);
      lines = para.map(x => x.trimLeft());
    } 
    text = lines.join('\n');
