...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_finalize(s,style) {
    ...
    var css = [];
    if(style.scale){
      css.push(`zoom:${style.scale*100}%`);
    }
    if (style && style.width) {
      let str = style.width;
      str = this.translator.string_to_css_width(str);
      css.push(`width:${str}`);
      text = `<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' fill='currentColor' stroke='currentColor' viewBox='0 0 ${this.fix(vw)} ${this.fix(vh)}' style='${css.join(";")}' >${s}</svg>`;
    } else {
      text = `<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' fill='currentColor' stroke='currentColor' viewBox='0 0 ${this.fix(vw)} ${this.fix(vh)}' style='${css.join(";")}' width='${this.fix(vw)}' height='${this.fix(vh)}' >${s}</svg>`;
    }
