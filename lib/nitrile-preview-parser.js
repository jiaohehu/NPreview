...
class NitrilePreviewParser {
  ...
  read_para (lines,n) {
    ...
    for (let i=0; n < lines.length; ++i,++n) {
      ...
      if (i==0 && (v=re_csvd.exec(line))!==null){
        type = 'csvd';
        body.push(v[2]);
        continue;
      }
  ...
  */
  ...
  async toGlyphs(fname) {
    ...
    for(var line of lines){
      var l = this.extract_all_unicode_patterns(line);
      o = o.concat(l);
    }
    return o.join('\n');
  }

  extract_all_unicode_patterns(line){
    /// 5  U+0000
    ///
  ...
  async toFontmap(fnames){
    ...
    for (let fname of fnames){
      var xxout = await this.readFileAsync(fname);
