...
class NitrilePreviewDiagram {
  ...
  constructor(translator) {
    ...
    this.MIN = 0.000001;
    /// regular expression
    this.re_foreach_loop = /^foreach\s+\(([A-Za-z,\s]+)\)\s+\[(.*?)\]:$/;
  ...
  toDiagram(para,notes) {
    ...
    var v;
    while(lines.length){
      var line = lines.shift();//remove top-line from lines
      ...
      if((v=this.re_foreach_loop.exec(line))!==null){
        ...
        //console.log('var_array',var_array);
        var seq_array = v[2].split(',').map(x => x.trim());
        var body = this.extract_loop_body(lines);
  ...
  extract_loop_body(lines){
    ...
    for (var n=0; lines.length; n++) {
      ...
      // this is the first lead, remember the leading whitespace
      if (n == 0) {
        var lead0 = lead;
        var line = lines.shift();//remove it from lines
        outlines.push(line);
      } else {
        ///else check to see if we have less of an indentation
        ///than the first line, if so we return without extracting
        ///this line
  ...
  readParamsStyle(str) {
    return this.translator.string_to_style(str);
  }
  ...
  parse_line(line,notes) {
    ...
    if ((v = this.re_pic_command.exec(line)) !== null) {
      ...
      if (pic) {
        pic = this.translator.join_backslashed_lines(pic);
        for (var s of pic) {
          var done = this.parse_line(s);
          if (done) {
            break;
          }
        }
