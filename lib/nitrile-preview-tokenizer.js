...
//console.log('s_=',s_);
  ...
  toSvg (v,addoutline = false) {
    ...
    ///
    var rate = this.cssfontsizeRate(this.parser.xcssfontsize);
    if (this.parser.config.HTML.bodyfontsizept) {
      var rate2 = this.parser.config.HTML.bodyfontsizept/12;
      rate = rate*rate2;
    }
    var vw = w*1.3333;
    var vh = h*1.3333;
...
${news}
  ...
  cssfontsizeRate(str){
    ///NOTE: the input str is CSS font size such as '12pt','small','x-small','xx-small', 
    /// so far the only conversion is small and x-small
    /// small -> 0.8
    /// x-small -> 0.8*0.8 = 0.64
    /// xx-small -> 0.8*0.8*0.8 = 0.512
    var p = pjson.cssfontsizeRate[str];
    if (p) {
      return p;
    }
    return 1;
