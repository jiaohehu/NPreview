...
class NitrilePreviewContex extends NitrilePreviewParser {
  ...
  do_samp(block){
    ...
    if(this.xfencecmd.style==1){
      ...
      body = body.map( x => this.escapeTT(x) );
      body = body.map( x => this.rubify(x) );
      body = body.map( x => this.polish(x,this.config.CONTEX.fssamp) );
      body = body.map(x => {
        if(!x){
          x = '~';
        }else{
  ...
  do_long(block){
    ...
    rows = rows.map ( (row,i) => {
      ...
      if(i==0){
        ...
        row = row.map(x => x.map(y => this.polish(y,this.config.CONTEX.fstabular)));
      }else{
        row = row.map(x => x.map(y => this.polish(y,this.config.CONTEX.fstabular))); 
      }
      ...
      if(j==0){
        ...
        if(maxn > 1){
          ...
          for(var n=0; n<maxn; ++n){
            ...
            pp = pp.map(x => `{\\bf ${x}}`);
            pp = pp.map(x => this.polish(x,this.config.CONTEX.fontsizetable));
