...
`;
  ...
  do_imgs(block) {
    ...
    for(var blk of data){
      ...
      var pp = blk.map( img => {
        ...
        imgsrcs.push(srcs[0]);
        images.push([srcs[0],imgid]);
        return(`<td style='text-align:center;vertical-align:bottom;width:${width};'> <img id='nitrileimg-${imgid}' style='width:100%;height:${height};' src='./${srcs[0]}'/> </td>`);
      });
      var qq = blk.map( img => {
        const {srcs,opts,sub} = img;
        var width = this.toHtmlLength(opts.width);
  ...
  do_tblr_long(block){
    ...
    padding:${t}px ${h}px;\
    ${(sig==='LONG')?'width:'+ww[i]+'%;':''}
    ${vlines.indexOf(i)>=0?'border-left:1px solid;':''}\
    ${vlines.indexOf(i+1)>=0?'border-right:1px solid;':''}\
    ...
    if (this.xname === 'table') {
      ...
      o.push(`<p id='${id}' class='P ${sig}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='font-size:${this.xcssfontsize};' >`);
      o.push(text);
      o.push(`</p>`);
    } 
    o.push('');
