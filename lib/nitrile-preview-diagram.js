...
class NitrilePreviewDiagram {
  ...
  initConfig(){
    ...
    this.config.labelgapx = 3; // in px
    this.config.labelgapy = 3; // in px
  ...
  parseLine(line) {
    ...
    if ((v = this.re_viewport_command.exec(line)) !== null) {
      this.config.width = this.assertInt(v[1], 25, 10, 100);
      this.config.height = this.assertInt(v[2], 10, 4, 100);
    ...
    if ((v = this.re_unit_command.exec(line)) !== null) {
      this.config.unit = this.assertUnit(v[1]);
      this.commands.push(this.do_comment(`unit: ${this.config.unit}`));
      this.do_setup();
      return false;
    }
    ...
    if ((v = this.re_shape_command.exec(line)) !== null) {
      ...
      else if(opt=='trapezoid'){
        var mypath = `(0,0)--(1,0)--(0.6,1)--(0.2,1)--cycle;`;
      }
      else if(opt=='parallelgram'){
...
console.log('james do_text',opt,txt,g,coords);
