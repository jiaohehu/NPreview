...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  do_identify(block, A) {
    ...
    var { sig, hdgn, islabeled, floatname, parser } = block;
  ...
  do_hdgs(block) {
    var {id,row1,row2,sig,hdgn,text,idnum,parser} = block;
    var o = [];
    var text = this.unmask(text);
        ...
        case 'LI': {
          if(ds){
            ...
            if (cat == 'rmap') {
              ...
              text = (`<strong>${key}</strong>${gap}${desc}`);
            }
            else if (cat == 'quoted') {
              key = `${q}${key}${q}`;
              key = this.polish(key);
              desc = this.unmask(desc);
              text = (`<tt><strong>${key}</strong></tt>${gap}${desc}`);
            }
            else {
              key = this.polish(key);
              desc = this.unmask(desc);
            ...
            dd = this.unmask(dd);
            if (isbroad) {
              o.push(`<dt>${dt}</dt><dd><p>${text}</p>${extra_text}</dd>`);
            }else{
              o.push(`<dt>${dt}</dt><dd>${text}</dd>`);
            }
          }else{
            text = this.unmask(text);
            if(isbroad){
