...
class NitrilePreviewLatex extends NitrilePreviewParser {
  ...
  do_hdgs(block){
    var o = [];
    var {id,row1,row2,sig,part,level,sublevel,dept,title,para,base,subrow,subfname} = block;
    ...
    if(part){
      if(this.config.LATEX.pages){
        o.push(`\\part{${text}}`);
        o.push('');
      }else{
        o.push(`\\section*{Part ${dept}~~${text}}`);
      ...
      if(this.config.LATEX.pages){
        ...
        o.push(`\\title{${this.escape(title)}}`);
        o.push(`\\author{${this.escape(author)}}`);
        o.push(`\\maketitle`);
        o.push('');
      }else{
        o.push(`\\noindent{\\huge ${text}}`);
  ...
  do_verb(block){
    var o = [];
    var {id,row1,row2,sig,data,caption,label,para,fencecmd,subfname} = block;
    subfname=subfname||''; o.push(`%${sig} ${subfname} ${1+row1} ${1+row2}`);
    var text = data;
    var s = [];
    if (this.xnumbers) {
      s.push(`\\begin{supertabular}{@{}l@{~~}l}`);
      var linenum = 0;
      for (var k=0; k < text.length; ++k) {
        var line = text[k];
        line = this.escapeSolid(line);
        var lineno = `${++linenum}`;
        lineno = this.escape(lineno);//pickup latfontsize
        s.push(`${lineno} & ${line}\\\\`);
      }
    ...
    if(label){
      o.push(`Listing ${this.xidnum} : ${this.caption_text} \\hfill\\\\`);
    }
    o.push('\\hrulefill\\\\');
    o.push(`${text} \\hfill\\\\`);
    o.push('\\hrulefill');
  ...
  do_samp(block){
    ...
    if(this.xfencecmd.style==1){
      ...
      for (var i=0; i < text.length; ++i) {
        var line = text[i];
        line = this.replaceSubstrings(line,this.mymapsmpl1);
        line = this.fontifyLATEX(line);
        line = this.rubify(line,this.rmap);
        if (line.length == 0) {
          line = '~';
        }
        o.push(`${line}\\\\`);
      ...
      text = this.trimSampPara(text);
      o.push(`\\begin{flushleft}`);
      o.push(`\\begin\{adjustwidth\}{${this.xfencecmd.left}mm}{}`)
      for (var i=0; i < text.length; ++i) {
        var line = text[i];
  ...
  do_text(block){
    var o = [];
    var {row1,row2,sig,data,subfname} = block;
    subfname=subfname||''; o.push(`%${sig} ${subfname} ${1+row1} ${1+row2}`);
    var v;
  ...
  do_long(block){
    ...
    var vlines = fencecmd.latvlines;
    var hlines = fencecmd.lathlines;
    var vlines = this.toArray(vlines);
    var hlines = this.toArray(hlines);
    var vpadding = parseInt(fencecmd.latvpadding);
    var pcol = this.insertTabularVlines(vlines,pcols);
    var header = rows.shift();
    var header = header.map(x => `{\\bfseries ${x}}`);
    o.push(`\\begin{xltabular}{\\linewidth}{${pcol}}`);
    if (fencecmd.name=='table') {
      o.push(`\\caption*{Table ${this.xidnum} : ${this.caption_text}}${this.latexlabelcmd}\\\\`);
    }
  ...
  do_tabr(block){
    var o = [];
    var {id,row1,row2,sig,data,caption,label,para,fencecmd,base,subrow,subfname} = block;
    subfname=subfname||''; o.push(`%${sig} ${subfname} ${1+row1} ${1+row2}`);
    caption=caption||'';
    label=label||'';
    var s = [];
  ...
  do_tabb(block){
    ...
    o.push(`\\begin{flushleft}`);
    o.push(`\\begin\{adjustwidth\}{${fencecmd.left}mm}{${fencecmd.right}mm}`)
    o.push(`\\begin{supertabular}{@{}${pcol}}`);
    o.push(text);
    o.push(`\\end{supertabular}`);
    o.push(`\\end{adjustwidth}`);
    o.push(`\\end{flushleft}`);
    o.push('');
    block.latex = o.join('\n');
  ...
  do_diag(block){
    ...
    o.push(text);
    if(label){
      o.push(`\\caption*{Figure ${this.xidnum} : ${this.caption_text}}${this.latexlabelcmd}`);
    }
    o.push('\\end{figure}');
    o.push('');
    block.latex = o.join('\n');
  ...
  do_pict(block){
    var o = [];
    var {id,row1,row2,sig,data,caption,label,subfname} = block;
    caption=caption||'';
    label=label||'';
    subfname=subfname||''; o.push(`%${sig} ${subfname} ${1+row1} ${1+row2}`);
    var s = [];
    for(var blk of data){
      ...
      var pp = images.map( img => {
        ...
        return (`\\includegraphics[width=${width}]{${imgsrc}}`);
      });
  ...
  do_frmd(block){
    ...
    subfname=subfname||''; o.push(`%${sig} ${subfname} ${1+row1} ${1+row2}`);
    var text = data;
    var [out, vw, vh] = this.toFramedLtpp(text,fencecmd);
...
\\usepackage[export]{adjustbox}`
  ...
  toLatexDocument() {
    ...
    if (!documentclass) {
      documentclass = (config.LATEX.pages)?'report':'article';
    }
    var opts = [];
...
`;
  ...
  toSubequations(o,id,sig,row1,row2,data,fencecmd) {
    var bls = data;
    var j = 0;
    var star = '';
  ...
  toMultline(o,id,sig,row1,row2,data,fencecmd) {
    var bls = data;
    var ss = [];
    for(var bl of bls) {
      var s0 = bl.join(' ');
  ...
  fontifyLATEX (text) {
    ...
    if(newtext){
      if(this.xmonospace && this.xlatfontsize){
        newtext = `{\\ttfamily\\${this.xlatfontsize} ${newtext}}`; 
      }else if(this.xlatfontsize){
        newtext = `{\\${this.xlatfontsize} ${newtext}}`; 
      }
    }
    return newtext;
  }
