...
class NitrilePreviewDiagramMP extends NitrilePreviewDiagram {
  ...
  _do_cartesian(opt,act,floats) {
    ...
    switch( act ) {
      ...
      case 'xtick':
        var withscaled = `xscaled(${this.refsx}) yscaled(${this.refsy})`;
        var withshifted = `shifted(${this.refx},${this.refy})`;
        var withtextcolor = (this.fontcolor) ? `withcolor ${this.to_colors(this.fontcolor)}` : '';
        for (var j=0; j < floats.length; ++j) {
          ...
          var lx = this.fix(lx);
          var ly = this.fix(ly);
          var tex_label = this.to_tex_label(floats[j]);
      ...
      case 'arrow':
        ...
        s.push(`drawarrow (${d}) ${this.withrefs()} ${this.draws()};`);
        break;
  ...
  do_p_path(points,p,g){
    ...
    var withscaled = `xscaled(${this.refsx}) yscaled(${this.refsy})`;
    var withshifted = `shifted(${this.refx},${this.refy})`;
    return(`draw (${d}) ${withscaled} ${withshifted} scaled(u) ${this.to_draws(g)};`);
  }
  ...
  to_draws(g) {
    var o = [];
    if (g.linedashed) {
      o.push(`dashed ${g.linedashed}`);
    }
    if (typeof g.linesize === 'number') {
      o.push(`withpen pencircle scaled ${g.linesize}`);
    }
    if (g.linecolor) {
  ...
  do_p_ellipse(x,y,Rx,Ry,angle,g){
    ...
    //s.push(`--(subpath (${ang1 / 45},${ang2 / 45}) of fullcircle xscaled(${this.fix(2*Rx)}) yscaled(${this.fix(2*Ry)}) rotated(${Phi}) shifted(${this.fix(Cx)},${this.fix(Cy)}))`);
    //s.push(`--(subpath (${ang2 / 45},${ang1 / 45}) of fullcircle xscaled(${this.fix(2*Rx)}) yscaled(${this.fix(2*Ry)}) rotated(${Phi}) shifted(${this.fix(Cx)},${this.fix(Cy)}))`);
    o.push(`draw fullcircle xscaled(${this.fix(2*Rx)}) yscaled(${this.fix(2*Ry)}) rotated(${Phi}) shifted(${Cx},${Cy}) ${this.withrefs()} ${this.draws()};`);
    return o.join('\n'); 
  }
  ...
  do_p_dot(x,y,g){
    ...
    ///***NOTE that drawdot cannot use shifted or scaled command
    ///   because there is no path before it
    return(`fill fullcircle scaled(${this.to_dotsizes(g)}) shifted(${x}*u,${y}*u) ${this.to_dotcolors(g)};`);
  }

  to_dotsizes(g) {
    if (typeof g.dotsize === 'number') {
      var v =(g.dotsize*0.75);
      v = this.fix(v);
      return v;
    } else {
      var v =(5*0.75);
