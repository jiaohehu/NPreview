...
///console.log(data);
        ...
        case 'LONG': {
          ...
          if (config.latexlongtablestyle == 'plain') {
            ///Plain style has no borders at all, not even \hline
            let header = text.shift();
            header = header.map(x => `\\textbf{${x}}`);
            var pcol = this.toPcolumn(ww,'plain');
            o.push(`\\begin{adjustwidth}{${left}cm}{}`);
            o.push(`\\begin{longtable}{${pcol}}`);
          ...
          } else if (config.latexlongtablestyle == 'boxed') { 
            let header = text.shift();
            header = header.map(x => `\\textbf{${x}}`);
            var pcol = this.toPcolumn(ww,'boxed');
            o.push(`\\begin{adjustwidth}{${left}cm}{}`);
            o.push(`\\begin{longtable}{${pcol}}`);
            if (fencecmd.caption && fencecmd.label) {
              o.push(`\\caption{${this.unmask(fencecmd.caption)}}\\label{${fencecmd.label}}\\\\`);
          ...
          } else if (config.latexlongtablestyle == 'full') { 
            let header = text.shift();
            header = header.map(x => `\\textbf{${x}}`);
            var pcol = this.toPcolumn(ww,'full');
            o.push(`\\begin{adjustwidth}{${left}cm}{}`);
            o.push(`\\begin{longtable}{${pcol}}`);
            if (fencecmd.caption && fencecmd.label) {
              o.push(`\\caption{${this.unmask(fencecmd.caption)}}\\label{${fencecmd.label}}\\\\`);
        ...
        case 'TABL': {
          ...
          var text = this.toFramedTabl(text, maxj, fencecmd);
          var pcol = this.toPcolumn(ww,'');
          o.push(`\\begin{flushleft}`);
          o.push(`\\begin{adjustwidth}{${left}cm}{}`);
        ...
        case 'SAMP': {
          ...
          if (config.sampstyle === 'code') {
            var text = this.wrapSample(data,'latex',config);
            ...
            for (var i=0; i < text.length; ++i) {
              var line = text[i];
              line = this.replaceSubstrings(line,this.mymap);
              line = this.fontifyLATEX(line);
              if (this.isAutoRuby('smpl',config)) {
                line = this.rubify(line);
              }
        ...
        case 'DESC': {
          ...
          if (mystyle === 'mono') {
            /// note that it must be style=unboxed, and cannot be style=nextline as otherwise it will be a blank line if there is no text
            o.push(`\\begin{description}\[nosep,style=unboxed,leftmargin=${xleft}cm,font=\\normalfont]`);
            for (var key of keys) {
              key = `${this.escape(bullet)}\\ttfamily\\bfseries{}${this.unmask(key)}`;
              o.push(`\\item\[${key}\]`);
            }
            ...
            o.push(`\\begin{description}\[nosep,style=unboxed,leftmargin=${xleft}cm,font=\\normalfont]`);
            for (var key of keys) {
              key = `${this.escape(bullet)}${this.unmask(key)}`;
              o.push(`\\item\[${key}\]`);
            }
          }
          if (text) {
...
\\usepackage[export]{adjustbox}`
  ...
  toFramedTabb (text, maxj, ww, fencecmd, config) {
    var text = text.map ( row => {
      row = row.map(x => x.split('\n'));
      row = row.map(x => x.map(y => this.unmask(y)));
      var kk = row.map(x => x.length); /// kk is an array of integers
      var kkm = kk.reduce((acc,curr) => acc>curr?acc:curr); /// kkm is the largest integer of all kk's
      ///
      // going into every element of $row so that its length is at least $kkm
      // 
      row = row.map(x => {while(x.length<kkm) x.push(''); return x;});
      return row;
