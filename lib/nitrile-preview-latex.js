...
class NitrilePreviewLatex extends NitrilePreviewParser {
  ...
  do_verb(block){
    ...
    if (name === 'listing') {
      this.toListing(o,id,sig,fname,row1,row2,data,fencecmd);
      ...
      for (var line of text) {
        ...
        if (!line) { line = '~'; }
        line = `{\\ttfamily${this.xlatexfontsize}{}${line}}\\\\`;
        o.push(line);
      }
      if (text.length) {
  ...
  do_samp(block){
    ...
    if (this.xparatype===0) {
      ...
      o.push(`\\begin\{adjustwidth\}{${this.xleft}}{}`)
      if (fencecmd.wrap) {
        var text = this.wrapSample(text,fencecmd.wrap);
      }
      for (var i=0; i < text.length; ++i) {
        ...
        var s = `{\\ttfamily${this.xlatexfontsize}{}${line}}`;
        o.push(`${s}\\\\`);
      }
      if (i > 0) {
        o.pop();
      ...
      text = text.map( x => `{${this.xlatexfontsize}{}${x}}` );
      text = text.join('\\\\\n');
      o.push(`\\begin{flushleft}`);
      o.push(`\\begin\{adjustwidth\}{${this.xleft}}{}`)
      o.push(text);
  ...
  do_math(block){
    ...
    if (name === 'aligns') {
      this.toEquations(o,id,sig,fname,row1,row2,data,fencecmd,true);
    } else if (name === 'gathers') {
      this.toEquations(o,id,sig,fname,row1,row2,data,fencecmd,false);
    } else if (name === 'subequations') {
      this.toSubequations(o,id,sig,fname,row1,row2,data,fencecmd,false);
    } else if (name === 'multline') {
      this.toMultline(o,id,sig,fname,row1,row2,data,fencecmd,false);
    } else if (name === 'split') {
      this.toEquation(o,id,sig,fname,row1,row2,data,fencecmd,true);
    } else if (name === 'equation') {
      this.toEquation(o,id,sig,fname,row1,row2,data,fencecmd,false);
    } else {
      this.toDisplayMath(o,id,sig,fname,row1,row2,data,fencecmd,false);
    }
    block.latex = o.join('\n');
  }
  do_frmd(block){
...
\\end{document}\n`;
  ...
  toListing(o,id,sig,fname,row1,row2,data,fencecmd) {
    var myopts=[];
    var onecolumn = fencecmd.onecolumn;
    myopts.push(`basicstyle=${this.xlatexfontsize}`);
