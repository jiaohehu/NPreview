...
class NitrilePreviewView {
  ...
  refreshView() {
    ...
    this.htmls.push(`</p>`);
    this.htmls = this.parser.translateHtml(this.config,this.blocks,this.isarticle,this.htmls);
    this.htmls.push(`</div>`);
  ...
  async saveAs (savefilepath) {
    ...
    try {
      ...
      if (this.saveastype === 'latex-chapter') {
        ...
        var olines = latex.translateLatex(this.config,this.blocks,false);
        data = olines.join('\n');

      } else if (this.saveastype === 'latex-complete') {

        var latex = new NitrilePreviewLatex();
        var olines = latex.translateLatex(this.config,this.blocks,this.isarticle);
