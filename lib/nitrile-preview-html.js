    ...
    for (var block of all) {
      ...
      switch (sig) {
        ...
        case 'IMGS': {
          ...
          o.push(`<p id='${id}' class='P ${sig}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='display:grid;grid-template-columns:repeat(${column},1fr);' >`);
          images.forEach(x => {
              var [image,src,sub] = x;
              sub = this.unmask(sub);
              o.push(`<span style='display:inline-block;padding:5px;text-align:center;'><img src='./${src}' style='vertical-align:text-bottom;width:100%;' alt='${src}'/><small>${sub}</small></span>`);
          });
          o.push('</p>');
          o.push('');
