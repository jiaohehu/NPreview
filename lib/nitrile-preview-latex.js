      ...
      switch (level) {
        case 0:
          if(this.config.LATEX.pages){
            var title = this.config.ALL.title ? this.config.ALL.title : 'Untitled'
            var author = this.config.ALL.author ? this.config.ALL.author : ''
            o.push(`\
...
`);
          ...
          break;
        case 1:
          if(this.config.LATEX.haschapter){
  ...
  do_verb(block){
    ...
    if (this.xnumbers) {
      ...
      for (var k=0; k < text.length; ++k) {
        ...
        s.push(`{\\scriptsize{}${++linenum}} & {${ffamily}{}${line}}\\\\`);
      }
...
\\usepackage[export]{adjustbox}`
  ...
  toLatexDocument() {
    ...
    if (!documentclass) {
      documentclass = (config.LATEX.haschapter==1)?'report':'article';
    }
    var opts = [];
