...
class NitrilePreviewParser {
  ...
  toFlow(lines,plevel,fname,o=null) {
    ...
    while (lines.length > 0) {
      ...
      if (block_type === 'SAMP' && block_type0 === 'PLST') {
        ...
        console.log('james: sig=',sig);
        console.log('james: brand=',brand);
        if (sig === 'imgs') {
        ...
        } else if (sig === 'flushright') {
          data = this.joinPara(data);
        } else if (sig === 'flushleft') {
          data = this.joinPara(data);
        } else if (sig === 'diagram') {
          ...
          o.push([id,row,row2,'SAMP','',data,para,fencecmd,star,base,label,fname]);
        }
        ...
        o.push([id,row,row2,'tabular','',data,para,fencecmd,'',base,label,fname]);
        block_type = 'tabular';
        continue
        ...
        if (block_type0 === 'longtablepara') {
          var bl = o.pop();
          var data = bl[5];
          var row = this.parseLongTablePara(para);
          data[0].push(row);
          bl[2] = row2;
          bl[5] = data;
          o.push(bl);
          ...
          var row = this.parseLongTablePara(para);
          var maxj = row.length;
          var data = [[row],maxj,this.toWW(maxj)];
          o.push([id,row,row2,'longtable','',data,para,fencecmd,'',base,label,fname]);
        }
      ...
      } else if (this.re_descblock.test(para[0])) {

        /// this is the at-sign (@) description block
        var data = this.parseDESC('mono',para,this.re_descblock);
        o.push([id,row,row2,'DESC','',data,para,fencecmd,'',base,label,fname]);
        block_type = 'DESC';
        continue;
  ...
  newERROblock (text,sublevel,fname) {
    ...
    var para = [];
    return ['',-1,-1,'ERRO',0,data,para,fencecmd,'',base,label,fname];
  ...
  toConfig (flags) {
    ...
    config.SAMP.fs='small';
    config.SAMP.wrap=0;

    config.latex.normalsize='12';
    config.listing.fs='small';
    config.tabular.fs='small';
    config.longtable.fs='small';
    config.imgs.fs='normalsize';
    config.verse.fs='normalsize';
    config.tabbing.fs='normalsize';
    config.dt.fs='normalsize';
    config.quot.fs='small';
    config.center.fs='normalsize';
    config.flushright.fs='normalsize';
    config.flushleft.fs='normalsize';
    config.math.fs='normalsize';
    config.verb.fs='normalsize';

    config.tabular.hlines = 'm';
    config.tabular.vpadding = 1;
    config.longtable.hlines = 't m b';
    config.longtable.vpadding = 4;
