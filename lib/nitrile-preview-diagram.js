...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  initConfig(){
    ...
    this.config.labeldx = 4; // in px
    this.config.labeldy = 2; // in px
  ...
  fetch_path(a) {
    ...
    return [[0, 0]]; //by default resturns a path with a single point that is the origin
  }
  ...
  readCoordsLine(line,istosave) {
    ...
    while (line.length) {
      ...
      } else if ((v = this.re_pathvar_single.exec(line)) !== null) {
        var symbol = v[1].trim();
        line = v[2];
        if (symbol.length == 0) {
          var from = this.lastcoords;
