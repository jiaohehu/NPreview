...
class NitrilePreviewDiagramMP extends NitrilePreviewDiagram {
  ...
  do_drawcontrolpoints(opt,txt,coords) {
    ...
    if (this.dotcolor) {
      var withcolor_s = `withcolor ${this.colors(this.dotcolor)}`;
    } else {
      var withcolor_s = `withcolor ${this.colors('black')}`;
    }
    s.push(`linecap := rounded;`);
    var x0 = null;
    var y0 = null;
    for (var i = 0; i < coords.length    ; i++) {
      ...
      if (join==='C') {
        ...
        s.push(`drawdot (${x}*u,${y}*u) withpen pensquare scaled ${r*2*0.75}pt ${withcolor_s};`);
        if (typeof x0 === 'number' && typeof y0 === 'number') {
          s.push(`drawdot (${x0}*u,${y0}*u) withpen pensquare scaled ${r*2*0.75}pt ${withcolor_s};`);
          x0 = null;
          y0 = null;
        }
        s.push(`drawdot (${i3}*u,${i4}*u) withpen pencircle scaled ${r*2*0.75}pt ${withcolor_s};`);
        s.push(`drawdot (${i5}*u,${i6}*u) withpen pencircle scaled ${r*2*0.75}pt ${withcolor_s};`);
      } if (join==='Q') {
        var i3 = this.localx(z0[3]);
        var i4 = this.localy(z0[4]);
        s.push(`drawdot (${x}*u,${y}*u) withpen pensquare scaled ${r*2*0.75}pt ${withcolor_s};`);
  ...
  do_drawanglearc(anglearc_opt,txt,coords) {
    var o = [];
    var f = this.readParamsLine(txt);
    var r = this.assertFloat(f[0],0.7,0,this.MAX);
    var r2 = this.assertFloat(f[1],1.45,0,this.MAX);
