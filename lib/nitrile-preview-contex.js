...
class NitrilePreviewContex extends NitrilePreviewParser {
  ...
  do_pict(block){
    var o = [];
    var {id,row1,row2,sig,rows,caption,label,islabeled} = block;
    o.push(this.to_info(block));
    var d = [];
    for(var blk of rows){
      let {mode,images} = blk;
      var n = images.length;
      d.push(`{\\centeraligned{\\startcombination[${n}*1]`);
  ...
  do_diag(block){
    var o = [];
    var {id,row1,row2,sig,body,parser,caption,label,islabeled} = block; 
    o.push(this.to_info(block));
