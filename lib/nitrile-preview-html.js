...
class NitrilePreviewHtml extends NitrilePreviewParser {
  ...
  do_pict(block) {
    ...
    o.push(`<figure ${this.toATTR(block)} >`);
    if(this.floatname){
      o.push(`<figcaption class='nicaption' style='text-align:center'> ${this.caption_text} </figcaption>`);
    }
    o.push(text);
    o.push('</figure>');
    o.push('');
  ...
  do_tabb(block){
    ...
    var text = s.join('\n');
    var text = `<table class='nitabb' border='0' style='border-collapse:collapse;'>${text}</table>`;
    o.push(`<figure ${this.toATTR(block)} >`);
    o.push(text);
    o.push('</figure>');
    o.push('');
...
${j==nrows-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</${j==0?'th':'td'}>`);
    ...
    var text = s.join('\n');
    text = `<table class='nitabb' border='0' style='margin-left:auto;margin-right:auto;border-collapse:collapse;'>${text}</table>`;
    o.push(`<figure ${this.toATTR(block)} >`);
    if(this.floatname){
      o.push(`<figcaption class='nicaption' style='text-align:center'> ${this.caption_text} </figcaption>`);
    }
...
${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
    ...
    s.push(`</tbody>`);
    var text = s.join('\n');
    text = `<table class='nitabb' border='0' style='border-collapse:collapse;width:100%;'>${text}</table>`;
    o.push(`<figure ${this.toATTR(block)} >`);
    if(0){
      o.push(`<figcaption class='nicaption'> ${this.caption_text} </figcaption>`);
    }
