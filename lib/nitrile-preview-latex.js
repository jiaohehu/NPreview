...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  do_tabu(block){
    ...
    o.push('');
    o.push(this.to_info(block));
    o.push(this.to_tabu_xltabular(cols));
    block.latex = o.join('\n');
  }
  do_tabr(block){
  ...
  smooth (unsafe) {
    ...
    unsafe = unsafe.replace(/(?<!\\)([A-Za-z])\^(\d)(?!\S)/g, (match,p1,p2) => {
          // I^1
          return  `${T6}${p1}${T1}${p2}${T6}`;  // octal code \01 is for caret
      })
    unsafe = unsafe.replace(/(?<!\\)([A-Za-z])_(\d)(?!\S)/g, (match,p1,p2) => {
          // I_1
          return `${T6}${p1}${T2}${p2}${T6}`;  // octal code \02 is for underscore
      })
    unsafe = unsafe.replace(/(?<!\\)\\(dot|ddot|bar|mathring|hat|check|grave|acute|breve|tilde)\{([A-Za-z])\}/g, (match,p1,p2) => {
          // \dot{x}, \vec{x}
          return `${T6}\0${p1}${T3}${p2}${T4}\0${T6}`;
      })
    unsafe = unsafe.replace(/(?<!\\)\\([A-Za-z][A-Za-z0-9]*)\;/g, (match,p1) => {
          // symbol
          try{
  ...
  smooth_for_var(s){
    var re_sup = /^(?<!\S)([A-Za-z])\^(\d)(?!\S)/; 
    var re_sub = /^(?<!\S)([A-Za-z])_(\d)(?!\S)/; 
    var re_dot = /(?<!\S)\\(dot|ddot|bar|mathring|hat|check|grave|acute|breve|tilde)\{([A-Za-z])\}(?!\S)/;
    var v;
    var t;
    var out='';
    while(s.length){
  ...
  escape_for_var(text){
    var text = this.smooth_for_var(text);
    var text = this.fontify_latex(text);
    var s = `${text}`;
    return s;
  }
