...
class NitrilePreviewParser {
  ...
  constructor() {
    ...
    this.re_rbit        = /^(%|\u{ff05})/u;
    this.re_rbit1       = /^(\S+?)(\/|\u{30fb})(\S+)\s*(.*)$/u;
    this.re_rbit2       = /^(%|\u{ff05})\s*(\S+?)(\/|\u{30fb})(\S+)\s*(.*)$/u;
    this.re_rbit3       = /^(%|\u{ff05})\s*#(.*)$/u;
    /*
    ％  ff05 jp,cn
    ＝  ff1d jp,cn
    ＃  ff03 jp,cn
  ...
  newConfig() {
    ...
    config.TEXT.csstextindent = '5';///5mm text indent for text that is not the first
    config.HDGS = {};
    config.PART = {};
    config.QUOT = {};
    config.QUOT.left = '5';///5mm margin
    config.QUOT.right = '5';///5mm margin
    config.QUOT.cssfontsize = '0.8em';
    config.QUOT.latfontsize = '\\footnotesize';
    config.QUOT.confontsize = '\\tfx';
    config.PARA = {};
    config.INCL = {};
    config.PLST = {};
  ...
  readFromLines(lines) {
    ...
    [3] sig: the signature of the blocks: TEXT, HRLE, MATH, PRIM, SECO,
        QUOT, PLST, TABB, DLST, LONG, TBLR, VERB, [LIST], [FRMD], [VERS].
    ...
    */
    ...
    while (lines.length > 0) {
      ...
      if (block_type === 'SAMP') {
      ...
      } else if ((v = this.re_rbit.exec(para[0])) !== null) {
        ...
        for( var s of para ) {
          ...
          if((v=this.re_rbit3.exec(s))!==null){
            /// % #number
            /// % #date
            /// % #vsuru
            /// % #builtin
            ...
            if(myid === 'builtin'){
              var re_type = /^\s*\[(.*?)\]\s*(.*)$/;
              const rubyitems = require('./nitrile-preview-rubyitems.json');
              for(var s of rubyitems){
              console.log('james',s);
                if((v=this.re_rbit1.exec(s))!==null){
                  var rb = v[1];
                  var rt = v[3];
                  var desc = v[4];
                  console.log('james',rb,rt,desc);
                  if((v=re_type.exec(desc))!==null) {
                    var type = v[1];
                    var desc = v[2];
                    var mydesc = `${rb}・${rt}・${desc}`;
                  console.log('james->',rb,rt,mydesc);
                    this.addItem(rb,rt,type,mydesc,rmap);
                  } else {
                    var mydesc = `${rb}・${rt}・${desc}`;
                  console.log('james->',rb,rt,mydesc);
              ...
              var types = ['number','date','vspecial','vsuru','v1','v5m','v5b','v5n','v5s','v5g','v5k','v5r','v5t','v5u','adji','exp','name','single'];
              for (var type of types) {
                var json = json_rubymap[type];
                for (var item of json) {
                  let [base,top,desc] = item;
                  if(desc){
                    var mydesc = `${base}・${top}・${desc}`;
                  } else {
                    var mydesc = `${base}・${top}`;
                  }
                  this.addItem(base,top,type,mydesc,rmap);
                }
              }
            } else if (myid === 'verb') {
              var types = ['vspecial','vsuru','v1','v5m','v5b','v5n','v5s','v5g','v5k','v5r','v5t','v5u'];
              for (var type of types) {
                var json = json_rubymap[type];
                for (var item of json) {
                  let [base,top,desc] = item;
