  ...
  translateHtml (all,config,ispreview=false,editorrow=-1,editorcolumn=-1) {
    ...
    for (var block of all) {
      ...
      switch (sig) {
        ...
        case 'PLST': {
          ...
          for (var item of data) {
            ...
            if (item.length === 3) {
              switch (lead) {
                case 'OL': {
                  o.push(`<ol style='padding-left:${xleft}cm;list-style-position:outside;'><li value='${bullet}'>${text}`);
                  break;
                }
                case 'UL': {
                  o.push(`<ul style='padding-left:${xleft}cm;list-style-position:outside;'><li value='${bullet}'>${text}`);
                  break;
                }
                case 'LI': {
        ...
        case 'VERB': {
          const xleft = config.LATEX.stepMargin;
          const xsize = this.toCssFontsize(config.VERB.fs);
          var text = data;
          text = text.map( x => this.escape(x) );
          text = text.join('\n');
          o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='${this.divcssstyle}; margin-left:${xleft}cm; font-size:${xsize};'>`);
        ...
        case 'ITEM': {
          const xleft = config.LATEX.stepMargin;
          const xsize = this.toCssFontsize(config.ITEM.fs);
          var text = data;
          text = text.join('\n');
          text = this.unmask(text);
          o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='${this.divcssstyle}; margin-left:${xleft}cm; font-size:${xsize};'>`);
        ...
        case 'SAMP': {
          const xleft = fencecmd.margin;
          const xsize = this.toCssFontsize(fencecmd.fs);
          var text = data;
          text = text.map( x => this.escape(x) );
...
//console.log(data);
        ...
        case 'TBLR': {
          ...
          o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='${this.divcssstyle} font-size:${xsize} '>`);
          o.push(text);
          o.push('</div>');
          o.push('');
          break;
...
///~~~
  ...
  toCssFontsize(xsize){
    return this.tokenizer.toCssFontsize(xsize);
  }
