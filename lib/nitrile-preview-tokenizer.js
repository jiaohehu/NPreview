...
module.exports = { 
  ...
  parse (str) {
    var l = this.toTokens(str);
    var g = this.toGroups(l);
console.error('after toGroups');
console.error(g);
    var g = this.toCleanup(g);
console.error('after toCleanup');
console.error(g);
    var g = this.toSubsup(g);
console.error('after toSubsup');
console.error(g);
    var g = this.toCommands(g);
console.error('after toCommands');
console.error(g);
    var s = this.toSvg(g);
    return s;
  },
    ...
    if (Array.isArray(g)) {
      ...
      if (key  === 'brace' || key  === 'leftright') {
        ...
        while (j < inner.length) {
          inner0 = inner[j]
          if (typeof inner0 === 'string' && re_empty.test(inner0)) {
            /// ignore
          } else {
        ...
        if (g.length == 2) {
          return [key,this.toCleanup(g[1])];
        }
        if (g.length == 3) {
          return [key,this.toCleanup(g[1]),this.toCleanup(g[2])];
        }
        if (g.length == 4) {
          return [key,this.toCleanup(g[1]),this.toCleanup(g[2]),this.toCleanup(g[3])];
        }
      }
    }
    return g;
    ...
    if (Array.isArray(g)) {
      ...
      if (key  === 'brace' || key  === 'leftright') {
        ...
        while (j < inner.length) {
          ...
          inner4 = inner[j+4]
          if (inner0 === '\\sqrt') {
            o.push([inner0,inner1]);
            j += 2;
          } else if (inner0 === '\\frac') {
            o.push([inner0,inner1,inner2]);
            j += 3;
          } else if (inner0 === '\\binom') {
            o.push([inner0,inner1,inner2]);
            j += 3;
          } else {
            o.push(inner0);
        ...
        if (g.length == 2) {
          return [key,this.toCommands(g[1])];
        }
        if (g.length == 3) {
          return [key,this.toCommands(g[1]),this.toCommands(g[2])];
        }
        if (g.length == 4) {
          return [key,this.toCommands(g[1]),this.toCommands(g[2]),this.toCommands(g[3])];
        }
      }
    }
    /// not an array
