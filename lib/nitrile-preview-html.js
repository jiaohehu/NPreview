...
`;
  ...
  do_math(block) {
    ...
    if (name === 'aligns') {
      ...
      var mydata1 = this.toMathTabular(data);
      var mydata2 = this.toMathTabularAligned(data);
      this.toParagraphMath(o,id,sig,dept,fname,row1,row2,data,fencecmd,false);
    } else {
      this.toParagraphMath(o,id,sig,dept,fname,row1,row2,data,fencecmd,false);
    }
    block.html = o.join('\n');
  ...
  toMathTabularAligned(data) {
    ...
    var data = data.map( x => {
        ...
        if (v) {
          ...
          s = s.map((y,i) => `<td style='text-aligned:${align_vals[i]};padding-right:${paddn_vals[i]}'>${y}</td>`);
        } else {
          var s = ['','',''];
