...
///console.log(`image ${fsrc} changed`);
  ...
  async toAll() {
    ...
    await this.parser.readSubs(subs);
  ...
  refreshView() {
    ...
    this.parser.translateBlocks({ispreview,editorrow,editorcolumn});
  ...
  updateBlocks() {
    ...
    this.toAll().then( (subs) => 
        ...
        this.title = this.parser.config.title ? this.parser.config.title : 'Untitled'
        this.author = this.parser.config.author ? this.parser.config.author : ''
        this.haschapter = this.parser.config.haschapter;
