...
class SphinxPreviewLatex extends SphinxPreviewParser {
  ...
  */
  toLINES (blocks,plevel,isarticle,o) {
    const step = 0.25;
    for (var block of blocks) {
      const [id,row1,row2,type,n,data,para,inst,inst_t] = block;
...
module.exports = {
  ...
  async toARTICLE (lines) {
    ...
    o = parser.toLINES(blocks,0,true,o);
    if (parser.title) {
      title = parser.title;
    }
...
\\end{document}`;
  ...
  async toBOOK (lines,dirname) {
    ...
    var the_bookdata = [];
    for (var block of blocks) {
      var [id,row1,row2,type,n,data,para,inst,inst_t] = block;
      if (type === '%BOOK') {
        var the_bookdata = data;
      }
    }
    if (the_bookdata.length == 0) {
      return "Cannot find a BOOK block";
    }
    ///
      ...
      if (v) {
        ...
        if (fname) {
          ...
          try {
            ...
            blocks = parser.toBLOCKS(lines,blocks,fname);
          } catch(e) {
            o.push('\\begin{flushleft}');
            o.push(`'${''+e}'`);
            o.push('\\end{flushleft}');
    ...
    var o = parser.toLINES(blocks,plevel,false,[]);
