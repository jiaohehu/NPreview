...
class NitrilePreviewParser {
    ...
    */
    ...
    while (lines.length > 0) {
      ...
      if (block_type === 'SAMP') {
        ...
        /// trim it first before parse
        this.addItemsToRubyMap(para,rmap);
        continue
  ...
  addItemsToRubyMap(para,rmap) {
    ...
    for( var s of para ) {
      if((v=this.re_rbit2.exec(s))!==null){
        ...
        var rt = v[4];
        var desc = v[5];
        if((v=re.exec(desc))!==null) {
          var type = v[1];
          var desc = v[2];
          this.addItem(rb,rt,type,desc,rmap);
        } else {
          rmap.push([rb,rt,desc]);
        }
        continue;
      }
    }
  ...
  addItem(base,top,type,desc,rmap) {
    /// "節約する","せつやくする","vsuru"  
