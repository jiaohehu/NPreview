...
`
  ...
  translateHtml (all,config,ispreview=false,editorrow=-1,editorcolumn=-1) {
    ...
    for (var block of all) {
      ...
      const [id,row1,row2,sig,_subrow,data,para,fencecmd,_caption,base,_label,fname] = block;
      this.block = block;
      this.base = base;
...
//console.log(data);
        ...
        case 'tabular':
          ...
          if (!hlines) {
            if (sig==='tabular') {
              if (config['fenceopt.tabular.hlines']) {
                hlines = config['fenceopt.tabular.hlines'];
              } 
            }
            if (sig==='longtable') {
              if (config['fenceopt.longtable.hlines']) {
                hlines = config['fenceopt.longtable.hlines'];
              } 
            }
          }
