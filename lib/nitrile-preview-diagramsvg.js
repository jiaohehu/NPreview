...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_finalize(s) {
    ...
    o.push('<defs>');
    o.push(`<marker id='markerArrow' markerWidth='3' markerHeight='4' refX='3' refY='2' orient='auto'> <path d='M0,0 L3,2 L0,4 z' stroke='context-stroke' fill='context-stroke'/> </marker>`);
    o.push(`<marker id='startArrow'  markerWidth='3' markerHeight='4' refX='0' refY='2' orient='auto'> <path d='M3,0 L3,4 L0,2 z' stroke='context-stroke' fill='context-stroke'/> </marker>`);
    o.push('</defs>');
    o.push(s);    
    return [o.join('\n'),vw,vh];
  }
  ...
  do_label(opt,txt,coords){
    ...
    for (var i = 0; this.valid(coords, i); i++) {
      ...
      if ((v=this.re_inlinemath.exec(t))!==null) {
        var [w,h,defs_,s_] = this.tokenizer.toDiagramSvg(v[1]);
        ...
        s.push(`<svg x='${x}' y='${y}' width='${w}' height='${h}' fill='inherit' stroke='inherit' viewBox='0 0 ${vw} ${vh+1}'>${defs_}${s_}</svg>`);
      } else {
  ...
  do_circle(opt,txt,coords) {
    ...
    for (var i = 0; this.valid(coords, i); i++) {
      ...
      if (opt==='') {
        ...
        var xm = x + radius   * Math.cos(my_angle / 180 * Math.PI);
        var ym = y + radius   * Math.sin(my_angle / 180 * Math.PI);
        var path = `(${x1},${y1})..(${xm},${ym})..(${x2},${y2})--(${x1},${y1})--cycle)`;
        var p = this.readCoordsLine(path);
        var d = this.coordsToD(p);
        s.push(`<path d='${d}' ${this.strokes()} ${this.fills()}/>`);
      }
  ...
  do_halfcircle(opt,txt,coords) {
    ...
    var path='(1,0)..(0,1)..(-1,0)--(1,0)--cycle ';
    var p0 = this.readCoordsLine(path);
    for (var i = 0; this.valid(coords, i); i++) {
      var z0 = this.point(coords, i);
      var x = (z0[0]);
  ...
  do_quadrant(opt,txt,coords) {
    ...
    var f = this.readParamsLine(txt);
    var r = this.assertFloat(f[0],1,0,this.MAX);
    var path='(1,0)..(0.7071067812,0.7071067812)..(0,1)--(0,0)--(1,0)--cycle ';
    var p0 = this.readCoordsLine(path);
    for (var i = 0; this.valid(coords, i); i++) {
      var z0 = this.point(coords, i);
      var x = (z0[0]);
  ...
  do_octant(opt,txt,coords) {
    ...
    var f = this.readParamsLine(txt);
    var r = this.assertFloat(f[0],1,0,this.MAX);
    var path='(1,0)..(0.9238795325112867,0.3826834323650898)..(0.7071067811865475,0.7071067811865475)--(0,0)--(1,0)--cycle ';
    var p0 = this.readCoordsLine(path);
    for (var i = 0; this.valid(coords, i); i++) {
      var z0 = this.point(coords, i);
      var x = (z0[0]);
  ...
  do_cartesian(opt,act,floats) {
    ...
    switch( act ) {
      ...
      case 'ellipse':
        ...
        var angle = this.assertFloat(floats[2],0);
        var Rx    = this.assertFloat(floats[3],2);
        var Ry    = this.assertFloat(floats[4],1);
  ...
  drawtext(x,y,label,justi) {
    ...
    /// label is a string which could be "``\sqrt{2}``" or "a"
    /// justi is a string such as "ctr", "urt", "top", etc.
    /// The return value is a SVG command such as "<svg...></svg>" or "<text.../>"
    ///
    ...
    if ((v=this.re_inlinemath.exec(label))!==null) {
      ...
      label = this.parser.escape(label);
      var anchor = 'middle', dy='0.3em';
      if (justi==='lrt') {
        anchor = 'start', dy='0.8em';
        //s.push(`<text text-anchor='start'  ${this.texts()} x='${x}' y='${y}'           dy='0.8em'>${label}</text>`);
      } else if (justi==='bot') {
        anchor = 'middle', dy='0.8em';
        //s.push(`<text text-anchor='middle' ${this.texts()} x='${x}' y='${y}'           dy='0.8em'>${label}</text>`);
      } else if (justi==='llft') {
        anchor = 'end', dy='0.8em';
        //s.push(`<text text-anchor='end'    ${this.texts()} x='${x}' y='${y}'           dy='0.8em'>${label}</text>`);
      } else if (justi==='urt') {
        anchor = 'start', dy='-0.2em';
        //s.push(`<text text-anchor='start'  ${this.texts()} x='${x}' y='${y}'           dy='-0.2em'>${label}</text>`);
      } else if (justi==='top') {
        anchor = 'middle', dy='-0.2em';
        //s.push(`<text text-anchor='middle' ${this.texts()} x='${x}' y='${y}'           dy='-0.2em'>${label}</text>`);
      } else if (justi==='ulft') {
        anchor = 'end', dy='-0.2em';
        //s.push(`<text text-anchor='end'    ${this.texts()} x='${x}' y='${y}'           dy='-0.2em'>${label}</text>`);
      } else if (justi==='rt') {
        anchor = 'start', dy='0.3em';
        //s.push(`<text text-anchor='start'  ${this.texts()} x='${x}' y='${y}'           dy='0.3em'>${label}</text>`);
      } else if (justi==='lft') {
        anchor = 'end', dy='0.3em';
        //s.push(`<text text-anchor='end'    ${this.texts()} x='${x}' y='${y}'           dy='0.3em'>${label}</text>`);
      } else if (justi==='ctr') {
        anchor = 'middle', dy='0.3em';
        //s.push(`<text text-anchor='middle' ${this.texts()} x='${x}' y='${y}'           dy='0.3em'>${label}</text>`);
      }
      s.push(`<text text-anchor='${anchor}' ${this.texts()} x='${x}' y='${y}' dy='${dy}'>${label}</text>`);
    }
    return s.join('\n');
  }
