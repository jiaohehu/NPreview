...
class NitrilePreviewTokenizer {
  ...
  constructor (parser) {
    this.parser = parser;
    this.fs = parser.config.HTML.bodyfontsizept;
    ...
    this.re_end = /^\\end\{(\w+)\}$/;
    this.re_fontvariants = /^\\(mathit|mathrm|mathbf|mathbb|mathcal)\s*\{(.*)\}$/;
  ...
  toMathSvg (str,isdisplaymath) {
    this.isdisplaymath = isdisplaymath;
    this.fs = this.parser.config.HTML.bodyfontsizept;
    var used = new Set();
    var l = this.toTokens(str);
    var g = this.toGroups(l);
    var g = this.toCleanup(g);
  ...
  toMathSvgArray(str, isdisplaymath) {
    this.isdisplaymath = isdisplaymath;
    this.fs = this.parser.config.HTML.bodyfontsizept;
    var l = this.toTokens(str);
    var g = this.toGroups(l);
  ...
  ///This method is called by Diagram to 
  ///generate a SVG to be inserted inside 
  ///a Diagram SVG. The only difference is that
  ///a Diagram's math would use a different font size
  toDiagMathSvg (str) {
    this.isdisplaymath = false;
    this.fs = this.parser.config.HTML.diagfontsizept;
    var used = new Set();
    var l = this.toTokens(str);
    var g = this.toGroups(l);
    var g = this.toCleanup(g);
    ...
    var [w,h,mid,s,op,q,id,shiftdist] = this.toInnerSvg(g,used);
    var defs = this.buildup_defs(used);
    return {s,w,h,mid,defs,shiftdist};
  }
  ...
  buildup_defs(used){
    ...
    for (var mathSymbol of pjson.mathSymbols) {
      ...
      width *= this.fs/12;
      if (used.has(id)) {
        defs.push(`<text id='${id}' dy='${this.dy}em' textLength='${width}pt' lengthAdjust='spacingAndGlyphs' style='stroke:none;fill:inherit;font-size:${this.fs}pt'>${unicode}</text>`);
      }
    }
    return defs;
  }
  ...
  toCleanup (g) {
    ...
    if (Array.isArray(g)) {
      ...
      if (key==='\\math' || key  === '\\brace' || key  === '\\leftright' || key === '\\beginend') {
        ...
        /// here the key could \sqrt, \binom, etc., in this case the 'g' is an array, where
        /// the first element is a string, such as '\\sqrt', '\\binom', etc. and the second
        /// and third elements might be a string, or another sub-group.
        console.log("Tokenizer: here A");
        var g = g.map(x => this.toCleanup(x));
        var g = g.filter(x => (typeof x === 'string' && re_empty.test(x))?false:true);
        return g;
      }
    }
    return g;
  ...
  toInnerSvg (v, used, compact=false) {
          ...
          if (name == 'cases' || 'matrix' || name == 'pmatrix' || name == 'bmatrix' || name == 'Bmatrix' || name == 'vmatrix' || name == 'Vmatrix' ) {
            ...
            for (let j=0; j < ncol; ++j) {
              for (let i=0; i < nrow; ++i) {
                let pv = p[i][j];
                if (pv) {
                  p[i][j] = this.toInnerSvg(['\\brace',pv,'','',''],used,compact);
                }
              }
            }
            ...
            //var [w_,h_,mid_,s_,g_,q_,id_] = this.toInnerSvg(['\\brace',p[0][0],'','',''],used,compact);
...
console.log('w1=',w1);
            ...
            return this.toInnerSvg(['\\brace',v[1],'','',v[4]],used,compact);
          }
          break;
        }
        ...
        case '\\leftright':
          ...
          var h = 0;
          var mid = 0;
          var results = g.map( (z) => this.toInnerSvg(z,used,compact) );
          var s = '';
          var o = [];
          ...
          if (v[0] === '\\math' || v[0] === '\\brace') {
            ...
            /// if it is \\math and shiftdist is still zero, we set it to
            /// the current width to express the width of this expression
            if(v[0] === '\\math' && shiftdist < 0){
              shiftdist = w;
            }
            return [w,h,mid,o.join('\n'),0,0,first_id,shiftdist];
          } else {
            /// \\leftright
            s = o.join('\n');
            var fence1 = v[2];
        ...
        case '\\widehat':
          var [w_,h_,mid_,s_,g_,q_,id_] = this.toInnerSvg(v[1],used,compact);
          var lead = 1;
          var sunk = 3;
          w = w_ + lead + lead;
          h = h_ + sunk;
        ...
        case '\\dif':
          ...
          var [w1_,h1_,mid1_,s1_,g1_,q1_,id1_] = this.toInnerSvg(v[1],used,compact);
          var w = wd_ + w1_;
          var h = h1_;
          var mid = mid1_;
          var o = [];
        ...
        case '\\od':
          ...
          var [w1_,h1_,mid1_,s1_,g1_,q1_,id1_] = this.toInnerSvg(v[2],used,compact);
          var [w2_,h2_,mid2_,s2_,g2_,q2_,id2_] = this.toInnerSvg(v[3],used,compact);
          var top_w_ = wd_ + nws_ + 1 + w1_;
          var bot_w_ = wd_ + w2_ + nws_;
          var top_h = Math.max(hd_+ext, h1_);
          var bot_h = Math.max(hd_, h2_+ext);
          w = Math.max(top_w_ , bot_w_);
          ...
          if (2) {
            ...
            var dy = top_h + 1;
            // letter 'd'
            o.push(`<use transform='translate(${dx*1.333},${(dy+bot_h-hd_)*1.333})' xlink:href='#${idd_}'/>`);
            // letter 'x'
            o.push(`<g transform='translate(${(dx+wd_)*1.333},${(dy+bot_h-h2_)*1.333})'> ${s2_} </g>`);
            // letter '2'
            o.push(`<use transform='translate(${(dx+wd_+w2_)*1.333},${(dy+bot_h-h2_-ext)*1.333}) scale(${this.sup_rate})' xlink:href='#${ids_}'/>`);
          }
        ...
        case '\\pd':
          ...
          o.push(`<line x1='0pt' y1='${hline_dy}pt' x2='${w}pt' y2='${hline_dy}pt' style='stroke:inherit; fill:none; font-size:${this.fs}pt;'/>`);
          this.fraclevel -= 1;
          mid = hline_dy;
          h = dy;
          var newv = [w,h,mid,o.join('\n'),0,0,''];
        ...
        case '\\sqrt':
          ...
          var [w_,h_,mid_,s_,g_,q_,id_] = this.toInnerSvg(v[2],used,compact);
          var lead = wr_;//already shrinked/expanded
          var sunk = 1 * this.fs/12;
          ...
          o.push(`<g transform='translate(0,${sunk*1.333})'>`);
          o.push(`<use x='0' y='0' transform='scale(1,${sy})' xlink:href='#${idr_}' />`);
          o.push(`<svg x='${lead}pt' y='0'>`);
          o.push(s_);
          o.push(`</svg>`);
          o.push(`</g>`);
          if(n){
          ...
          o.push(`<line x1='${lead}pt' y1='${sunk/2}pt' x2='${w}pt' y2='${sunk/2}pt' style='stroke:inherit; fill:none; font-size:${this.fs}pt;'/>`);
          return [w,h,mid,o.join('\n'),0,0,''];
        ...
        case '\\frac':
          ...
          this.fraclevel += 1;
          var reduce_rate = 1;
          if (this.isdisplaymath && !this.isinmatrix){
            if(this.fraclevel == 1) {
              reduce_rate = 1;
            }else if(this.fraclevel == 2) {
          ...
          var [w1_,h1_,mid1_,s1_,g1_,q1_,id1_] = this.shrinkSvg(this.toInnerSvg(v[1],used,compact),reduce_rate);
          var [w2_,h2_,mid2_,s2_,g2_,q2_,id2_] = this.shrinkSvg(this.toInnerSvg(v[2],used,compact),reduce_rate);
          this.fraclevel -= 1;
          w = (w1_ > w2_) ? w1_ : w2_;
          w += 2; /// make it 1pt longer than either one of the numerator/denomitator
          var hh_ = 1; /// add 1pt gap between numerator and denominator
          h = hh_ + h1_ + h2_;
          mid = h1_ + hh_/2;
          var o = [];
          if (w1_ < w2_) {
            var dx = 1+ (w2_ - w1_)/2;
            //o.push(`<svg x='${dx}pt' y='0'>`);
            o.push(`<g transform='translate(${dx*1.333},0)'>`);
            o.push(s1_);
            o.push(`</g>`);
            //o.push(`</svg>`);
            //o.push(`<svg x='1pt' y='${hh_+h1_}pt'>`);
            o.push(`<g transform='translate(1.333,${(hh_+h1_)*1.333})'>`);
            o.push(s2_);
            o.push(`</g>`);
            //o.push(`</svg>`);
            o.push(`<line x1='0pt' y1='${mid}pt' x2='${w}pt' y2='${mid}pt' style='stroke:inherit; fill:none; font-size:${this.fs}pt;'/>`);
            var newv = [w,h,mid,o.join('\n'),0,0,''];
            return newv;
          } else {
            var dx = 1+ (w1_ - w2_)/2;
            //o.push(`<svg x='1pt' y='0'>`);
            o.push(`<g transform='translate(1.333,0)'>`);
            o.push(s1_);
            o.push(`</g>`);
            //o.push(`</svg>`);
            //o.push(`<svg x='${dx}pt' y='${hh_+h1_}pt'>`);
            o.push(`<g transform='translate(${dx*1.333},${(hh_+h1_)*1.333})'>`);
            o.push(s2_);
            o.push(`</g>`);
            //o.push(`</svg>`);
            o.push(`<line x1='0pt' y1='${hh_/2+h1_}pt' x2='${w}pt' y2='${hh_/2+h1_}pt' style='stroke:inherit; fill:none; font-size:${this.fs}pt;'/>`);
            var newv = [w,h,mid,o.join('\n'),0,0,''];
            return newv;
          }
          break;
        case '\\sfrac':
          var [w1_,h1_,mid1_,s1_,g1_,q1_,id1_] = this.shrinkSvg(this.toInnerSvg(v[1],used,compact),this.frac_rate);
          var [w2_,h2_,mid2_,s2_,g2_,q2_,id2_] = this.shrinkSvg(this.toInnerSvg(v[2],used,compact),this.frac_rate);
          var [id3_,w3_,op3_] = this.findIdById('mySOLIDUS',used);
          var h3_ = this.fs;
          var w = w1_ + w3_ + w2_;
          ...
          var o = [];
          o.push(s1_);
          o.push(`<g transform='translate(${(w-w2_)*1.3333},${(h-h2_)*1.3333})'>`);
          o.push(s2_);
          o.push(`</g>`);
          o.push(`<use x='${w1_}pt' y='0' xlink:href='#${id3_}' />`);
          return [w,h,mid,o.join('\n'),0,0,''];
          break;
        case '\\binom':
          var [w1_,h1_,mid1_,s1_,g1_,q1_,id1_] = this.toInnerSvg(v[1],used,compact);
          var [w2_,h2_,mid2_,s2_,g2_,q2_,id2_] = this.toInnerSvg(v[2],used,compact);
          w = (w1_ > w2_) ? w1_ : w2_;
          h = 1 + h1_ + h2_;
          mid = h1_;
          var o = [];
          ...
          if (w1_ < w2_) {
            ...
            o.push(s2_);
            o.push(`</svg>`);
            o.push(`<svg x='0' y='0' width='${T}pt' height='${h}pt' viewBox='0 0 ${w1*1.333} ${h1*1.333}' preserveAspectRatio='none'>`);
            o.push(`<use x='0' y='0' xlink:href='#${id1}' />`);
            o.push(`</svg>`);
            o.push(`<svg x='${T+w}pt' y='0' width='${T}pt' height='${h}pt' viewBox='0 0 ${w2*1.333} ${h2*1.333}' preserveAspectRatio='none'>`);
            o.push(`<use x='0' y='0' xlink:href='#${id2}' />`);
            o.push(`</svg>`);
        ...
        case '\\sup':
          ...
          if (v[0] === '\\subsup') {
            var [w1_,h1_,mid1_,s1_,g1_,q1_,id1_] = this.toInnerSvg(v[1],used,compact1);
            var [w2_,h2_,mid2_,s2_,g2_,q2_,id2_] = this.toInnerSvg(v[2],used,compact2);
            var [w3_,h3_,mid3_,s3_,g3_,q3_,id3_] = this.toInnerSvg(v[3],used,compact3);
          } else if (v[0] === '\\sub') {
            var [w1_,h1_,mid1_,s1_,g1_,q1_,id1_] = this.toInnerSvg(v[1],used,compact1);
            var [w2_,h2_,mid2_,s2_,g2_,q2_,id2_] = this.toInnerSvg(v[2],used,compact2);
            ...
            var [w3_,h3_,mid3_,s3_,g3_,q3_,id3_] = this.toInnerSvg(v[2],used,compact3);
          }

          if (!this.isinmatrix && this.isdisplaymath && (
                  v[1] === '\\sum' || 
                  v[1] === '\\prod' || 
                  v[1] === '\\lim'  )) {
            var o = [];
            var nw1_ = w1_    *1;///the returned size would have already been adjusted for isdisplaymath
            var nh1_ = h1_    *1;
            var nmid1_ = mid1_*1;
      ...
      var m;
      if ((m=this.re_fontvariants.exec(v))!==null) {
        var key = m[1];
        var str = m[2];
        var id = str[0];
        is_mathvariant = true;
      ...
      if (id === 'myINT') { 
      ...
      } else if (id === 'mySUM' || id === 'myPROD') { /// THIS is \\sum symbol by itself it is enlarged if 'isdisplaymath' is on
        ...
        ///such as \Rot
        var v = m[1];
        var [w, h, mid, s, g, q, id] = this.to_text_element(v);
        var g = 3; ///set g to 3 so that it exhibit a \log like behavior
  ...
  to_text_element(text,keepwhitespace){
    ...
    var totalw = this.measureText(text,this.fs);
    text = this.parser.escape(text);
