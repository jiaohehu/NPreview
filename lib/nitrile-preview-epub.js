...
class NitrilePreviewEpub {
  ...
  async generateAsync (title,author,htmls,chaps,imageidmap,imagemap,isarticle) {
...
console.error('imgbuff='+imagemap.get(imgid));
        var imgbuff = imagemap.get(imgid);
      } else {
        var imgbuff = new Buffer('');
      }
    ...
    if (isarticle) {
      ...
      tocs.push([`content${i0}.xhtml`,text0]);
