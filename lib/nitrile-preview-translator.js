...
class NitrilePreviewTranslator extends NitrilePreviewBase {
    ...
    while ((v = re_unmask.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        if(key.length==1){
          ...
          newtext += this.to_style(cnt, 'code');
        }
      } 
  ...
  untouch (line) {
    ...
    var start_i = 0;
    var newtext = '';
    const re_untouch = /(?<!\\)(\\[A-Za-z]+\{)|(?<!\\)(\\\()|(?<!\\)(\\\[)|(?<!\\)(\\\\[A-Za-z0-9]+)/g; //cannot be global because it is used recursively
    while ((v = re_untouch.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        try {
          ...
          var cnt_text = this.smooth(cnt_text);//this will have to be smooth becaus \dot{x} is processed here
          newtext += cnt_text;
        }
        ...
        var match_text = this.polish(match_text);
        var match_text = this.to_style(match_text,'code');
        newtext += match_text;
