...
const global_rubymap = [];
///this.buildRubyMap(json_rubymap);
...
class NitrilePreviewParser {
  ...
  constructor() {
    ...
    this.re_ntrflag     = /^nitrile\.([\w]+)\.([\w]+)\s*\=\s*(.*)\s*$/;
    ...
    */
    ...
    /// initialize the this.config
    this.config.ALL = {};
    this.config.ALL.title = '';
    this.config.ALL.author = '';

    /// All the layout dimensions are in 'mm'
    this.config.CONTEX = {};
    this.config.CONTEX.backspace=38;
    this.config.CONTEX.width=130;
    this.config.CONTEX.topspace=30;
    this.config.CONTEX.header=0;
    this.config.CONTEX.footer=0;
    this.config.CONTEX.height=250;
    this.config.CONTEX.papersize='A4';

    /// All the layout dimensions are in 'mm'
    this.config.LATEX = {};
    this.config.LATEX.leftmargin=44;
    this.config.LATEX.rightmargin=44;
    this.config.LATEX.topmargin=30;
    this.config.LATEX.papersize='a4paper';
    this.config.LATEX.twoside='';///set to 'twoside' to enable
    this.config.LATEX.twocolumn='';///set to 'twocolumn' to enable
    this.config.LATEX.latexengine='';///set to 'pdflatex' or 'lualatex', default to 'lualatex'
    this.config.LATEX.tableofcontents='';///set to 'tableofcontents' to enable
    this.config.LATEX.documentclass='';///set to 'book','scrbook', etc., default to 'article'

    /// All the layout dimensins are in 'cm'
    this.config.HTML = {};
    this.config.HTML.leftmargin=44;
    this.config.HTML.rightmargin=44;
    this.config.HTML.topmargin=30;
    this.config.HTML.bodyfontsizept=11.5;
    this.config.HTML.diagfontsizept=10.0;

    this.config.PREVIEW = {};
    this.config.PREVIEW.leftmargin=44;
    this.config.PREVIEW.rightmargin=44;
    this.config.PREVIEW.topmargin=30;
    this.config.PREVIEW.bodyfontsizept=11.5;
    this.config.PREVIEW.diagfontsizept=10.0;
    ...
    this.config.DLST = {};
    this.config.TBLR = {};
    this.config.TBLR.hlines = 'm';
    this.config.TBLR.vlines = '';
    this.config.TBLR.vpadding = 1;
    this.config.TBLR.hpadding = 6;
    this.config.TBLR.fs = 'small';
    this.config.LONG = {};
    this.config.LONG.hlines = 't m b r';
    this.config.LONG.vlines = '*';
    this.config.LONG.vpadding = 3;
    this.config.LONG.hpadding = 6;
    this.config.LONG.fs = 'small';
    this.config.SAMP = {};
    this.config.SAMP.samptype=0;///0=pre;1=line;2=list
    this.config.TABB = {};
    this.config.IMGS = {};
    this.config.DIAG = {};
    this.config.VERB = {};
    this.config.VERB.numbers = 'left';
    this.config.VERB.numbersep = '2mm';
    this.config.VERB.monospace = 0;
    this.config.VERB.fs = 'footnotesize';
    this.config.FRMD = {};
    this.config.FRMD.frameborder = 1;

  }

  readFromLines(lines,fname) {
  ...
  new_hdgs_block (text) {
    ...
    var level = 0;
    var hdgn = 0;
    return ({id,row1,row2,sig,html,level,hdgn,data,para,fencecmd,subrow,fname});
  ...
  rubify (src) {
    ...
    while (j < src.length) {
      i0 = src.length;
      found = 0;
      for (var rubyitem of global_rubymap) {
  ...
  idenBlocks() {
    ...
    autonum.idenBlocks(this.blocks,this.config,this.isepub);
  }
  ...
  translateBlocks(options={}) {
    ...
    this.block = [];
    this.haschapter = this.config.ALL.haschapter;
    for (var block of this.blocks) {
      ...
      this.xleft = this.to_stepmargin(this.config.ALL.stepmargin);
      this.xnumbers = this.to_numbers(fencecmd.numbers);
      this.xnumbersep = this.to_numbersep(fencecmd.numbersep);
      this.xbodyfontsizept = (this.config.HTML.bodyfontsizept);
      this.xdiagfontsizept = (this.config.HTML.diagfontsizept);
...
`;
  ...
  async readFromFile(fname) {
    ...
    var lines = out.split('\n');
    this.readFromLines(lines,fname);
  }

  async readSubs(subs) {
    for( var sub of subs ) {
      let { subfname, sublevel, subrow } = sub;
      var fsubfname = path.join(path.dirname(this.fname),subfname);
      sub.subparser = new NitrilePreviewParser();
      sub.subprom = await sub.subparser.readFromFile(fsubfname);
    }
    for( var sub of subs ) {
      await sub.subprom;
    }
