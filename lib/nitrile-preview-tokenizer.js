...
class NitrilePreviewTokenizer {
  ...
  toMathSvgArray(str, isdisplaymath,wide) {
    this.isdisplaymath = isdisplaymath;
    this.fs = this.parser.config.HTML.bodyfontsizept;
    var l = this.toTokens(str);
    var g = this.toGroups(l);
  ...
  toCleanup (g) {
    ...
    if (Array.isArray(g)) {
      var key = g[0];
      if (key==='' || key==='\\math' || key  === '\\brace' || key  === '\\leftright' || key === '\\beginend') {
        var inner  = g[1];
        var fence1 = g[2];
        var fence2 = g[3];
...
console.log('w1=',w1);
        ...
        case '\\leftright':
          ...
          for (let result of results) {
            ...
            if(cmdname == '\\math' && id_ == 'myAMPERSAND'){
              ...
              if(shiftdist < 0){//not yet been assigned
                shiftdist = w;
                if(i>0){
        ...
        case '\\operatorname':
          ...
          if(cmdname=='\\image'){
            return this.to_image_element(str);
          }else if(cmdname=='\\text'){
        ...
        case '\\sqrt':
          ...
          o.push(`<svg width='${lead}pt' height='${h}pt' viewBox='0 0 ${wr_*1.333} ${hr_*1.333}'>`);
  ...
  to_outer_svg(s,w,h,mid,defs){
    ...
    var s = `<svg class='math' ${opt}> <defs>${defs.join('\n')}</defs> ${s} </svg>`;
    return s;
  }
