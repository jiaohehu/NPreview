...
class NitrilePreviewParser extends NitrilePreviewBase {
      ...
      if(sig=='SAMP'){
        ...
        if(sig0==='FRMD'||
          ...
          if(sig0==='FRMD'){
            ...
            blk.lines = this.join_backslashed_lines(blk.body);
            blk.lines = blk.lines.map(x => x.trimLeft());
          }else if(sig0==='PICT'){
  ...
  async read_mode_async() {
    ...
    /// check to see if any one of the following mode has a 'only' flag
    var isonly = this.mode.reduce((acc, x) => acc || x.only, 0);
