...
class NitrilePreviewContex extends NitrilePreviewParser {
  ...
  do_csvd(block){
    this.do_csvd_tabulate(block);
  }
  do_csvd_btable(block){
    var o = [];
    ...
    for (var j = 0; j < nrows; ++j) {
      ...
      pp = pp.map(x => this.polish_csvd(x));
...
\\stoptext
  ...
  polish_csvd(s){
    return this.polish(s,this.conf('nicsvd'));
  }
  polish_prog(s){
