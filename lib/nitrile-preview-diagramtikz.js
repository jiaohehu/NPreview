...
class NitrilePreviewDiagramTikz extends NitrilePreviewDiagram {
  ...
  do_finalize(s) {
    ...
    if(this.is_dia){
      var p = `\\draw[help lines, step=${xm*unit}mm,lightgray,very thin] (0,0) grid (${xm*unit}mm,${ym*unit}mm);`;
    }else{
      var p = `\\draw[help lines, step=${unit}mm,lightgray,very thin] (0,0) grid (${xm*unit}mm,${ym*unit}mm);`;
    }
    p = p.trim();
    o.push(p);
    o.push(s);
