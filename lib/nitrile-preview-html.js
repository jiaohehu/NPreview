...
//console.log(data);
        ...
        case 'tabular': 
          ...
          var text = this.toFramedTabular(text,maxj,ww,vlines,hlines);
          o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='${this.divcssstyle}; '>`);
          o.push(`${showidtext}`);
          o.push(text);
          o.push('</div>');
  ...
  */
  ...
  toFramedTabular (rows, maxj, ww, vlines, hlines) {
    rows = rows.map ( row => {
    ...
    var header = header.map((x,i) => `<td style='text-align:left; padding:2; ${vlines.indexOf(i)>=0?'border-left:1px solid black;':''} ${vlines.indexOf(i+1)>=0?'border-right:1px solid black;':''} ${hlines.indexOf('t')>=0?'border-top:1px solid black;':''} ${hlines.indexOf('t')>=0?'border-bottom:1px solid black;':''} '>${x}</td>`);
    o.push(`<tr>${header.join('')}</tr>`);
    var n = rows.length;
    for (var j=0; j < rows.length; ++j) {
      var row = rows[j];
      var row = row.map((x,i) => `<td style='text-align:left; padding:2; ${vlines.indexOf(i)>=0?'border-left:1px solid black;':''} ${vlines.indexOf(i+1)>=0?'border-right:1px solid black;':''} ${j>0&&hlines.indexOf('*')>=0?'border-top:1px solid black;':''} ${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid black;':''} '>${x}</td>`);
