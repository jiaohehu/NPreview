...
`;
  ...
  do_verb(block) {
    ...
    if (this.xnumbers) {
      ...
      for (var k=0; k < text.length; ++k) {
        ...
        s.push(`<tr><td style='font-size:xx-small;padding-right:${this.xnumbersep};'>${++linenum}</td><td style='white-space:pre;width:100%;'>${line}</td></tr>` );
        while (lines.length > 0) {
          ...
          s.push(`<tr><td></td><td style='white-space:pre;width:100%;'>${line}</td></tr>` );
        }
      }
    } else {
      for (var k=0; k < text.length; ++k) {
        ...
        while (lines.length > 0) {
          ...
          s.push(`<tr><td style='white-space:pre;width:100%;'>${line}</td></tr>` );
        }
      }
    }
    text = s.join('\n');
  ...
  do_samp(block) {
    ...
    if(fencecmd.style==1){
      ...
      text = text.join('\n');
      o.push(`<dl id='${id}' class='P ${sig}' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' style='margin-left:${fencecmd.left}mm;margin-top:${this.xtop};margin-bottom:${this.xbot};'>${text}</dl>`);
      o.push('');
    }
    else{
      text = this.trimSampPara(text);
      ...
      text = text.map( x => `<dt style='white-space=pre'>${x}</dt>` );
      text = text.join('\n');
      o.push(`<dl id='${id}' class='P ${sig}' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' style='margin-left:${fencecmd.left}mm;margin-top:${this.xtop};margin-bottom:${this.xbot};};'>${text}</dl>`);
      o.push('');
    } 
