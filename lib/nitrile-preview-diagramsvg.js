...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  p_fill(coords,g){
    ...
    var x = this.to_marker_arrow_str(g);
    var o = [];
    for(var item of items){
      var {iscycled,d} = item; 
      o.push(`<path d='${d}' ${x} ${this.to_fillonly_str(g,1)}/>`);  
    }
    return o.join('\n');
  }
  ...
  p_path(coords,g){
    ...
    for(var item of items){
      var {iscycled,d} = item; 
      if(iscycled){
        o.push(`<path d='${d}' ${x} ${this.to_fillonly_str(g)}/>`);
  ...
  to_drawonly_str(g) {
    ...
    if (g.linecolor) {
      o.push(`stroke='${this.string_to_color_name(g.linecolor)}'`);
    } else {
