...
class NitrilePreviewLatex extends NitrilePreviewParser {
  ...
  do_hdgs(block){
    var o = [];
    var {id,row1,row2,sig,name,sublevel,hdgn,dept,text,para,base,subrow,subfname} = block;
    ...
    if(this.ismaster){
      if(name=='part'){
        ...
        o.push(`\\addcontentsline{toc}{chapter}{${dept} ${raw}}`);
        o.push('%');
      }
      else if(level==1){
        o.push(`\\section*{${dept}~~${text}}${this.latexlabelcmd}`);
        o.push(`\\addcontentsline{toc}{section}{${dept} ${raw}}`);
        o.push('%');
      }
      else if(level==2){
        o.push(`\\subsection*{${dept}~~${text}}${this.latexlabelcmd}`);
        o.push(`\\addcontentsline{toc}{subsection}{${dept} ${raw}}`);
        o.push('%');
      }
      else{
