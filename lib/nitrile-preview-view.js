...
class NitrilePreviewView {
  ...
  hasImageId (src) {
    return this.imageidmap.has(src);
  }
  getImageId (src) {
    return this.imageidmap.get(src);
  }
  setImageId (src,imgid) {
    return this.imageidmap.set(src,imgid);
  }
...
console.log(`load image from imagemap: ${imgid}, ${src}`);
      setTimeout( () => {
        ...
        if (this.imagemap.has(imgid)) {
          let imgbuff = this.imagemap.get(imgid);
...
console.log(`load image from file: ${imgid}, ${src}`);
      ...
      this.parser.readImageFileAsync(fsrc)
        .then( imgbuff => {
            this.imagemap.set(imgid,imgbuff);
            var node = document.querySelector(`img#nitrile-preview-img-${imgid}`);
            if (node) {
              node.src = `data:image/png;base64,${imgbuff.toString('base64')}`;///TODO: harded coded to PNG, 
            }
  ...
  refreshView() {
    ...
    var xrefs_old = this.autonum.xrefs;
    this.autonum.start();

    if (this.isarticle) {
      [this.htmls,this.chaps] = this.parser.translateHtml(this.autonum,this.config,this.blocks,this.isarticle,isepub,0,0,this.htmls,this.chaps);
    } else {
      for (var i=0; i < this.subs.length; ++i) {
        [this.htmls,this.chaps] = this.parser.translateHtml(this.autonum,this.config,this.subs[i].flow[0],this.isarticle,isepub,this.subs[i].subrow,this.subs[i].sublevel,this.htmls,this.chaps);
      }
    }

    this.autonum.end();
    var xrefs_new = this.autonum.xrefs;

    if (this.isEqual(xrefs_old,xrefs_new)) {
      console.log('xrefs_changed is false.');
      this.xrefs_changed = false;
    } else {
      console.log('xrefs_changed is true.');
      this.xrefs_changed = true;
      this.parser.xrefs = xrefs_new;
    }
...
`;
        ...
        /// regenerate HTML with 'isepub' flag on
        ...
        this.autonum.start();
        if (this.isarticle) {
          [htmls,chaps] = this.parser.translateHtml(this.autonum,this.config,this.blocks,this.isarticle,isepub,0,0,htmls,chaps);
        } else {
          for (var i=0; i < this.subs.length; ++i) {
            [htmls,chaps] = this.parser.translateHtml(this.autonum,this.config,this.subs[i].flow[0],this.isarticle,isepub,this.subs[i].subrow,this.subs[i].sublevel,htmls,chaps);
          }
        }
        this.autonum.end();
...
`;
        ...
        data = await epub.generateAsync(this.parser.escape(this.title),this.parser.escape(this.author),htmls,chaps,this.imageidmap,this.imagemap,this.isarticle,this.dirname());
      ...
      } else if (this.saveastype === 'zip') {

        var zip = new JSZip();
        zip.file("Hello.txt", "Hello World\n");
        var img = zip.folder("images");
        var imgData = fs.readFileSync('/Users/james/sphinx/notes/d3js-notes/tree.png');
        img.file('mytree.png', imgData);
        if (0) {
          zip.generateNodeStream({type:'nodebuffer',streamFiles:true})
            .pipe(fs.createWriteStream('/Users/james/temp/out.zip'))
            .on('finish', function () {
              // JSZip generates a readable stream with a "end" event,
              // but is piped here in a writable stream which emits a "finish" event.
              console.error("/Users/james/temp/out.zip written.");
            });
        } else {
