...
///console.log(data);
          ...
          var text = this.toFramedImgs(column,data);
        ...
        case 'TABL': {
          ...
          if (fencecmd.table === 'tight') {

            var [text,maxj,ww] = data;
            var text = this.toFramedTabu(text, maxj, ww, fencecmd, config);
            ...
            var text = this.toFramedTabb2(text, maxj, fencecmd);
...
///console.log(pcol);
        ...
        case 'VERB': {
          ...
          if (1) {
              out =  `{\\resizebox{\\linewidth}{!}{\\fbox{${out}}} }`;
          } else {
            if (config.verbframe) {
        ...
        case 'DESC': {
          ...
          if (cat === 'mono') {
            ...
            for (var key of keys) {
              key = `${this.escape(bullet)}\\ttfamily\\bfseries{}${this.escape(key)}`;
              o.push(`\\item\[${key}\]`);
            }
          } else if (cat === 'strong') {
            o.push(`\\begin{description}\[nosep,style=unboxed,leftmargin=${xleft}cm,font=\\normalfont]`);
            for (var key of keys) {
              key = `${this.escape(bullet)}\\bfseries{}${this.escape(key)}`;
              o.push(`\\item\[${key}\]`);
            }
          } else {
            o.push(`\\begin{description}\[nosep,style=unboxed,leftmargin=${xleft}cm,font=\\normalfont]`);
            for (var key of keys) {
              key = `${this.escape(bullet)}\\bfseries\\itshape{}${this.escape(key)}`;
              o.push(`\\item\[${key}\]`);
            }
          }
          if (text) {
...
\\usepackage{xifthen}`
  ...
  toFramedTabb2 (text, maxj, fencecmd) {
    var formats = this.toArray(fencecmd.format);
    var text = text.map ( pp => {
      pp = pp.map(x => x.split('\n'));
      var kk = pp.map(x => x.length); /// an new array of length
    ...
    for (var pp of text) {
      ...
      for (var k=0; k < kkm; ++k) {
        ...
        if (rowcount === 1) {
          ...
          var ppk = ppk.map((p) => this.unmask(p));
          o.push(ppk.join(' & & ') + ' \\\\');
        }
      }
      if (rowcount === 1) {
  ...
  toFramedTabu (text, maxj, ww, fencecmd, config) {
    var text = text.map ( pp => {
      pp = pp.map(x => x.split('\n'));
      var kk = pp.map(x => x.length); /// an new array of length
      var kkm = kk.reduce((acc,curr) => acc>curr?acc:curr); /// max length
...
///console.log(column);
	...
	sub = this.unmask(sub);
        return `{\\begin{minipage}{\\linewidth}\\includegraphics[width=\\linewidth,frame]{${src}}\\captionof*{figure}{${sub}}\\end{minipage}}`;
    });
