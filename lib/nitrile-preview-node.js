...
module.exports = {
  ...
  async toLatex (filename) {
    ...
    let lines = out.split('\n');
    let [blocks,flags,book] = latex.toBlocks(lines,'my',0);
...
\\end{document}\n`;
  ...
  async toHtml (filename) {
    ...
    let lines = out.split('\n');
    var [blocks,flags,book] = previewer.toBlocks(lines,'my',0);
    if (book.length) {
      [blocks,flags] = await previewer.toBookBlocks(book,path.dirname(filename));
    }
