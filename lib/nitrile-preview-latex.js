...
class NitrilePreviewLatex extends NitrilePreviewParser {
  ...
  translateLatex (all,config) {
    ...
    for (var block of all) {
      const [id,row1,row2,sig,star,data,para,_fencecmd,unused,base,subrow,fname] = block;
      this.block = block;
      this.base = base;
      this.row1 = row1;
      this.row2 = row2;
      ...
      const label_text = (star.length == 0 && fencecmd.label)?`${base}-${fencecmd.label}`:'';
...
\\end{document}\n`;
  ...
  toEquation(o,id,sig,dept,fname,row1,row2,data,fencecmd,isalignequalsign) {
    var bls = data;
    var ss = [];
    var mylabeltext = this.label_text;
    ...
    o.push(`\\begin{equation${star}}\\label{${mylabeltext}}`);
    o.push(`\\begin{split}`);
    o.push(ss.join('\\\\\n'));
    o.push(`\\end{split}`);
    o.push(`\\end{equation${star}}`);
