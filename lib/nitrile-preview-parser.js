...
class NitrilePreviewParser extends NitrilePreviewBase {
  ...
  read_para (lines,n) {
    ...
    if(type == 'spcl'){
      ...
      return {para,sig,wide,lines,caption,label,islabeled,floatname,body};
    } 
    else if (type == 'data'){
  ...
  parse_tabr(para) {
    ...
    for(; j < para.length; ++j){
      ...
      if((v=re_item.exec(s))!==null){
        var coli = this.col_letter_to_num(v[1]);
        if(Number.isFinite(coli)){
          pp[coli] = v[2];
        }
      }
    }
    ...
    rows = rows.filter(pp => {
      let qq = pp.map(x => re_bars.test(x) ? 1 : 0);
      let n = qq.reduce((acc, cur) => acc + cur);
      if (n == qq.length) {
        ww = pp.map(x => re_bars.exec(x)[1]);
        return [];
      }
      return pp;
    })
