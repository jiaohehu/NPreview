...
class NitrilePreviewParser {
  ...
  toFlow(lines,plevel,fname,o=null) {
    ...
    [10] subrow: initially set to empty string, but will be filled with a number by
        this.mergeBlocks() if this document is a sub-document---a document included by
    ...
    while (lines.length > 0) {
      ...
      if (block_type === 'SAMP' && (block_type0 === 'plistpara' || 
        ...
        if (fence) {
          ...
          o.push([id,row,row2,'LIST','',data,para,fencecmd,'',base,'',fname]);
        } else {
        ...
        sig==='multline') {
          ...
          o.push([id, row, row2, 'FRMD', '', data, para, fencecmd, '', base, '', fname]);
          block_type = 'FRMD';

        } else if (sig==='verse'){

          o.push([id, row, row2, 'VERS', '', data, para, fencecmd, '', base, '', fname]);
          block_type = 'VERS';
