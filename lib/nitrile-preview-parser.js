...
class NitrilePreviewParser {
  ...
  constructor() {
    ...
    this.re_unmask = /(?<!\S)_{2}(.*?)_{2}|(?<!\S)_{1}(.*?)_{1}|``(.*?)``|`(.*?)`|\[(.*?)\]\((.*?)\)|\[\[(\S+?)\]\]|(?<!\S)\'(\S+)\'/g;
    ...
    this.re_plst = /^(\s*)(\u{30fc}|\u{ff0d}|-|\d+\.|\d+\))\s+(.*)$/u;
    ...
    this.re_long_table = /^(\d+\))\s*(.*)$/;
    this.re_short_table = /^(\|{1,}|\u{ff5c}{1,})\s*(.*)$/u;
    this.re_unmask_1 = /^&(\w+);$/;/// ENTITY: [[&deg;]] or [[&amp;]]
  ...
  unmask (line) {
    ...
    while ((v = this.re_unmask.exec(line)) !== null) {
      ...
      else if (v[7] !== undefined) { /// [[\S+]]   
        var re_1 = this.re_unmask_1;
        var re_2 = this.re_unmask_2;
        var re_3 = this.re_unmask_3;
        var cnt = v[7];
        var w;
        if ((w=re_1.exec(cnt))!==null) { 
          newtext += this.entity(w[1]);
        } 
        else if ((w=re_2.exec(cnt))!==null) {
          newtext += this.ruby(w[1],w[2]);
        }
        else if ((w=re_3.exec(cnt))!==null) {
  ...
  toXhtmlDocument(config, htmlines, stylesheet_html) {
    var geometry_opts = [];
...
`;
  ...
  toHtmlDocument(config, htmlines, stylesheet_html) {
    var geometry_opts = [];
    geometry_opts.push(`padding-left:${config.HTML.leftmargin}mm`);
    geometry_opts.push(`padding-right:${config.HTML.rightmargin}mm`);
    geometry_opts.push(`padding-top:${config.HTML.topmargin}mm`);
