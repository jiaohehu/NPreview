...
class NitrilePreviewContext extends NitrilePreviewTranslator {
  ...
  constructor(name='CONTEXT') {
    ...
    this.xconfig.bodyfontsizept = 11;
    this.xconfig.diagfontsizept = 12;
    this.xconfig.papersize = 'A4';///210 x 297
  ...
  do_hdgs(block,conf){
    var o = [];
    var {id,row1,row2,sig,name,subn,hdgn,label,text} = block;
    ...
    if(this.xparser.ismaster){
      if(name=='part'){
        o.push(`\\startpart[title={${text}},reference={${label}},bookmark={${raw}}]`);
  ...
  */
  ...
  to_context_document() {
    ...
    if(this.xconfig.layout){
      var layout_s = `\\setuplayout[width = ${ this.xconfig.width }mm,
    ...
    if(this.xconfig.papersize){
      var papersize_s=`\\setuppapersize[${this.xconfig.papersize}]`;
    }else{
      var papersize_s='';
    }
    var data = `\
