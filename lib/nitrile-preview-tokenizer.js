...
class NitrilePreviewTokenizer {
  ...
  toMathSvg (str,dstyle) {
    this.isdmath = dstyle;
    this.isdstyle = dstyle;
    this.fs = this.translator.xconfig.mathfontsizept;
    var used = new Set();
    var l = this.toTokens(str);
    var g = this.toGroups(l);
    var g = this.toCleanup(g);
  ...
  toDiagMathSvg (str) {
    this.isdmath = 0;
    this.isdstyle = 0;
    this.fs = this.translator.xconfig.diagfontsizept;
    var used = new Set();
    var l = this.toTokens(str);
    var g = this.toGroups(l);
    var g = this.toCleanup(g);
        ...
        case '\\leftright':
          ...
          if(v[0]==='\\leftright'){
            ...
            if(v[0] === '\\pipe' || v[0] === '\\math'){
              var defs = this.buildup_defs(used);
              s = this.translator.to_outer_svg(s,w,h,mid,defs);
            }
  ...
  to_text_element(text,keepwhitespace){
    ...
    text = this.translator.escape(text);
