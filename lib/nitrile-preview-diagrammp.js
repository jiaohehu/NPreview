...
class NitrilePreviewDiagramMP extends NitrilePreviewDiagram {
  ...
  to_tex_label(txt,ts){
    ...
    if (ts==2) {//math
      ...
      ///return something like: $A_0$+$B_0$
      var s = this.translator.escape_for_var(txt);
      var s = `{\\fontsize{${fs}}{${fs}}\\selectfont{}${s}}`;
    } else {
      ///return sth like: A_0+B_0
      var s = this.translator.escape(txt);
      var s = `{\\fontsize{${fs}}{${fs}}\\selectfont{}${s}}`;
    }
    return `btex ${s} etex`;
  }
  ...
  do_box(opt,txt,ts,g,coords){
    ...
    for (var i = 0; i < coords.length    ; i++) {
      ...
      if (label) {
        ...
        var tex_label = this.to_tex_label(label,ts);
  ...
  p_path(coords,p,g){
    ...
    for(var item of items) {
      ...
      d = d.trim();
      if(!d) continue;
      if(iscycled && g.fillcolor){
        o.push(`fill (${d}) ${withscaled} ${withshifted} scaled(u) ${this.to_fills(g)};`);
      }
      if(p.arrowstart && p.arrowend){
        o.push(`drawdblarrow (${d}) ${withscaled} ${withshifted} scaled(u) ${this.to_draws(g)};`);
      }else if(p.arrowstart){
        o.push(`drawarrow reverse(${d}) ${withscaled} ${withshifted} scaled(u) ${this.to_draws(g)};`);
      }else if(p.arrowend){
        o.push(`drawarrow (${d}) ${withscaled} ${withshifted} scaled(u) ${this.to_draws(g)};`);
      }else{
        o.push(`draw (${d}) ${withscaled} ${withshifted} scaled(u) ${this.to_draws(g)};`);
      }
    }
    return o.join('\n');
  }
  ...
  p_label(x,y,txt,ts,ta,g){
    ...
    var tex_label = this.to_tex_label(txt,ts);
    var dx = this.assertFloat(g.dx,0);
    var dy = this.assertFloat(g.dy,0);
    x += dx;
    y += dy;
    return(`label${myopt} (${tex_label}, (${x}*u,${y}*u)) ${this.to_texts(g)};`);
  }
  ...
  p_ellipse(x,y,Rx,Ry,angle,g){
    ...
    if (g.fillcolor) {
      o.push(`fill fullcircle xscaled(${this.fix(2*Rx)}) yscaled(${this.fix(2*Ry)}) rotated(${Phi}) shifted(${this.fix(Cx)},${this.fix(Cy)}) ${this.withrefs()} ${this.to_fills(g)};`);
    }
    o.push(`draw fullcircle xscaled(${this.fix(2*Rx)}) yscaled(${this.fix(2*Ry)}) rotated(${Phi}) shifted(${this.fix(Cx)},${this.fix(Cy)}) ${this.withrefs()} ${this.to_draws(g)};`);
    return o.join('\n'); 
  }
  ...
  p_shape(x,y,p,g){
    ...
    for( var item of items ) {
      ...
      if(!d) continue;
      if(iscycled && g.fillcolor) {
        o.push(`fill (${d}) scaled(${this.refs}) xscaled(${sx}) yscaled(${sy}) shifted(${x},${y}) scaled(u) ${this.to_fills(g)};`);
      }
      o.push(`draw (${d}) scaled(${this.refs}) xscaled(${sx}) yscaled(${sy}) shifted(${x},${y}) scaled(u) ${this.to_draws(g)};`);
    }
    return o.join('\n');
  }
