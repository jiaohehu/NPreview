...
`;
  ...
  do_item(block) {
    ...
    var text = data;
    text = this.unmask(text);
    o.push(`<p id='${id}' class='P ${sig}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='margin-left:${this.xleft};'>${text}</p>`);
    o.push('');
    block.html = o.join('\n');
  }
  ...
  do_text(block) {
    ...
    if (leadn && leadn>0) {
      ...
      text = this.unmask(text);
      o.push(`<p id='${id}' class='P TEXT' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='margin-top:0;margin-bottom:0;text-indent:${this.xindent}mm; ' >`);
      o.push(text);
      o.push('</p>');
    } 
    o.push('');
