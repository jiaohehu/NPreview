...
class NitrilePreviewTokenizer {
  ...
  buildup_defs(){
    ...
    for (var mathSymbol of pjson.mathSymbols) {
      ...
      width *= this.fs/12;
      if (this.used.has(id)) {
        defs.push(`<text id='${id}' dy='${dy}' textLength='${width}pt' lengthAdjust='spacingAndGlyphs' style='stroke:none;fill:inherit;font-size:${this.fs}pt'>${unicode}</text>`);
      }
    }
    return defs;
  }
    ...
    if (v === undefined || v === null) {
      ...
      switch (v[0]) {
        ...
        case '\\nroot':
          ...
          o.push(`</svg>`);
          /// draw overhead line
          o.push(`<line x1='${nlead+lead}pt' y1='0.8pt' x2='${nlead+lead+w}pt' y2='0.8pt' style='stroke:inherit; fill:none; font-size:${this.fs}pt;'/>`);
        ...
        case '\\sqrt':
          ...
          if(1){
            ...
            o.push(`<line x1='${lead}pt' y1='0.8pt' x2='${w}pt' y2='0.8pt' style='stroke:inherit; fill:none; font-size:${this.fs}pt;'/>`);
            this.used.add('mySQRT');
            if(n){
              var [ids_,ws_,ops_] = this.findIdByElement(n);
              let nws_ = ws_*this.nroot_rate;
              let nhs_ = this.fs*this.nroot_rate;
        ...
        case '\\sup':
          ...
          if (!this.isinmatrix && this.isdisplaymath && (
            ...
            var nw3_ = w3_*this.sup_rate;
            var nh3_ = h3_*this.sup_rate;
            /// following two variables adjusts the vertical coordinates for super- and subscript     
            var supdy = nh3_*0.30;
            var subdy = nh2_*0.30;
            o.push(`<svg y='${supdy}pt'>`);
            o.push(s1_);
            o.push(`</svg>`);
            o.push(`<svg x='${w1_}pt' y='${supdy+h1_-nh2_+subdy}pt' width='${nw2_}pt' height='${nh2_}pt' viewBox='0 0 ${w2_*1.333} ${h2_*1.333}'>`);
            o.push(s2_);
            o.push(`</svg>`);
            o.push(`<svg x='${w1_}pt' width='${nw3_}pt' height='${nh3_}pt' viewBox='0 0 ${w3_*1.333} ${h3_*1.333}'>`);
            o.push(s3_);
            o.push(`</svg>`);
            return [w1_+this.max(nw2_,nw3_),h1_+(subdy)+(supdy),mid1_+(supdy),o.join('\n'),g1_,q,''];
          }
          break;
  ...
  to_text_element(text,keepwhitespace){
    ...
    /// display it as text.
    var s = `<text x='0' y='0' dy='0.75em' textLength='${totalw}pt' lengthAdjust='spacingAndGlyphs' style='stroke:none;fill:inherit;font-size:${this.fs}pt'>${newtext}</text>`
    var w = totalw;
    var h = this.fs;
    var mid = this.fs/2;
