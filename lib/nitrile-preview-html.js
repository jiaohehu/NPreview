...
`;
  ...
  do_plst(block) {
    ...
    var STYLE=`padding-left:${fencecmd.padding}em;list-style-position:outside;`;
    var count=0;
    for (var item of data) {
  ...
  do_hlst(block){
    var {id,row1,row2,sig,dept,data,para,fencecmd,base,subrow} = block;
    var o = [];
    var style=`${this.xstyle};padding-left:${fencecmd.padding}em;list-style-position:outside`;
...
${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
  ...
  do_tabr(block){
    ...
    var {id,row1,row2,sig,data,caption,label} = block;
    var o = []; 
    var vlines = this.xfencecmd.cssvlines;
    var hlines = this.xfencecmd.csshlines;
    var vpadding = this.xfencecmd.cssvpadding;
