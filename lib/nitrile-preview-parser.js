...
class NitrilePreviewParser {
    ...
    while (lines.length > 0) {
      ...
      if (block_type === 'SAMP') {
        ...
        if(v){
          ...
          o.push({mode,rmap,row1,row2,sig,html,part,hdgn,level,data,para,fencecmd,subrow});
        }
  ...
  */
  ...
  translateBlocks() {
    ...
    this.needblank = 0;
    for (var block of this.blocks) {
      var {rmap,id,row1,row2,sig,dept,level,data,para,fencecmd,subrow,subfname} = block;
      this.block = block;
      ...
      this.xbaselabel = fencecmd.label||'';
      if(1){
        this.iscaption = 1;
      ...
      htmlattrs.push(`style='${cssstyles.join(';')}'`);
      this.xhtmlattr=htmlattrs.join(' ');
