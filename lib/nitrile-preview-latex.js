...
class NitrilePreviewLatex extends NitrilePreviewParser {
  ...
  do_imgs(block){
    ...
    for(var blk of data){
      j++;
      var n = blk.length;
      s.push(`\\begin{tabulary}{\\linewidth}{${'L'.repeat(n)}}`);
      if(j >= 2){
        s.push(`\\noalign{\\smallskip}`);
      }
      for(var img of blk){
        ...
        if (!width) {
          width = this.toLatexLength(`${100/n}%`);
        }
        s.push(`{\\includegraphics[width=\\linewidth]{${src}}}`);
        s.push(`&`);
      }
      s.pop();
      s.push(`\\\\`);
      for(var img of blk){
        const {srcs,opts,sub} = img;
        var sub_text = this.unmask(sub);
        s.push(`{\\begin{minipage}{\\linewidth}\\centering \\small ${sub_text} \\end{minipage}}`);
        s.push(`&`);
      }
      s.pop();
      s.push(`\\\\`);
