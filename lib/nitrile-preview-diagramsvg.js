...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  constructor(translator,tokenizer) {
    super(translator);
  ...
  drawtext(x,y,label,justi,g) {
    ...
    if ((v=this.re_inlinemath.exec(label))!==null) {
      ...
      label = this.translator.smooth_for_diag(label);
      ...
      d.push(`<text font-size='${this.translator.xconfig.diagfontsizept}pt' text-anchor='${anchor}' ${this.to_textdraws(g)} x='${x}' y='${y}' dy='${dy}'>${label}</text>`);
    }
    return d.join('\n');
  }
  ...
  p_label(x,y,txt,a,g){
    ...
    var X = this.localx(x+dx);
    var Y = this.localy(y+dy);
