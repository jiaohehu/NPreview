...
class NitrilePreviewDiagram {
  ...
  parseLine(line) {
    ...
    if ((v = this.re_set_command.exec(line)) !== null) {
      ...
      if (key === 'linesize') { this.linesize = this.assertLength(val, this.def_linesize); } ///such as '0', '0.1', '1pt', '1mm'
      if (key === 'fillcolor') { this.fillcolor = this.assertColor(val, this.def_fillcolor); } ///such as 'red', '0.5red', or '0.8[red,green]'
      if (key === 'linecolor') { this.linecolor = this.assertColor(val, this.def_linecolor); } ///such as 'red', '0.5red', or '0.8[red,green]'
      if (key === 'dotcolor') { this.dotcolor = this.assertColor(val, this.def_dotcolor); } ///such as 'red', '0.5red', or '0.8[red,green]'
      if (key === 'dotsize') { this.dotsize = this.assertLength(val, this.def_dotsize); } ///such as '4pt' or '4mm'
      if (key === 'tick') { this.tickcolor = this.assertColor(val, this.def_tickcolor); } ///such as 'red'
      if (key === 'ticksize') { this.ticksize = this.assertLength(val, this.def_ticksize); } ///such as '1pt' or '1mm'
      if (key === 'tickprotrude') { this.tickprotrude = this.assertFloat(val, this.def_tickprotrude, 0.1, 1.0); } ///such as '0.33'
      if (key === 'fontcolor') { this.fontcolor = this.assertColor(val, this.def_fontcolor); }/// such as 'green', 'rgb(200,100,255)'
      if (key === 'fontsize') { this.fontsize = this.assertLength(val, this.def_fontsize); }/// such as '14pt'
