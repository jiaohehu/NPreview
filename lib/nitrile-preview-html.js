...
`;
  ...
  do_verb(block) {
    ...
    if (this.xwraplines > 0) {
      data = this.wrapSample(data,this.xwraplines);
    }
    var text = data;
    if (this.xnumbers) {
      var linenum = 0;
      text = text.map( x => this.escape(x) );
      text = text.map( x => `<td style='font-size:xx-small;padding-right:6px;'>${++linenum}</td><td style='font-family:monospace;white-space:pre;width:100%;'>${x}</td>` );
      text = text.map( x => `<tr>${x}</tr>`);
    } else {
      text = text.map( x => this.escape(x) );
      text = text.map( x => `<td style='font-family:monospace;white-space:pre;width:100%;'>${x}</td>` );
      text = text.map( x => `<tr>${x}</tr>`);
    }
    text = text.join('\n');
