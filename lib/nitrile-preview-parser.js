...
class NitrilePreviewParser {
  ...
  constructor() {
    ...
    */
    ...
    this.features = {};///the feature of the active block
    ...
    while (lines.length > 0) {
      ...
      if (block_type === 'SAMP') {
        ...
        if (block_type0 === 'SAMP') {
          var bl = o.pop();
          var data = bl.data;
          var mydata = this.trimParaAt(para,N_sampspaces); /// m is to be ignored
          ...
          var data = mydata;
          sig = 'SAMP';
          o.push({features,rmap,row1,row2,sig,html,data,para,fencecmd,subrow});
        }
        continue
        ...
        if (v2) {
          ...
          sig = 'HDGS';
          var level = hdgn;
          o.push({features,rmap,row1,row2,sig,html,hdgn,level,data,para,fencecmd,subrow});
          continue;
        }
      ...
      } else if (this.isDmathPara(para)) {

        if (block_type0 === 'mathpara') {
          var bl = o.pop();
          var data = bl.data;
          var item = this.parseDmathPara(para,fencecmd);
          data.push(item);
          bl.row2 = row2;
          bl.data = data;
          o.push(bl);
        ...
        var items = this.parsePictPara(para);
        ...
        var data = text;
        sig = 'TEXT';
        o.push({features,rmap,row1,row2,sig,html,lead,leadn,data,para,fencecmd,subrow});
        block_type = 'TEXT';
        ...
        if((v=re1.exec(s))!==null){
          ...
          sig = 'SAMP';
          var mode = 1;
          o.push({features,rmap,row1,row2,mode,sig,html,data,para,fencecmd,subrow});
          block_type = 'SAMP';
          ...
          var standalone = 1;
          o.push({features,rmap,row1,row2,sig,html,standalone,data,para,fencecmd,subrow});
          block_type = 'TEXT';
        }
  ...
  */
  ...
  isDlstPara(para) {
    ...
    if (this.re_dlst_astr.test(s0)) {
      ...
      return true;
    } else if (this.re_dlst_plus.test(s0)) {
      return true;
    }
    return false;
  }
  ...
  isDmathPara(para) {
    ...
    if(para.length>2 && this.re_dmath.test(para[0]) &&
         this.re_dmath.test(para[para.length-1])){
      return true;
    }
    return false;
  }
  ...
  parseDmathPara(para) {
    ...
    var data = para.slice(1,para.length-1);
  ...
  translateBlocks() {
    ...
    for (var block of this.blocks) {
      var {features,row1,row2,sig,level,data,para,fencecmd,subrow} = block;
      this.block = block;
