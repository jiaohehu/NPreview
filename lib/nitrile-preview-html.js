...
//console.log(data);
        ...
        case 'TABB': {
          ...
          var text = this.toFramedTabb2(text,maxj,ww,fencecmd,config);
          if (fig) {
        ...
        case 'DESC': {
          ...
          for (i=0; i < keys.length; ++i) {
            if (cat === 'mono') {
              o.push(`<dt style='${this.dtcssstyle}' >${bullet} <code style='${this.codecssstyle}' ><b>${this.escape(keys[i])}</b></code></dt>`);
            } else if (cat === 'strong') {
              o.push(`<dt style='${this.dtcssstyle}' >${bullet} <b>${this.escape(keys[i])}</b></dt>`);
            } else {
              o.push(`<dt style='${this.dtcssstyle}' >${bullet} <b><i>${this.escape(keys[i])}</i></b></dt>`);
            }
          }
  ...
  toFramedImgs (column, margin, gap, pcol, text, config, ispreview) {
    ...
    while (pp.length) {
      ...
      if (n == 0) {
        ...
        o.push(`<span style='display:inline-block; width:${pcol[0]}%'>${p}</span>`);
        continue;
      }
      if (n == column) {
        o.push(`<span style='display:inline-block; width:${margin}%'></span>`);
      ...
      o.push(`<span style='display:inline-block; width:${pcol[m]}%'>${p}</span>`);
      n += 1;
      m += 1;
    }
    while (n < column) {
...
console.log(o);
  ...
  toFramedTabb2 (text, maxj, ww, fencecmd, config) {
    var tableww = 'table-layout:fixed; width:100%; max-width:100%';
    if (fencecmd.adjust) {
      ww = this.toAdjustedColumns(maxj,fencecmd.adjust);
    }
  ...
  toFramedTabu (text, maxj, ww, fencecmd, config) {
    ...
    for (var pp of text) {
      ...
      if (this.isAutoRuby('tabu',config)) {
        pp = pp.map(x => x.map(y => this.rubify(y)));
      }
      pp = pp.map(x => x.join('<br/>'));
      o.push(`<tr >`);
