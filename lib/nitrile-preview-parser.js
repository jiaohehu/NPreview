  ...
  */
  toBLOCKS(lines,fname,plevel,o) {
    ...
    while (lines.length > 0) {
      ...
      /// build the 'id' string that include part of the 'fname'
      var v = fname.match(/^[\w\-]+/);
      if (v) {
        id = `${v[0]}:${nblocks}`; /// such as 'd3-intro-15'
      }
  ...
  unmask (text) {
    ...
    while ((v = re.exec(str)) !== null) {
      ...
      if (v[1]) {
        ...
        var id = '';
        if (v1) {
          id = labl;  /// this is a fully-qualified label
        } else if (v2) {
          /// this is a label that refers to the first heading
          /// of the sub-document; in this case the id is
          /// assumed 'labl:1'.
          id = labl+':1';
        } else if (v3) {
          /// this id does not have a 'fname' portion, so get
          /// assume it is the same as that of its containing block
