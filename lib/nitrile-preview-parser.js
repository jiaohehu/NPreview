...
class NitrilePreviewParser {
  ...
  constructor() {
    ...
    this.re_unmask = /\s\_{2}(.*?)\_{2}|\s\_{1}(.*?)\_{1}|\s\`{3}\s*(.*?)\s*\`{3}|\s\`{2}\s*(.*?)\s*\`{2}|\s\`\s*(.*?)\s*\`|\[\^\^(.*?)\]|\[\^U\+([0-9a-fA-F]+)\]|\[(.*?)\]\((.*?)\)|\[\^\/(.*?)\]/g;
  }
  ...
  unmask (text) {
    ...
    while ((v = this.re_unmask.exec(str)) !== null) {
      ...
      if (v[1]) {
        ...
        /// uri: [yahoo](www.yahoo.com)
        newtext += this.style('uri',[v[8],v[9]]);

      } else if (v[9]) {
        /// uri: [](www.yahoo.com)
        newtext += this.style('uri',['',v[9]]);
