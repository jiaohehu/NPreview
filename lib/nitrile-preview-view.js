...
///console.log(`image ${fsrc} changed`);
  ...
  async getFlows() {
    ...
    var [blocks,flags] = this.parser.toFlow(this.lines,fname);
    var isarticle = !utils.getBool(flags.book);
    var subs = [];
    if (!isarticle) {
      var subs = this.parser.toSubs(blocks);
  ...
  async getSubFlow (sub) {
    ...
    if (sublevel < 0) {
      var o = [];
      o.push(this.parser.newPARTblock (subrow,subpart));
      return [o,{},[]];
    }
  ...
  updateBlocks() {
    ...
    this.getFlows().then( ([blocks,flags,isarticle,subs]) => {
