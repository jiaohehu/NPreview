...
class NitrilePreviewParser {
    ...
    while (lines.length > 0) {
      ...
      if (block_type === 'SAMP' && block_type0 === 'PLST') {
        ...
        if (sig === 'imgs') {
          ...
          o.push([id,row,row2,'IMGS','',data,para,fencecmd,star,base,'',fname]);

        } else if (sig === 'diagram') {

          data = this.parseDIAGRAM(data);
          o.push([id,row,row2,'DIAG','',data,para,fencecmd,star,base,'',fname]);

        } else if (sig === 'tabbing') {

          data = this.parseTABL(data);
          o.push([id,row,row2,'TABB','',data,para,fencecmd,star,base,'',fname]);

        } else if (sig==='math'||sig==='equation'||sig==='subequations'||sig==='equations'||sig==='multline') {

          data = this.parseEQTN(data);
          o.push([id,row,row2,'MATH','',data,para,fencecmd,star,base,'',fname]);

        } else if (sig === 'tabular') {

          data = this.parseTABL(data);
          o.push([id,row,row2,'TBLR','',data,para,fencecmd,star,base,'',fname]);

        } else if (sig === 'dt') {

          data = this.parseDT(data);
          o.push([id,row,row2,'DLST','',data,para,fencecmd,star,base,'',fname]);

        } else {

          o.push([id,row,row2,'VERB','',data,para,fencecmd,star,base,'',fname]);
  ...
  mergeBlocks (main,subs) {
    ...
    var o = [];
    for (var m of main) {
      const [id,row1,row2,brand,n,data,para,fencecmd,star,base,_label,fname] = m;
      switch (brand) {
        case 'CITE': {
          for (var sub of data) {
            var {subfname} = sub;
