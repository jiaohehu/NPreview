...
class NitrilePreviewLatex extends NitrilePreviewParser {
  ...
  do_dlst(block){
    ...
    for (var item of items) {
      ...
      if(type=='text'){
        ...
        text = this.unmask(text);
        if(rb&&rt){
          o.push(`\\item[-]{\\jp{}\\ruby{${rb}}{${rt}}} ${text}`);
        } else {
          o.push(`\\item[-]{\\jp{}${key}} ${text}`);
        }
      }else if(type=='quot'){
        key = this.escape(key);
        text = this.unmask(text);
        ...
        key = this.escape(key);
        text = this.unmask(text);
        o.push(`\\item[-] {\\itshape{}${key}} ${text}`);
      }else if(type=='code'){
        key = this.escape(key);
        key = this.polish(key,this.conf('nicode'));
        text = this.unmask(text);
        o.push(`\\item[-] {\\ttfamily{}${key}} ${text}`);
      }
    }
  ...
  do_prog(block){
    ...
    while(n < body.length){
      ...
      if(1){
        ...
        if(this.floatname){
          var caption_text = cont + this.caption_text;
  ...
  do_samp(block){
    ...
    var body = body.map( x => this.polish(x,this.conf('nicode')));
  ...
  do_tabb(block){
    ...
    for (var j = 0; j < nrows; ++j) {
      ...
      pp = pp.map(x => this.polish(x,this.conf('nitabb')));
      pp = pp.join(' \\> ');
      pp = `${pp} \\\\`;
      d.push(pp);
    }
  ...
  do_csvd(block){
    ...
    cols.forEach(x => {
      ...
      var width = x.reduce((acc,s) => Math.max(acc,this.measureText(s)),0);
      ww.push(width);
    });
    var d = [];
    for (var j = 0; j < nrows; ++j) {
    ...
    o.push(`\\begin{xltabular}[l]{\\linewidth}{@{}p{${this.conf('step')}mm}@{}${pcol}@{}}`);
    o.push(d.join('\n'));
      ...
      if(j==0){
        ...
        if(maxn > 1){
          ...
          for(var n=0; n<maxn; ++n){
            ...
            var pp = pp.map(x => this.polish(x,this.conf('nitabb'),'\\bfseries'));
            s.push(`${pp.join(' & ')}\\\\`);
          }
          s.push(`\\HL`);
        } else {
        ...
        var pp = pp.map(x => this.unmask(x));
        var pp = pp.map(x => this.polish(x,this.conf('nitabb')));
        s.push(`\\noalign{\\vspace{${vpadding}pt}}`);
        s.push(`${pp.join(' & ')}\\\\`);
      }
    }
    ...
    for (var j=0; j < this.blocks.length; ++j) {
      ...
      if(sig=='MATH'){
        ...
        if( str.localeCompare(label)===0) {
          ...
          return `${secsign}\\hyperref[${str.slice(1)}]{${idnum}}`;
          break;
        }
      }else{
        if( str.localeCompare(label)===0) {
          return `${floatname}.\\hyperref[${str.slice(1)}]{${idnum}}`;
          break;
        }
      }
    }
...
`
  ...
  toLatexDocument() {
    ...
    var latexengine = this.conf('latexengine')||'lualatex';
    var titlelines = [];
    var mytitle = this.conf('title');
    var myauthor = this.conf('author');
    if(mytitle){
      titlelines.push(`\\title{${this.escape(mytitle)}}`);
      titlelines.push(`\\author{${this.escape(myauthor)}}`);
...
`;
  ...
  polish_tabb_header(s){
    return this.polish(s,this.conf('nitabb'),'\\bfseries');
  }
  polish_code(s){
