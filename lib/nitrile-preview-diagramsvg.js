...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_finalize(s,style) {
    ...
    o.push('</defs>');
    o.push(`<rect width="${this.fix(vw)}" height="${this.fix(vh)}" fill="url(#smallgrid)" />`);
    o.push(s);    
    var s = o.join('\n');
    if (style && style.width) {
      let str = style.width;
      str = this.translator.str_to_css_width(str);
      var text = `<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' fill='currentColor' stroke='currentColor' viewBox='0 0 ${this.fix(vw)} ${this.fix(vh)}' style='width:${str}; ' >${s}</svg>`;
    } else {
      var text = `<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' fill='currentColor' stroke='currentColor' width='${this.fix(vw)}' height='${this.fix(vh)}' >${s}</svg>`;
    }
    s = text;
    return {s};
  }
  ...
  do_rcard(opt,txt,ts,g,coords){
    ...
    for (var i = 0; i < coords.length; i++) {
      ...
      var x = this.localx(z0[0]);
      var y = this.localy(z0[1]);
      s.push(`<path d='M${x+rx},${y} h${dx} q${rx},${0},${rx},${-ry} v${-dy} q${0},${-ry},${-rx},${-ry} h${-dx} q${-rx},${0},${-rx},${ry} v${dy} q${rx},${0},${rx},${ry} z' ${this.to_fillonlys(g)} />`);
      s.push(`<path d='M${x+rx},${y} h${dx} q${rx},${0},${rx},${-ry} v${-dy} q${0},${-ry},${-rx},${-ry} h${-dx} q${-rx},${0},${-rx},${ry} v${dy} q${rx},${0},${rx},${ry} z' ${this.to_drawonlys(g)} />`);
    }
    return s.join('\n');
  }
  ...
  p_label(x, y, txt, ts, ta, g) {
    ...
    if (ts==2) {
      ...
      d.push(`<text font-size='${fontsize?fontsize:12}pt' text-anchor='${anchor}' ${this.to_textdraws(g)} x='${x}' y='${y}' dy='${dy}'>${txt}</text>`);
    }
    return d.join('\n');
  }
  ...
  p_fill(coords,g){
    ...
    for(var item of items){
      var {iscycled,d} = item; 
      o.push(`<path d='${d}' ${x} ${this.to_fillonlys(g,1)}/>`);  
    }
    return o.join('\n');
  }
  ...
  p_path(coords,g){
    ...
    for(var item of items){
      var {iscycled,d} = item; 
      if(iscycled){
        o.push(`<path d='${d}' ${x} ${this.to_fillonlys(g)}/>`);
  ...
  p_circle(cx, cy, r, g) {
    ...
    o.push(`<circle cx='${this.fix(cx)}' cy='${this.fix(cy)}' r='${this.fix(r)}' ${this.to_fillonlys(g)}/>`);
    o.push(`<circle cx='${this.fix(cx)}' cy='${this.fix(cy)}' r='${this.fix(r)}' ${this.to_drawonlys(g)}/>`);
    return o.join('\n');
  }
  ...
  p_rect(x,y,w,h,g){
    ...
    y = y - h;
    o.push(`<rect x='${this.fix(x)}' y='${this.fix(y)}' width='${this.fix(w)}' height='${this.fix(h)}' ${this.to_fillonlys(g)} />`);
    o.push(`<rect x='${this.fix(x)}' y='${this.fix(y)}' width='${this.fix(w)}' height='${this.fix(h)}' ${this.to_drawonlys(g)} />`);
    return o.join('\n');
  }
  ...
  p_pie(cx,cy,radius,angle1,angle2,g){
    ...
    // part of a circle
    let d = [];
    d.push(`<path d='M ${cx} ${cy} L ${x1} ${y1} A ${r} ${r} 0 ${bigflag} 0 ${x2} ${y2} Z' ${this.to_fillonlys(g)} />`);
    d.push(`<path d='M ${cx} ${cy} L ${x1} ${y1} A ${r} ${r} 0 ${bigflag} 0 ${x2} ${y2} Z' ${this.to_drawonlys(g)} />`);
    return d.join('\n');
  }
  ...
  p_cseg(cx,cy,radius,angle1,angle2,g){
    ...
    // part of a circle
    let d = [];
    d.push(`<path d='M ${x1} ${y1} A ${r} ${r} 0 ${bigflag} 0 ${x2} ${y2} Z' ${this.to_fillonlys(g)} />`);
    d.push(`<path d='M ${x1} ${y1} A ${r} ${r} 0 ${bigflag} 0 ${x2} ${y2} Z' ${this.to_drawonlys(g)} />`);
    return d.join('\n');
  }
  ...
  to_drawonlys(g) {
    ...
    } else if (this.config.linecap) {
      o.push(`stroke-linecap='${this.to_linecaps(this.config.linecap)}'`);
    }
    if (g.linejoin) {
      o.push(`stroke-linejoin='${this.to_linejoins(g.linejoin)}'`);
    } else if (this.config.linejoin) {
      o.push(`stroke-linejoin='${this.to_linejoins(this.config.linejoin)}'`);
    }
    o.push(`fill='none'`);
    return o.join(' ');
  }
  ...
  p_ellipse(x,y,Rx,Ry,angle,g){
    ...
    ///anti-clockwise
    o.push(`<path d='M${x1},${y1} A${Rx},${Ry},${-angle},1,0,${x2},${y2}' ${this.to_fillonlys(g)} />`);//anti-clockwise
    o.push(`<path d='M${x1},${y1} A${Rx},${Ry},${-angle},1,0,${x2},${y2}' ${this.to_drawonlys(g)} />`);//anti-clockwise
    ///clockwise
    o.push(`<path d='M${x1},${y1} A${Rx},${Ry},${-angle},1,1,${x2},${y2}' ${this.to_fillonlys(g)} />`);//clockwise
    o.push(`<path d='M${x1},${y1} A${Rx},${Ry},${-angle},1,1,${x2},${y2}' ${this.to_drawonlys(g)} />`);//clockwise
    return o.join('\n');
  }
  ...
  p_shape(x,y,p,g){
    ...
    for(var item of items){
      var {iscycled,d} = item;
      if(iscycled){
        o.push(`<path d='${d}' ${this.to_fillonlys(g)} />`);
