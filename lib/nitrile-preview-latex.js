...
class NitrilePreviewLatex extends NitrilePreviewParser {
  ...
  do_prog(block){
    ...
    var mylabelcmd = this.to_latexlabelcmd(label);
    var cont = '';
    if(!islabeled){
      o.push('');
      o.push('\\noindent');
    ...
    while(n < body.length){
      ...
      if(islabeled){
        ...
        if(caption){
          ...
          o.push(`\\caption{${caption_text}}${mylabelcmd}`);
          mylabelcmd = '';//so that it is not used again for the second float
          cont = ' (Cont.)';
        }
      } 
      o.push(text);
  ...
  do_tabr(block){
    ...
    if(islabeled){
      ...
      if(caption){
        ...
        o.push(`\\caption{${caption_text}}${this.to_latexlabelcmd(label)}`);
      }
    }else{
      o.push('\\begin{center}');
    }
  ...
  do_pict(block){
    var o = [];
    var {id,row1,row2,sig,wide,rows,caption,label,islabeled} = block;
    o.push(this.to_info(block));
    var s = [];
    for(var blk of rows){
      let {mode,images} = blk;
