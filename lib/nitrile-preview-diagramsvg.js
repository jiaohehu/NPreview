...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  to_textdraw_str(g) {
    var d = [];
    if (g.fontcolor) {
      d.push(`fill='${this.to_colors(g.fontcolor)}'`);
    }
    d.push(`stroke='none'`);
    return d.join(' ');
  }
  ...
  p_path(coords,g){
    ...
    for(var item of items){
      ...
      if(iscycled){
        ...
        o.push(`<path d='${d}' ${x} ${this.to_drawonly_str(g)}/>`);
      } else {
        o.push(`<path d='${d}' ${x} ${this.to_drawonly_str(g)}/>`);
      }
    }
  ...
  p_line(x1,y1,x2,y2,g){
    ...
    var x = this.to_marker_arrows(g);
    o.push(`<line x1='${this.fix(x1)}' y1='${this.fix(y1)}' x2='${this.fix(x2)}' y2='${this.fix(y2)}' ${x} ${this.to_drawonly_str(g)} />`);
    return o.join('\n');
  }
  ...
  p_qbezier_line(x0,y0, x1,y1, x2,y2, g){
    ...
    var y2 = this.localy(y2);
    var x = this.to_marker_arrows(g);
    o.push(`<path d='M ${this.fix(x0)},${this.fix(y0)} Q ${this.fix(x1)},${this.fix(y1)},${this.fix(x2)},${this.fix(y2)}' ${x} ${this.to_drawonly_str(g)} />`);
    return o.join('\n');
  }
