...
console.log(this.data)
    ...
    this.editor = null;
    this.path = data; /// from constructor
    this.lines = [];
    this.blocks = [];
    this.htmls = [];
    this.row = -1;
    ...
    /// open the editor for this path and get the lines from the editor
    this.switchPath(this.path);
  }
  ...
  */
  ...
  serialize() {
console.log('serialize of NitrilePreviewView called');
console.log(this.path);
    return {
      deserializer: 'NitrilePreviewView',
      data: this.path
    }
  }
  ...
  update() {
    ...
    /// for article
    if (!this.isBookBlock) {
      this.blocks = parser.autoBLOCKS(this.blocks,true);
      this.htmls = [];
  ...
  async generateContentsBlocks(blocks,dirname) {
    ...
    var parser1 = new NitrilePreviewParser(); 
    contentsBlocks = parser1.autoBLOCKS(contentsBlocks,false);
  ...
  switchPath(data) {
    ...
    /// note that data could be undefined
...
console.log(this.path);
    ...
    atom.workspace.open(this.path,{pending:true,searchAllPanes:true,activatePane:false,activateItem:false})
        .then((editor) => {
          this.editor = editor;
...
\\end{document}`;
        ...
        var translator = new NitrilePreviewLatex();
        var lines1 = translator.translate(this.contentsBlocks,false);
        const { title = '', author = '' } = this.contentsFlags;
        data = `\% \!TEX program = lualatex
\\documentclass{book}
\\usepackage{luatexja-fontspec}
\\usepackage{luatexja-ruby}
