...
`
  ...
  translateHtml (all,config,ispreview=false,editorrow=-1,editorcolumn=-1) {
    ...
    for (var block of all) {
      ...
      const [id,row1,row2,sig,_subrow,data,para,fencecmd,caption,base,label,fname] = block;
      this.block = block;
      this.base = base;
      ...
      switch (sig) {
        ...
        case 'DESC': {
          ...
          o.push(`<dd style='margin-left:${xleft}cm; ${this.ddcssstyle}'>${text}</dd>`);
          o.push('</dl>');
          o.push('</div>');
          o.push('');
          break;
        ...
        case 'SAMP': {
          ...
          text = text.join('\n');
          o.push(`<pre style='${this.samplecssstyle}; overflow:hidden; white-space:pre-wrap; word-wrap:break-word; word-break:break-all; padding-left:0;' >${text}</pre>`);
          o.push(`</div>`);
          o.push('');
...
//console.log(data);
        ...
        case 'tabbing': 
          ...
          var text = this.toFramedTabbing(text,maxj,ww,fencecmd,config);
          o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='${this.divcssstyle}; '>`);
          o.push(`${showidtext}`);
          o.push(text);
          o.push('</div>');
        ...
        case 'tabular': 
          ...
          var text = this.toFramedTabular(text,maxj,ww,vlines,hlines,vpadding);
          o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='${this.divcssstyle}; '>`);
          o.push(`${showidtext}`);
          o.push(text);
          o.push('</div>');
        ...
        case 'tabulary': {
          var [text,maxj,ww] = data;
          var text = this.toFramedTabulary(text,maxj,ww,fencecmd,config);
          o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='${this.divcssstyle}; '>`);
          o.push(`${showidtext}`);
          o.push(text);
          o.push('</div>');
        ...
        case 'tabularx': 
          var [text,maxj,ww] = data;
          var text = this.toFramedTabularx(text,maxj,ww,fencecmd,config);
          o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='${this.divcssstyle}; '>`);
          o.push(`${showidtext}`);
          o.push(text);
          o.push('</div>');
        ...
        case 'longtabu': 
          var [text,maxj,ww] = data;
          var text = this.toFramedLongtabu(text,maxj,ww,fencecmd,config);
          o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='${this.divcssstyle}; '>`);
          o.push(`${showidtext}`);
          o.push(text);
          o.push('</div>');
        ...
        case 'quot': {
          ...
          var xleft = config.latexStepMargin;
          o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='${this.divcssstyle}; '>`);
          o.push(`${showidtext}`);
          for (var p of text) {
            o.push(`<p style='${this.quotcssstyle}; margin:0; margin-bottom:0.5em; padding-left:${xleft}cm; padding-right:${xleft}cm;'>${p}</p>`);
          }
          o.push('</div>');
          o.push('');
          break;
        ...
        case '_diagram': {
          ...
          o.push(`<svg xmlns:xlink='http://www.w3.org/1999/xlink' width='100%' height='6em' viewBox='0 0 ${80*8*1.3333} ${text.length*12*1.3333}'\
                  style='outline:1px solid orange;' role='img' focusable='false' xmlns='http://www.w3.org/2000/svg' >`);
          o.push(`<defs><style type="text/css"><![CDATA[.MyText { stroke: none; fill: black; font-size: 12pt; }]]></style></defs>`);
          text = text.map( (x,i) => `<text class='MyText' x='0' y='${i}em' dy='0.8em'>${x}</text>` );
          o.push(text.join('\n'));
          o.push(`</svg>`);
          o.push(`</p>`);
          o.push('</div>');
        ...
        case 'diagram': {
          var [text,vw,vh] = new NitrilePreviewDiagramSVG(this).parse(data);
          o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='${this.divcssstyle}; '>`);
          o.push(`${showidtext}`);
          o.push(`<p style='${this.paracssstyle}; text-align:left; margin:0; line-height:1; font-size:small;' >`);
        ...
        case 'math': {
          ...
          for (var j in text) {
            ...
            if (fencecmd.label) {
              ...
              if (text.length == 0) subnum = '';
              var numtext = `<span style='position:absolute;left:12pt;top:0;'>(${num}${subnum})</span>`;
            } else {
              var numtext = '';
            }
            if (s.length==1) {
            ...
            s = s.join('');
            s = `<tr>${s}<td style='position:relative;'>${numtext}</td></tr>`;
  ...
  */
  ...
  toFramedTabbing(text, maxj, ww, fencecmd, config) {
    ...
    for (var pp of text) {
      ...
      pp = pp.map(x => x.join('<br/>'));
      o.push(`<tr >`);
      pp = pp.map((x,i) => `<td style='text-align:left; padding:2; width:${100*ww[i]}%; '>${x}</td>`);
      var v = pp.join('');
      o.push(v);
      o.push('</tr>');
    }
  ...
  toFramedTabular (rows, maxj, ww, vlines, hlines, vpadding) {
    rows = rows.map ( row => {
    ...
    var header = header.map((x,i) => `<td style='text-align:left; padding:${t}px 6px; ${vlines.indexOf(i)>=0?'border-left:1px solid black;':''} ${vlines.indexOf(i+1)>=0?'border-right:1px solid black;':''} ${hlines.indexOf('t')>=0?'border-top:1px solid black;':''} ${hlines.indexOf('t')>=0?'border-bottom:1px solid black;':''} '>${x}</td>`);
    o.push(`<tr>${header.join('')}</tr>`);
    var n = rows.length;
    for (var j=0; j < rows.length; ++j) {
      var row = rows[j];
      var row = row.map((x,i) => `<td style='text-align:left; padding:${t}px 6px; ${vlines.indexOf(i)>=0?'border-left:1px solid black;':''} ${vlines.indexOf(i+1)>=0?'border-right:1px solid black;':''} ${j>0&&hlines.indexOf('*')>=0?'border-top:1px solid black;':''} ${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid black;':''} '>${x}</td>`);
  ...
  toFramedTabulary (text, maxj, ww, fencecmd, config) {
    var o = [];
    o.push(`<table border='0' style='${this.paracssstyle}; ${this.tablecssstyle}; margin:0 auto; ' >`);
    for (var pp of text) {
      pp = pp.map(x => x.split('\n'));
      ...
      pp = pp.map(x => x.join('<br/>'));
      o.push(`<tr >`);
      pp = pp.map((x) => `<td style='text-align:left; padding:2; '>${x}</td>`);
      var v = pp.join('');
      o.push(v);
      o.push('</tr>');
    }
  ...
  toFramedTabularx (text, maxj, ww, fencecmd, config) {
    var o = [];
    o.push(`<table border='0' style='${this.paracssstyle}; ${this.tablecssstyle}; width:100%; ' >`);
    for (var pp of text) {
      pp = pp.map(x => x.split('\n'));
  ...
  toFramedLongtabu (text, maxj, ww, fencecmd, config) {
    var o = [];
    o.push(`<table border='0' style='${this.paracssstyle}; ${this.tablecssstyle}; width:100%; ' >`);
    for (var pp of text) {
      pp = pp.map(x => x.split('\n'));
