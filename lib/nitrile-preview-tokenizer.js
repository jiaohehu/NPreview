...
class NitrilePreviewTokenizer {
  ...
  toDiagramSvg (str) {
    ...
    for (var mathSymbol of pjson.mathSymbols) {
      ...
      if (this.used.has(id)) {
        defss.push(`<text dy='${dy}' dx='${dx}' class='MyText' id='${id}' textLength='${width}pt' lengthAdjust='spacingAndGlyphs'>${unicode}</text>`);
      }
    }
    var defs= `\
<defs>
...
${defss.join('\n')}
  ...
  /// position this SVG so that it is centered around the rectangle
  /// that is bw width and bh height, starting at (bx,by). The SVG
  /// will not be shrinked or expanded; it is shown at its original
  /// size
  centerSvg (v,bx,by,bw,bh) {
  ...
  toCleanup (g) {
    ...
    if (Array.isArray(g)) {
      var key = g[0];
      if (key  === '\\brace' || key  === '\\leftright' || key === '\\beginend') {
        var inner  = g[1];
        var fence1 = g[2];
        var fence2 = g[3];
...
//console.log('s_=',s_);
        ...
        case '\\sup':
          ...
          if (!this.isinmatrix && this.isdisplaymath && v[1] === '\\sum'
            ...
            if (0) {
              ...
              o.push(`<svg x='0' y='0'           width='${nw1_}pt' height='${nh1_/2}pt' viewBox='0 -2 ${wdtop_*1.333} ${12*1.333+2}' preserveaspectratio='none'>`);
              o.push(`<use xlink:href='#${idtop_}' />`);
              o.push(`</svg>`);
              o.push(`<svg x='0' y='${nh1_/2}pt' width='${nw1_}pt' height='${nh1_/2}pt' viewBox='0 0 ${wdbot_*1.333} ${12*1.333+2}' preserveaspectratio='none'>`);
              o.push(`<use xlink:href='#${idbot_}' />`);
              o.push(`</svg>`);
            }
            var dx2 = nw1_ * 0.666;
            var dx3 = nw1_;
      ...
      if (id === 'myINT') {
        ...
        var [idbot_,wdbot_,opbot_] = this.findIdByElement('\\intbot');
        var s = [];
        s.push(`<svg x='0' y='0'   width='${wdtop_}pt' height='7pt' viewBox='0 -2 ${wdtop_*1.3333} ${12*1.3333+2}'>`);
        s.push(`<use xlink:href='#${idtop_}' />`);
        s.push(`</svg>`);
        s.push(`<svg x='0' y='7pt' width='${wdbot_}pt' height='7pt' viewBox='0 0 ${wdbot_*1.3333} ${12*1.3333+2}'>`);
        s.push(`<use xlink:href='#${idbot_}' />`);
        s.push(`</svg>`);
        s = s.join('\n');
  ...
  toSvg (v,addoutline = false) {
    ...
    ///NOTE: we need to shrink this SVG to match the font that was currently being
    ///used. The current fontsize being used is stored as this.parser.fontsize.
    var rate1 = this.toRateFontsize(this.parser.fs);///this.parser.fs= 'normalfont', 'small', 'fontnotesize', etc.
    var rate2 = this.toRateNormalsize(this.parser.xnormalfontsize);///this.parser.xnormalfontsize= '11.5pt', '10pt', ...
...
${news}
  ...
  toRateNormalsize(fs){
    ///this function is to return a rate so that we can shink the SVG
    ///based on the current setting of the xnormalfontsize. Thus, if
    ///the xnormalfontsize setting is set to '11.5' we need to shrink
    /// by 0.9583 because 11.5/12 = 0.9583... The SVG is always modeled
    /// after a fontsize 12pt.
    return parseFloat(fs.slice(0,fs.length-2))/12.0;
  }
  ...
  toResizedSvg(str,dmath,x,y,rate,color) {

    this.used.clear();
    this.isdisplaymath = dmath;
    var l = this.toTokens(str);
    var g = this.toGroups(l);
    var g = this.toCleanup(g);
    var g = this.toCommands(g);
    var g = this.toSubsup(g);
    var [w,h,mid,s_] = this.toInnerSvg(g);
    var pp = [];
    for (var mathSymbol of pjson.mathSymbols) {
      let {dy,dx,id,unicode,width} = mathSymbol;
      if (this.used.has(id)) {
        pp.push(`<text dy='${dy}' dx='${dx}' class='MyText' id='${id}' textLength='${width}pt' lengthAdjust='spacingAndGlyphs'>${unicode}</text>`);
      }
    }
    var defs= `\
<defs>
...
${pp.join('\n')}
    ...
    s.push(`<svg x='${x}' y='${y}' width='${w}' height='${h}' fill='${color}' stroke='${color}' viewBox='0 0 ${vw} ${vh+1}'>${defs}${s_}</svg>`);
  }
