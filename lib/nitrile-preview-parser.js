...
class NitrilePreviewParser {
  ...
  newConfig() {
    ...
    config.LATEX.fslineno='footnotesize';
    ...
    config.ILST = {};
    config.ILST.padding = '2.0';///2.0em padding-left for <UL> and <OL>
    config.ILST.style = 2;///0=plain;1=monospace;2=italic
    config.HLST = {};
    config.DLST = {};
    config.NLST = {};
    config.NLST.padding = '2.0';///2.0em
    config.LONG = {};
...
//console.log(block);
      ...
      if(sig == 'LONG'){
        ...
        if(sig0 == 'MATH'){
          ...
          blk.row2 = block.row2;
          blk.para = blk.para.concat(block.para);
          var maths = block.maths;
          var label = block.label;
          blk.more.push({maths,label});
          o.push(blk);
        } else {
          block.more = [];
          o.push(block);
  ...
  read_para (lines,n) {
    ...
    /// continue to read after skipping blank lines or %-lines                   
    ...
    for (let i=0; n < lines.length; ++i,++n) {
      ...
      if(i==0 && (v=re_spcl.exec(line))!==null){
        ...
        else if((v=re_caption.exec(s))!==null){
          label = v[1];
          caption = v[2];
          iscaption = 1;
          continue;
      ...
      if(i==0 && (v=re_ilst.exec(line))!==null){
        type = 'ilst';
        body.push(line);
        continue;  
      }
      ...
      if(i==0 && (v=re_note.exec(line))!==null){
        type = 'note';///set to empty string so that this block will be ignored
        body.push(v[2]);
        continue;  
      }
      if(i==0){
    ...
    /// block reading has finished
    /// slice off some the lines that are read
    ...
    if(type == 'spcl'){
      if(fenceid == 'framed'){
        ...
        return {para,sig,caption,label,body};
      } 
        ...
        return {para,sig,caption,label,rows};
      }
      ...
      var maths = this.parse_math(body);
      return {para,sig,label,maths,body};
    }
    else if(type == 'hdgs'){
      if(n0==0){
        ///if it starts at line 1, 
        ///then it is hardcoded to be HDGS/0
        hdgn=0;
      }
  ...
  */
  ...
  async readModeAsync(dirname) {
    ...
    for(let d of this.mode){
      let {name,sublevel,part,subfname,subparser} = d;
      if(name==='root' && subparser){
        ...
        let sig='HDGS';
        let hdgn=0;
        this.blocks.push({sig,hdgn,part});
        this.ismaster = 1;
  ...
  parse_tabr_vertical_bar(para){
    ...
    /// process the extra and see if we can extra
    /// [^1], [^2], etc.
    var re_key   = /^\[\^(\d+)\]$/;
    var re_item  = /^\[\^(\d+)\]\s+(.*)$/;
    var defs = {};
    ...
    if(1){    
      ...
      /// check the first row to replace any [^1] with the text
  ...
  parse_tabr(para){
    ...
    if(para.length && para[0].indexOf('|') >= 0){
      ...
      return this.parse_tabr_hyphen_minus(para);
    }
  }
