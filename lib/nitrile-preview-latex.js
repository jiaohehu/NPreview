...
`;
  ...
  make_line(math,label){
    ...
    if(label) {
      line += `\\label{${label.slice(1)}}`;
    }
    return {line, splitflag};
  }
  ...
  make_math(o,math,label,more,gather){
    ...
    var lines = [];
    var n=0;
    var {line} = this.make_line(math,label);
    lines.push(line);
    for(let i=0; more && i < more.length; ++i){
      let x = more[i];
      var {line} = this.make_line(x.math,x.label);
      lines.push(line);
      if(x.label){
        n++;
      }
