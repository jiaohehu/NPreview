...
class NitrilePreviewHtml extends NitrilePreviewParser {
  ...
  do_hdgs(block) {
    var {id,row1,row2,sig,name,sublevel,hdgn,text,dept,subfname} = block;
    var o = [];
    var text = this.escape(text);
    ///note that sublevel and hdgn guarenteed to be integers
    if(sublevel === undefined){
      var level = hdgn;
    }else{
      var level = sublevel + hdgn;
    }
    if(name=='part'){
...
${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
  ...
  do_diag(block){
    ...
    var o = []; 
    var diagram = new NitrilePreviewDiagramSVG(this, parser.notes, this.tokenizer);
    var {s,vw,vh} = diagram.toDiagram(body);
