...
console.log('james: Cy=',Cy);
        if (Number.isFinite(Cx) && Number.isFinite(Cy)) {
          if (position=='top') {
            var ang1 = Math.atan2(Y2-Cy,X2-Cx)/Math.PI*180;
            var ang2 = Math.atan2(Y1-Cy,X1-Cx)/Math.PI*180;
            o.push(`draw subpath (${ang1/45},${ang2/45}) of fullcircle xscaled(${2*Rx}) yscaled(${2*Ry}) scaled(u) shifted(${Cx}*u,${Cy}*u);`);
          } else {
            var ang1 = Math.atan2(Y1-Cy,X1-Cx)/Math.PI*180;
            var ang2 = Math.atan2(Y2-Cy,X2-Cx)/Math.PI*180;
            o.push(`draw subpath (${ang1/45},${ang2/45}) of fullcircle xscaled(${2*Rx}) yscaled(${2*Ry}) scaled(u) shifted(${Cx}*u,${Cy}*u);`);
          }
        }
        continue;
