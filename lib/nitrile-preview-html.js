...
`;
  ...
  do_dlst(block) {
    ...
    var o = []; 
    const dtstyle=`text-indent:-${this.config.HTML.step}em;margin-left:${this.config.HTML.step}em;`;
    ...
    for (var item of items) {
      ...
      if(type=='text'){
        ...
        key = this.escape(key);
        text = this.unmask(text);
        o.push(`<dt style='${dtstyle}'>&#x2022; &#160; <tt> <span class='NICODE'>${key}</span> &#160; ${text}</dt>`);
      }
    }
    o.push('</dl>');
    o.push('');
  ...
  do_nlst(block){
    ...
    for (var item of items) {
      ...
      if(type=='nlst'){
        ...
        text = body.join('\n');
        o.push(`<pre><span class='NICODE'>${text}</span></pre>`);
        o.push('');
      }else if(type=='pass'){
  ...
  do_verb(block) {
    ...
    var linenum = 0;
    body = body.map( x => this.escapeBR(x) );
    body = body.map( x => `<tr><td><span class='NIPROG'>${++linenum}</span></td><td>&#160;</td><td style='white-space:pre'><span class='NIPROG'>${x}</span></td></tr>`);
    text = body.join('\n');
    text = `<table border='0' style='border-collapse:collapse;'>${text}</table>`;
    o.push(`<figure ${this.toATTR(block)} >`);
    if(this.floatname){
      o.push(`<figcaption> <span class='NICAP'>${this.caption_text}</span> </figcaption>`);
    }
    o.push(text);
    o.push('</figure>');
    o.push('');
  ...
  do_samp(block) {
    ...
    var text = body.join('\n');
    o.push(`<pre ${this.toATTR(block)} style='margin-left:${this.config.HTML.step}em;'><span class='NICODE'>${text}</span></pre>`);
    o.push('');
    block.html = o.join('\n');
  }
  do_pass(block) {
  ...
  do_pict(block) {
    ...
    for(var blk of rows){
      ...
      var qq = imgs.map( img => {
        ...
        return(`<td style='${tdstyle}'> <span class='NICAP'>${sub_text}</span> </td>`);
      });
      if(mode && mode.width){
  ...
  do_tabb(block){
    ...
    for(var j=0; j<nrows; ++j){
      ...
      pp = pp.map(x => `<span class='NITAB'>${x}</span>`);
...
${j==nrows-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</${j==0?'th':'td'}>`);
  ...
  do_long(block){
    ...
    rows = rows.map ( row => row.map(x => `<span class='NITAB'>${x}</span>`));
    ww = this.wwToHundred(ww);
    var s = [];
...
${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
  ...
  style (type, text) {
    ...
    switch (type) {
      case 'code': {
        text = text.trim();
        return `<tt><span class='NICODE'>${this.escape(text)}</span></tt>`
        break;
      }
      case 'em': {
        text = text.trim();
  ...
  make_math_svg(math,label,idnum,more,gather){
    ...
    if(1){///always a gather
      var data = data.map( x => {
          ...
          var math_style = `left:${dist}em;position:relative;text-align:left;margin:${this.mathmargin}pt 0;`;
          var idnum_style = `text-align:right;`;
          return (`<td style='${math_style}'>${s}</td><td style='${idnum_style}'><span class='NICAP'>${idnum_text}</span></td>`);
      });
