...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  unmask (line) {
    ...
    while ((v = re_unmask.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        var cnt = v[2]; 
        newtext += this.do_style('code',cnt);
  ...
  is_symbol(cnt){
    return this.tokenizer.is_symbol(cnt);
  }
  ...
  untouch (line) {
    ...
    var start_i = 0;
    var newtext = '';
    var re_untouch = /(?<!\\)(\\\\)|(?<!\\)(\\[A-Za-z][A-Za-z0-9]*)|(?<!\\)(\\\()|(?<!\\)(\\\[)/g;
  ...
  exec_cmd(key,cnt){
    //note that 'cnt' has not yet been escaped 
    var newtext = '';
    if(key=='em'){
