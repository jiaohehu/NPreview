...
class NitrilePreviewDiagram {
  ...
  parseLine(line) {
    ...
    if ((v = this.re_cartesian_command.exec(line)) !== null) {
      var opt = v[1].trim();
      var [act,line] = this.readActionText(v[2]);
      var floats = this.readFloatsLine(line);
      this.commands.push(this.do_cartesian(opt,act,floats));
      return false;
    }
    if ((v = this.re_barchart_command.exec(line)) !== null) {
      ...
      var [act,line] = this.readActionText(v[2]);
      var floats = this.readFloatsLine(line);
      this.commands.push(this.do_barchart(opt,act,floats));
      return false;
    }
    if(line.length>0){
      this.commands.push(this.do_comment(`${line}`));
  ...
  do_barchart(opt,act,floats) {
    ...
    switch( act ) {
      case 'setup':
        ...
        A.xgrid = this.assertFloat(floats[2],1,this.MIN,this.MAX);
        A.ygrid = this.assertFloat(floats[3],1,this.MIN,this.MAX);
        break;
       
      case 'bbox':
        var x1 = this.assertFloat(floats[0],0);
        var y1 = this.assertFloat(floats[1],0);
        var x2 = this.assertFloat(floats[2],0);
        var y2 = this.assertFloat(floats[3],0);
        x1 /= A.xgrid;
        x2 /= A.xgrid;
        x1 += A.xorigin;
        x2 += A.xorigin;
        ...
        y1 += A.yorigin;
        y2 += A.yorigin;
        s.push(this.do_path_primitive([x1,y1,x2,y1,x2,y2,x1,y2],true));
        break;
