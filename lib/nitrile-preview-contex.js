...
\\page`);
  ...
  do_samp(block){
    var {id,row1,row2,sig,mode,rmap,data,para,fencecmd,base,subrow,fname} = block;
    var o = [];
    var text = data;
    o.push(`%SAMP`);
    if (!mode) {
  ...
  do_frmd(block){
    ...
    var text = text.map((x,i) => `label.rt("\\tt ${x}", (5,-${i}*o));`);
    var solid = '\\ '.repeat(80);
    o.push(`\\blank`);
    o.push(`\\startMPcode`);
    o.push(`numeric o; o := BodyFontSize;`);
    o.push(`label.rt("\\tt ${solid}", (5,0));`);
    data.forEach((x,i) => {
      x = this.escapeSolid(x);
      o.push(`label.rt("\\tt ${x}", (5,-${i}*o));`);
    });
    o.push(`numeric tw; tw := \\the\\textwidth;`);
    o.push(`numeric pc; pw := bbwidth(currentpicture);`);
