...
${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
  ...
  smooth_for_var(unsafe) {
    ...
    unsafe = unsafe
      ...
      .replace(/²/g,  "&#x00B2;")
      .replace(/³/g,  "&#x00B3;")
      .replace(/(?<!\\)([A-Za-z])\^(\d)(?!\S)/g, (match,p1,p2) => {
      ...
      .replace(/(?<!\\)([A-Za-z])_(\d)(?!\S)/g, (match,p1,p2) => {
      ...
      .replace(/(?<!\\)\\(dot|ddot|bar|mathring|hat|check|grave|acute|breve|tilde)\{([A-Za-z])\}(?!\S)/g, (match,p1,p2) => {
