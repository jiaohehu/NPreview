...
class NitrilePreviewDiagram {
  ...
  initSettings() {
    ...
    this.def_sweepflag = this.sweepflag = 0;
    this.def_anglearcradius = this.anglearcradius = 0.7;
  ...
  parse(para) {
    ...
    for (row in para) {
      ...
      if ((v = this.re_set_command.exec(line)) !== null) {
        ...
        if (key === 'radius') { this.radius = this.assertFloat(val, this.def_radius, 0.1, 100); } ///such as '1'
        if (key === 'start-angle') { this.angle1 = this.assertFloat(val, this.def_angle1, 0, 360); } ///such as '90'
        if (key === 'end-angle') { this.angle2 = this.assertFloat(val, this.def_angle2, 0, 360); } //suchas '180'
        ...
        if (key === 'bigarcflag') { this.bigarcflag = this.assertInt(val, this.def_bigarcflag, 0, 1); } //suchas '0'
        if (key === 'sweepflag') { this.sweepflag = this.assertInt(val, this.def_sweepflag, 0, 1); } //suchas '0'
          ...
          /// [a:rx,ry,angle,bigarcflag,sweepflag,dx,dy] 
            ...
            if (val.length == 7) {
              ...
              if (Number.isFinite(X1) &&
                ...
                lastjoin = 'arcpath';
                coords.push([X2, Y2, lastjoin, Rx,Ry,Phi,bigarcflag,sweepflag]);
                lastpt[0] = X2;
                lastpt[1] = Y2;
              }
            }
            ...
            if (val.length == 4) {
              ...
              if (Number.isFinite(x1) &&
                ...
                var [C0,C1,C2,C3] = this.quadrilaticToCubic(lastpt,[x1,y1],[x,y]);
                coords.push([x, y, '..', C1[0], C1[1], C2[0], C2[1]]);
                lastpt[0] = x;
                lastpt[1] = y;
                lastjoin = '..';
              }
