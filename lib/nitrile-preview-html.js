...
${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
  ...
  to_inlinemath (str,dstyle) {  
    var {s} = this.to_svgmath(str,dstyle);
    if(dstyle){
  ...
  to_svgmath(line,dstyle){
    ...
    var {s,w,h,mid,shiftdist,defs} = this.tokenizer.to_svgmath(line,dstyle);
    var vw = w*1.333;
    var vh = h*1.333;
  ...
  make_line_svg(math,label,islabeled,idnum,wide){
    var d = this.tokenizer.to_svgmath_array(math);
    d = d.filter(x => (x.w > 0));
