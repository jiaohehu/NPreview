...
class NitrilePreviewLatex extends NitrilePreviewParser {
  ...
  do_hdgs(block){
    ...
    switch (level) {
      case 0:
        if(this.config.LATEX.coverpage){
...
`;
  ...
  do_pict(block){
    var {id,mode,row1,row2,sig,data,para,fencecmd,base,subrow,fname} = block;
    var o = [];
    var s = [];
    for(var blk of data){
      var n = blk.length;
...
\\begin{document}
${(config.LATEX.coverpage)?coverpage:''}
${(config.LATEX.toc)?'\\tableofcontents':''}
\\bigskip
${texlines.join('\n')}
\\end{document}
`;
    ...
    for (var j=0; j < text.length; ++j) {
      ...
      if (fns0) {
        ...
        if (fn) {
          ...
          var c0 = c0 + c;
          if(this.xlatfontsize){
            var s0 = `{\\${fn}\\${this.xlatfontsize}{}${c0}}`;
          }else{
            var s0 = `{\\${fn}{}${c0}}`;
          }
