...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  drawtext(x,y,label,justi,g) {
    ...
    if ((v=this.re_inlinemath.exec(label))!==null) {
      ...
      label = this.translator.smooth(label);
      ...
      d.push(`<text font-size='${this.translator.config.HTML.diagfontsizept}pt' text-anchor='${anchor}' ${this.to_textdraws(g)} x='${x}' y='${y}' dy='${dy}'>${label}</text>`);
    }
    return d.join('\n');
  }
