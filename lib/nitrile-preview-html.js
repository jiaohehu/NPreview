...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  constructor() {
    ...
    this.xconfig.title = '';
    this.xconfig.author = '';
    this.xconfig.step = 2;//similar to CONTEX.step but specifing in 'em'
    this.xconfig.mathfontsizept = 12;
    this.xconfig.diagfontsizept = 12;
    this.xconfig.css = '';//to be added using +=
    this.xconfig.width = 130;///130mm textwidth in MAIN element
    this.xconfig.margin = 4;///4mm margins left/right/top/bottom for MAIN element
    this.xconfig.leftmargin = 44;
    this.xconfig.rightmargin = 44;
    this.xconfig.topmargin = 30;
    this.xconfig.toc = 0;///when set to 1 a customized TOC will be inserted
  }
  set_view (view) {
    this.view = view;
  }
  do_identify(block, A) {
    ...
    if (this.xismaster) {
      if (sig == 'HDGS') {
        ...
        if (name == 'part') {
          A.parts++;
          idnum = this.to_part_num(A.parts);
        } else if (hdgn == 0) {
          A.chapters++;
          A.sections = 0;
          A.subsections = 0;
  ...
  do_dlst(block) {
    ...
    o.push(`<dl class='${sig}' ${this.to_attr(block)} >`);
    var bull = '&#x2022;';
  ...
  do_nlst(block){
    var o = [];
    var {id,row1,row2,sig,items,para} = block;
    o.push(`<ol class='${sig}' ${this.to_attr(block)} style='${this.to_lstyle(block)};'>`);
    for (var item of items) {
      ...
      if(type=='nlst'){
        ...
        body = body.map( x => this.escape_with_nbsp(x) );
        text = body.join('\n');
  ...
  do_hlst(block) {
    ...
    var o = []; 
    o.push(`<dl class='HLST' ${this.to_attr(block)} >`);
    for (var item of items) {
      var {key,text} = item;
      key = this.unmask(key);
      text = this.unmask(text);
    ...
    for (var item of items) {
      ...
      switch (bull) {
        case 'OL': {
          if(count==0){
            count++;
            o.push(`<ol class='PLST' ${this.to_attr(block)} style='${this.to_lstyle(block)}'><li value='${value}'>${text}`);
          }else{
            o.push(`<ol class='PLST' style='${this.to_lstyle(block)}'><li value='${value}'>${text}`);
          }
          break;
        }
        case 'UL': {
          if(count==0){
            count++;
            o.push(`<ul class='PLST' ${this.to_attr(block)} style='${this.to_lstyle(block)}'><li value='${value}'>${text}`);
          }else{
            o.push(`<ul class='PLST' style='${this.to_lstyle(block)}'><li value='${value}'>${text}`);
          }
          break;
        }
        case 'LI': {
  ...
  do_llst(block) {
    ...
    var d = body.map((x, i) => {
      var line = x;
      var lineno = `${i + 1}`;
      var n = 5 - lineno.length;
      lineno += '&#160;'.repeat(n>0?n:0);
      var lineno = `<tt>${lineno}</tt>`;
  ...
  do_samp(block) {
    ...
    var o = []; 
    var body = body.map( x => this.escape_with_nbsp(x) );
    var text = body.join('\n');
    o.push(`<pre class='SAMP' ${this.to_attr(block)} style='margin-left:${this.xconfig.step}em;'>${text}</pre>`);
    o.push('');
    block.html = o.join('\n');
  }
  do_pass(block) {
    ...
    o.push(`<p class='PASS' ${this.to_attr(block)} style='margin-left:${this.xconfig.step}em;'>${text}</p>`);
    o.push('');
    block.html = o.join('\n');
  }
  do_hrle(block) {
    ...
    var o = []; 
    text = this.unmask(text);
    o.push(`<hr class='${sig}' ${this.to_attr(block)} style='text-align:center' />`);
    o.push('');
    block.html = o.join('\n');
  }
  do_text(block) {
    ...
    ///output to 'o'
    o.push(`<p class='${sig}' ${this.to_attr(block)} >${text}</p>`);
    o.push('');
    block.html = o.join('\n');
  }
  do_pict(block) {
    ...
    var imgs = images.map( img => {
      ...
      var imgid='';
      if (this.view) {
        var {imgsrc,imgid} = this.view.query_imagemap_info(src);
    ...
    var mypp = imgs.map( img => {
      ...
      var img = `<td style='${td_style}'> <img id='${imgid}' style='${img_style}' src='${imgsrc}' alt='${src}'/> </td>`;
  ...
  do_quot(block) {
    ...
    text = `${lq}${text}${rq}`;
    var o = [];
    o.push(`<blockquote class='${sig}' ${this.to_attr(block)} style='margin-left:${this.xconfig.step}em;right-margin:0;${this.xstyle}'>`);
    o.push(text);
    o.push(`</blockquote>`);
    o.push('');
    block.html = o.join('\n');
  ...
  do_tabu(block){
    ...
    var text = s.join('\n');
    var text = `<table class='TABU' ${this.to_attr(block)} border='0' style='${this.to_mstyle(block)};border-collapse:collapse;'>${text}</table>`;
    o.push(text);
    o.push('');
    block.html = o.join('\n');
  }
...
${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
    ...
    s.push(`</tbody>`);
    var text = s.join('\n');
    text = `<table class='LONG' ${this.to_attr(block)} border='0' style='border-collapse:collapse;width:100%;'>${text}</table>`;
    o.push(text);
    o.push('');
    block.html = o.join('\n');
  } 
  do_tabb(block){
    ...
    s.push(`</tbody>`);
    var text = s.join('\n');
    text = `<table class='TABB' ${this.to_attr(block)} border='0' style='border-collapse:collapse;width:100%;'>${text}</table>`;
    o.push(text);
    o.push('');
    block.html = o.join('\n');
  } 
  do_diag(block){
    var {id,row1,row2,sig,notes,body,idnum,floatname,caption,label,islabeled} = block;
    var o = []; 
    var {s,vw,vh} = this.diagram.toDiagram(body,notes);
    var text = `<svg class='DIAG' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' fill='currentColor' stroke='currentColor' viewBox='0 0 ${vw} ${vh}' style='width:100%; ' >${s}</svg>`;
    ...
    if(islabeled){          
      o.push(`<figure ${this.to_attr(block)} >`);
      o.push(`<figcaption class='CAPTION'> ${this.to_caption_text(floatname,idnum,caption)} </figcaption>`);
    }else{
  ...
  do_ref (sig,label,floatname,idnum,refid,id,chapters){
    if(sig=='HDGS'){
      ...
      return `<a class='niref' href='#${id}'>${secsign}${idnum}</a>`;
    }
    else if(sig=='MATH'){
      var secsign = String.fromCharCode(0xA7);
      return `<a class='niref' href='#${id}'>${secsign}(${idnum})</a>`;
    ...
    if(floatname){
      var secsign = String.fromCharCode(0xA7);
      return `<a class='niref' href='#${id}'>${floatname}&#160;${secsign}${idnum}</a>`;
    }
...
`;
  ...
  to_xhtml_document() {
    ...
    mytitle = this.escape(mytitle);
    myauthor = this.escape(myauthor);
    var mytoc = this.xconfig.toc?this.to_toc():'';
    var data = `\
