    ...
    for (var pp of text) {
      ...
      var kk = pp.map( x => {
          ...
          if (this.view) {
            if (config.previewimages && ispreview) { 
              if( this.view.imagemap.has(src)) {
                let imgbuff = this.view.imagemap.get(src);
                imgsrc = `data:image/png;base64,${imgbuff.toString('base64')}`;///TODO: harded coded to PNG,
              } else {
          ...
          return `<img id='nitrile-preview-img-${imgid}' alt='${src}' src='${imgsrc}' 
                  style='vertical-align:text-bottom; object-fit:contain; object-position:left; 
                  width:${100.0*width}%; max-width:${100.0*width}%;
