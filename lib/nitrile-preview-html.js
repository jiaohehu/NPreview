...
class NitrilePreviewHtml extends NitrilePreviewParser {
  ...
  /*
    Only HDGS are translated
    */
  translateContents (blocks,o) {
    o = o || [];
    o.push(`<div style='box-sizing:border-box;width:8.5in;padding:1in 1.25in 1in 1.5in;margin:0;font-size:10pt;background-color:white;'>`);
    var chapnum = 0;
        ...
        case 'HDGS': {
          ...
          switch (cat) {
            case 0:
              ...
              this.subsubsecnum = 0;
              o.push(`<h1>`);
              o.push(`${chapnum} &#160; ${this.escape(text)}`);
    ...
    for (var block of blocks) {
      ...
      switch (type) {
          ...
          switch (cat) {
            case 0:
              if (isarticle) {
                ...
                o.push(`<p>`);
                o.push(`${this.escape(new Date().toLocaleString())}`);
                o.push(`</p>`);
                o.push(`</div>`);
              } else {
                o.push(`<div id='${id}' class='${type}' rows='${row1} ${row2}'>`);
        ...
        case 'LSTG': {
          ...
          var linenum = 0;
          text = text.map( x => `<code><span>${++linenum}</span>${x}</code>` );
          var text = text.join('\n');
          o.push(`<pre>${text}</pre>`);
          o.push(`</figure>`);
          o.push(`</div>`);
        ...
        case 'CODE': {
          ...
          if (fencecmd['n']) {
            ...
            var linenum = 0;
            text = text.map( x => `<code><span><small>${this.expandString(++linenum,4,' ')}</small></span>${x}</code>` );
            var text = text.join('\n');
            o.push(`<pre>${text}</pre>`);
            o.push(`</div>`);
            o.push('');
