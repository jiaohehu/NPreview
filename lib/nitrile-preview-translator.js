...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  constructor(parser) {
    ...
    this.config={};
    this.issymbolmode=0;
    this.re_all_sups = /(?<!\w)([A-Za-z])\^([0-9ni])/g;
    this.re_all_subs = /(?<!\w)([A-Za-z])_([0-9aeoxhklmnpst])/g;
    ...
    while ((v = re_untouch.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        try {
          ...
          var cnt_text = this.polish(cnt_text);//this will have to be polish so that we can see all native codes
          newtext += cnt_text;
        }
        ...
        var match_text = this.to_style(match_text,'verb');
        newtext += match_text;
