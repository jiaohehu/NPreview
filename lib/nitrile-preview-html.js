...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  do_FLOA(block) {
    switch(block.floatname){
      ...
      case 'Vocabulary': {
        let itms = this.para_to_vocabulary_itms(block.body,block.style);
        itms.forEach(p => {
      ...
      case 'Listing': {
        ...
        if(1){
          let o = [];
          o.push(`<figure >`);
          o.push(this.to_caption_text(block.style.idtext,block.style.idnum,block.style.caption));
          o.push(text);
          o.push('</figure>');
          text = o.join('\n');
        }
  ...
  fence_to_math(ss, style) {
    ...
    if(style.floatname=='Equation'){
      let os = '';
      os = `<td style='width:0%'><nobr>(${style.idnum})</nobr></td><td style='width:100%'>${text}</td>`;
      os = `<tr>${os}</tr>`;
      os = `<table style='width:100%'>${os}</table>`;
      text = os;
    }
    return text;
  }
  ...
  fence_to_tabulate(ss, style) {
    let rows = super.fence_to_tabulate_rows(ss,style);
    ...
    if(style.floatname=='Table'){
      let o = [];      
      o.push(`<figure >`);
      o.push(this.to_caption_text(style.idtext, style.idnum, style.caption));
      o.push(text);
      o.push('</figure>');
      text = o.join('\n');
    }
  ...
  para_to_longtable(ss, style) {
    let itms = this.para_to_longtable_itms(ss,style);
    itms = itms.map(s => {
      s = this.unmask(s);
      return s;
    });
    let text = this.itms_to_longtable(itms, style);
    if (style.floatname == 'Table') {
      let o = [];
      o.push(`<figure >`);
      o.push(this.to_caption_text(style.idtext, style.idnum, style.caption));
...
`;
  ...
  to_caption_text(idtext,idnum,caption){
  ...
  itms_to_tabulate(rows,style){
    ...
    if(border==1){
      for (var j = 0; j < nrows; ++j) {
        ...
        pp = pp.map((x, i) => `<td style='padding:0 0.5em;border:1px solid;'>${x}</td>`);
        var p = pp.join('');
        var p = `<tr>${p}</tr>`;
        d.push(p);
      }
      ...
      for (var j = 0; j < nrows; ++j) {
        ...
        pp = pp.map((x, i) => {
          if(i==0){
            return `<td style='padding:0 0.5em;border-left:1px solid;border-top:${border_top};border-bottom:${border_bot}'>${x}</td>`;
          }else if(i==pp.length-1){
            return `<td style='padding:0 0.5em;border-right:1px solid;border-top:${border_top};border-bottom:${border_bot}'>${x}</td>`;
          }else{
            return `<td style='padding:0 0.5em;border-top:${border_top};border-bottom:${border_bot}'>${x}</td>`;
          }
        });
        var p = pp.join('');
        var p = `<tr>${p}</tr>`;
      ...
      for (var j = 0; j < nrows; ++j) {
        ...
        let border_bot = (j == nrows - 1) ? '1px solid' : '';
        pp = pp.map((x, i) => {
          return `<td style='padding:0;border-top:${border_top};border-bottom:${border_bot};'>${x}</td>`;
        });
      ...
      for (var j = 0; j < nrows; ++j) {
        ...
        pp = pp.map((x, i) => `<td style='padding:0;border-top:${border_top};'>${x}</td>`);
