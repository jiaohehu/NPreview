        ...
        this.generateContentsBlocks(this.blocks,this.dirname)
          .then((out) => {
              ...
              if (contentsBlocks.length) {
                ...
                this.htmls = [];
                this.htmls.push(`<div style='${this.divStyle}'>`);
                this.htmls = parser.translateHdgsOnly(title,author,contentsBlocks,this.htmls);
                this.htmls.push(`</div>`);
                const { scrollTop } = this.element;
                this.element.textContent = ''
                this.element.innerHTML = this.htmls.join('\n');
...
console.log(the_blocks);
          ...
          /// need to find the <div> node that is the parent
          /// of this <mark> node and then insert htmls as 
          /// follow on siblings of this <div> node
          var div_node = this.findParentDivElementById( node.parentNode, id);
          if (div_node) {
            var parent_div_node = div_node.parentNode;
            if (parent_div_node) {
              for (var block of the_blocks) {
                var element = document.createElement('div');
                var html = translator.translate([block],false);        
                element.innerHTML = html.join('\n');
                parent_div_node.insertBefore(element,div_node.nextSibling);
              }
            }
          }
        }
  ...
  findParentDivElementById (node, id) {
    while (node ) {
      var nodeName = ''+node.nodeName;
      var className = ''+node.className;
      var idName = ''+node.id;
...
console.log('this.contentsBlocks.length');
    for (var j=0; j < this.contentsBlocks.length; ++j) {
      ...
      if (type1 === 'HDGS' && id1 === id) {
        for (var k=j+1; k < this.contentsBlocks.length; ++k) {
