...
class NitrilePreviewNode {
  ...
  async toEpub (filename) {
    ...
    const parser = new NitrilePreviewHtml('EPUB');
    parser.isepub = 1;
    await parser.readFromFileAsync(filename)
    await parser.readModeAsync();
    parser.idenBlocks();
  ...
  async runFontmap(args){
    ...
    return new NitrilePreviewNode().toFontmap(myfiles);
  }

  ///Needed for nih
  async runGlyphs(args){
    ...
    new NitrilePreviewNode().toGlyphs(myfile).then(f => console.log(f)).catch(err => console.log(err)) 
  }
