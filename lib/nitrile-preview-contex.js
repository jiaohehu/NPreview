...
class NitrilePreviewContex extends NitrilePreviewParser {
  ...
  do_hdgs(block){
    var o = [];
    var {id,row1,row2,sig,name,sublevel,hdgn,dept,text,subfname} = block;
    o.push(this.to_info(block));
    var raw = text;
    var text = this.escape(text);
    ///note that 'sublevel' and 'hdgn' are guarrenteed to be
    //integers
    if(level === undefined){
      level=hdgn;
    }else{
  ...
  do_math(block){
    var o = [];
    var {id,row1,row2,sig,math,label,islabeled,more,gather,subfname} = block;
    o.push(this.to_info(block));
