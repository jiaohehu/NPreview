...
class NitrilePreviewDiagramMP extends NitrilePreviewDiagram {
  ...
  do_label(opt,label,coords){
    ...
    for (var i = 0; this.valid(coords, i); ++i) {
      ...
      if (this.fontsize) {
        ...
        tex_label = `\\fontsize{12pt}{12pt}\\selectfont{}${tex_label}`;
      }
  ...
  do_crate(opt,txt,coords) {
    var mypath = '(3,2)--(0,2)--(0,0)--(3,0)--(3,2)--(0,2)--(1,3)--(4,3)--(3,2)--(3,0)--(4,1)--(4,3)--(3,2)--cycle';
  ...
  do_box(opt,txt,coords) {
    ...
    for (var i = 0; i < coords.length    ; i++) {
      ...
      if (label) {
        ...
        s.push(`label (btex {${tex_label}} etex, (${labelx}*u,${labely}*u)) ${withcolor};`);
      }
    }
    return s.join('\n');
  }
    ...
    switch( act ) {
      ...
      case 'xticks':
        ...
        for (var j=0; j < floats.length; ++j) {
          ...
          var lx = this.fix(lx);
          var ly = this.fix(ly);
          s.push(`label.bot (btex {${tex_label}} etex, (${lx}*u,${ly-protrude}*u)) ${withtextcolor};`);
        }
        break;
      ...
      case 'yticks':
        ...
        for (var j=0; j < floats.length; ++j) {
          ...
          var lx = this.fix(lx);
          var ly = this.fix(ly);
          s.push(`label.lft (btex {${tex_label}} etex, (${lx}*u,${ly}*u)) ${withtextcolor};`);
        }
        break;
      ...
      case 'ellipse':
        ...
        var angle = this.assertFloat(floats[2],0);///angle is always anti-clockwise
      ...
      case 'text' :
        ...
        for( var j=0,n=0; j < floats.length; j+=2,n++ ) {
          ...
          if (Number.isFinite(x)&&Number.isFinite(y)){
            ...
            s.push(`label${label_alignment} (btex {${tex_label}} etex, (${x}*u,${y}*u)) ${this.withtextcolors()};`);
          }
        }
        break;
