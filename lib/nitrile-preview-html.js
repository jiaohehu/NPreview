      ...
      switch (sig) {
        ...
        case 'FIGE': {
          ...
          for (var pp of text) {
            var kk = pp.map( x => {
                ...
                if (config.previewimages && this.view && this.view.hasImage(src)) {
                  ...
                  if (config.previewimages && this.view) {
                    this.view.requestImage(src);
                  }
                }
                return `<img alt='${src}' src='${imgdata}' width='${100.0*width}%' style='vertical-align:text-bottom; min-width:${100.0*width}%; min-height:100px; outline:1px solid #333;' />`;
            });
            o.push(`<nobr>`);
            o.push(kk.join(''));
            o.push(`</nobr>`);
            ...
            var kk = pp.map( (x,j) => {
                var [image,width,opts,src,srcs,sub] = x;
                return `<span style='display:inline-block; width:${100.0*width}%; text-align:center; '>(${this.toSubfigNum(j)}) ${this.unmask(sub)}</span>`;
            });
            o.push(`<nobr>`);
            o.push(kk.join(''));
            o.push(`</nobr>`);
