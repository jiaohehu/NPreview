...
`;
  ...
  do_hdgs(block) {
    var {id,row1,row2,sig,part,level,data,para,dept,fencecmd,base,subrow,subfname} = block;
    var o = [];
...
${j==nrows-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</${j==0?'th':'td'}>`);
  ...
  do_diag(block){
    var {id,row1,row2,sig,data,para,dept,fencecmd,base,subrow,subfname} = block;
    var o = []; 
    if (fencecmd.star) {
