...
class NitrilePreviewParser extends NitrilePreviewBase {
  ...
  async read_mode_async() {
    ...
    for (let d of this.mode) {
      ...
      if (name === 'config') {
        ...
        let sig = 'PART';
        let subseq = 0;
        this.blocks.push({ sig, text, refid, subseq });///the 'part' block will have a refid
  ...
  translate_blocks(translator) {
    ...
    if(1){
      for (var block of this.blocks) {
        let hdgn = block.hdgn||'';
        let name = block.name||'';
        console.log('->',block.sig,`'${hdgn}'`,`'${name}'`,`"${block.text}"`,`${block.row1}:${block.row2}`);
      } 
    }
  ...
  to_key_item(s){
    ...
    let q;
    var v;
    const re_key_braced = /^\{\s+(.*?)\s+\}\s*(.*)$/m;
    const re_key_quoted = /^'(.*?)'\s*(.*)$/m;
    const re_key_dquoted = /^"(.*?)"\s*(.*)$/m;
    const re_key_item = /^(\S+)\s*(.*)$/m;
    if ((v = re_key_braced.exec(s)) !== null) {
      /// { Apple tree } 
      ///   is good
      key = v[1];
