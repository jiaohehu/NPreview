...
class NitrilePreviewParser extends NitrilePreviewBase {
  ...
  read_para (lines,n) {
    ...
    if(type == 'spcl'){
      ...
      if(floatid == 'figure'){
        ...
        items = this.parse_body_for_vocb(body);
        sig = 'VOCB';
      }
  ...
  async read_mode_async() {
    ...
    for (let d of this.mode) {
      ...
      if (name === 'config') {
        ...
        let hdgn = subn;
        this.blocks.push({sig,title,hdgn,label,refid,subseq,name,subf,subn});
  ...
  parse_dlst(para) {
    ...
    for (var j = 0; j < para.length; ++j) {
      ...
      if((v=re_dlst.exec(s)) !== null) {
        ...
        if((v=re_rubyitem.exec(key)) !== null) {
          type   = 'rmap';
          rb     = v[1];
          rt     = v[2];
          this.rmap.push([rb,rt]);
    ...
    if(1){
      ...
      for (var block of this.blocks) {
        ...
        switch(block.sig){
          ...
          case 'VOCB': translator.do_VOCB(block); break;
          default: break;
        }
      }
  ...
  to_key_item(ss){
    ...
    if ((v = re_key_bracked.exec(s)) !== null) {
      ...
      if ((v = re_rubyitem.exec(word)) !== null) {
        cat = 'rmap';
        rb = v[1];
        rt = v[2];
        this.rmap.push([rb, rt]);
