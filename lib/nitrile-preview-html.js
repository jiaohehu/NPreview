...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  do_hdgs(block) {
    ...
    dept = dept||'';
    if(name=='part'){
      o.push(`<h1 ${this.to_attr(block)} >Part ${dept} <br/> ${text}</h1>`);
    } 
    else if(level==0) {
      o.push(`<h1 ${this.to_attr(block)} >${dept} ${text}</h1>`);
    } 
    else if(level==1) {
      o.push(`<h2 ${this.to_attr(block)} >${dept} ${text}</h2>`);
    } 
    else if(level==2) {
      o.push(`<h3 ${this.to_attr(block)} >${dept} ${text}</h3>`);
    } 
    else {
      o.push(`<h4 ${this.to_attr(block)} >${dept} ${text}</h4>`);
    }
    o.push('');
    block.html = o.join('\n');
  }
...
`;
  ...
  to_caption_text(floatname,idnum,caption){
    return `${floatname} ${idnum}: ${this.unmask(caption)}`;
  }
