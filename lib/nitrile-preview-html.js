    ...
    for (var block of blocks) {
      ...
      switch (type) {
        ...
        case 'LSTG': {
          this.lstnum += 1;
          o.push(`<div id='${id}' class='${type}' rows='${row1} ${row2}' style='margin-left:${left}'>`);
          o.push(`<p>Listing ${this.lstnum}: <small><mark>[${id}]</mark></small> ${this.unmask(ins_text)}</p>`);
        ...
        case 'PICT': {
          if (ins === 'figure') {
            ...
            if (ins_local) {
              var base = id.split(':')[0];
              var label = base + ':' + ins_local;
            } else {
              var label = id;
            }
            o.push(`<div id='${id}' class='${type}' rows='${row1} ${row2}'>`);
            o.push(`<p>Figure ${this.fignum}: <small><mark>[${label}]</mark></small> ${this.unmask(ins_text)}</p>`);
          } else {
            o.push(`<div id='${id}' class='${type}' rows='${row1} ${row2}' style='margin-left:${left}'>`);
          }
          o.push('<p>');
