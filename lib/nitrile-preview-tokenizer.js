...
console.log('w1=',w1);
        ...
        case '\\pd':
          ...
          if (2) {
            ...
            // letter 'x'
            o.push(`<g transform='translate(${(dx+wd_)*1.333},${(bot_h-h2_)*1.333})'> ${s2_} </g>`);
            // letter '2'
            o.push(`<use transform='translate(${(dx+wd_+w2_)*1.333},${(bot_h-h2_-ext)*1.333}) scale(${this.sup_rate})' xlink:href='#${ids_}'/>`);
            var bot_s = o.join('\n');
          }
          var reduce_rate = this.to_frac_reduce_rate();
          if(reduce_rate < 1){
            top_w *= reduce_rate;
            top_h *= reduce_rate;
            bot_w *= reduce_rate;
          ...
          var w = Math.max(top_w,bot_w) + 4;
          var h = top_h + 1 + bot_h;
          if (top_w < bot_w) {
            var top_dx = 2 + (bot_w - top_w)/2;
            var bot_dx = 2;
          } else {
            var top_dx = 2;
            var bot_dx = 2 + (top_w - bot_w)/2;
          }
          var mid = top_h + 0.4;
          var o = [];
          var top_x = top_dx;
