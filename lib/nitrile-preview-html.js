...
`;
    ...
    for (var item of data) {
      ...
      switch (bull) {
        case 'OL': {
          if(count==0){
            count++;
            o.push(`<ol ${TOPATTRS} style='${TOPSTYLE};padding-left:${fencecmd.padding}em;list-style-position:outside;'><li value='${value}'>${text}`);
          }else{
            o.push(`<ol style='padding-left:${fencecmd.padding}em;list-style-position:outside;'><li value='${value}'>${text}`);
          }
          break;
        }
        case 'UL': {
          if(count==0){
            count++;
            o.push(`<ul ${TOPATTRS} style='${TOPSTYLE};padding-left:${fencecmd.padding}em;list-style-position:outside;'><li value='${value}'>${text}`);
          }else{
            o.push(`<ul style='padding-left:${fencecmd.padding}em;list-style-position:outside;'><li value='${value}'>${text}`);
          }
          break;
        }
        case 'LI': {
  ...
  do_verb(block) {
    ...
    text = s.join('\n');
    text = `<table border='0' style='line-height:1;border-collapse:collapse;border-top:solid 1px;border-bottom:solid 1px;'>${text}</table>`;
  ...
  do_samp(block) {
    ...
    if(fencecmd.style==1){
      ...
      text = text.join('\n');
      o.push(`<dl ${this.xattr} style='${this.xstyle};line-height:1;'>${text}</dl>`);
      o.push('');
    }
    else{
      text = this.trimSampPara(text);
  ...
  do_tabb(block){
    ...
    var text = s.join('\n');
    var text = `<table border='0' style='border-collapse:collapse;line-height:1'>${text}</table>`;
    o.push(`<figure ${this.xattr} style='${this.xstyle}'>`);
    o.push(text);
    o.push('</figure>');
    o.push('');
...
${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
    ...
    s.push(`</tbody>`);
    var text = s.join('\n');
    text = `<table border='0' style='width:100%;border-collapse:collapse;line-height:1'>${text}</table>`;
    if (this.xname === 'table') {
      o.push(`<figure ${this.xattr} style='${this.xstyle}'>`);
...
${j==nrows-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</${j==0?'th':'td'}>`);
    ...
    var text = s.join('\n');
    text = `<table border='0' style='margin-left:0;margin-right:auto;border-collapse:collapse;line-height:1;'>${text}</table>`;
    if (this.xname === 'table') {
      o.push(`<figure ${this.xattr} style='${this.xstyle}'>`);
      o.push(`<figcaption style='text-align:left;margin-top:${this.xtop};margin-bottom:${this.xbot};' > Table ${this.xidnum} : ${this.caption_text} </figcaption>`);
      o.push(text);
