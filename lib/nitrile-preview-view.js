...
class NitrilePreviewView {
  ...
  refreshView() {
    ...
    if (this.isarticle) {
      ...
      this.htmls = [];
      this.chaps = [];
      this.htmls.push(`<main class='PAGE' style='${geometry_text}' >`);
    ...
    if (this.path) {
      switch (this.saveastype) {
        ...
        case 'latex-complete': 
          ...
          break;
        case 'epub': 
          basename = path.basename(this.path + ".zip");
          break;
        default: 
...
`;
        ...
        epub.setElement(this.element.firstElementChild.firstElementChild,this.isarticle);
