...
class NitrilePreviewParser {
  ...
  newConfig() {
    ...
    config.ALL.title = '';
    config.ALL.author = '';
    config.ALL.samp=0;//1=lines,0=code
...
//console.log(block);
  ...
  parseIMGS (para) {
    ...
    para = this.join_backslashed_lines(para);
  ...
  parse_ilst (para) {
    ...
    var re_word = /^(\S+)\s*(.*)$/;
    for (var line of para) {
      if((v = this.re_ilst.exec(line))!==null){
        var text = v[2];
        items.push(text);
        continue;
      } 
  ...
  */
  ...
  translateBlocks() {
    ...
    for (var block of this.blocks) {
      ...
      this.xpadding=fencecmd.padding||'0';
