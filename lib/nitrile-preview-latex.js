...
class NitrilePreviewLatex extends NitrilePreviewParser {
  ...
  do_hdgs(block){
    var o = [];
    var {id,row1,row2,sig,name,subn,hdgn,dept,text,para} = block;
    ...
    if(name=='part'){
      ...
      if(name=='h'){
        o.push(`\\chapter${star}{${dept}${text}}${this.latexlabelcmd}`);
      ...
      o.push(`\\section${star}{${dept}${text}}${this.latexlabelcmd}`);
      if(star) o.push(`\\addcontentsline{toc}{section}{${dept} ${raw}}`);
      o.push('%');
    }
    else if(level==2){
      o.push(`\\subsection${star}{${dept}${text}}${this.latexlabelcmd}`);
      if(star) o.push(`\\addcontentsline{toc}{subsection}{${dept} ${raw}}`);
      o.push('%');
    }
    else{
      o.push(`\\subsubsection${star}{${dept}${text}}${this.latexlabelcmd}`);
      o.push('%');
    }
    block.latex = o.join('\n');
  }
  ...
  do_prog(block){
    var o = [];
    var {id,row1,row2,sig,wide,body,caption,label} = block;
    o.push(this.to_info(block));
    var n = 0;
  ...
  do_tabr(block){
    var o = [];
    var {id,row1,row2,sig,wide,cols,caption} = block;
    o.push(this.to_info(block));
  ...
  do_diag(block){
    var {body,wide,parser,caption} = block;
  ...
  do_pict(block){
    var o = [];
    var {id,row1,row2,sig,wide,rows,caption} = block;
    o.push(this.to_info(block));
    var s = [];
    for(var blk of rows){
      let {mode,images} = blk;
  ...
  do_frmd(block){
    var o = [];
    var {id,row1,row2,sig,wide,body,caption} = block;
    o.push(this.to_info(block));;
    var s = [];
  ...
  do_vbarchart (cnt) {
    ...
    if(w && h && data){
      ...
      o.push(`draw unitsquare xscaled(w) yscaled(h) ;`);
      o.push(`endfig`);
