...
class NitrilePreviewDiagramMP extends NitrilePreviewDiagram {
  ...
  p_shape(x,y,p,g){
    ...
    for( var s of ss ) {
      if (s) {
        if (g.fillcolor) {
          o.push(`fill (${s}--cycle) xscaled(${sx}) yscaled(${sy}) shifted(${x},${y}) scaled(u) ${this.to_fills(g)};`);
        }
        o.push(`draw (${s}) xscaled(${sx}) yscaled(${sy}) shifted(${x},${y}) scaled(u) ${this.to_draws(g)};`);
      }
    }
    return o.join('\n');
  }
