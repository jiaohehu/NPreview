    ...
    for (var block of all) {
      ...
      switch (sig) {
        ...
        case 'math': {
          ...
          var count = ss.length;
          if (count == 1) {
            o.push(`\\begin{${(fencecmd.eq==='1')?'equation':'equation*'}}${label_cmd}`);
            o.push(ss[0].join(' '));
            o.push(`\\end{${(fencecmd.eq==='1')?'equation':'equation*'}}`);
            o.push('');
          } else {
            ss = ss.map(x => x.join(' & '));
            ss = ss.map(x => `${x} \\\\`);
            if (fencecmd.alignequalsign) {
              ...
              o.push(`\\begin{${(fencecmd.eq==='1')?'gather':'gather*'}}${label_cmd}`);
              o.push(ss.join('\n'));
              o.push(`\\end{${(fencecmd.eq==='1')?'gather':'gather*'}}`);
              o.push('');
            }
          }
          break;
...
///console.log(o);
  ...
  toLatexDocument(config,texlines) {
    ...
    var title = config.latex.title ? config.latex.title : 'Untitled'
    var author = config.latex.author ? config.latex.author : ''
