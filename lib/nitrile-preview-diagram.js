...
console.log(`%${line}`);
        ...
        if (key === 'tick-size'     ){ this.ticksize  = this.toLength(val,this.def_ticksize); } ///such as '1pt' or '1mm'
        if (key === 'tick-protrude' ){ this.tickprotrude = this.assertFloat(val,this.def_tickprotrude,0.1,1.0); } ///such as '0.33'
        if (key === 'font-color'    ){ this.fontcolor = this.toColor(val,this.def_fontcolor); }/// such as 'green'
        if (key === 'font-size'     ){ this.fontsize = this.toString(val,this.def_fontsize); }/// such as '14pt'
        continue;
      }
...
///console.log('C=',C);
  ...
  toColor(val,def_val) {
console.log('toColor: val=',val);
console.log('toColor: def_val=',def_val);
    ///such as 'red', '0.5red', or 0.8[red,green]
    var re_color = /^([\d\.]*)(\w+)$/;
    var re_color2 = /^([\d\.]*)\[(\w+)\,(\w+)\]$/;
    var v = null;
    if (!val) {
console.log('toColor: return=',def_val);
      return def_val;
    }
...
console.log( `\\mpcolor{${color1}!${first}!${color2}!${second}!}`); 
      ...
      first = first.toFixed(0);
      if (first == 100) {
console.log(   `\\mpcolor{${color}}` );
        return `\\mpcolor{${color}}`;
      } else {
console.log(   `\\mpcolor{${color}!${first}!}`);
