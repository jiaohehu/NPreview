...
class NitrilePreviewContext extends NitrilePreviewLatex {
  ...
  do_hdgs(block,conf){
    var {hdgn,label,text,parser} = block;
    var o = [];
    o.push(this.to_info(block));
    //var raw = text;
    var raw = this.smooth(text);
      ...
      switch (bull) {
        ...
        case 'LI': {
          if(dl){
            ...
            if (desc) {
              o.push(`${cmd}{{\\bf{}${key}}} \\\\ ${desc} ${extra_text} \\par`);
            } else {
              o.push(`${cmd}{{\\bf{}${key}}} {} \\par`);
            }
          } else if (ds) {
            ...
            if (cat == 'rmap') {
              key = `${rb}${String.fromCharCode('0xb7')}${rt}`;
              key = this.polish(key);
              o.push(`\\item {\\bf{}${key}} ${nbsp}${desc} ${extra_text}`);
            }
            else if (cat == 'quoted') {
              key = this.polish(key);
              key = `${q}${key}${q}`;
              o.push(`\\item {\\tt\\bf{}${key}} ${nbsp}${desc} ${extra_text}`);
            }
            else {
              key = this.polish(key);
              o.push(`\\item {\\bf{}${key}} ${nbsp}${desc} ${extra_text}`);
  ...
  to_tabular(rows) {
    ...
    let text = this.rows_to_tabulate(rows, ww, {}, 0);
    return text;
  }
  to_style (text,type) {
