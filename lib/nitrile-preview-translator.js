...
class NitrilePreviewTranslator extends NitrilePreviewBase {
  ...
  to_ref (target) {
    for (var j=0; j < this.parser.blocks.length; ++j) {
      ...
      var {id,sig,label,islabeled,floatname,idnum,more,refid,id} = block;
      if(more&&more.length){
        for(let k=0; k < more.length; k++){
          let x = more[k];
          if(x.islabeled && x.label && target.localeCompare(x.label)===0){
            return this.do_ref(sig,x.label,floatname,x.idnum,refid,id);
            break;
          }
        }
      }else{
        if( islabeled && label && target.localeCompare(label)===0) {
          return this.do_ref(sig,label,floatname,idnum,refid,id);
          break;
        }
      }
    }
    target = this.polish(target);
    return this.to_style(target,'overstrike');
  }
  ...
  untext(ss,style={}) {
    ...
    if(n>1&&ss[0].startsWith('~~~')&&ss[n-1].startsWith('~~~')){ 
      ...
      if(this.para_is_math(ss)){
        let itms = this.para_to_math(ss);
        return this.to_math(itms,style);
      }
      if(this.para_is_longtable(ss)){
        let rows = this.para_to_longtable(ss);
        return this.to_longtable(rows,style);
      }
      if(this.para_is_multi(ss)){
        let rows = this.para_to_multi(ss);
        return this.to_multi(rows,style);
      }
      if(this.para_is_cols(ss)){
        let cols = this.para_to_cols(ss);
        return this.to_cols(cols,style);
      }
      if(this.para_is_rows(ss)){
        let itms = this.para_to_rows(ss);
        return this.to_rows(itms,style);
      }
      if(this.para_is_itemized(ss)){
        let itms = this.para_to_itemized(ss);
        return this.to_itemized(itms,style);
      }
      if(this.para_is_tabulate(ss)){
        let data = this.para_to_tabulate(ss);
        return this.to_tabulate(data,style);
      }
      if(this.para_is_imgrid(ss)){
        let data = this.para_to_imgrid(ss);
        return this.to_imgrid(data,style);
      }
      ss = this.join_backslashed_lines(ss);
      ss = this.join_indented_lines(ss);
      return this.to_verse(ss,style);
    } 
    if(n>1&&ss[0].startsWith('"""') && ss[n - 1].startsWith('"""')) {
      let s0 = ss[0].slice(3);
      ss = ss.slice(1, n - 1);
      ...
      ss = this.trim_samp_body(ss);
      return this.to_blockquote(ss,style);
    }
    return this.to_story(ss,style);
  }
  uncode(line) {
  ...
  unmask(line) {
    ...
    while ((v = re_unmask.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        let ss = cnt.split('\n');
        if(ss.length){
          let {fenceid,style} = this.str_to_fenceid(str);
          let nss = ss.length;
          ss = ss.slice(0, nss - 1);
          ss = this.trim_samp_body(ss);
          if(fenceid==='diagram'){
            newtext += this.to_diagram(ss,style);
          }else if(fenceid==='framed'){
            newtext += this.to_framed(ss,style);
          }else if(fenceid==='math'){
            let itms = this.para_to_math(ss);
            newtext += this.to_math(itms,style);
          }else{
            newtext += this.to_verbatim(ss,style);
          }
        }        
      } 
    ...
    while ((v = re_untouch.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        if(key=='ruby'){
          ...
          var g = this.to_fig(cnt);
          newtext += this.do_vbarchart(g);
        }
        else if(key=='xyplot'){
          var g = this.to_fig(cnt);
          newtext += this.do_xyplot(g);
        }
        else if(key=='colorbox'){
          var g = this.to_fig(cnt);
          newtext += this.do_colorbox(g);
        }
        else if(key=='em'){
          newtext += this.to_style(this.untouch(cnt),'em');
        }
        else if(key=='b'){
          newtext += this.to_style(this.untouch(cnt),'b');
        }
        else if(key=='br'){
          newtext += this.to_br();
        }
        else if(key=='vspace'){
          newtext += this.to_vspace(cnt);
        }
        else {
  ...
  equation_to_math(ss){
    var pp = this.to_clustered_lines(ss);
    var pp = pp.map(x => x.join(' '));
    var pp = pp.map(x => x.replace(/`/g,''));
    var pp = pp.map(x => x.trimLeft());
    var pp = pp.map(s => {
      let v;
