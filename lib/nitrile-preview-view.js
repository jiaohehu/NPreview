  ...
  refreshView() {
    ...
    if (this.blocks.length) {
      ...
      if (this.isarticle) {
        ...
        this.htmls = this.parser.translateHtml(autonum,this.config,this.xrefs,this.blocks,this.isarticle,0,0,this.htmls);
      } else {
        var autonum = new NitrilePreviewAutonum();
        for (var i=0; i < this.subs.length; ++i) {
          this.htmls = this.parser.translateHtml(autonum,this.config,this.xrefs,this.subs[i].flow[0],this.isarticle,this.subs[i].subrow,this.subs[i].sublevel,this.htmls);
        }
      }
  ...
  async saveAs (savefilepath) {
    ...
    try {
      ...
      if (this.saveastype === 'latex-chapter') {
        ...
        if (this.isarticle) {
          var autonum = new NitrilePreviewAutonum();
          var olines = latex.translateLatex(autonum,this.config,this.xrefs,this.blocks,this.isarticle,0,0,[]);
        } else {
          var autonum = new NitrilePreviewAutonum();
          var olines = [];
          for (var i=0; i < this.subs.length; ++i) {
            olines = latex.translateLatex(autonum,this.config,this.xrefs,this.subs[i].flow[0],this.isarticle,this.subs[i].subrow,this.subs[i].sublevel,olines);
          }
        }
