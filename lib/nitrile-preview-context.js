...
class NitrilePreviewContext extends NitrilePreviewTranslator {
  ...
  constructor() {
    ...
    this.xconfig.bodyfontsizept = 10
  ...
  do_hdgs(block,conf){
    var o = [];
    var {subn,hdgn,label,text} = block;
    o.push(this.to_info(block));
    var raw = text;
    var text = this.escape(text);
    subn = subn || 0;
  ...
  do_llst(block){
    var o = [];
    var {id,row1,row2,sig,wide,body,idnum,caption,label,islabeled,parts,chapters} = block;
    o.push('');
    o.push(this.to_info(block));
    var d = body.map((x,i) => {
      var line = x;
  ...
  do_ref (sig,label,floatname,idnum,refid,id,chapters){
    if(sig=='HDGS'){
      ...
      var secsign = String.fromCharCode(0xA7);
      if(chapters){
        return `${secsign}Listing~${chapters}.${idnum}`;
      }else{
        return `${secsign}Listing~${idnum}`;
      }
    }
    if(floatname){  
      var secsign = String.fromCharCode(0xA7);
