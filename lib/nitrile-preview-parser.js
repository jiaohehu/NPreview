...
class NitrilePreviewParser extends NitrilePreviewBase {
  ...
  read_md_lines(lines) {
    ...
    while (nlines < lines.length) {
      ...
      if(o.length){
        sig0 = o[o.length-1].sig;
      }

      if (sig === 'FRMD') {

        block.lines = this.trim_samp_body(block.body);
        o.push(block);
      
      } 
      else if (sig === 'DIAG') {
      
        block.lines = block.body;
        o.push(block);
      } 
      else if (sig === 'PICT') {
        
        var { opts, images } = this.parse_pict(block.body);
        block.opts = opts;
        block.images = images;
        o.push(block);
        ...
        var pp = this.to_clustered_lines(block.body);
        block.more = [];
        block.islabeled = 1;
        pp.forEach((lines,i) => {
          let {math,label} = this.parse_quat_one(lines);
          block.more.push({label,math});
        });
        o.push(block);
        ...
        var rows = this.parse_data(block.body,0);
        block.rows = rows;
        block.sig = 'DATA';
        o.push(block);
        ...
        var rows = this.parse_data(block.body,1);
        block.rows = rows;
        block.sig = 'DATA';
        o.push(block);
        ...
        var { rows, ww } = this.parse_tabr(block.body);
        block.rows = rows;
        block.ww = ww;
        o.push(block);
      ...
      /// assign the last sig
      /// this line will have to be here for all cases because
      /// for 'isspcl' where the leading one could be 'PICT'
      /// and the second one could be 'SAMP', it will 
      /// need to do a 'continue' so that after processing
      /// the second one the 'sig0' isn't changed to 'SAMP'
      sig0 = sig;
    }
  ...
  read_para (lines,n) {
    ...
    for (let i=0; !done && n < lines.length; ++i,++n) {
      ...
      if(i==0){
        is_text = 1;
        type = 'text';
        is_fenced = this.line_is_fence(line);
        body.push(line);
        continue;  
      }
      if(is_spcl){
      ...
      if(is_text){
        ...
        if(is_fenced&&this.line_is_fence(line)){
          is_fenced=0;
          line = line.trimLeft();
          body.push(line);
          done = 1;
        ...
        if(this.line_is_fence(line)){
          ///fence it
          nspace = this.get_line_nspace(line);
          line = line.trimLeft();
          body.push(line);
          is_fenced = 1;
          continue;
    ...
    if(type == 'spcl'){
      ...
      else if (fenceid == 'data'){
        var floatname = 'DATA';
        sig = 'DATA';
      }
      else if (fenceid == 'note'){
        if(islabeled){
      ...
      return {para,sig,wide,lines,caption,label,islabeled,style,floatname,body};
    } 
    else if(type == 'math'){
      ...
      var items = this.parse_plitems(plitems);
      return {para,sig,items,body,isbroad};
    }
    else if(type == 'hlst'){
      ...
      data = this.parse_para(body);
      return {para,sig,data,body};
    }
    else if(type == 'samp'){
      var lines = this.trim_samp_body(body);
      return {para,sig,body,lines};
    } 
    else if(type == 'text'){
      ...
      if(body.length==1 && n0==0 && n==1){
        ...
        var {leadn,lead,text,isverse,isstory} = this.parse_text(body);
        return {para,sig,leadn,lead,text,isverse,isstory,body};
      }
    } 
    else if(type == 'note'){
      let name = body[0];
  ...
  async read_mode_async() {
    ...
    for (let d of this.mode) {
      ...
      if (name === 'config') {
        ...
        console.log('h', subf, refid, 'blocks',subparser.blocks.length);
  ...
  parse_text(para) {
    ...
    if ((v1 = re_prim.exec(para[0])) !== null ||
      ...
      let line0 = para[0];
      let linen = para[para.length-1];
      if(this.line_is_verse(line0)&&this.line_is_verse(linen)){
        let mybody = para.slice(1,para.length-1);
        mybody = mybody.map(x => x.trimLeft());
        mybody = this.join_backslashed_lines(mybody);
        text = mybody.join('\n');
        isverse = 1;
      }else if(this.line_is_story(line0)&&this.line_is_story(linen)){
        let mybody = para.slice(1,para.length-1);
        mybody = mybody.map(x => x.trimLeft());
        text = mybody.join('\n');
        isstory = 1;
    ...
    return { leadn, lead, text, isverse, isstory };
  }
  ...
  parse_data(para,isspace){
    ...
    var n = tt.reduce((acc,cur) => Math.max(acc,cur));
    para = para.map(pp => {
      while(pp.length < n){
        pp.push('');
      }
  ...
  translate_blocks(translator) {
    console.log('translate block, translator name',translator.name);
    console.log('translate block, total blocks',this.blocks.length);
    if(1){
      //call do_starttranslate
    ...
    for (var block of this.blocks) {
      this.block = block;
      //console.log('translate block',block.sig);
      switch(block.sig){
        ...
        case 'DATA': translator.do_data(block); break;
        case 'TABR': translator.do_tabr(block); break;
        case 'LONG': translator.do_long(block); break;
        case 'PARA': translator.do_para(block); break;
  ...
  line_is_fence(line){
    return re_fence.test(line.trim());
  }
  line_is_verse(line) {
    ...
    items.forEach(x => {
      if(x.more){
        x.more = x.more.map(plitem => {
          if(plitem[0].trim()==='```'){
            plitem = plitem.slice(1);
            if(plitem[plitem.length-1].trim()==='```'){
              plitem = plitem.slice(0,plitem.length-1);
            }
