  ...
  read_md_lines(lines) {
    ...
    while (nlines < lines.length) {
      ...
      if (sig === 'FRMD') {
        ...
        block.more = [];
        pp.forEach((lines,i) => {
          let x = this.parse_quat_one(lines);
          block.more.push(x);
        });
        let more_islabeled = block.more.map(x => x.islabeled).reduce((acc,cur) => Math.max(acc,cur),0);
        block.islabeled = Math.max(block.islabeled,more_islabeled);
        o.push(block);
        ...
        if(sig0 == 'MATH'){
          ...
          var islabeled = 0;
          var blk = o.pop();
          blk.more.push({math,label,islabeled});
          blk.row2 = block.row2;
          blk.para = blk.para.concat(block.para);
          o.push(blk);
          ...
          var islabeled = 0;
          block.more = [];
          block.more.push({math,label,islabeled});
          o.push(block);
