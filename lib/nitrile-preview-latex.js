...
const P_lstlisting_numbers_xleftmargin = 0.75;/// additional left margin for when lstlisting is numbered
...
class NitrilePreviewLatex extends NitrilePreviewParser {
    ...
    for (var block of blocks) {
      ...
      switch (sig) {
        ...
        case 'CODE':  
          ...
          if (fig) {
            var text = data;
            const numbers = (fencecmd.n) ? 'left' : 'none';
            const xleft = (fencecmd.n) ? P_lstlisting_numbers_xleftmargin : 0;
            const increment = 40;
            for (var k=0; k < text.length; k += increment) {
              if (k == 0) {
                o.push(`\\begin{lstlisting}[float=*,basicstyle=\\ttfamily,breaklines=true,postbreak=\\textrightarrow\\space,firstnumber=${k+1},numbers=${numbers},xleftmargin=${xleft}cm,label=${label},caption={${this.unmask(caption)}}]`);
              } else {
                o.push(`\\begin{lstlisting}[float=*,basicstyle=\\ttfamily,breaklines=true,postbreak=\\textrightarrow\\space,firstnumber=${k+1},numbers=${numbers},xleftmargin=${xleft}cm,]`);
              }
            ...
            var text = data;
            const numbers = (fencecmd.n) ? 'left' : 'none';
            const xleft = (fencecmd.n) ? left+P_lstlisting_numbers_xleftmargin : left;
            o.push(`\\begin{lstlisting}[basicstyle=\\ttfamily,breaklines=true,postbreak=\\textrightarrow\\space,xleftmargin=${xleft}cm,numbers=${numbers},]`);
        ...
        case 'VERB': {
          ...
          if (fig) {
            if (fencecmd.frame) {
              o.push(`\\begin{program*}`);
              o.push(`\\caption{${this.unmask(caption)}}${label_text}`);
              o.push(`\\begin{Verbatim}[fontsize=\\footnotesize,frame=single]`)
              ...
              o.push(`\\begin{program*}`);
              o.push(`\\caption{${this.unmask(caption)}}${label_text}`);
              o.push(`\\begin{Verbatim}[fontsize=\\footnotesize]`)
              for (var line of text) {
                o.push(line);
              }
              o.push('\\end{Verbatim}')
          ...
          } else if (fencecmd['frame']) {
            o.push(`\\begin{lstlisting}[basicstyle=\\ttfamily\\footnotesize,breaklines=true,postbreak=\\textrightarrow\\space,xleftmargin=${left}cm,columns=fullflexible,breakindent=6pt,frame=single]`)
            for (var line of text) {
              o.push(line);
            }
            o.push('\\end{lstlisting}')
            ...
            o.push(`\\begin{flushleft}`);
            o.push(`\\begin\{adjustwidth\}{${left}cm}{}`)
            var [out, vw, vh] = this.toFramedTikz(text);
            o.push(`\\ifthenelse{\\dimtest{${vw}}{>}{\\linewidth}}{\\resizebox{\\linewidth}{!}{`);
            o.push('');
            o.push(out);
            o.push('');
            o.push(`}}{`);
  ...
  */
  ...
  toRequiredPackages (isarticle,config) {
    ...
    const texfamily = config.texfamily;
    ...
    if (config.twocolumn) {
      ...
      geometry_opts.push(`left=${leftmargin}cm`);
      geometry_opts.push(`right=${rightmargin}cm`);
      geometry_opts.push(`top=${topmargin}cm`);
    } else {
    ...
    geometry_text = `\\usepackage[${geometry_opts.join(',')}]{geometry}`;
...
`
  ...
  toFramedTikz (para ) {
    ...
    ///lappend o "\\draw (0,0) rectangle ($width,$height);"
    ...
    for (var line of para) {
      ...
      line = this.replaceSubstrings(line,this.mymapcode);
      line = this.fontifyLATEX(line);
      o.push( `\\draw (10pt,${i*10}pt) node\[right\]\
