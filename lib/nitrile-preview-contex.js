...
class NitrilePreviewContex extends NitrilePreviewParser {
  ...
  do_hdgs(block){
    ...
    if (!this.haschapter) {
      switch (level) {
        ...
        case 1:
          o.push(`\\startsection[title={${dept}~~${text}},reference={${this.baselabel}},bookmark=${raw}]`);
          o.push('');
          break;
        case 2:
          o.push(`\\startsubsection[title={${dept}~~${text}},reference={${this.baselabel}},bookmark=${raw}]`);
          o.push('');
          break;
        case 3:
          o.push(`\\startsubsubsection[title={${dept}~~${text}},reference={${this.baselabel}},bookmark=${raw}]`);
          o.push('');
          break;
        default:
          o.push(`\\startsubsubsubject[title={${dept}~~${text}},reference={${this.baselabel}},bookmark=${raw}]`);
          o.push('');
          break;
      }
    } else {
      switch (level) {
        case 0:
          break;///this needs to be ignore specifically otherwise it
                ///will show through 'default'
        case 1:
          o.push(`\\startchapter[title={${dept}~~${text}},reference={${this.baselabel}},bookmark=${raw}]`);
          o.push('');
          break;
        case 2:
          o.push(`\\startsection[title={${dept}~~${text}},reference={${this.baselabel}},bookmark=${raw}]`);
  ...
  do_verb(block){
    ...
    if (this.xnumbers) {
      ...
      for (var k=0; k < text.length; ++k) {
        ...
        s.push(`\\NC {\\tfx ${++linenum}} \\NC {${ffamily}${this.xconfontsize}{}${line}} \\NC\\NR`);
  ...
  do_long(block){
    ...
    var header = rows.shift();
    var header = header.map(x => `{${this.xconfontsize}{}${x}}`);
    var s = [];
    ww = this.wwToOne(ww);
...
\\setuphead[section][style=bfc,number=no]
\\setuphead[subsection][style=bfb,number=no]
\\setuphead[subsubsection][style=bfa,number=no]
\\setuphead[subsubsubsection][style=bold,number=no]
