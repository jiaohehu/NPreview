...
class NitrilePreviewHtml extends NitrilePreviewTranslator {
  ...
  do_part(block) {
    ...
    if(this.conf('html.partpage')){
      ...
      o.push(`<h1 ${this.to_attr(block)} > <small> Part ${idnum} </small> <br/> ${title} </h1>`);
    }
    o.push('');
    block.html = o.join('\n');
  }
  do_hdgs(block) {
    var {id,row1,row2,sig,hdgn,subn,name,title,idnum,parser} = block;
    var o = [];
    var title = this.unmask(title);
    ...
    idnum = idnum||'';
    if(level==0) {
      o.push(`<h1 ${this.to_attr(block)} >${idnum} ${title}</h1>`);
    } 
    else if(level==1) {
      o.push(`<h2 ${this.to_attr(block)} >${idnum} ${title}</h2>`);
    } 
    else if(level==2) {
      o.push(`<h3 ${this.to_attr(block)} >${idnum} ${title}</h3>`);
    } 
    else {
      o.push(`<h4 ${this.to_attr(block)} >${idnum} ${title}</h4>`);
    }
    o.push('');
    block.html = o.join('\n');
  }
    ...
    for (var item of items) {
      ...
      if(more && more.length){
        more.forEach((plitem) => {
          let {lines} = plitem;
          if(isbroad){
            extra_text += `<p>${this.untext(lines)}</p>`;
          }else{
            extra_text += `<div>${this.untext(lines)}</div>`;
          }
        });
      }
      switch (bull) {
        ...
        case 'LI': {
          if(dl){
            ...
            if (isbroad) {
              ...
              o.push(`<dt>${dt}</dt><dd>${dd}${extra_text}</dd>`);
            }
          }
          else if(ds){
            let { keys, desc } = ds;
            ...
            if(isbroad){
              ...
              o.push(`<li value='${value}'>${text}${extra_text}</li>`);
            }
          }
          break;
        }
  ...
  do_llst(block) {
    var { body, floatname, caption, idnum, islabeled } = block;
    var o = [];
    var s = [];
    var d = body.map((x, i) => {
      var line = x;
  ...
  do_samp(block) {
    ...
    var o = []; 
    var text = this.to_verbatim(body);
  ...
  do_prim(block) {
    ...
    if (hdgn === 1) {
      ...
      text = `${indent}<strong><i>${title}</i></strong>  ${text}`;
    }
...
${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
  ...
  do_math(block) {
    var {id,row1,row2,sig,caption,label,islabeled,wide,idnum,items} = block;
    var o = []; 
    var data = this.make_math_svg(caption,label,islabeled,idnum,items,wide);
    var text = data.join('\n');
  ...
  do_fige(block) {
    var {floatname,idnum,caption,style,islabeled,body} = block;
    var o = []; 
  ...
  do_frmd(block) {
    var {id,row1,row2,floatname,idnum,caption,sig,style,lines,islabeled} = block;
    var o = []; 
    lines=lines||[];
    var {s} = this.framed.to_framed(lines,'100%');
    var text = s;///`<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' fill='currentColor' stroke='currentColor' viewBox='0 0 ${vw} ${vh}' style='width:100%; ' >${s}</svg>`;
    if (islabeled) {
      o.push(`<figure ${this.to_attr(block)} >`);
      o.push(this.to_caption_text(floatname, idnum, caption));
      o.push(text);
  ...
  to_math(pp,style) {
    let islabeled = style.islabeled||0;
    var text = this.math_to_htmltable(pp,islabeled);
    return text
  }
  to_verbatim(ss,style){
    ss = ss.map(x => this.polish(x));
    return `<code style='display:block;white-space:pre-wrap'>${ss.join('<br/>')}</code>`;
  }
  to_verse(ss){
  ...
  to_cols(itms,style){
    ...
    let text = this.itms_to_cols(itms,style.n);
    return text;
  }
  to_tabulate(rows,style) {
    rows = rows.map((ss) => ss.map(s => {
      s = this.unmask(s);
      s = this.rubify(s);
      return s;
    }));
    let text = this.rows_to_tabulate(rows,style.border);
    return text;
  }
  to_longtable(itms, style, caption, label) {
    itms = itms.map(s => {
      s = this.unmask(s);
      return s;
    });
    ...
    return this.itms_to_longtable(itms, style.fr, style.hline, style.glue, style.n, caption, label);
  }
  to_multi(itms,style){
    itms = itms.map(s => {
      s = this.unmask(s);
    ...
    let text = this.itms_to_multi(itms,style.fr,style.hline,style.n);
    return text;
  }
  to_itemized(itms,style) {
    ...
    let text = this.itms_to_ul_or_ol(itms,style);
    return text;
  }
  to_blockquote(ss) {
    let text = ss.join('\n').trim();
  ...
  to_imgrid(imgs,style){
    ...
    var text = this.imgrid_to_htmltable(imgs,style.frame,style.n);
    return text;
  }
  to_story(ss){
    let text = ss.join('\n');
  ...
  to_vspace(s){
    return `<span style='display:block;height:${s}'></span>`;
  }
  to_style(text,type) {
  ...
  make_line_svg(math,label,islabeled,idnum,wide){
    ...
    s = this.to_outer_svg(s,w,h,mid,defs,this.tokenizer.fs,0,'make_line_svg');
    return {s,w,h,mid,shiftdist,defs,label,idnum};
  }

  make_math_svg(caption,label,islabeled,idnum,items,wide){
    var data = [];
...
`;
  ...
  rows_to_tabulate(rows,border){
    ...
    if(border==1){
      for(var j=0; j<nrows; ++j){
        ...
        pp = pp.map((x,i) => `<td style='border:1px solid'>${x}</td>`);
        var p = pp.join(gap);
        var p = `<tr>${p}</tr>`;
        d.push(p);
      }
      ...
      for (var j = 0; j < nrows; ++j) {
        var pp = rows[j];
        if (this.pp_is_hline(pp)) {
          let ww = 'x'.repeat(ncols).split('').map(x => '');
          d.push(`<tr><td style='border-top:1px solid' colspan='${ncols + ncols - 1}'></td></tr>`);
          continue;
        }
        pp = pp.map((x, i) => `<td>${x}</td>`);
        var p = pp.join(gap);
  ...
  itms_to_ul_or_ol(itms,style){
    ...
    if(itms.length && itms[0].bull=='OL'){
      let pp = itms.map(p => {
        if(p.type=='A'){
          return `<li type='A' value='${p.value}'>${p.text}</li>`;
        }
        if(p.type=='a'){
          return `<li type='a' value='${p.value}'>${p.text}</li>`;
        }
        if(typeof p.value == 'number'){
          return `<li value='${p.value}'>${p.text}</li>`  
        }
        return `<li>${p.text}</li>`
      });
      if (style && style.n && style.n > 1) {
        return this.pp_to_multi_itemized(pp, style.fr, style.n, 'ol')
      }
      return `<ol>${pp.join('\n')}</ol>`;
    }
    return `<ul><li></li></ul>`;
  ...
  pp_to_multi_itemized(pp,fr,n,name){
    ...
    for (let i = 0; i < n; ++i) {
      ...
      tn = k;
      let fr = frs[i];
      d.push(`<td style='vertical-align:top;width:${fr*w*100}%'><${name} style='margin:0'>\n${kk.join('\n')}\n</${name}></td>`);
      if (i < n - 1) {
        d.push(`<td style='width:${gap * 100}%'></td>`);
      }
    }
  ...
  imgrid_to_htmltable(imgs,frame,n) {
    var d = [];
    var imgsrcs = [];
    var n = parseInt(n);
    var n = n || imgs.length;
  ...
  itms_to_longtable(itms, fr, hline, glue, n, caption, label) {
    var caption = caption || '';
    var label = label || '';
    var fr = fr || '';
    var glue = glue || '';
...
${j+n >= itms.length && hlines.indexOf('b') >= 0 ? 'border-bottom:1px solid;' : ''} '>${x}</td>`);
  ...
  math_to_htmltable(pp,islabeled) {
    var data = pp.map( p => {
        var line = this.make_line_svg(p.math, p.label, islabeled, p.idnum, 0);
        return line;
    });
    ...
    if (gather) {///always a gather
      var data = data.map(x => {
        ...
        //var math_style = `left:${dist}pt;position:relative;text-align:left;padding:${this.mathpadding}pt 0;`;
        var math_style = `text-align:left;padding:${this.mathpadding}pt 0;`;
        var idnum_style = `text-align:right;`;
        return (`<td style='${math_style}'>${s}</td><td style='${idnum_style}'>${idnum_text}</td>`);
      });
      var data = data.map(x => `<tr>${x}</tr>`);
