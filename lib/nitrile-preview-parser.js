...
class NitrilePreviewParser {
  ...
  toBlocks(lines,fname,subrow,plevel,o=null) {
    ...
    while (lines.length > 0) {
      ...
      if (v) {
        if (v[1] === 'caption') {
          caption = this.joinPara(para.slice(1));
        } else if (v[1] === 'label') {
          label = para.slice(1).join('');
          label = `${base}:${label}`;
        }
        continue
      }
      ...
      if (para.length >= 3 &&
        ...
        var [data, n] = this.trimPara(para);
        var fencecmd = {};
        var [data,adjust] = this.parseAltTABB(data);
        fencecmd['adjust'] = adjust;
        o.push([id,row,row2,'TABB',n,data,para,fencecmd,dept,fig,caption,base,label,fname,subrow,plevel]);
        caption = ''; label = '';
        continue;
  ...
  parseSample (para, config) {
    var o = [];
    const maxn = config.twocolumn ? 42 : 80;
    for (var s of para) {
      var leadn = 0;
      var v = /^(\s*)/.exec(s);
      if (v) {
      ...
      while (s.length > maxn) {
        ...
        var s2 = '';
        if (s.length > maxn) {
          var k = this.findSpaceBeforeWordAt(s, maxn);
          var s1 = s.slice(0,k);
          var s2 = s.slice(k);
        }
        o.push(s1);
        ...
        /// try to find the number of leading blanks
        /// in 's'
        s = s2;    
      } 
      o.push(s);
