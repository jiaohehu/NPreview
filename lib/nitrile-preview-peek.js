...
class NitrilePreviewPeek extends NitrilePreviewTranslator {
  ...
  do_part(block) {
    ...
    if(this.conf('partpage')){
      ...
      o.push(`<h1 ${this.to_attr(block)} > <small> Part ${idnum} </small> ${text} </h1>`);
    }
    o.push('');
    block.html = o.join('\n');
  }
  ...
  do_samp(block) {
    ...
    var lines = lines.map( x => this.escape_with_nbsp(x) );
    var text = lines.join('\n');
    o.push(`<pre class='SAMP' ${this.to_attr(block)} style='margin-left:${this.conf('step')}em;'>${text}</pre>`);
    o.push('');
    block.html = o.join('\n');
  }
  do_pass(block) {
    ...
    o.push(`<p class='PASS' ${this.to_attr(block)} style='margin-left:${this.conf('step')}em;'>${text}</p>`);
    o.push('');
    block.html = o.join('\n');
  }
  do_hrle(block) {
  ...
  do_quot(block) {
    ...
    text = `${lq}${text}${rq}`;
    var o = [];
    o.push(`<blockquote class='${sig}' ${this.to_attr(block)} style='margin-left:${this.conf('step')}em;right-margin:0;${this.xstyle}'>`);
    o.push(text);
    o.push(`</blockquote>`);
    o.push('');
    block.html = o.join('\n');
...
${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
  ...
  do_tabb(block){
    ...
    d.push(`</tbody>`);
    var text = d.join('\n');
    o.push(`<table class='TABB' ${this.to_attr(block)} border='0' style='border-collapse:collapse;width:100%;'>`);
    o.push(text);
    o.push(`</table>`);
    o.push('');
    block.html = o.join('\n');
  ...
  smooth_for_var(unsafe) {
    ...
    return unsafe
      ...
      .replace(/(?<!\w)(\w)_(\d)(?!\w)/g, (match,p1,p2) => {
