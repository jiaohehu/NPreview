...
`;
  ...
  do_samp(block) {
    ...
    if(parser.samp==1){
      ...
      o.push(`<p ${this.xattr} style='margin-left:${this.config.HTML.step}em;'>${text}</p>`);
    }
      ...
      var body = body.map( x => this.rubify(x)); 
      var text = body.join('\n');
      o.push(`<pre ${this.xattr} style='font-size:${this.config.HTML.fscode}em;margin-left:${this.config.HTML.step}em;'>${text}</pre>`);
    } 
    o.push('');
    block.html = o.join('\n');
  }
