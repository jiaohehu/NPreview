...
class NitrilePreviewParser extends NitrilePreviewBase {
  ...
  constructor() {
    ...
    this.sample = 0;///set to 1 if this document is detected to be a master document
    ...
    while (nlines < lines.length) {
      ...
      if(sig=='SAMP'){
        ...
        if(sig0==='FRMD'||
          ...
          /// re-parse the new 'body'

          if(sig0==='FRMD'){
            blk.lines = this.trim_samp_body(blk.body);
          }else if(sig0==='DIAG'){
            blk.lines = blk.body;
          }else if(sig0==='PICT'){
            var {opts,images} = this.parse_pict(blk.body);
            blk.opts=opts;
            blk.images=images;
          }else if(sig0==='TABR'){
            blk.cols = this.parse_tabr(blk.body);
          }else if(sig0==='LLST'){
            blk.lines = this.trim_samp_body(blk.body);
          }else{
            blk.lines = this.trim_samp_body(blk.body);
          ...
          /// re-insert the blk back
          o.push(blk);

          /// turn 'SAMP' to whatever 'sig0' is, so that
          /// at the end of this if where 'sig0=sig' statement
          /// executes 'sig0' is still the same
          sig = sig0;
        ...
        if(sig0 == 'LONG'){
          ...
          var blk = o.pop();
          blk.rows = blk.rows.concat(block.rows);
          blk.para = blk.para.concat(block.para);
          blk.row2 = block.row2;
          o.push(blk);
        }else{ 
  ...
  read_para (lines,n) {
    ...
    for (let i=0; n < lines.length; ++i,++n) {
      ...
      if(i==0 && (v=re_long_one.exec(line))!==null){
        type = 'long_one';
        body.push(v[2]);
        continue;  
      }
      ...
      if (i==0 && (v=re_data.exec(line))!==null){
        wide = (v[1].length==2) ? 1:0;
        type = 'data';
        body.push(v[2]);
        continue;
      }
    ...
    if(type == 'spcl'){
      ...
      var caption = this.join_para(body);
      var lines = [];
      if(fenceid == 'framed'){
      ...
      return {para,sig,wide,lines,caption,label,islabeled,floatname};
    } 
    else if (type == 'data'){
      var cols = this.parse_data(body,wide);
      return {para,sig,wide,caption,label,islabeled,cols,body};
    }
    else if(type == 'math'){
      ...
      let {rows,ww} = this.parse_long(body);
      return {para,sig,rows,ww,body};
    }
    else if(type == 'long_one'){
      let {rows,ww} = this.parse_long_one(body);
      sig = 'LONG';
      return {para,sig,rows,ww,body};
    }
    else if(type == 'tabb'){
      ...
      //double-check to make sure all lines are indented,
      //not just the first line
      if(this.is_samp_body(body)){
        var lines = this.trim_samp_body(body);
        return {para,sig,body,lines};
      }else{
        sig='TEXT';
        var text = this.join_para(body);
        return {para,sig,body,text};
      }
    } 
    else if(type == 'pass'){
      var lines = this.trim_pass_body(body);
      if(bull.length==2){
      ...
      console.err(`unhandled type: (${type})`);
      console.err(para);
  ...
  parse_long_one(para) {
    ...
    for (var j = 0; j < para.length; ++j) {
      ...
      var ww = pp.map(x => re_fr.test(x)?1:0);
      var n = ww.reduce((acc,cur) => acc+cur);
      if(n == pp.length){
        ww = pp.map(x => re_fr.exec(x)[1]);
        continue;
      }
      var ss = pp.map(x => re_bars.test(x)?1:0);
      var n = ss.reduce((acc,cur) => acc+cur);
      if(n == pp.length){
        continue;
      }
      rows.push(pp);
    ...
    if(ww.length==0 && rows.length){
      ww = '1'.repeat(rows[0].length).split('');
      ww = pp.map(x => parseInt(x));       
    }
  ...
  parse_data(para,wide){

    if(wide){  // separated by spaces
