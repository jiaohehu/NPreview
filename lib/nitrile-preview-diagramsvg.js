...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_box(opt,txt,ts,g,coords){   
    ...
    for (var i = 0; i < coords.length    ; i++) {
      ...
      label = label||'';
      if (label) {
        s.push(this.p_label(x,y,label,0,'ctr',g));
      }
    }
    return s.join('\n');
  }
  ...
  p_label(x, y, txt, ts, ta, g) {
    ...
    var gapx = 2; //hard coded to move away additional spaces from the anchor point
    var gapy = 2;
    var dx = 0;
    var dy = 0;
    if (ta==='lrt') {
  ...
  p_path(coords,p,g){
    ...
    for(var item of items){
      var {iscycled,d} = item; 
      if(iscycled){
        o.push(`<path d='${d}' ${this.to_filldraws(g)} ${x.join(' ')}/>`);
      } else {
        o.push(`<path d='${d}' ${this.to_drawonlys(g)} ${x.join(' ')}/>`);
      }
    }
    return o.join('\n');
  }
