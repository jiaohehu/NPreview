...
${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
  ...
  inlinemath (str,cssfontrate) {
    this.tokenizer.cssfontrate = cssfontrate;///a float between 0-1 such 0.8
    this.tokenizer.isdisplaymath = false;
    var s = this.tokenizer.parse(str);
    return s;
  }

  displaymath (str) {
    this.tokenzier.isdisplaymath = true;
    var s = this.tokenizer.parse(str);
    return s;
  }
...
///~~~
  ...
  make_line(maths,label,idnum){
    ...
    this.tokenizer.isdisplaymath = true;
    var {s,nw,nh,shiftdist} = this.tokenizer.toMathSvg(line);
