...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_label(opt,txt,coords){
    ...
    for (var i = 0; this.valid(coords, i); i++) {
      ...
      if ((v=this.re_inlinemath.exec(t))!==null) {
        ...
        if (opt==='.lrt') {
          s.push(`<text text-anchor='start'  ${this.texts()} x='${x}' y='${y}' dx='2'    dy='0.9em'>${t}</text>`);
        } else if (opt==='.bot') {
          s.push(`<text text-anchor='middle' ${this.texts()} x='${x}' y='${y}'           dy='0.9em'>${t}</text>`);
        } else if (opt==='.llft') {
          s.push(`<text text-anchor='end'    ${this.texts()} x='${x}' y='${y}' dx='-2'   dy='0.9em'>${t}</text>`);
        } else if (opt==='.urt') {
  ...
  do_drawanglearc(opt,txt,coords) {
    ...
    if (opt==='.sq') {
      ...
      var x1 = x0 + 0.75*radius * Math.cos(ang1 / 180 * Math.PI);
      var y1 = y0 + 0.75*radius * Math.sin(ang1 / 180 * Math.PI);
      var x2 = x0 + 0.75*radius * Math.cos(ang2 / 180 * Math.PI);
      var y2 = y0 + 0.75*radius * Math.sin(ang2 / 180 * Math.PI);
