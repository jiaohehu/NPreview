    ...
    while ((v = re_unmask.exec(line)) !== null) {
      ...
      if (v[1] !== undefined) {
        ...
        if(key.length==1){
          ...
          if(ss.length){
            let nss = ss.length;
            let {fenceid,style} = this.str_to_fenceid(ss[0]);
            if(fenceid==='diagram'){
              newtext += this.to_diagram(ss.slice(1,nss-1),style);
            }else if(fenceid==='framed'){
              newtext += this.to_framed(ss.slice(1,nss-1),style);
            }else if(fenceid==='itemized'){
              let myss = ss.slice(1,nss-1);
              let myitms = this.para_to_itemized(myss);
              newtext += this.to_itemized(myitms,style);
            }else if(fenceid==='math'){
              newtext += this.to_math(ss.slice(1,nss-1),style);
            }else{
              newtext += this.to_verbatim(ss.slice(1,nss-1),style);
            }
          }
        }
      } 
