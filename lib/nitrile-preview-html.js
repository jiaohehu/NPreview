...
`
  ...
  translateHtml (all,config,ispreview=false,editorrow=-1,editorcolumn=-1) {
    ...
    for (var block of all) {
      ...
      const [id,row1,row2,sig,_subrow,data,para,_fencecmd,_caption,base,_label,fname] = block;
      this.block = block;
      this.base = base;
      ...
      switch (sig) {
        ...
        case 'LIST': {
          ...
          o.push(`${showidtext}`);
          o.push(`<p style='${this.paracssstyle}' >`);
          o.push(`<ul style='padding-left:${config.latex.stepMargin}cm;list-style-position:outside;'>`);
        ...
        case 'PLST': {
          ...
          for (var item of data) {
            ...
            if (item.length === 3) {
              switch (lead) {
                case 'OL': {
                  o.push(`<ol style='padding-left:${config.latex.stepMargin}cm;list-style-position:outside;'><li value='${bullet}'>${text}`);
                  break;
                }
                case 'UL': {
                  o.push(`<ul style='padding-left:${config.latex.stepMargin}cm;list-style-position:outside;'><li value='${bullet}'>${text}`);
                  break;
                }
                case 'LI': {
...
//console.log(data);
        ...
        case 'tabular':
          ...
          var text = this.toFramedTable(rows,maxj,ww,vlines,hlines,vpadding,fs,sig);
          o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='${this.divcssstyle}; '>`);
          o.push(`${showidtext}`);
          o.push(text);
          o.push('</div>');
        ...
        case 'quot': {
          ...
          var xleft = config.latex.stepMargin;
          o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='${this.divcssstyle}; '>`);
          o.push(`${showidtext}`);
          for (var p of text) {
        ...
        case 'equations': 
          o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='${this.divcssstyle};position:relative;' >`);
          o.push(`${showidtext}`);
          ...
          for (var j in text) {
            ...
            if (sig==='equations'){
              var num = this.autonum.idenEquation(this.isarticle);
              var subnum = '';
            ...
            ///always attached the equation number to the last cell
            s[2] = `${s[2]}${numtext}`;
        ...
        case 'subequations': 
          ...
          for (var j in text) {
            ...
            if (sig==='equations'){
              var num = this.autonum.idenEquation(this.isarticle);
              var subnum = '';
              var numtext = `<span style='position:absolute;right:0pt;top:0;'>(${num}${subnum})</span>`;
            } else {
              var num = num || this.autonum.idenEquation(this.isarticle);
              var subnum = this.toSubfigNum(j);
              var numtext = `<span style='position:absolute;right:0pt;top:0;'>(${num}${subnum})</span>`;
        ...
        case 'math': 
          o.push(`<div id='${id}' class='${sig}' dept='${dept}' fName='${fname}' rows='${row1} ${row2}' style='${this.divcssstyle};' >`);
          o.push(`${showidtext}`);
          o.push(`<table border='0' style='${this.paracssstyle};width:100%;position:relative;' >`);
          var text = data;
          var j = 0;
          var num = '';
          for (var j in text) {
            ...
            s = s.map((x,i)=>`<td style='text-align:${align_vals[i]};padding-right:${paddn_vals[i]}'>${x}</td>`);
            s = s.join('');
            s = `<tr>${s}<td style='position:relative;'></td></tr>`;
            o.push(s);
          }
          if (sig==='equation') {
            var num = num || this.autonum.idenEquation(this.isarticle);
            var numtext = `<span style='position:absolute;right:0pt;'>(${num})</span>`;
            o.push(numtext);
          } 
          o.push(`</table>`);
          o.push('</div>');
  ...
  */
  ...
  toFramedTable (rows, maxj, ww, vlines, hlines, vpadding, fs, sig) {
    rows = rows.map ( row => {
    ...
    if (sig==='longtable') {
      o.push(`<table border='0' style='${this.paracssstyle}; ${this.tablecssstyle}; font-size:${fs}; width:100%; ' >`);
    }else{
      o.push(`<table border='0' style='${this.paracssstyle}; ${this.tablecssstyle}; font-size:${fs}; margin:0 auto; ' >`);
    }
    var header = rows.shift();
    var header = header.map(x => `<strong>${x}</strong>`);
    var header = header.map((x,i) => `<td style='text-align:left;\
padding:${t}px 6px;\
...
${hlines.indexOf('m')>=0?'border-bottom:1px solid black;':''} '>${x}</td>`);
    ...
    for (var j=0; j < rows.length; ++j) {
      var row = rows[j];
      var row = row.map((x,i) => `<td style='text-align:left;\
padding:${t}px 6px;\
${(sig==='longtable')?'width:'+ww[i]+'%;':''}
${vlines.indexOf(i)>=0?'border-left:1px solid black;':''}\
${vlines.indexOf(i+1)>=0?'border-right:1px solid black;':''}\
