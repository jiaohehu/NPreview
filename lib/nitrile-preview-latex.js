...
class NitrilePreviewLatex extends NitrilePreviewParser {
  ...
  do_dlst(block){
    ...
    for (var item of items) {
      ...
      if(type=='text'){
        ...
        key = this.escape(key);
        key = this.polish(key,this.config.LATEX.fscode);
        text = this.unmask(text);
  ...
  do_nlst(block){
    ...
    o.push(this.to_info(block));
    o.push(`\\begin{enumerate}[leftmargin=${this.config.LATEX.step}mm]`);
    for (var item of items) {
  ...
  _do_verb(block){
    ...
    o.push(this.to_info(block));
    var fs = this.config.LATEX.fslisting;
    var s = [];
    if(1){
      s.push(`\\begin{xltabular}[l]{\\linewidth}{@{} >{\\${fs}}l >{\\${fs}}l @{}}`);
      if(this.floatname){
  ...
  _do_verb(block){
    ...
    if(1){
      ...
      body.forEach(line => {
        ...
        line = this.smooth_samp(line);
        line = this.fontify_latex(line);
        line = this.polish(line,fs);
        lineno = this.polish(lineno,fs);
        d.push(`${lineno} \\> ${line}\\\\`);
      });
    } 
    var text = d.join('\n');
    ...
    if(1){
      ...
      o.push('\\begin{tabbing}');
      linenum = this.polish(linenum,fs);
      o.push(`${linenum}0 \\= \\kill`);
      o.push(text);
      o.push('\\end{tabbing}');
  ...
  do_verb(block){
    ...
    o.push(this.to_info(block));
    var n = 0;
    const maxn = parseInt(this.config.LATEX.maxn);
    ...
    while(n < body.length){
      ...
      if(1){
        o.push(`\\begin{Program${star}}[ht]`);
        if(this.floatname){
          o.push(`\\caption*{${cont}${this.caption_text}}${latexlabelcmd}`);
  ...
  do_pict(block){
    ...
    for(var blk of rows){
      ...
      var qq = images.map( img => {
        ...
        var sub_text = this.polish(sub_text,this.config.LATEX.fssubcaption);
        return (`${sub_text}`);
      });
      s.push(pp.join(' & '));
      s.push(`\\\\`);
  ...
  escape_diag(text){
    var fs = this.config.LATEX.diagfontsizept;
    var fs = `${fs}pt`;
    var text = this.smooth(text);
    var text = this.fontify_latex(text);
  ...
  */
  ...
  toExtraPackages(config) {
    var extra = config.LATEX.extra;
    return extra;
  }
