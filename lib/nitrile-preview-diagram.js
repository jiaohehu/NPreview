...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  constructor(translator) {
    ...
    // clippath, path, fn, var
    this.re_clippath_command = /^clippath\s+(.*?)\s*\=\s*(.*)$/;
  ...
  parse_line(line,env={}) {
    ...
    if ((v = this.re_clippath_command.exec(line)) !== null) {
      var symbol = v[1];
      line = v[2];
      var coords = this.readCoordsLine(line);
      if (this.re_word.test(symbol)) {
        this.my_clippaths.set(symbol, coords);
        this.commands.push(this.do_comment(`clippath ${symbol}=${this.coords_to_string(this.my_clippaths.get(symbol))}`));
      } else {
