...
`;
  ...
  do_samp(block) {
    ...
    if (this.features.mode1) {
      ...
      text = text.join('\n');
      o.push(`<dl id='${id}' class='P ${sig}' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' style='margin-left:${fencecmd.left}mm;font-size:${fencecmd.cssfontsize};'>${text}</dl>`);
      o.push('');
    } else if (this.features.mode3) {
      text = this.joinBackslashedLines(text);
      text = text.map( x => this.escape(x) );
  ...
  do_quot(block) {
    ...
    text = `${lq}${text}${rq}`;
    var o = [];
    o.push(`<blockquote id='${id}' class='P ${sig}' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' style='margin-left:${fencecmd.left}mm;margin-right:${fencecmd.right}mm;font-size:${fencecmd.cssfontsize};'>${text}</blockquote>`);
    o.push('');
    block.html = o.join('\n');
  }
  do_tabb(block){
...
${j==nrows-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</${j==0?'th':'td'}>`);
  ...
  do_math(block) {
    ...
    text = `<table style='margin-left:${fencecmd.left}mm;margin-right:0;border-collapse:collapse;width:100%;'>${text}</table>`;
    o.push(`<figure id='${id}' class='P ${sig}' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' style='margin-left:0;margin-right:0;margin-top:${this.xtop};margin-bottom:${this.xbot};'>`);
    o.push(text);
    o.push('</figure>');
    o.push('');
...
///~~~
  ...
  toMathTabularAligned(data) {
    var data = data.map( row => row.map((x,i) => {
        x = this.tokenizer.parse(x,true);
        var align = (i==0)?'right':'left';
        if(row.length===1) {
          align = 'left';
        }
        return (`<td style='text-align:${align};padding-right:2pt;'>${x}</td>`);
    }));
    return data;
  }
