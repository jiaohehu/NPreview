...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  constructor(translator) {
    super(translator);
    this.tokenizer = new NitrilePreviewTokenizer(translator);//a new tokenizer because tokenizer.fs is set differently than that of Html
    this.re_unit = /^(\d+)mm$/;
    this.re_inlinemath = /^\`\`(.*)\`\`$/;
  }
  ...
  do_finalize(s) {
    ...
    if(this.config.grid && this.config.grid == 'boxed'){
      for (var x = 0; x <= this.config.width; x+=this.config.width) {
        x1 = x * u;
        x2 = x * u;
        y1 = 0;
        y2 = this.config.height * u;
        o.push(`<line x1='${x1}' y1='${y1}' x2='${x2}' y2='${y2}' stroke-opacity='0.10' stroke='inherit'/>`);
      }
      for (var y = 0; y <= this.config.height; y+=this.config.height) {
        y1 = y * u;
        y2 = y * u;
        x1 = 0;
        x2 = this.config.width * u;
        o.push(`<line x1='${x1}' y1='${y1}' x2='${x2}' y2='${y2}' stroke-opacity='0.10' stroke='inherit'/>`);
      }
  ...
  coordsToD(coords,multi=false) {
    ...
    for (var i in coords) {
      ...
      if (i == 0) {
        ...
        var sweepflag  = pt[11];        
        o.push(`A${this.fix(Rx)},${this.fix(Ry)},${-this.fix(Phi)},${this.fix(bigarcflag)},${this.fix(sweepflag)},${this.fix(x)},${this.fix(y)}`);
        x0 = x;
        y0 = y;
      }
      else {
  ...
  p_label(x, y, txt, ts, ta, g) {
    ...
    var gapx = this.config.labeldx; //hard coded to move away additional spaces from the anchor point
    var gapy = this.config.labeldy;
    var dx = 0;
    var dy = 0;
    if (ta==='lrt') {
    ...
    var d = [];
    var myfontsizept=this.translator.conf('diagfontsizept');
    if (ts==2) {//math
      this.tokenizer.fs = myfontsizept;
      var {s,w,h,defs} = this.tokenizer.to_math_svg(txt);
      ...
      if(ts==1){//var
        txt = this.translator.smooth_for_var(txt);
      }else{
      ...
      if(ts==1){//var
        d.push(`<text font-size='${myfontsizept}pt' font-style='italic' text-anchor='${anchor}' ${this.to_textdraws(g)} x='${x}' y='${y}' dy='${dy}'>${txt}</text>`);
      }else{
        d.push(`<text font-size='${myfontsizept}pt' text-anchor='${anchor}' ${this.to_textdraws(g)} x='${x}' y='${y}' dy='${dy}'>${txt}</text>`);
      }
    }
    return d.join('\n');
  }
  ...
  p_line(x1,y1,x2,y2,g){
    ...
    o.push(`<line x1='${this.fix(x1)}' y1='${this.fix(y1)}' x2='${this.fix(x2)}' y2='${this.fix(y2)}' ${this.to_drawonlys(g)} />`);
    return o.join('\n');
  }
