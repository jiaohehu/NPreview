...
`;
  ...
  do_hdgs(block) {
    var {id,row1,row2,sig,part,level,title,dept,subfname} = block;
    var o = [];
    var text = this.escape(title);
    if(subfname && this.config.HTML.trace){
      text += ` <span style='font-size:.8em;font-style:italic'>${subfname}</span>`;
    }
    if(part){
      o.push(`<h2 ${this.xattr} style='${this.xstyle}'>Part ${dept} &#160; ${text}</h2>`);
    } else if(level==0) {
      o.push(`<h2 ${this.xattr} style='${this.xstyle}'>${text}</h2>`);
    } else if(level==1) {
  ...
  do_verb(block) {
    ...
    for (var k=0; k < text.length; ++k) {
      ...
      //if (fencecmd.monospace) {
        //line = `<code>${line}</code>`;
      //} 
      line = `<td style='white-space:pre;'>${line}</td>`;
      if(this.xnumbers){
        var ln = ++linenum;
        ln = this.escape(ln);//so that it gets the cssfontsize
        line = `<td><small>${ln}</small></td> <td>&#160;&#160;</td> ${line}`;
      }
      line = `<tr>${line}</tr>`;
      s.push(line);
    }
    ...
    o.push(`<figure ${this.xattr} style='${this.xstyle}'>`);
  ...
  do_samp(block) {
    var {id,rmap,row1,row2,sig,body} = block;
    var o = []; 
    if(this.xfencecmd.style==1){
      body = this.trimSampPara(body);
      body = this.getMode1SampPara(body);
      ///**make sure not to use escapeBRTT because it does not wrap line
      ///when viewed by calibre
      body = body.map( x => this.escapeBR(x) );
      body = body.map( x => this.rubify(x,rmap)); 
      body = body.map( x => `<tr><td>${x}</td></tr>` );
      body = body.join('\n');
    }
    else{
      body = this.trimSampPara(body);
    ...
    body = `<table border='0' style='border-collapse:collapse;'>${body}</table>`;
    o.push(`<figure ${this.xattr} style='${this.xstyle}'>`);
    o.push(body);
    o.push(`</figure>`);
    o.push('');
  ...
  _do_samp(block) {
    ...
    if(fencecmd.style==1){
      ...
      ///**make sure not to use escapeBRTT because it does not wrap line
      ///when viewed by calibre
      text = text.map( x => this.escapeBR(x) );
      text = text.map( x => this.rubify(x,rmap)); 
      text = text.map( x => `<dt>${x}</dt>` );
      text = text.join('\n');
      o.push(`<dl ${this.xattr} style='${this.xstyle};'>${text}</dl>`);
      o.push('');
    }
    else{
      text = this.trimSampPara(text);
      text = text.map( x => this.escapeBRTT(x) );
      text = text.map( x => this.rubify(x,rmap)); 
      text = text.map( x => `<dt>${x}</dt>` );
      text = text.join('\n');
      o.push(`<dl ${this.xattr} style='${this.xstyle};'>${text}</dl>`);
  ...
  do_quot(block) {
    ...
    text = `${lq}${text}${rq}`;
    var o = [];
    o.push(`<blockquote ${this.xattr} style='${this.xstyle}'>`);
    o.push(text);
    o.push(`</blockquote>`);
    o.push('');
    block.html = o.join('\n');
...
${j==nrows-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</${j==0?'th':'td'}>`);
  ...
  do_diag(block){
    var {id,row1,row2,sig,body,label,subfname} = block;
    var o = []; 
    if (0) {
      (new NitrilePreviewDiagramSVG(this,this.tokenizer)).def_pic(body);
      block.html = o.join('\n');
      return;
    }
    var [text,vw,vh] = new NitrilePreviewDiagramSVG(this,this.tokenizer).parse(body);
    var s = [];
  ...
  _do_math(block) {
    ...
    if (fencecmd.name === 'equation') {
      var mydata = this.toMathTabularAligned(data,fencecmd);
  ...
  fontifyHTML (s) {
    ...
    var opts = [];
    if(this.xcssfontsize){
      opts.push(`font-size:${this.xcssfontsize}em`);
    }
    if(this.xmonospace){
  ...
  escape (text) {
    ...
    var text = this.fontifyHTML(text);
    return text;
  }
...
///~~~
  ...
  make_math(maths,label,idnum,more){
    ...
    var data = data.map( x => {
        ...
        return (`<td style='left:${dist}px;position:relative;text-align:left;'>${s}</td><td style='width:100%;text-align:right;'>(${idnum})</td>`);
    });
...
`;
  ...
  toTOC(){
    var items = this.blocks.map(x => {
      let {id,sig,dept,data} = x; 
      data = this.escape(data);
      if(sig=='HDGS'){
        return `<li><a href='#${id}'>${dept} ${data}</a></li>`;
      } else {
        return '';
      }
    });
