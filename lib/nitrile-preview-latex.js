...
class NitrilePreviewLatex extends NitrilePreviewTranslator {
  ...
  do_hdgs(block){
    var o = [];
    var {id,row1,row2,sig,name,subn,hdgn,dept,text,para,label} = block;
    ...
    if(name=='part'){
      ...
      if(name=='h'){
        o.push(`\\chapter${star}{${dept}${text}}${this.to_latexlabelcmd(label)}`);
        if(star) o.push(`\\addcontentsline{toc}{chapter}{${dept} ${raw}}`);
      }else{
      ...
      o.push(`\\section${star}{${dept}${text}}${this.to_latexlabelcmd(label)}`);
      if(star) o.push(`\\addcontentsline{toc}{section}{${dept} ${raw}}`);
      o.push('%');
    }
    else if(level==2){
      o.push(`\\subsection${star}{${dept}${text}}${this.to_latexlabelcmd(label)}`);
      if(star) o.push(`\\addcontentsline{toc}{subsection}{${dept} ${raw}}`);
      o.push('%');
    }
    else{
      o.push(`\\subsubsection${star}{${dept}${text}}${this.to_latexlabelcmd(label)}`);
      o.push('%');
    }
    block.latex = o.join('\n');
  }
  do_dlst(block){
    ...
    for (var item of items) {
      ...
      if(type=='text'){
        ...
        text = this.unmask(text);
        if(rb&&rt){
          o.push(`\\item[${bull} {\\jp{}\\ruby{${rb}}{${rt}}}] ${nbsp}${text}`);
        } else {
          o.push(`\\item[${bull} {\\jp{}${key}}] ${nbsp}${text}`);
        }
      }else if(type=='quot'){
        key = this.escape(key);
        text = this.unmask(text);
