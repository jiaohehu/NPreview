    ...
    for (var block of this.parser.blocks) {
      ...
      if (sig === 'HDGS') {
        ...
        if(name=='part'){
          ...
          level = 1 + hdgn;  // level starts with 2
          block.name = 'h';
          block.level = level;
        ...
        } else if (sig  === 'DIAG' && float) {
          block.idnum = this.idenFigure();
          block.floatname = 'Figure';
        } else if (sig  === 'FRMD' && float) {
          block.idnum = this.idenFigure();
          block.floatname = 'Figure';
        } else if (sig === 'VERB' && float) {
          block.idnum = this.idenListing();
          block.floatname = 'Listing';
        } else if (sig === 'TABR' && float) {
          block.idnum = this.idenTable();
          block.floatname = 'Table';
        } else if (sig == 'MATH' && label) {
          block.idnum = this.idenEquation();
          block.floatname = 'Equation';
          if(block.more){
            var n = 0;
