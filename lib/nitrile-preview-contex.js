...
class NitrilePreviewContex extends NitrilePreviewParser {
  ...
  do_hdgs(block){
    var o = [];
    var {id,row1,row2,sig,name,level,hdgn,dept,text,subfname} = block;
  ...
  do_dlst(block){
    ...
    for (var item of items) {
      ...
      if(type=='text'){
        ...
        text = this.unmask(text);
        if(rb&&rt){
          o.push(`\\latexdesc{-} \\switchtobodyfont[jp]{\\ruby{${rb}}{${rt}}} ${text} \\par`);
        } else {
          o.push(`\\latexdesc{-} \\switchtobodyfont[jp]{${key}} ${text} \\par`);
        }
      }else if(type=='quot'){
        key = this.escape(key);
        text = this.unmask(text);
        o.push(`\\latexdesc{-} \\quotation{${key}} ${text} \\par`);
      }else if(type=='emph'){
        key = this.escape(key);
        text = this.unmask(text);
        o.push(`\\latexdesc{-} {\\it{}${key}} ${text} \\par`);
      }else if(type=='code'){
        key = this.escape(key);
        text = this.unmask(text);
        o.push(`\\latexdesc{-} {\\tt{}${key}} ${text} \\par`);
      }
    }
    o.push('');
    block.latex = o.join('\n');
  ...
  do_nlst(block){
    ...
    for (var item of items) {
      ...
      if(type=='nlst'){
        ...
        body = body.map( x => this.polish_code(x) );
        body = body.map( x => (x)?x:'~');
        body = body.map( x => `{\\tt ${x}}`);
        body = body.join('\n');
  ...
  do_text(block){
    var o = [];
    var {id,row1,row2,sig,leadn,lead,text,subfname} = block;
    o.push(this.to_info(block));
    var v;
  ...
  _do_csvd(block){
    ...
    for (var j = 0; j < nrows; ++j) {
      ...
      pp = pp.map(x => this.polish_prog(x));
      pp = pp.map(x => `\\NC ${x}`);
      var p = pp.join(' ');
      p = `${p} \\NC\\NR`;
      s.push(p);
  ...
  do_csvd(block){
    ...
    var h=3;
    var t=0;
    s.push(`\\bTABLE[loffset=${h}pt,roffset=${h}pt,toffset=${t}pt,boffset=${t}pt,split=yes]`);
    for (var j = 0; j < nrows; ++j) {
      var pp = cols.map(x => x[j] || '');
      pp = pp.map(x => this.escape_verb(x));
      pp = pp.map(x => this.polish_prog(x));
      pp = pp.map(x => `\\bTD ${x} \\eTD`);
      s.push(`\\bTR ${pp.join(' ')} \\eTR`);
    }
    s.push(`\\eTABLE`);
    var text = s.join('\n');
    o.push(`\\blank`);
  ...
  _do_tabr(block){
      ...
      if(j==0){
        ...
        if(maxn > 1){
          ...
          for(var n=0; n<maxn; ++n){
            ...
            pp = pp.map(x => this.polish_tabb_header(x));
    ...
    if(this.floatname){
      ...
      o.push(`{${this.polish_caption(this.caption_text)}}`);//already unmasked
      o.push('{%');
      o.push(text);
      o.push('}');
    } else {
  ...
  do_long(block){
    ...
    rows = rows.map((row,i) => row.map(x => (i==0)?this.polish_tabb_header(x):this.polish_tabb(x))); 
  ...
  do_pict(block){
    ...
    for(var blk of rows){
      ...
      for(var img of images){
        ...
        if(mode && mode.width){
          var mywidth = this.toContexLength(`${mode.width*frac}%`);
        }else{
    ...
    if(this.floatname){
      o.push(`\\placefloat`);   
      o.push(`[here]`);   
      o.push(`[${this.label_text}]`);//with # stripped off already
      o.push(`{${this.polish_caption(this.caption_text)}}`);//already unmasked
      o.push('{%');
      o.push(text);
      o.push('}');
    ...
    for (var j=0; j < this.blocks.length; ++j) {
      ...
      if(sig=='MATH'){
        if( str.localeCompare(label)===0) {
          return `${floatname}.\\in[${str.slice(1)}]`;
          break;
        }
        if(more&&more.length){
        ...
        if( str.localeCompare(label)===0) {
          ...
          return `${secsign}{\\goto{${idnum}}[${str.slice(1)}]}`;
          break;
        }
      }else{
        if( str.localeCompare(label)===0) {
          return `${floatname}.{\\goto{${idnum}}[${str.slice(1)}]}`;
          break;
        }
      }
    }
  ...
  math_diag(text){
    ...
    var fs = this.conf('diagfontsizept');
    var s = `{\\switchtobodyfont[${fs}pt]\\math{${s}}}`;
    return s;
  }

  inlinemath(str) {
    var s = this.tokenizer.toCmath(str);
    return `\\math{${s}}`;
  }
  ...
  */
  ...
  toContexDocument() {
    ...
    /// two column setting
    var istwocol = this.conf('twocolumns');
    ...
    for(var i=0; i < this.blocks.length; ++i){
      ...
      if(this.conf('twocolumns') && sig=='HDGS' && hdgn==0){      
    ...
    if(this.conf('twocolumns') && my_chapters.length){
      my_chapters.pop();
      texlines.push(`\\stopcolumns`);
    }
...
\\setuphead[chapter][style=${this.conf('chapter')},number=no]
\\setuphead[section][style=${this.conf('section')},number=no]
\\setuphead[subsection][style=${this.conf('subsection')},number=no]
\\setuphead[subsubsection][style=${this.conf('subsubsection')},number=no]
\\setuphead[subsubsubsection][style=${this.conf('subsubsubsection')},number=no]
...
\\stoptext
  ...
  make_line(math,label,mainlabel){
    if (label == '#') {
      label = '[+]';
      var islabel=1;
    } else if (label) {
  ...
  make_math(o,math,label,more,gather){
    ...
    ///   \NC a_2 x + b_2 y \NC = c_2 \NR[eq:a]
    ///   \stopmathalignment \stopformula
    var line = this.make_line(math,label,label);
    o.push(line);
    more.forEach(x => {
      var line = this.make_line(x.math,x.label,label);
      o.push(line);
    });
  }
  ...
  polish_tabb_header(s){
    return this.polish(s,this.conf('nitabb'),'bf');
  }
  polish_code(s){
    return this.polish(s,this.conf('nicode'));
  }
  polish_prog(s){
