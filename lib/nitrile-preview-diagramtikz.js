    ...
    for (var i in coords) {
      ...
      if (i == 0) {
        ...
        if (sweepflag) {//clockwise
          o.push(`[rotate around={${this.fix(Phi)}:(${this.fix(x0*unit)}mm,${this.fix(y0*unit)}mm)}] arc [start angle=${ang2},end angle=${ang1},x radius=${this.fix(Rx*unit)}mm,y radius=${this.fix(Ry*unit)}mm];`);
        } else {//counter-clockwise
          o.push(`[rotate around={${this.fix(Phi)}:(${this.fix(x0*unit)}mm,${this.fix(y0*unit)}mm)}] arc [start angle=${ang1},end angle=${ang2},x radius=${this.fix(Rx*unit)}mm,y radius=${this.fix(Ry*unit)}mm];`);
        }
        x0 = x;
        y0 = y;
      }
