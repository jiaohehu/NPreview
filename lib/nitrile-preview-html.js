...
${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
  ...
  to_math_svg_array(line,isdisplaymath,cssfontrate){
    ...
    d = d.map( md => {
      ...
      opt += ` style='vertical-align:${va}pt; outline:${this.config.HTML.mathoutline}px solid orange'`;
  ...
  to_math_vertical_align(h,mid){
    var descent = (1 - this.tokenizer.dy)*this.config.HTML.bodyfontsizept;
    var va = -descent-(h-this.config.HTML.bodyfontsizept)/2;
    va -= h/2 - mid;
    return va;
  }
