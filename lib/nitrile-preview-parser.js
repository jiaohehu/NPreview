...
//console.log('mode',mode);
  ...
  getNextComponent(line){
    ...
    if(line.charCodeAt(0)==123){
      ...
      while(i < line.length){
        ...
        if(line.charCodeAt(i)==92 && line.charCodeAt(i+1)==123){
          i+=2;
        }else if(line.charCodeAt(i)==92 && line.charCodeAt(i+1)==125){
          i+=2;
      ...
      if(s.charCodeAt(s.length-1)==125){
        s = s.substring(1,s.length-1);
      }else{
        s = s.substring(1,s.length);
      }
      return [s,line.slice(i)];
    }
  ...
  splitLineComponents (line) {
    ...
    var i = 0;
    while(line.length > 0){
      var [s,line] = this.getNextComponent(line);
      o.push(s);
    }
    o = o.map(x => x.trim());
    return o;
  ...
  */
  ...
  parseTabbPara(para){
    ...
    var data = data.map(row => this.splitLineComponents(row));
