...
class NitrilePreviewContex extends NitrilePreviewParser {
  ...
  do_hdgs(block){
    ...
    if(level == 0) {
      ...
      if(this.config.CONTEX.pages){
        block.head = 'part';
        o.push(`\\startpart[title={Part ${dept}~~${text}},reference={${this.xbaselabel}},bookmark={Part ${dept} ${raw}}]`);
        o.push(`\\dontleavehmode`);
        o.push(`\\blank[60mm]`);
        ...
        block.head = 'section';
        o.push(`\\startsection[title={Part ${dept}~~${text}},reference={${this.xbaselabel}},bookmark={Part ${dept} ${raw}}]`);
      }
    } else if(level == 1){
      if(this.config.CONTEX.pages){
        block.head = 'chapter';
