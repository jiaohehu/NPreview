...
//console.log('nmid1_=',nmid1_);
      ...
      if (id === 'myINT') { /// THIS is \\int symbol by itself, it SHOULD also be enlarged to match the 
                            /// height of the next formular if possible but current it is only shown to be at a height of 14pt hardcoded --TODO
        
        var [id1_,w1_,op1_] = this.findIdByElement('\\int');
        var h1_ = 12;
        var mid1_ = 6;

        var nw1_   = w1_    ;
        var nh1_   = h1_    * this.int_rate;
        var nmid1_ = mid1_  * this.int_rate;
        ...
        if(1){
          var sy = nh1_/12;///as a ratio to 12pt, thus if it is 13pt height then the ratio is 13/12
          o.push(`<use transform='scale(1,${sy})' xlink:href='#${id1_}' />`);
        }
        s = o.join('\n');
        return [w,h,mid,s,op1_,1,id];
