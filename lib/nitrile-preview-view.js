    ...
    if (!this.isBookBlock) {
      ...
      this.generateContentsBlocks(bookBlock,this.dirname())
        .then((out) => {
            ...
            this.htmls.push(`${parser.escape(new Date().toLocaleDateString())}`);
            this.htmls.push(`</p>`);
            this.htmls = parser.translateOutlineOnly(contentsBlocks,this.htmls);
            this.htmls.push(`</div>`);
            const { scrollTop } = this.element;
            this.element.textContent = ''
            this.element.innerHTML = this.htmls.join('\n');
...
console.log("generateContentsBlocks");
    ...
    for (var line of data) {
      ...
      if (v) {
        ...
        if (dirname && fname1) { 
          ...
          var data1 = '';
          try {
            await fs.statSync(ffname1);
            var data1 = await fs.readFileSync(ffname1,'utf8');
          } catch(e) {
            var parser1 = new NitrilePreviewParser(); 
            contentsBlocks.push( parser1.toErrorBlock(`No file: '${ffname1}'`,fname1,plevel1));
            console.log(e.stack);
            continue;
          }
          var parser1 = new NitrilePreviewParser(); 
  ...
  async saveAs (savefilepath) {
    ...
    try {
      ...
      if (this.saveastype === 'lualatex-article') {
        ...
        this.blocks = translator.autoBLOCKS(this.blocks,true);
