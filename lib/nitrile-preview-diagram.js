...
class NitrilePreviewDiagram extends NitrilePreviewBase {
  ...
  initConfig(){
    ...
    this.config.noderadius = 1; // in grid
    this.config.boxwidth   = 1; // in grid
    this.config.boxheight  = 1; // in grid
  }
  ...
  is_arg_coords(a){
    if(a.coords && Array.isArray(a.coords) && a.coords.length){
      return true;
    }
    return false;
  }
  ...
  is_arg_scalar(a){
    if(a.scalar && Number.isFinite(a.scalar)){
      return true;
    }
    return false;
  }
  ...
  exec_path_func(line, fun_str, arg_str) {

    var ret_val = [];
    var args = arg_str.split(',');
    args = args.map(x => x.trim());
    args = args.filter(x => x.length?true:false);
    switch (fun_str) {

      case 'midpoint':

        var myargs = args.map(x => this.fetch_arg(x));
        var z0,z1,fraction;
        if(args.length == 1 && this.is_arg_coords(myargs[0])){
          var z0 = this.point(myargs[0].coords, 0);
          var z1 = this.point(myargs[0].coords, 1);
          var fraction = 0.5;
        } else if(args.length == 2 && this.is_arg_coords(myargs[0]) && this.is_arg_scalar(myargs[1])){
          var z0 = this.point(myargs[0].coords,0);
          var z1 = this.point(myargs[0].coords,1);
          var fraction = myargs[1].scalar;
        } else if(args.length == 2 && this.is_arg_coords(myargs[0]) && this.is_arg_coords(myargs[1])){
          var z0 = this.point(myargs[0].coords,0);
          var z1 = this.point(myargs[1].coords,0);
          var fraction = 0.5;
        } else if(args.length == 3){
          var z0 = this.point(myargs[0].coords, 0);
          var z1 = this.point(myargs[1].coords, 0);
          var fraction = myargs[2].scalar;
        }
      ...
      case 'perpoint':
        ...
        var myargs = args.map(x => this.fetch_arg(x));
        if (args.length == 3 && 
        this.is_arg_coords(myargs[0]) && 
        this.is_arg_coords(myargs[1]) &&
        this.is_arg_scalar(myargs[2])) {
          var z0, z1, magni;
          var z0 = this.point(myargs[0].coords, 0);
          var z1 = this.point(myargs[1].coords, 0);
          var magni = myargs[2].scalar;
        ...
        this.is_arg_coords(myargs[0]) &&
        this.is_arg_coords(myargs[1]) &&
        this.is_arg_coords(myargs[2])) {
          var z0 = this.point(myargs[0].coords, 0);
          var z1 = this.point(myargs[1].coords, 0);
          var z2 = this.point(myargs[2].coords, 0);
          var z0x = parseFloat(z0[0]);
          var z0y = parseFloat(z0[1]);
          var z1x = parseFloat(z1[0]);
          var z1y = parseFloat(z1[1]);
          var z2x = parseFloat(z2[0]);
          var z2y = parseFloat(z1[1]);
      ...
      case 'shiftpoints':
        ...
        if (args.length == 3) {
          var coords = this.fetch_path(args[0]);
          var dx = parseFloat(args[1]);
          var dy = parseFloat(args[2]);
          if (coords && Number.isFinite(dx) && Number.isFinite(dy)) {
      ...
      case 'allpoints':
        ...
        for (var i in args) {
          var coords = this.fetch_path(args[i]);
          if (coords) {
            for (var j in coords) {
      ...
      case 'lineintersect': {
        ...
        if (args.length == 2) {
          var coords0 = this.fetch_path(args[0]);
          var coords1 = this.fetch_path(args[1]);
      ...
      case 'linecircleintersect': {
        ...
        if (args.length == 3) {
          ...
          var coords0 = this.fetch_path(a);
          let p0 = this.point(coords0, 0);
          let p1 = this.point(coords0, 1);
          let [A, B, C] = this.computeStandardLineForm(p0, p1);
          var coords0 = this.fetch_path(c);
      ...
      case 'circlecircleintersect': {
        if (args.length == 4) {
          ...
          R = parseFloat(R);
          r = parseFloat(r);
          var coords1 = this.fetch_path(a);
          var coords2 = this.fetch_path(b);
          if(coords1.length&&coords2.length&&R>0&&r>0){
            let pt1 = this.point(coords1,0);
            let pt2 = this.point(coords2,0);
            let [x1,y1] = pt1;
            let [x2,y2] = pt2;
            let dx = x2-x1;
            let dy = y2-y1;
  ...
  read_action_coords(line) {
    var coords = this.readCoordsLine(line,1);
    for(var j=0; j < coords.length; ++j){
      var join = coords[2];
  ...
  */
  ...
  do_edge(opts,txt,ts,g,coords){
    ...
    if(this.my_nodes.has(name1)&&this.my_nodes.has(name2)){
      ...
      let [p0,p1,p2] = this.dir_to_qbezier(dir, r, node1.pt[0],node1.pt[1],node2.pt[0],node2.pt[1]);        
      if(dir){
  ...
  dir_to_qbezier(dir, r, x1,y1, x2,y2){
    var p0 = [x1,y1];
    var p2 = [x2,y2];
    // 
    var dx = +x2-x1;
    ...
    ang1 += dir;
    p0[0] += r * Math.cos(ang1);
    p0[1] += r * Math.sin(ang1);
    //re-compute p2 starting point
    var ang2 = Math.PI + Math.atan2(dy,dx);
    ang2 -= dir;
    p2[0] += r * Math.cos(ang2);
    p2[1] += r * Math.sin(ang2);
  ...
  do_box(opt,txt,ts,g,coords){
    var o = [];
    var w = this.config.boxwidth;
    var h = this.config.boxheight;
    w = this.scaledist(w);
    h = this.scaledist(h);
    for (var j = 0; j < coords.length; j++) {
