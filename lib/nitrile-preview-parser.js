...
class NitrilePreviewParser {
  ...
  constructor() {
    ...
    this.re_dlst_astr2  = /^(\*|\u{ff0a})\s+\{(.*?)\}\s*(.*)$/u;
    ...
    this.re_rbit2       = /^(%)\s*(\S+?)(\/|\u{30fb})(\S+)\s*(.*)$/u;
    this.re_flag        = /^(%)\s*#(.*)$/u;
    /*
    ＆  ff06 jp
    ％  ff05 jp,cn
  ...
  newConfig() {
    ...
    config.CONTEX.bodyfontsizept=10;
    config.CONTEX.papersize='A4';
    config.CONTEX.coverpage='';
    config.CONTEX.chapter = '\\bfd';
    config.CONTEX.section = '\\bfc';
    config.CONTEX.subsection = '\\bfb';
    config.CONTEX.subsubsection = '\\bfa';
    ...
    config.LATEX.tableofcontents='';///set to '1' to enable
    config.LATEX.documentclass='';///set to 'book','scrbook', etc., default to 'article' if 'config.haschapter' is null, or 'report' if not
    ...
    config.TEXT.vtop=2;///2mm margin top
    config.TEXT.vbot=2;///2mm margin bot
    config.MATH = {};
    config.MATH.left='5';///5mm left margin
    config.MATH.vtop=2;///2mm margin top
    config.MATH.vbot=2;///2mm margin bot
    config.MATH.vmargin='1';///1mm top/bottom margin
    config.HDGS = {};
    config.PART = {};
    config.PARA = {};
    config.INCL = {};
    config.PLST = {};
    config.PLST.csspaddingleft = '5';///5mm padding left for each list group
    config.PLST.vtop=2;///2mm margin top
    config.PLST.vbot=2;///2mm margin bot
    config.DLST = {};
    config.DLST.left = '10';
    config.DLST.vtop=2;///2mm margin top
    config.DLST.vbot=2;///2mm margin bot
    config.TBLR = {};
    config.TBLR.hlines = 'm';
    config.TBLR.vlines = '';
    config.TBLR.vpadding = 1;
    config.TBLR.hpadding = 6;
    config.TBLR.cssfontsize = '0.8em';
    config.TBLR.latfontsize = '\\footnotesize';
    config.TBLR.confontsize = '\\tfx';
    config.TBLR.vtop=2;///2mm margin top
    config.TBLR.vbot=2;///2mm margin bot
    config.LONG = {};
    config.LONG.hlines = 't m b r';
    config.LONG.vlines = '*';
    config.LONG.vpadding = 3;
    config.LONG.hpadding = 6;
    config.LONG.cssfontsize = '0.8em';
    config.LONG.latfontsize = '\\footnotesize';
    config.LONG.confontsize = '\\tfx';
    config.LONG.vtop=2;///2mm margin top
    config.LONG.vbot=2;///2mm margin bot
    config.SAMP = {};
    config.SAMP.left='5';///5mm margin
    config.SAMP.cssfontsize = '0.8em';
    config.SAMP.latfontsize = '\\footnotesize';
    config.SAMP.confontsize = '\\tfx';
    config.SAMP.csspaddingleft = '5';///5mm padding left for each list group
    config.SAMP.vtop=2;///2mm margin top
    config.SAMP.vbot=2;///2mm margin bot
    config.TABB = {};
    config.TABB.cssfontsize = '0.8em';
    config.TABB.latfontsize = '\\footnotesize';
    config.TABB.confontsize = '\\tfx';
    config.TABB.vtop=2;///2mm margin top
    config.TABB.vbot=2;///2mm margin bot
    config.IMGS = {};
    config.IMGS.vtop=2;///2mm margin top
    config.IMGS.vbot=2;///2mm margin bot
    config.DIAG = {};
    config.DIAG.vtop=2;///2mm margin top
    config.DIAG.vbot=2;///2mm margin bot
    config.VERB = {};
    config.VERB.numbers = 'left';
    config.VERB.numbersep = '2mm';
    config.VERB.monospace = 0;
    config.VERB.cssfontsize = '0.8em';
    config.VERB.latfontsize = '\\footnotesize';
    config.VERB.confontsize = '\\tfx';
    config.VERB.vtop=2;///2mm margin top
    config.VERB.vbot=2;///2mm margin bot
    config.FRMD = {};
    config.FRMD.frameborder = 1;
    config.FRMD.vtop=2;///2mm margin top
    config.FRMD.vbot=2;///2mm margin bot
  ...
  readFromLines(lines) {
    ...
    Standard blocks: TEXT, SAMP, HRLE, MATH, IMGS, DIAG,
        PLST, TABB, DLST, LONG, TBLR, VERB, FRMD
    ...
    */
    ...
    while (lines.length > 0) {
      ...
      if (block_type === 'SAMP') {
        ...
        sig = 'SAMP';
        o.push({rmap,row1,row2,sig,html,data,para,fencecmd,subrow});
        continue;
        ...
        if (v2) {
          ...
          sig = 'HDGS';
          var level = hdgn;
          o.push({rmap,row1,row2,sig,html,hdgn,level,data,para,fencecmd,subrow});
          continue;
        }
      ...
      } else if (this.isLineTable(para)) {

        var data = this.parseLineTable(para);
        sig = 'TBLR';
        o.push({rmap,row1,row2,sig,html,data,para,fencecmd,subrow});
        block_type = 'tabular';
        continue
      ...
      } else if ((v = this.re_feature.exec(para[0])) !== null) {

        /// trim it first before parse
        var re_type = /^\s*\[(.*?)\]\s*(.*)$/;
        var v;
        for( var s of para ) {
          if((v=this.re_flag.exec(s))!==null){
            /// %#rubymap
            var myid = v[2];
            if(myid === 'rubymap'){
              for(var s of json_rubyitems){
                if((v=this.re_rbit1.exec(s))!==null){
                  var rb = v[1];
                  var rt = v[3];
                  var desc = v[4];
                  if((v=re_type.exec(desc))!==null) {
                    var type = v[1];
                    var desc = v[2];
                    var mydesc = `${rb}・${rt}・${desc}`;
                    this.addItem(rb,rt,type,mydesc,rmap);
                  } else {
                    var mydesc = `${rb}・${rt}・${desc}`;
                    rmap.push([rb,rt,mydesc]);
                  }
                }
              }
            } else if (myid==='texmath'){
              this.flag_texmath = 1;
            }
          }
          if((v=this.re_rbit2.exec(s))!==null){
            var rb = v[2];
            var rt = v[4];
            var desc = v[5];
            if((v=re_type.exec(desc))!==null) {
        ...
        var data = text;
        sig = 'TEXT';
        o.push({rmap,row1,row2,sig,html,lead,leadn,data,para,fencecmd,subrow});
        block_type = 'TEXT';
        ...
        if((v=re1.exec(s))!==null){
          ...
          sig = 'SAMP';
          var mode = 1;
          o.push({rmap,row1,row2,mode,sig,html,data,para,fencecmd,subrow});
          block_type = 'SAMP';
          ...
          var standalone = 1;
          o.push({rmap,row1,row2,sig,html,standalone,data,para,fencecmd,subrow});
          block_type = 'TEXT';
        }
      }
      else {
  ...
  unmask (line) {
    ...
    while ((v = this.re_unmask.exec(line)) !== null) {
      ...
      else if (v[6] !== undefined) { /// 'file.txt'
        var cnt = v[0]; // get the original 'file.txt'
  ...
  mergeSubs (subs) {
    ...
    for (var sub of subs) {
      ...
      subflows[subfname] = subparser.blocks;
  ...
  parseLineTable(para){
    ...
    for (var j=1; j < para.length; j++){
      ...
      if (row) {
        ...
        var pp = pp.map(x => x=='{}'?'':x);
        o.push(pp);
        maxj = Math.max(pp.length,maxj);
      }
    }
    /// expand every row so that it contains at least this number elements
    var o = o.map( x => this.expandArray(x,maxj,'') );
    ///for TBLR ww does not matter
    var ww = [];
    return [o,maxj,ww];
  }
  ...
  translateBlocks() {
    ...
    this.haschapter = this.config.haschapter;
    for (var block of this.blocks) {
      ...
      this.xtop = `${fencecmd.vtop}mm`;
      this.xbot = `${fencecmd.vbot}mm`;
