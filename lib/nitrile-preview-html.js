...
`;
  ...
  do_hdgs(block) {
    var {id,row1,row2,sig,part,hdgn,text,dept,subfname} = block;
    var o = [];
    var text = this.escape(text);
    if(subfname && this.config.HTML.trace){
      text += ` <span style='font-size:.8em;font-style:italic'>${subfname}</span>`;
    }
    if(part){
      text = this.escape(part);
      o.push(`<h1 ${this.xattr} style='${this.xstyle}'>Part ${dept} &#160; ${text}</h1>`);
    } else if(hdgn==0) {
      if(this.isepub){
      }else{
        o.push(`<h1 ${this.xattr} style='${this.xstyle}'>${text}</h1>`);
      }
    } else if(hdgn==1) {
      o.push(`<h2 ${this.xattr} style='${this.xstyle}'>${dept} &#160; ${text}</h2>`);
    } else if(hdgn==2) {
      o.push(`<h3 ${this.xattr} style='${this.xstyle}'>${dept} &#160; ${text}</h3>`);
    } else if(hdgn==3) {
      o.push(`<h4 ${this.xattr} style='${this.xstyle}'>${dept} &#160; ${text}</h4>`);
    } else {
      o.push(`<h4 ${this.xattr} style='${this.xstyle}'>${dept} &#160; ${text}</h4>`);
    }
    o.push('');
  ...
  do_dlst(block) {
    var {id,row1,row2,sig,items,para,dept,fencecmd,base,subrow} = block;
    var o = []; 
    o.push(`<dl ${this.xattr} style='${this.xstyle}'>`);
  ...
  do_nlst(block){
    var o = [];
    var {id,row1,row2,sig,items,para,fencecmd,base,subrow,subfname} = block;
    var TOPATTRS=this.xattr;
    var TOPSTYLE=this.xstyle;
    var STYLE=`padding-left:${this.xpadding}em;list-style-position:outside;`;
    o.push(`<ol ${TOPATTRS} style='${TOPSTYLE};${STYLE}'>`);
    for (var item of items) {
      ...
      if(type=='nlst'){
        ...
        body = body.map( x => this.escapeBRTT(x) );
        body = body.map( x => this.rubify(x,this.rmap)); 
        body = body.map( x => `<span style='font-family:monospace'>${x}</span>`);
        text = body.join('\n');
        o.push(`<p style='white-space:pre;'>${text}</p>`);
        o.push('');
      }else if(type=='text'){
        text = this.unmask(text);
        o.push(`<p>${text}</p>`);
  ...
  do_plst(block) {
    ...
    var TOPATTRS=this.xattr;
    var TOPSTYLE=this.xstyle;
    var STYLE=`padding-left:${this.xfencecmd.padding}em;list-style-position:outside;`;
    var count=0;
    for (var item of items) {
      ...
      switch (bull) {
        case 'OL': {
          if(count==0){
            count++;
            o.push(`<ol ${TOPATTRS} style='${TOPSTYLE};${STYLE}'><li value='${value}'>${text}`);
          }else{
        ...
        case 'UL': {
          if(count==0){
            count++;
            o.push(`<ul ${TOPATTRS} style='${TOPSTYLE};${STYLE}'><li value='${value}'>${text}`);
          }else{
  ...
  do_ilst(block){
    var {id,row1,row2,sig,items} = block;
    var o = [];
    var style=`${this.xstyle};padding-left:${this.xfencecmd.padding}em;list-style-position:outside`;
    o.push(`<ul ${this.xattr} style='${style}'>`);
    for(var item of items){
      ...
      if(dt){
        ...
        if(this.xfencecmd.style==1){
          text = `<li><span style='font-family:monospace'>${dt}</span> ${sep} ${text}</li>`;
        }else if(this.xfencecmd.style==2){
          text = `<li><span style='font-style:italic'>${dt}</span> ${sep} ${text}</li>`;
        }else{
          text = `<li>${dt} ${sep} ${text}</li>`;
        }
      }
      o.push(text);
    }
  ...
  do_verb(block) {
    ...
    var linenum = 0;
    body = body.map( x => this.escapeBRTT(x) );
    body = body.map( x => `<tr><td style='font-size:${this.config.HTML.fslineno}em'>${++linenum}</td><td>&#160;</td><td style='font-size:${this.config.HTML.fslisting}em;white-space:pre'>${x}</td></tr>`);
    text = body.join('\n');
    text = `<table border='0' style='border-collapse:collapse;border-top:solid 1px;border-bottom:solid 1px;'>${text}</table>`;
    o.push(`<figure ${this.xattr} style='margin-left:0;${this.xstyle}'>`);
    if(this.floatname){
  ...
  do_samp(block) {
    ...
    var o = []; 
    if(this.config.ALL.samp==1){
      body = this.getMode1SampPara(body);
      body = body.map( x => this.escapeBRTT(x) );
      body = body.map( x => this.rubify(x,this.rmap)); 
      var text = body.join('\n');
      o.push(`<p style='margin-left:${this.config.HTML.step}em;'>${text}</p>`);
    }
    else{
      body = body.map( x => this.escapeBRTT(x) );
      body = body.map( x => this.rubify(x,this.rmap)); 
      body = body.map( x => `<span style='font-family:monospace;font-size:${this.config.HTML.fscode}em'>${x}</span>`);
      var text = body.join('\n');
      o.push(`<p style='margin-left:${this.config.HTML.step}em;white-space:pre;'>${text}</p>`);
    } 
    o.push('');
    block.html = o.join('\n');
  }
  ...
  do_text(block) {
    ...
    ///output to 'o'
    o.push(`<p ${this.xattr} style='${this.xstyle}'>${text}</p>`);
    o.push('');
    block.html = o.join('\n');
  }
  do_pict(block) {
    var {id,row1,row2,sig,rows,caption,label,para,dept,fencecmd,base,subrow} = block;
    var s = [];
    var imgsrcs = [];
    for(var blk of rows){
      let {mode, images} = blk;
  ...
  do_quot(block) {
    ...
    text = `${lq}${text}${rq}`;
    var o = [];
    o.push(`<blockquote ${this.xattr} style='margin-left:${this.config.HTML.step}em;right-margin:0;${this.xstyle}'>`);
    o.push(text);
    o.push(`</blockquote>`);
    o.push('');
    block.html = o.join('\n');
  ...
  do_tabb(block){
    ...
    for(var j=0; j<nrows; ++j){
      ...
      pp = pp.map(x => this.polish(x,this.config.HTML.fstabular));
    ...
    o.push(`<figure ${this.xattr} style='margin-left:${this.config.HTML.step}em;${this.xstyle}'>`);
    o.push(text);
    o.push('</figure>');
    o.push('');
    block.html = o.join('\n');
  ...
  do_long(block){
    ...
    var o = []; 
    var maxj = ww.length;
    //var vlines = this.xfencecmd.cssvlines;
    //var hlines = this.xfencecmd.csshlines;
    //var vlines = this.toArray(vlines);
    //var hlines = this.toArray(hlines);
    //var vpadding = this.xfencecmd.cssvpadding;
    //var hpadding = this.xfencecmd.csshpadding;
    ...
    rows = rows.map ( row => row.map(x => this.polish(x,this.config.HTML.fstabular)));
    ww = this.wwToHundred(ww);
    var s = [];
...
${j==n-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</td>`);
    ...
    s.push(`</tbody>`);
    var text = s.join('\n');
    text = `<table border='0' style='width:100%;border-collapse:collapse;'>${text}</table>`;
    o.push(`<figure ${this.xattr} style='margin-left:0;${this.xstyle}'>`);
    if(0){
      o.push(`<figcaption> Table ${this.xidnum} : ${this.caption_text} </figcaption>`);
    }
    o.push(text);
    o.push(`</figure>`);
    o.push('');
...
${j==nrows-1&&hlines.indexOf('b')>=0?'border-bottom:1px solid;':''} '>${x}</${j==0?'th':'td'}>`);
  ...
  do_diag(block){
    ...
    var o = []; 
    var [text,vw,vh] = new NitrilePreviewDiagramSVG(this,parser.notes,this.tokenizer).parse(body);
    var s = [];
    var text = `<svg xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' fill='currentColor' stroke='currentColor' viewBox='-1 -1 ${vw+2} ${vh+2}' focusable='false' style='display:inline-block;width:100%' >${text}</svg>`;
    ...
    o.push(`<figure ${this.xattr} style='margin-left:0;${this.xstyle}'>`);
    if(label){
      o.push(`<figcaption> Figure ${this.xidnum} : ${this.caption_text} </figcaption>`);
    }
    o.push(text);
    o.push('</figure>');
    o.push('');
  ...
  do_math(block) {
    var {id,row1,row2,sig,maths,label,idnum,more} = block;
    var o = []; 
    var data = this.make_math(maths,label,idnum,more);
    var text = data.join('\n');
  ...
  do_frmd(block) {
    ...
    if (this.xfencecmd.frameborder==1) {
      var text = `<img style='width:100%;max-width:${vw}px;outline:1px solid;padding:3px;box-sizing:;' src="data:image/svg+xml;charset=UTF-8,${encodeURIComponent(out)}" />`;
    } else {
      var text = `<img style='width:100%;max-width:${vw}px;' src="data:image/svg+xml;charset=UTF-8,${encodeURIComponent(out)}" />`;
    }
    o.push(`<p ${this.xattr} style='${this.xstyle}'>`);
    o.push(text);
    o.push('</p>');
    o.push('');
    block.html = o.join('\n');
  ...
  ruby (rb,rt) {
    ...
    if(this.xcssfontsize && this.xmonospace){
      return `<span style='font-family:monospace;font-size:${this.xcssfontsize}em'>${s}</span>`;
    } else if(this.xcssfontsize){
      return `<span style='font-size:${this.xcssfontsize}em'>${s}</span>`;
    }else{
      return s;
    }
  }
  ...
  toFramedSvg (para ) {
    ...
    var vh = (npara)*10*1.333333; /// from pt -> px
    var fontsize = 10*1.333333; /// from pt -> px
    var extra_dy = 0.75;

    var o = [];
    o.push( `<svg xmlns='http://www.w3.org/2000/svg' width='${vw}' height='${vh}' viewBox='0 0 ${vw} ${vh}' >` );
    ...
    for (var i=0; i < npara; ++i) {
      ...
      var x = 0;
      o.push( `<tspan y='${(i+extra_dy)*10*1.333333}px' x='0'>${s}</tspan>` );
    }
    o.push( `</text>`);
    o.push( "</svg>" );
    return [o.join('\n'), vw, vh];
...
///~~~
  ...
  make_line(maths,label,idnum){
    if(maths.length > 1){
      var line = maths.join(' &');
    } else {
      var line = maths.join(' ');
    }
    var {s,nw,nh,shiftdist} = this.tokenizer.toMathSvg(line,true);
    return {s,nw,nh,shiftdist,label,idnum};
  }

  make_math(maths,label,idnum,more){
    var data = [];
    var line = this.make_line(maths,label,idnum);
    data.push(line);
    if(more&&more.length){
      for(let k=0; k < more.length; k++){
        let x = more[k];
        var line = this.make_line(x.maths,x.label,x.idnum);
        data.push(line);
      }
    }
    ///figure out maxshiftdist
    var maxshiftdist = 0;
    data.forEach( x => {
        var {shiftdist} = x;
    ...
    var data = data.map( x => {
        var {s,nw,nh,shiftdist,label,idnum} = x;
        ...
        let math_style = `left:${dist}px;position:relative;text-align:left;`;
        let idnum_style = `width:100%;text-align:right;`;
  ...
  toMathTabularAligned(data,fencecmd) {
    if(fencecmd.isalignequalsign){
      var data = data.map( row => row.join(' &') );
    } else {
      var data = data.map( row => row.join(' '));
    }
