  ...
  toInnerSvg (v, compact=false) {
    ...
    if (v === undefined || v === null) {
      ...
      switch (v[0]) {
        ...
        case '\\nroot':  
          var n = v[1];
          var [w1_,h1_,mid1_,s1_,g1_] = this.shrinkSvg(this.toInnerSvg(['\\brace',n.split('')],compact));
          var nw1_ = w1_*this.sup_rate;
          var nh1_ = h1_*this.sup_rate;
          var nlead = nw1_;
          nlead -= 5;
          if (nlead < 0) nlead = 0;
          var [w_,h_,mid_,s_,g_] = this.toInnerSvg(v[2],compact);
          var lead = 8;
          w = w_;
          h = h_;
          mid = h/2;
        ...
        case '\\sqrt':  
          var [w_,h_,mid_,s_,g_] = this.toInnerSvg(v[1],compact);
          var lead = 8;
          w = w_ + lead;
          h = h_;
          mid = h/2;
        ...
        case '\\frac':  
          var rate = 0.75;
          var [w1_,h1_,mid1_,s1_,g1_] = this.shrinkSvg(this.toInnerSvg(v[1],compact),rate);
          var [w2_,h2_,mid2_,s2_,g2_] = this.shrinkSvg(this.toInnerSvg(v[2],compact),rate);
          w = (w1_ > w2_) ? w1_ : w2_;
        ...
        case '\\binom':  
          var [w1_,h1_,mid1_,s1_,g1_] = this.toInnerSvg(v[1],compact);
          var [w2_,h2_,mid2_,s2_,g2_] = this.toInnerSvg(v[2],compact);
          w = (w1_ > w2_) ? w1_ : w2_;
          h = 1 + h1_ + h2_;
          mid = h1_;
          var o = [];
        ...
        case '\\sup':  
          ...
          if (v[0] === '\\subsup') {
            var [w1_,h1_,mid1_,s1_,g1_] = this.toInnerSvg(v[1],compact1);
            var [w2_,h2_,mid2_,s2_,g2_] = this.toInnerSvg(v[2],compact2);
            var [w3_,h3_,mid3_,s3_,g3_] = this.toInnerSvg(v[3],compact3);
          } else if (v[0] === '\\sub') {
            var [w1_,h1_,mid1_,s1_,g1_] = this.toInnerSvg(v[1],compact1);
            var [w2_,h2_,mid2_,s2_,g2_] = this.toInnerSvg(v[2],compact2);
            var [w3_,h3_,mid3_,s3_,g3_] = [0,0,0,'',0];
          } else { 
            var [w1_,h1_,mid1_,s1_,g1_] = this.toInnerSvg(v[1],compact1);
            var [w2_,h2_,mid2_,s2_,g2_] = [0,0,0,'',0];
            var [w3_,h3_,mid3_,s3_,g3_] = this.toInnerSvg(v[2],compact3);
          }
          var o = [];
...
console.error(str);
  ...
  ///<text dy='.7em' class='MyText' id='myPLUS'>+</text>
    ...
    var [w,h,mid,s,g] = this.toInnerSvg(v);
    var news = `<svg>${s}</svg>`
    var dv = -h + mid + 4;
    var defs = [];
    for (var mathSymbol of pjson.mathSymbols) {
