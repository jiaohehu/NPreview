...
class NitrilePreviewLatex extends NitrilePreviewParser {
  ...
  do_dlst(block){
    ...
    o.push(`\\begin{description}\[nosep,style=unboxed,leftmargin=${this.config.DLST.margin}mm,font=\\normalfont]`);
  ...
  do_plst(block){
    ...
    for (var item of data) {
      ...
      if (item.length === 3) {
        switch (lead) {
          case 'OL': {
            o.push(`\\begin{enumerate}[nosep,leftmargin=${this.xleft}mm]`);
            o.push(`\\item\[${value}\] ${text}`);
            break;
          }
          case 'UL': {
            o.push(`\\begin{itemize}[nosep,leftmargin=${this.xleft}mm]`);
            o.push(`\\item ${text}`);
            break;
          }
          case 'LI': {
  ...
  do_verb(block){
    ...
    if (this.xnumbers) {
      ...
      for (var k=0; k < text.length; ++k) {
        ...
        s.push(`{\\scriptsize{}${++linenum}} & {${ffamily}${this.xlatexfontsize}{}${line}}\\\\`);
        while (lines.length > 0) {
          var line = lines.shift();
          line = this.escapeSolid(line);
          s.push(`& {${ffamily}${this.xlatexfontsize}{}${line}}\\\\`);
        }
      }
  ...
  do_list(block){
    ...
    text.unshift(`\\begin{itemize}[nosep,leftmargin=*,labelindent=${this.xleft}mm,font=\\normalfont${this.xlatexfontsize}]`);
  ...
  do_quot(block){
    ...
    o.push('\\begin{flushleft}');
    o.push(`\\begin\{adjustwidth\}{${this.xleft}mm}{${this.xleft}mm}`)
    o.push(`${lq}${text}${rq}`);
    o.push('\\end{adjustwidth}')
    o.push('\\end{flushleft}')
    block.latex = o.join('\n');
...
\\usepackage[export]{adjustbox}`
  ...
  toLatexDocument(config,texlines) {
    ...
    var documentclassopt = (config.LATEX.twocolumn)?"twocolumn":"";
    var latexengine      = (config.LATEX.latexengine)?config.LATEX.latexengine:'lualatex';
    var data = `\
...
\\end{document}\n`;
  ...
  insertTabularVlines(vlines,pp){
    ...
    for(var j=0; j < pp.length; ++j){
      if(vlines.indexOf(j)>=0 || vlines.indexOf('*')) {
        oo.push('|');
        oo.push(pp[j]);
      } else {
        oo.push(pp[j]);
    ...
    if(vlines.indexOf(pp.length)>=0 || vlines.indexOf('*')){
      oo.push('|');
    }
    return oo.join('');
  }
