  ...
  do_FLOA(block) {
    switch(block.floatname){
      ...
      case 'Equation': {
        ...
        let os = '';
        os = `<div style='position:relative;'><div style='position:absolute;top:50%;transform:translate(0,-50%);'>${mynum}</div><div style='margin-left:${step}em'>${text}</div></div>`;
        text = os;
        block.html = text;
        break;
      }
  ...
  to_ref (cnt){
    if(this.ref_map && this.ref_map.has(cnt)){      
      ...
      if(g.idtext){
        ...
        if(g.chnum){
          return `<a href='#${id}'>${secsign}${g.chnum}.${g.idnum}</a>`;  
        }else{
          return `<a href='#${id}'>${secsign}${g.idnum}</a>`;
        }
      }
    }
