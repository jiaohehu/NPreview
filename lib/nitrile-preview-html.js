...
`;
  ...
  do_text(block) {
    ...
    /// watch out for "this.textblockcount"
    ...
    if (standalone) {
      ...
      var vtop=`${this.config.HTML.vmargin}mm`;
      var vbot=`${this.config.HTML.vmargin}mm`;
    ...
    ///output to 'o'
    o.push(`<p id='${id}' class='P TEXT' dept='${dept}' fName='${this.tofName()}' rows='${row1} ${row2}' style='margin-left:${left};margin-top:${vtop};margin-bottom:${vbot};text-indent:${csstextindent};' >${text}</p>`);
    o.push('');
    block.html = o.join('\n');
  }
  do_quot(block) {
