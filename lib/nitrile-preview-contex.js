...
class NitrilePreviewContex extends NitrilePreviewParser {
  ...
  do_verb(block){
    ...
    caption=this.caption_text;
    if(label){
      caption = `Listing ${this.xidnum} : ${caption}`;
    }
    var text = body;
    var s = [];
    if(this.xnumbers){
  ...
  do_tabr(block){
    ...
    caption=this.caption_text;
    if(label){
      caption = `Table ${this.xidnum} : ${caption}`;
    }
    var s = [];
  ...
  do_pict(block){
    ...
    caption=this.caption_text;
    if(label){
      caption = `Figure ${this.xidnum} : ${caption}`;
    }
    var s = [];
    for(var blk of rows){
      let {mode,images} = blk;
  ...
  do_diag(block){
    var o = [];
    var {id,row1,row2,sig,caption,label,body,subfname} = block; 
    subfname=subfname||''; o.push(`%${sig} ${subfname} ${1+row1} ${1+row2}`);
    caption = this.caption_text;
    if(label){
      caption = `Figure ${this.xidnum} : ${caption}`;
