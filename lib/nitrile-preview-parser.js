...
class NitrilePreviewParser {
    ...
    */
    ...
    while (lines.length > 0) {
      ...
      if (the_fence === '@@@') {
        ...
        id = this.toSid(id,fencecmd.label);
        o.push([id,row,row2,'SPEC',n,data,para,fencecmd,dept,fig,caption,hidden,fname,plevel]);
        continue;
      }
  ...
  hideBlocks (blocks,hiddendepts) {
    ...
    for (var block of blocks) {
      const [id,row1,row2,type,n,data,para,fencecmd,dept,fig,caption,hidden,fname,plevel] = block;
      if (hiddendepts.hasOwnProperty(dept)) {
        block[P_hidden] = true;
      } else {
        block[P_hidden] = false;
      }
      o.push(block);
    }
    return o;
  ...
  idenBlocks (blocks,isarticle,isoutline) {
