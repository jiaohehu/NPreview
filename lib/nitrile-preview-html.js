...
`;
  ...
  do_plst(block) {
    ...
    var TOPATTRS=this.xattr;
    var TOPSTYLE=this.xstyle;
    var STYLE=`padding-left:${this.config.HTML.step}em;list-style-position:outside;`;
    var count=0;
    for (var item of items) {
  ...
  do_ilst(block){
    var {id,row1,row2,sig,items} = block;
    var o = [];
    var style=`${this.xstyle};padding-left:${this.config.HTML.step}em;list-style-position:outside`;
    o.push(`<ul ${this.xattr} style='${style}'>`);
    for(var item of items){
      var {dt,sep,style,text} = item;
      text = this.unmask(text);
      if(dt){
        dt = this.unmask(dt);
        if(!sep){
  ...
  do_samp(block) {
    ...
    if(this.config.ALL.samp==1){
      ...
      body = body.map( x => this.rubify(x,this.rmap)); 
      var text = body.join('\n');
      o.push(`<p style='margin-left:${this.config.HTML.step}em;white-space:pre-wrap;'>${text}</p>`);
    }
    else{
      body = body.map( x => this.escapeBRTT(x) );
      body = body.map( x => this.rubify(x,this.rmap)); 
  ...
  do_hrle(block) {
    var {id,row1,row2,sig,data,para,dept,base,subrow} = block;
    var o = []; 
    var text = data;
    text = this.unmask(text);
    o.push(`<figure ${this.xattr} style='margin-left:0;${this.xstyle}'>`);
