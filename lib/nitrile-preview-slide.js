...
<style>
  ...
  .slide {
    ...
    max-width:128mm;
    min-height:96mm;
    font-family:${this.conf('css.font-family')};
    font-size:${this.conf('css.font-size')};
  }
...
`;
  ...
  to_html(tops,the_slides){
    ...
    tops.forEach((o,i) => {
      ...
      if(o.sig=='FRNT'){
        let data = this.to_frontmatter(o,the_slides);
        d.push(data);
      }
    });
    return d.join('\n');
  ...
  to_html_frame(tops,the_slides){
    ...
    if(d.length==1){
      ...
      ///main slide
      let id = my.idnum;
      all.push(`<div id='slide${mid}' class='slide'>`);
      all.push(d.join('\n'));
      all.push(`<p>`);
      ...
      w.forEach(({title,body},i) => {
        ...
        all.push(`<div id='slide${sid}' class='slide'>`);
        all.push(d.join('\n'));///frame title only
        all.push(`<p>&#x2611; ${title}</p>`);
        all.push(body.join('\n'));
        all.push('</div>');
  ...
  to_frontmatter(block,the_slides){
    let title = '';
    let subtitle = '';
    let institute = '';
    let author = '';
