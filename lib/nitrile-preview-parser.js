...
class NitrilePreviewParser {
  ...
  read_para (lines) {
    ...
    if(type == 'pict'){
      ...
      return {para,lines,text,caption,label,title,hdgn,sig,items,isfenced,fenceid};
    }
    else if(type == 'hlst'){
      ...
      return {para,lines,text,caption,label,title,hdgn,sig,body,isfenced,fenceid};
    }
    else {
      var sig = 'TEXT';
      var {leadn,lead,text} = this.parse_text(body);
      return {para,lines,leadn,lead,text,sig};
    }
