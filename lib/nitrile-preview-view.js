...
console.log('NitrilePreviewView::destroy() called');
  ...
  updateBlocks() {
    ...
    this.getBlocks(parser).then( ([blocks,flags,book]) => {
      ...
      blocks = parser.idenBlocks(blocks,isarticle,isoutline);
  ...
  async saveAs (savefilepath) {
    ...
    try {
      ...
      if (this.saveastype === 'latex-chapter') {
        ...
        var olines = parser.translateLatex(this.toConfig(),this.blocks,false);
        data = olines.join('\n');

      } else if (this.saveastype === 'latex-complete') {

        var parser = new NitrilePreviewLatex();
        var olines = parser.translateLatex(this.toConfig(),this.blocks,this.article);
        var texFamily = atom.config.get('nitrile-preview.texFamily');

        data = `\% \!TEX program = ${texFamily}
\\documentclass{${this.documentclass}}
${parser.toRequiredPackages(texFamily)}
\\title{${this.title}}
\\author{${this.author}}
\\begin{document}
\\maketitle
