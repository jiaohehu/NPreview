...
console.error(`load image from imagemap: ${imgid}, ${src}`);
      setTimeout( () => {
        ...
        if (this.imagemap.has(imgid)) {
          let imgdata = this.imagemap.get(imgid);
...
console.error(`load image from file: ${imgid}, ${src}`);
      ...
      this.parser.readImageFileAsync(fsrc)
        .then( imgdata => {
            this.imagemap.set(imgid,imgdata);
            var node = document.querySelector(`img#${imgid}`);
            if (node) {
              node.src = imgdata;
            }
...
`;
        ...
        epub.setContents(this.parser.escape(this.title),this.parser.escape(this.author),this.htmls,this.chaps,this.isarticle);
        data = await epub.generateAsync(); ///data is a Nodejs Buffer
        fs.writeFileSync(savefilepath, data);
        atom.notifications.addSuccess(savefilepath + ' saved, '
              + data.length + ' byte(s)');
