...
class NitrilePreviewNode {
  ...
  async toLatex (filename) {
    ...
    let [all,flags] = await this.toAll(filename,parser);
    var config = parser.toConfig(flags);
    ...
    if (!documentclass) {
      documentclass = config.latexIsBook ? "book" : "article";
    }
...
${parser.toRequiredPackages(config)}
...
\\end{document}\n`;
  ...
  async toHtml (filename) {
    ...
    var ispreview = false;
    var config = parser.toConfig(flags);
    var [htmls,chaps] = parser.translateHtml(all,config,ispreview);
    /// replace all REFS
    htmls = parser.replaceRef(htmls,chaps);
    /// get TOC
    var toc = this.getXHtmlToc(config.latexIsArticle,chaps);
    /// construct the final LATEX file
    let title = config.latexTitle ? config.latexTitle : 'Untitled'
    let author = config.latexAuthor ? config.latexAuthor : ''
