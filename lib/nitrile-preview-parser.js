...
class NitrilePreviewParser {
  ...
  newConfig() {
    ...
    config.CONTEX.titlepage=0;//1=title page will be generated
    ...
    config.LATEX.titlepage=0;//1=title page will be generated
...
//console.log(block);
  ...
  read_para (lines) {
    ...
    if(type == 'pict'){
      ...
      return {para,lines,sig,hdgn,label,title};
    } 
    else if(type == 'quot'){
  ...
  async readModeAsync(dirname) {
    ...
    for(let d of this.mode){
      ...
      if(name==='root' && subparser){
        ...
        let part=1;
        let title=subtitle;
        this.blocks.push({sig,hdgn,part,title});
        this.ismaster = 1;
