...
if (typeof y !== 'number') { console.error('y not a number:',y); console.error('coords=',coords); }
  ...
  readCoordsLine (o, line, changeall=true) {
    ...
    while (line.length) {
      ...
      if ((v = this.re_asterisk.exec(line)) !== null) {
        var raw = v[0];
        var d = v[1];
        line = v[2];
        console.log('readCoordsLine: d=',d);
        if (d.length == 1) {
          var from = this.lastcoords;
        } else {
          var symbol = d.slice(1); /// such as *p
          console.log('readCoordsLine: symbol=',symbol);
          var from = this.fetchVariable(o, symbol);
        }
        for (var i=0; i < from.length; ++i) {
          console.log('readCoordsLine: i=',i,' length = ',from.length);
          ///***NOTE: must call point() so that it converts any variable
          ///   to real coords.;
          let pt = this.point(o, from,i);
          coords.push(pt);
        }
        continue;
      }
