...
///console.log(`image ${fsrc} changed`);
  ...
  async toAll() {
    ...
    var [main, config] = this.parser.toFlow(this.lines, 0, fname);
    ...
    main = this.parser.insertSubs(main, subs);
    this.parser.idenBlocks(main,config);
  ...
  refreshView() {
    ...
    var ispreview = true;
    var editorrow = this.row;
    var editorcolumn = this.column;
    this.parser.translateBlocks(this.all,this.config,{ispreview,editorrow,editorcolumn});
    this.htmls = this.all.map(x=>x.html);
    this.innerHTMLs = [];
  ...
  async saveAs (savefilepath) {
    ...
    try {
      ...
      if (this.saveastype === 'latex-complete') {
        ...
        var texlines = parser.translateBlocks(this.all,this.config);
        ...
        parser.translateBlocks(this.all,this.config);
        var htmlines = this.all.map(x=>x.html);
        const cssrules = []
