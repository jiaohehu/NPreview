    ...
    for (var block of all) {
      ...
      switch (sig) {
        ...
        case 'TBLR': {
          ...
          var header = header.map(x => `{\\${xsize}{}${x}}`);
          o.push(`\\begin{center}`);
          ...
          o.push(this.toSingleLineRows(hlines,vpadding,xsize,rows));
        ...
        case 'LONG': {
          ...
          o.push(`\\endfoot`);
          o.push(this.toParagraphRows(hlines,vpadding,xsize,rows));
        ...
        case 'listing': {
          var myopts=[];
          const xsize = config.listing.fs;
          var onecolumn = fencecmd.onecolumn;
          myopts.push(`basicstyle=\\ttfamily\\${xsize}`);
...
///console.log(o);
  ...
  toParagraphRows(hlines,vpadding,xsize,rows) {
    var o = [];
    if (rows.length == 0) {
      o.push('(empty) ');
    } else {
  ...
  toSingleLineRows(hlines,vpadding,xsize,rows) {
    var o = [];
    if (rows.length == 0) {
      o.push('(empty) ');
    } else {
  ...
  toLatexDocument(config,texlines) {
    ...
    if (!documentclass) {
      documentclass = (this.isarticle) ? "article" : "report";
    }
