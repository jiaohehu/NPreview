...
class NitrilePreviewDiagramSVG extends NitrilePreviewDiagram {
  ...
  do_p_path(coords,p,g){
    var d = this.coordsToD(coords);
    var f = [];
    if(p.arrowstart){
      f.push(`marker-start='url(#startArrow)'`);
    }
    if(p.arrowend){
      f.push(`marker-end='url(#markerArrow)'`);
    }
    return(`<path d='${d}' ${this.to_strokes(g)} ${this.to_fills(g)} ${f.join(' ')}/>`);
  }
