    ...
    if (blocks.length == 1 && blocks[0][3] === '!BOOK') {
      ...
      this.toBookPreviewBlocks(this.bookBlock,this.dirname())
        .then((out) => {
            ...
            this.htmls.push(`</p>`);
            this.htmls = parser.translateBook(this.blocks,this.htmls);
            this.htmls.push(`</div>`);
...
\\end{document}`;
        ...
        var translator = new NitrilePreviewLatex();
        var olines = translator.translate(this.blocks,false);
        data = olines.join('\n');
...
\\end{document}`;
        ...
        const olines = translator.translate(this.blocks,isarticle);
